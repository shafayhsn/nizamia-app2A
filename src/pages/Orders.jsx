    ...
    <div style={{ display:'flex', flexDirection:'column', flex:1, minHeight:0, background:'#fff' }}>
      <div style={{ padding:'12px 24px', borderBottom:'1px solid #f3f4f6', display:'flex', alignItems:'center', gap:14, flexShrink:0, minWidth:0 }}>
        ...
        <div style={{ display:'flex', gap:8, alignItems:'stretch', minWidth:0 }}>
          ...
          <div style={{ padding:'7px 12px', border:'1px solid var(--border)', borderRadius:8, background:'#fff', minWidth:170 }}>
        ...
        <div style={{ marginLeft:'auto', display:'flex', gap:8, alignItems:'center', flexShrink:0 }}><button className="btn btn-secondary" style={{ height:36 }} disabled={selected.length === 0} onClick={exportCSV}>Export</button><button className="btn btn-secondary" style={{ height:36 }} disabled={selected.length === 0} onClick={printList}>Print</button><button className="btn btn-primary" style={{ height:36, background:actionMode === 'activate' ? '#16a34a' : actionMode === 'reset' ? '#374151' : '#e5e7eb', color:actionDisabled ? '#9ca3af' : '#fff', borderColor:'transparent', minWidth:128 }} disabled={actionDisabled} title={actionMode === 'mixed' ? 'Mixed selection — select same state queues' : ''} onClick={runAction}>{actionMode === 'reset' ? 'Reset Queue' : 'Activate Queue'}</button></div>
      </div>
      <div style={{ padding:'8px 24px', borderBottom:'1px solid #f3f4f6', display:'flex', alignItems:'center', gap:8, flexShrink:0, overflowX:'auto', overflowY:'hidden' }}>
        <div style={{ position:'relative', width:230, flex:'0 0 230px' }}><Search size={13} style={{ position:'absolute', left:9, top:'50%', transform:'translateY(-50%)', color:'#9ca3af' }} /><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search Q#, label, style, buyer..." style={{ width:'100%', paddingLeft:28, paddingRight:10, height:32, border:'1px solid var(--border)', borderRadius:7, fontSize:12, fontFamily:'var(--font)', outline:'none', background:'#fafafa' }} /></div>
