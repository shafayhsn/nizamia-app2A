...
function QueuesTab({ onEditOrder }) {
  ...
  async function runAction(mode = actionMode) {
    ...
    if (mode === 'reset') {
      ...
      const label = orderIds.length === 1 ? 'this order' : `${orderIds.length} orders`
      if (!window.confirm(`Reset queue setup for ${label}? This will delete all generated Qs and allow queue creation from start.`)) return
      const { error: delErr } = await supabase.from('order_queues').delete().in('order_id', orderIds)
      if (delErr) { window.alert(delErr.message || 'Queue reset failed.'); return }
      const { error: orderErr } = await supabase.from('orders').update({ queue_split_rule:'None' }).in('id', orderIds)
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', flex:1, minHeight:0, background:'#fff' }}>
      <div style={{ padding:'12px 24px', borderBottom:'1px solid #f3f4f6', display:'flex', alignItems:'center', gap:14, flexShrink:0, minWidth:0 }}>
        ...
        <div style={{ marginLeft:'auto', display:'flex', gap:8, alignItems:'center', flexShrink:0 }}><button className="btn btn-secondary" style={{ height:34, padding:'0 14px' }} disabled={filtered.length === 0} onClick={exportCSV}>Export</button><button className="btn btn-secondary" style={{ height:34, padding:'0 14px' }} disabled={filtered.length === 0} onClick={printList}>Print</button><button className="btn btn-primary" style={{ height:34, padding:'0 14px', background:selectedInactive.length && selectedInactive.length === selected.length ? '#16a34a' : '#e5e7eb', color:selectedInactive.length && selectedInactive.length === selected.length ? '#fff' : '#9ca3af', borderColor:'transparent', minWidth:112 }} disabled={!(selectedInactive.length && selectedInactive.length === selected.length)} title={selected.length && selectedInactive.length !== selected.length ? 'Select only inactive/queued rows to activate' : ''} onClick={()=>runAction('activate')}>Activate Queue</button><button className="btn btn-secondary" style={{ height:34, padding:'0 14px', background:selectedActive.length && selectedActive.length === selected.length ? '#fee2e2' : '#f3f4f6', color:selectedActive.length && selectedActive.length === selected.length ? '#dc2626' : '#9ca3af', borderColor:'transparent', minWidth:100 }} disabled={!(selectedActive.length && selectedActive.length === selected.length)} title={selected.length && selectedActive.length !== selected.length ? 'Select only active Q rows to reset' : ''} onClick={()=>runAction('reset')}>Reset Queue</button></div>
      </div>
