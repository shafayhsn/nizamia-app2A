...
export default function Orders() {
  ...
  const [visibleColumns, setVisibleColumns] = useState(() => {
    try {
      const saved = localStorage.getItem('orders.visibleColumns.v1')
      if (saved) return JSON.parse(saved)
    } catch (e) {}
    return { po: true, factory_ref: true, qty: true, value: true, ship_date: true, placed: true, merchandiser: true, q_count: true, season: false, po_count: false, sampling_status: false }
  })
  ...
  useEffect(() => {
    try { localStorage.setItem('orders.visibleColumns.v1', JSON.stringify(visibleColumns)) } catch (e) {}
  }, [visibleColumns])
  ...
  const configurableColumns = [
    { key: 'po', label: 'PO #', sort: null },
    { key: 'factory_ref', label: 'Factory Ref', sort: null },
    { key: 'qty', label: 'Qty / Ship / Balance', sort: 'total_qty' },
    { key: 'value', label: 'Value (USD)', sort: null },
    { key: 'ship_date', label: 'Ship Date', sort: 'ship_date' },
    { key: 'placed', label: 'Placed', sort: 'created_at' },
    { key: 'merchandiser', label: 'Merch.', sort: null },
    { key: 'q_count', label: '#Qs', sort: null },
    { key: 'season', label: 'Season', sort: null },
    { key: 'po_count', label: 'PO Count', sort: null },
    { key: 'sampling_status', label: 'Sampling Status', sort: null },
  ]
  const visibleConfigColumns = configurableColumns.filter(c => visibleColumns[c.key])
  const resetVisibleColumns = () => setVisibleColumns({ po: true, factory_ref: true, qty: true, value: true, ship_date: true, placed: true, merchandiser: true, q_count: true, season: false, po_count: false, sampling_status: false })
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 980 }}>
                <thead>
                  <tr style={{ background: '#fafafa', position: 'sticky', top: 0, zIndex: 10 }}>
                    ...
                    <th style={{ fontSize: 10, fontWeight: 600, color: '#9ca3af', textTransform: 'uppercase', letterSpacing: '0.5px', padding: '9px 12px', textAlign: 'left', borderBottom: '1px solid #f3f4f6', whiteSpace: 'nowrap' }}></th>
                    {[{ label: 'Job / Status', col: 'job_number' }, { label: 'Buyer', col: 'buyer_name' }, { label: 'Style / Desc', col: null }, ...visibleConfigColumns.map(c => ({ label: c.label, col: c.sort }))].map(({ label, col }) => (
        ...
        <div style={{ position:'fixed', inset:0, zIndex:260, pointerEvents:'none' }}>
          ...
          <div style={{ position:'absolute', top:0, right:0, width:300, height:'100%', background:'#fff', borderLeft:'1px solid #e5e7eb', boxShadow:'-16px 0 40px rgba(15,23,42,0.14)', pointerEvents:'auto', display:'flex', flexDirection:'column' }}>
            ...
            <div style={{ padding:18, overflowY:'auto', flex:1 }}>
              ...
              <div style={{ fontSize:12, fontWeight:800, color:'#374151', marginBottom:10 }}>Visible Columns</div>
              <div style={{ display:'flex', flexDirection:'column', gap:11, paddingBottom:16, borderBottom:'1px solid #f3f4f6', marginBottom:16 }}>
                {configurableColumns.filter(c => visibleColumns[c.key]).map(c => (
                  <label key={c.key} style={{ display:'flex', alignItems:'center', gap:10, fontSize:13, fontWeight:700, color:'#374151', cursor:'pointer' }}>
                    <input type="checkbox" checked={!!visibleColumns[c.key]} onChange={() => toggleColumn(c.key)} style={{ width:14, height:14, accentColor:'#111827' }} />
                    <span style={{ flex:1 }}>{c.label}</span>
