    ...
    <div style={{ flex: 1, overflowY: 'auto', padding: '16px 24px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
        ...
        <div style={{ marginLeft:'auto', display:'flex', gap:8, alignItems:'center' }}>
          ...
          <button className="btn btn-secondary btn-sm" disabled={selectedQueues.length === 0} onClick={() => setShowPrintModal(true)}><Printer size={14}/> Print Reports</button>
        ...
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.4)', zIndex:300, display:'flex', alignItems:'center', justifyContent:'center' }}>
          <div style={{ background:'#fff', borderRadius:10, padding:24, width:360, boxShadow:'0 16px 48px rgba(0,0,0,0.2)' }}>
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:12, marginBottom:12 }}>
              <div>
                <div style={{ fontSize:16, fontWeight:900 }}>Queue Print Center</div>
                <div style={{ fontSize:11, color:'#6b7280', fontWeight:600, marginTop:3 }}>{selectedQueues.length} selected queue{selectedQueues.length === 1 ? '' : 's'}</div>
              </div>
              <button className="btn btn-ghost" style={{ height:30, width:30, padding:0 }} onClick={() => setShowPrintModal(false)}><X size={18}/></button>
            </div>
            <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
              {[
                { label:'Queue Summary', desc:'Print current selected queue table summary', action: async () => exportQueuesPDF() },
                { label:'IOS', desc:'Internal Order Sheet for selected Qs', kind:'Internal Order Sheet (IOS)' },
                { label:'Fabric Program', desc:'Fabric demand/program using finalized layout', kind:'Fabric Demand' },
                { label:'Cutting Program', desc:'Cutting plan/program using finalized layout', kind:'Cutting Plan' },
                { label:'Trims Program', desc:'Trims demand/program using finalized layout', kind:'Trims Demand' },
                { label:'Washing', desc:'Washing program for selected Qs', kind:'Washing' },
                { label:'Embellishment', desc:'Embellishment program for selected Qs', kind:'Embellishment' },
                { label:'Finishing', desc:'Finishing/packing program for selected Qs', kind:'Finishing' },
                { label:'Print Production File', desc:'Full production file bundle', kind:'Print Production File' },
              ].map(report => (
                <button key={report.label} className="btn btn-secondary btn-sm" style={{ justifyContent:'space-between', textAlign:'left', height:'auto', padding:'9px 10px' }} onClick={async () => { setShowPrintModal(false); if (report.action) await report.action(); else await printSelectedPrograms(report.kind) }}>
                  <span style={{ display:'flex', flexDirection:'column', gap:2 }}>
                    <span style={{ fontWeight:900 }}>{report.label}</span>
                    <span style={{ fontSize:10.5, color:'#6b7280', fontWeight:600 }}>{report.desc}</span>
                  </span>
                  <Printer size={14}/>
                </button>
              ))}
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', flex:1, minHeight:0, background:'#fff' }}>
      <div style={{ padding:'12px 24px', borderBottom:'1px solid #f3f4f6', display:'flex', alignItems:'center', gap:14, flexShrink:0, minWidth:0 }}>
        ...
        <div style={{ marginLeft:'auto', display:'flex', gap:8, alignItems:'center', flexShrink:0 }}><button className="btn btn-secondary" style={{ height:34, padding:'0 14px' }} disabled={filtered.length === 0} onClick={exportCSV}>Export</button><button className="btn btn-secondary" style={{ height:34, padding:'0 14px' }} disabled={filtered.length === 0} onClick={printList}>Print</button><button className="btn btn-primary" style={{ height:34, padding:'0 14px', background:selectedInactive.length && selectedInactive.length === selected.length ? '#16a34a' : '#e5e7eb', color:selectedInactive.length && selectedInactive.length === selected.length ? '#fff' : '#9ca3af', borderColor:'transparent', minWidth:112 }} disabled={!(selectedInactive.length && selectedInactive.length === selected.length)} title={selected.length && selectedInactive.length !== selected.length ? 'Select only inactive/queued rows to activate' : ''} onClick={()=>runAction('activate')}>Activate Queue</button><button className="btn btn-secondary" style={{ height:34, padding:'0 14px', background:selected.length ? '#fee2e2' : '#f3f4f6', color:selected.length ? '#dc2626' : '#9ca3af', borderColor:'transparent', minWidth:100 }} disabled={!selected.length} title={selected.length ? 'Reset the whole queue split for this order' : 'Select any Q to reset its order'} onClick={()=>runAction('reset')}>Reset Queue</button></div>
      </div>
