...
import { Search, Printer, Layers, Plus, X, Check, AlertCircle, Copy, Upload, Download, FileText, Table, Trash2, RefreshCcw, Settings, ShoppingCart, UserRound, Truck, ClipboardList } from 'lucide-react'
...
function QueuesTab({ onEditOrder }) {
  ...
  async function fetchQDocData(q) {
    ...
    const selectedGroupSizes = expandSelectedSizes(q.label || q.size_group_label || rawGroupName, allGroupSizes)
    const effectiveSizes = selectedGroupSizes.length ? selectedGroupSizes : allGroupSizes
    const groupName = selectedGroupSizes.length ? (q.label || rawGroupName) : rawGroupName

    const sizeMap = {}
    qSgs.forEach(g => {
      const relColors = colors.filter(c => c.size_group_id === g.id).filter(c => !q.color_name || c.color_name === q.color_name)
      relColors.forEach(c => {
        ;(effectiveSizes.length ? effectiveSizes : (g.sizes || [])).forEach(sz => {
          const row = bd.find(b => b.size_group_id === g.id && b.color_id === c.id && String(b.size) === String(sz))
          const qty = parseFloat(row?.qty) || 0
          if (qty > 0) sizeMap[String(sz)] = (sizeMap[String(sz)] || 0) + qty
        })
      })
    })

    const sizeOrder = effectiveSizes.length ? effectiveSizes : (qSgs.length ? ((qSgs[0].sizes || []).map(String)) : Object.keys(sizeMap))
      ...
      <div style="width:210mm;height:297mm;padding:5mm 4mm 4mm 4mm;font-family:Arial,sans-serif;color:#000;background:#fff;box-sizing:border-box;overflow:hidden;display:flex;flex-direction:column;gap:1mm;">
        ...
        <table style="width:100%;border-collapse:collapse;table-layout:fixed;margin-bottom:1mm;">
          <colgroup>
            ...
            <col style="width:14%"/>
            <col style="width:22%"/>
            ${sizeCols.map(() => '<col style="width:8.2%"/>').join('')}
            <col style="width:8.8%"/>
          </colgroup>
          <thead>
            <tr>
...
export default function Orders() {
  ...
  async function delinkSelectedJobs() {
    ...
    setSelected(new Set())
    markTabUpdated('ordersPage', 'jobs')
    setTabNotices(getPageTabNoticeMap('ordersPage', ['orders','jobs','queues']))
    await loadOrders()
  ...
  const filtered = orders.filter(o => {
    ...
    else if (col === 'job_number') { va = visibleJobNumber(a) || 'zzz'; vb = visibleJobNumber(b) || 'zzz' }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: '#fff' }}>
      ...
      <div style={{ padding: '12px 24px', borderBottom: '1px solid #f3f4f6', display: 'flex', alignItems: 'center', gap: 14, flexShrink: 0 }}>
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'center', flex: 1, minWidth: 0 }}>
          {[
            { label: 'Urgent Shipments', value: dueThisWeek.length, Icon: AlertCircle, color: '#ef4444', bg: '#fff1f2', border: '#fecdd3' },
            { label: 'Pending Purchase', value: active.filter(o => (parseFloat(o.balance_qty) || 0) > 0).length, Icon: ShoppingCart, color: '#f97316', bg: '#fff7ed', border: '#fed7aa' },
            { label: 'Unassigned Jobs', value: active.filter(o => !o.job_id).length, Icon: UserRound, color: '#f59e0b', bg: '#fffbeb', border: '#fde68a' },
            { label: 'Ready for Shipment', value: active.filter(o => (parseFloat(o.balance_qty) || 0) === 0 && (parseFloat(o.total_qty) || 0) > 0).length, Icon: Truck, color: '#2563eb', bg: '#eff6ff', border: '#bfdbfe' },
            { label: 'Draft Orders', value: active.filter(o => (o.status || 'Draft') === 'Draft').length, Icon: ClipboardList, color: '#64748b', bg: '#f8fafc', border: '#e2e8f0' },
          ].map(kpi => (
            <div key={kpi.label} style={{ display:'flex', alignItems:'center', gap:10, padding:'9px 12px', border:`1px solid ${kpi.border}`, borderRadius:9, background:'#fff', minWidth:150, flex:'1 1 0', maxWidth:230 }}>
              <span style={{ width:28, height:28, borderRadius:8, display:'inline-flex', alignItems:'center', justifyContent:'center', background:kpi.bg, color:kpi.color, flexShrink:0 }}><kpi.Icon size={17} /></span>
              <span style={{ fontSize:13, fontWeight:800, color:'#1f2937', whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{kpi.label} <span style={{ color:kpi.color }}>({kpi.value})</span></span>
            </div>
          ))}
        </div>
        <div style={{ marginLeft: 'auto', display: 'flex', gap: 8, alignItems: 'center' }}>
      ...
      {tab === 'orders' && (
        <>
          ...
          <div style={{ padding: '8px 24px', borderBottom: '1px solid #f3f4f6', display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
            ...
            <div style={{ marginLeft:'auto', display:'flex', gap:8, alignItems:'center' }}>
              <button className="btn btn-secondary" style={actionBtnStyle} disabled={selected.size === 0} onClick={() => setShowAssignJob(true)}>Assign Job</button>
              <button className="btn btn-secondary" style={actionBtnStyle} disabled={selected.size === 0} onClick={delinkSelectedJobs}>Delink Job</button>
              ...
              <button className="btn btn-secondary" style={squareBtnStyle} title="Column Settings" onClick={() => alert('Column settings will be configured later.', { title:'Column Settings' })}>
                <Settings size={15} />
              </button>
            </div>
          </div>
          ...
          {/* Footer */}
          <div style={{ padding: '10px 24px', borderTop: '1px solid var(--border)', background: '#fff', flexShrink: 0 }}>
            <div style={{ display:'grid', gridTemplateColumns:'repeat(6, minmax(140px, 1fr))', gap:0, border:'1px solid #eef0f4', borderRadius:8, overflow:'hidden', background:'#fff' }}>
              {[
                { label:'Total Orders', value: openFiltered.length, sub:'', Icon: ClipboardList, color:'#1f2937', bg:'#f8fafc' },
                { label:'Total Order Qty', value: totalQty.toLocaleString(), sub:'pcs', Icon: ClipboardList, color:'#1f2937', bg:'#f8fafc' },
                { label:'Total Shipped', value: openFiltered.reduce((s,o)=>s+(parseFloat(o.shipped_qty)||0),0).toLocaleString(), sub:'pcs', Icon: Truck, color:'#10b981', bg:'#ecfdf5' },
                { label:'Total Balance', value: openFiltered.reduce((s,o)=>s+(parseFloat(o.balance_qty)||0),0).toLocaleString(), sub:'pcs', Icon: ClipboardList, color:'#ef4444', bg:'#fff1f2' },
                { label:'Total Value (USD)', value:`$${totalValue.toLocaleString(undefined,{maximumFractionDigits:0})}`, sub:'', Icon: Layers, color:'#2563eb', bg:'#eff6ff' },
                { label:'', value:'Download Summary', sub:'', Icon: Download, color:'#1f2937', bg:'#f8fafc', button:true },
              ].map((item, idx) => (
                <button key={idx} type="button" disabled={!item.button} title={item.button ? 'Download Summary' : item.label}
                  style={{ minHeight:44, border:0, borderRight: idx < 5 ? '1px solid #eef0f4' : 0, background:'#fff', display:'flex', alignItems:'center', gap:10, padding:'8px 14px', textAlign:'left', cursor:item.button?'pointer':'default', fontFamily:'var(--font)' }}>
                  <span style={{ width:26, height:26, borderRadius:7, display:'inline-flex', alignItems:'center', justifyContent:'center', background:item.bg, color:item.color, flexShrink:0 }}><item.Icon size={15} /></span>
                  <span style={{ minWidth:0 }}>
                    {item.label && <span style={{ display:'block', fontSize:10, color:'#64748b', fontWeight:700, lineHeight:1.1 }}>{item.label}</span>}
                    <span style={{ display:'block', fontSize:15, color:item.color, fontWeight:900, lineHeight:1.15, whiteSpace:'nowrap' }}>{item.value} {item.sub && <span style={{ fontSize:12, color:item.color, fontWeight:800 }}>{item.sub}</span>}</span>
                  </span>
                </button>
              ))}
            </div>
