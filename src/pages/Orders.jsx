      ...
      {showPrintModal && (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.4)', zIndex:300, display:'flex', alignItems:'center', justifyContent:'center' }}>
          <div style={{ background:'#fff', borderRadius:10, padding:24, width:380, boxShadow:'0 16px 48px rgba(0,0,0,0.2)' }}>
            <div style={{ fontSize:16, fontWeight:800, marginBottom:4 }}>Queue Print Center</div>
    ...
    <div style={{ width:'min(1380px, 98vw)', height:'min(780px, 94vh)', background:'#fff', borderRadius:12, boxShadow:'0 22px 70px rgba(0,0,0,0.25)', display:'flex', flexDirection:'column', overflow:'hidden' }}>
      ...
      {step === 1 && <div style={{ flex:1, display:'flex', alignItems:'center', justifyContent:'center' }}>
        <div onDragOver={e=>e.preventDefault()} onDrop={e=>{ e.preventDefault(); handleFile(e.dataTransfer.files?.[0]) }} style={{ width:560, minHeight:290, border:'2px dashed #cbd5e1', borderRadius:14, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:16, textAlign:'center', padding:30 }}>
          ...
          <div><div style={{ fontSize:18, fontWeight:900 }}>Upload Order Data (.csv / .xlsx)</div><div style={{ fontSize:13, color:'#6b7280', marginTop:8, lineHeight:1.45 }}>Drag and drop your populated template file here, or click to browse.<br/>Preview rows before importing into the system.</div></div>
      ...
      {step !== 1 && <>
        ...
        {message && <div style={{ margin:'10px 22px 0', padding:'9px 12px', borderRadius:8, background:message.type==='error'?'#fef2f2':'#ecfdf5', color:message.type==='error'?'#dc2626':'#15803d', fontSize:12, fontWeight:800 }}>{message.text}</div>}
