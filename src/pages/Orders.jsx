...
function OrderMetadataModal({ order, onClose, onEdit }) {
  ...
  const bomRows = (data?.bom || []).map((b, idx) => {
    const poMatches = (data.poItems || []).filter(i => String(i.source_id || '') === String(b.id || '') || [i.description, i.specification].some(x => String(x || '').toLowerCase().includes(String(b.name || b.detail || '').toLowerCase())))
    const poQty = poMatches.reduce((s,i)=>s+(Number(i.qty)||0),0)
    const poNo = poMatches.map(i=>i.po_number).filter(Boolean)[0] || '—'
    const status = poMatches.length ? (poMatches.some(i => String(i.po_status || '').toLowerCase().includes('partial')) ? 'PARTIAL' : (poQty >= Number(b.final_qty || b.base_qty || 0) ? 'RECEIVED' : (poMatches[0].po_status || 'ISSUED'))) : (b.po_status || 'NOT ISSUED')
    return { no:idx+1, desc:b.name || b.detail || b.category || 'Material', spec:b.specification || b.detail || '—', uom:b.unit || '—', req:b.final_qty || b.base_qty || '—', po:poNo, poQty:poQty || '—', status }
  })
