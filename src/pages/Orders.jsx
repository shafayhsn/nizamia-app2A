...
const libMap = Object.fromEntries((libs || []).map(x => [x.id, x]))
  ...
  function buildTrimsProgramHTML(d) {
    ...
    const trimRows = (d.trimItems || []).flatMap((item) => {
      ...
      const parent = `<tr><td style="${td};font-weight:900;">${esc(trimType(item))}</td><td style="${td};font-weight:900;">${esc(item.name || '—')}</td><td style="${td};font-weight:900;">${esc(item.shade || item.specification || item.detail || '—')}</td><td style="${tdC}">&nbsp;</td><td style="${tdC};white-space:nowrap;">${esc(formatPackLogic(item))}</td><td style="${tdR};font-weight:900;">${esc(formatReq(item.q_qty, item))}</td><td style="${tdC};font-weight:900;">${esc(poText(item.po_number))}</td><td style="${tdC};font-weight:900;">${esc(poText(item.po_status))}</td><td style="${tdC}">${circle}</td><td style="${tdC}">${circle}</td></tr>`
