  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', flex:1, minHeight:0, background:'#fff' }}>
      <div style={{ padding:'12px 24px', borderBottom:'1px solid #f3f4f6', display:'flex', alignItems:'center', gap:14, flexShrink:0, minWidth:0 }}>
        ...
        <div style={{ marginLeft:'auto', display:'flex', gap:8, alignItems:'center', flexShrink:0 }}><button className="btn btn-secondary" style={{ height:36 }} disabled={filtered.length === 0} onClick={exportCSV}>Export</button><button className="btn btn-secondary" style={{ height:36 }} disabled={filtered.length === 0} onClick={printList}>Print</button><button className="btn btn-primary" style={{ height:36, background:actionMode === 'activate' ? '#16a34a' : actionMode === 'reset' ? '#374151' : '#e5e7eb', color:actionDisabled ? '#9ca3af' : '#fff', borderColor:'transparent', minWidth:128 }} disabled={actionDisabled} title={actionMode === 'mixed' ? 'Mixed selection — select same state queues' : ''} onClick={runAction}>{actionMode === 'reset' ? 'Reset Queue' : 'Activate Queue'}</button></div>
      </div>
