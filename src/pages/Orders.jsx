...
import { getPageTabNoticeMap, markTabSeen, markTabUpdated, TAB_NOTICE_EVENT } from '../lib/tabNotices'
...
function QueuesTab({ onEditOrder }) {
  ...
  useEffect(() => {
    loadAll()
    const refreshQueues = (evt) => {
      if (evt?.detail?.page && evt.detail.page !== 'ordersPage') return
      if (!evt?.detail?.tab || evt.detail.tab === 'queues') loadAll()
    }
    window.addEventListener(TAB_NOTICE_EVENT, refreshQueues)
    window.addEventListener('storage', refreshQueues)
    window.addEventListener('focus', refreshQueues)
    return () => {
      window.removeEventListener(TAB_NOTICE_EVENT, refreshQueues)
...
export default function Orders() {
  ...
  useEffect(() => {
    ...
    const refreshOrderPageNotices = (evt) => {
      if (evt?.detail?.page && evt.detail.page !== 'ordersPage') return
      setTabNotices(getPageTabNoticeMap('ordersPage', ['orders','queues']))
      if (!evt?.detail?.tab || evt.detail.tab === 'orders' || evt.detail.tab === 'queues') loadOrders()
    }
    window.addEventListener(TAB_NOTICE_EVENT, refreshOrderPageNotices)
    window.addEventListener('storage', refreshOrderPageNotices)
    window.addEventListener('focus', refreshOrderPageNotices)
    return () => {
      window.removeEventListener(TAB_NOTICE_EVENT, refreshOrderPageNotices)
      window.removeEventListener('storage', refreshOrderPageNotices)
      window.removeEventListener('focus', refreshOrderPageNotices)
    }
  }, [])
