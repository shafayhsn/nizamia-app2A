...
function QueuesTab({ onEditOrder }) {
  ...
  async function handleActivateQueue() {
    if (!selectedQueue || selectedQueue.q_number) return
    const { data: existing } = await supabase.from('order_queues').select('q_number').not('q_number', 'is', null)
    const maxQ = Math.max(0, ...((existing || []).map(r => parseInt(String(r.q_number || '').replace(/\D/g, ''))).filter(n => Number.isFinite(n))))
    const nextQ = `Q${maxQ + 1}`
    const { error } = await supabase.from('order_queues').update({ q_number: nextQ, status: selectedQueue.status === 'Queued' ? 'Pending' : selectedQueue.status }).eq('id', selectedQueue.id)
    if (error) { await alert(error.message || 'Could not activate queue.', { title:'Activation Failed' }); return }
    setSelected(selectedQueue.id)
    await loadAll()
    await alert(`${nextQ} assigned to ${selectedQueue.label || 'queue'}.`, { title:'Queue Activated' })
  }
  ...
  const statusColor = { Queued: '#9ca3af', Pending: '#d97706', 'In Progress': '#2563eb', Completed: '#16a34a' }
  const statusBg    = { Queued: '#f3f4f6', Pending: '#fff7ed', 'In Progress': '#eff6ff', Completed: '#f0fdf4' }
  ...
  const handleRollbackOrderQueues = async () => {
    ...
    const proceed = await confirm(`Reset queue configuration for ${selectedOrder?.style_number || 'this order'}? This will remove all generated Qs for the order.`, { title:'Reset Queue Configuration', confirmText:'Reset', tone:'danger' })
    if (!proceed) return
    if (selectedOrderLocked) {
      const pw = await prompt('This order has Qs already used in a Purchase Order. Admin password required to reset queues:', { title:'Admin Override Required', confirmText:'Confirm', password:true, placeholder:'Enter admin password' })
      if (pw !== 'admin') {
        await alert('Incorrect admin password. Queue reset cancelled.', { title:'Access Denied' })
        return
      }
    }
    const { error: delErr } = await supabase.from('order_queues').delete().eq('order_id', selectedOrderId)
    if (delErr) { await alert(delErr.message, { title:'Queue Reset Failed' }); return }
    await supabase.from('orders').update({ queue_split_rule: 'None' }).eq('id', selectedOrderId)
    setSelected(null)
    await loadAll()
    await alert('Queue configuration reset. You can now reconfigure the split rule and regenerate Qs.', { title:'Queues Reset' })
  }
  ...
  async function fetchQDocData(q) {
    ...
    const [bom, washing, embellishments, processes, finishing, packs, sgs] = await Promise.all([
      ...
      safeQuery(supabase.from('order_processes').select('*').eq('order_id', q.order_id).order('sort_order'), []),
  ...
  async function printProgram(kind) {
    ...
    if (!printWin) {
      alert('Popup blocked. Please allow popups for this site and try again.', { title:'Popup Blocked' })
      return
    }
    ...
    if (fn) printHTML(fn(), printWin)
      ...
      const msg = err?.message || 'Unable to prepare this document.'
      printWin.document.open()
      printWin.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>Print error</title><style>body{font-family:Arial,sans-serif;padding:24px;color:#111} .box{border:1px solid #e5e7eb;border-radius:8px;padding:16px;max-width:640px} .muted{color:#6b7280;font-size:12px}</style></head><body><div class="box"><h2 style="margin:0 0 8px">Could not prepare print</h2><div class="muted">${esc(kind)} · ${esc(selectedQueue?.q_number || 'Queued')}</div><p style="margin-top:16px">${esc(msg)}</p><p class="muted">Please try again. If this keeps happening, reopen the order and queue, then print again.</p></div></body></html>`)
      printWin.document.close()
    }
  }
      ...
      {selected && selectedQueue && (
          <div style={{ marginTop: 16, background: '#fff', border: '1px solid #e8e8e6', borderRadius: 8, padding: '16px 20px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12, flexWrap:'wrap' }}>
              <span style={{ fontFamily: 'var(--font-mono)', fontWeight: 800, fontSize: 15 }}>{selectedQueue.q_number || 'Queued'}</span>
              ...
              {!selectedQueue.q_number && <button className="btn btn-primary btn-sm" onClick={handleActivateQueue}>Activate Q</button>}
            </div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: '#fff' }}>
      ...
      {showChangeStatus && (
        <ChangeStatusModal
          ...
          onDone={() => { markTabUpdated('ordersPage', 'orders'); setTabNotices(getPageTabNoticeMap('ordersPage', ['orders','jobs','queues'])); setShowChangeStatus(false); setSelected(new Set()); loadOrders() }}
        />
      )}
      {showAssignJob  && (
        <AssignJobModal
