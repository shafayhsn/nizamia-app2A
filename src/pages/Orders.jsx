...
export default function Orders() {
  ...
  const [groupBy, setGroupBy] = useState(() => {
    try { return localStorage.getItem('orders.groupBy.v1') || 'none' } catch (e) { return 'none' }
  })
  ...
  useEffect(() => {
    try { localStorage.setItem('orders.groupBy.v1', groupBy) } catch (e) {}
  }, [groupBy])
  ...
  const groupKeyValue = (o) => {
    if (groupBy === 'style_number') return o.style_number || 'No Style'
    if (groupBy === 'job_number') return visibleJobNumber(o) || 'No Job'
    if (groupBy === 'store_name') return o.store_name || 'No Store'
    if (groupBy === 'delivery') return o.ship_date || 'No Delivery Date'
    return 'All Orders'
  }
  const groupLabel = (key) => {
    if (groupBy === 'style_number') return `Style ${key}`
    if (groupBy === 'job_number') return `Job ${key}`
    if (groupBy === 'store_name') return `Store ${key}`
    if (groupBy === 'delivery') return key === 'No Delivery Date' ? key : `Delivery ${fmtDate(key)}`
    return key
  }
  const groupedOrders = (() => {
    ...
    return Array.from(groups.entries()).map(([key, rows]) => ({
      key,
      rows,
      qty: rows.reduce((sum, o) => sum + (parseFloat(o.total_qty) || 0), 0),
      value: rows.reduce((sum, o) => sum + (parseFloat(o.total_value_usd) || 0), 0),
    }))
  })()
  const tableColumnCount = 1 + (visibleColumns.picture ? 1 : 0) + 3 + visibleConfigColumns.length
  const displayRows = groupBy === 'none' ? filtered : groupedOrders.flatMap(group => [{ __group: true, ...group }, ...group.rows])
      ...
      {tab === 'orders' && (
        <>
          ...
          <div style={{ padding: '8px 24px', borderBottom: '1px solid #f3f4f6', display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
            ...
            <select
              value={sort.col}
              onChange={e => setSort({ col: e.target.value, dir: 'asc' })}
              style={sortSelectStyle}
              title="Sort orders"
            >
              {sortOptions.map(opt => <option key={opt.col} value={opt.col}>Sort: {opt.label}</option>)}
            </select>
            <button
              className="btn btn-secondary"
              style={{ ...squareBtnStyle, fontSize: 13, fontWeight: 800 }}
            ...
            <select
              ...
              style={{ ...selStyle, minWidth: 155, fontWeight: 600 }}
              title="Group orders"
            >
              {groupOptions.map(opt => <option key={opt.value} value={opt.value}>Group: {opt.label}</option>)}
            </select>
            <div style={{ marginLeft:'auto', display:'flex', gap:8, alignItems:'center' }}>
                  ...
                  {displayRows.map(row => {
                    if (row.__group) return (
                      <tr key={`group-${row.key}`}>
                        <td colSpan={tableColumnCount} style={{ padding: '8px 18px', background: '#f8fafc', borderTop: '1px solid #e5e7eb', borderBottom: '1px solid #e5e7eb' }}>
                          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 14 }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                              <span style={{ fontSize: 11, fontWeight: 800, color: '#111827', textTransform: 'uppercase', letterSpacing: '0.4px' }}>{groupLabel(row.key)}</span>
                              <span style={{ fontSize: 10, fontWeight: 700, color: '#6b7280', background: '#fff', border: '1px solid #e5e7eb', borderRadius: 999, padding: '2px 8px' }}>{row.rows.length} order{row.rows.length > 1 ? 's' : ''}</span>
                            </div>
...
}}{row.value.toLocaleString(undefined, { maximumFractionDigits: 0 })}</strong></span>
                            </div>
                          </div>
                        </td>
                      </tr>
