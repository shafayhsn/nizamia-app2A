...
const libMap = Object.fromEntries((libs || []).map(x => [x.id, x]))
  ...
  function buildCuttingPlanHTML(d) {
    ...
    const fabricRows = (d.fabricItems || []).length ? (d.fabricItems || []).map(item => `<tr>
    ...
    const emptyRows = Array.from({ length: 10 }).map(() => `<tr>${Array.from({ length: 13 }).map(() => `<td style="${td};height:8.3mm;">&nbsp;</td>`).join('')}</tr>`).join('')
    ...
    return `<div style="width:210mm;min-height:297mm;padding:6mm 8mm;box-sizing:border-box;font-family:Arial,sans-serif;color:#000;background:#fff;">
      ...
      <div class="cp-page" style="min-height:285mm;display:flex;flex-direction:column;">
        ...
        <table style="width:100%;border-collapse:collapse;table-layout:fixed;margin-bottom:4mm;"><thead><tr>${['Date','Cut #','Wash','Size Range','Shrinkage','Width','Cutting Qty','Marker Length','Layers','Fabric Used','Body Consump','Lining Used','Cut Issued To'].map(h => `<th style="${th}${blk}">${h}</th>`).join('')}</tr></thead><tbody>${emptyRows}</tbody></table>
