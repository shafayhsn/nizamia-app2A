...
function QueuesTab({ onEditOrder }) {
  ...
  async function fetchQDocData(q) {
    ...
    const [bom, washing, embellishments, finishing, packs, sgs, fittingBlocks, poHeadersAll, poItemsAll, libItems] = await Promise.all([
      safeQuery(supabase.from('bom_items').select('*').eq('order_id', q.order_id).order('sort_order'), []),
      safeQuery(supabase.from('washing').select('*').eq('order_id', q.order_id).order('created_at'), []),
      safeQuery(supabase.from('embellishments').select('*').eq('order_id', q.order_id).order('created_at'), []),
      ...
      safeQuery(supabase.from('purchase_orders').select('id,po_number,status,created_at,order_id').eq('order_id', q.order_id).order('created_at', { ascending: false }), []),
      safeQuery(supabase.from('purchase_order_items').select('po_id,description,specification,source_id,source_type,library_item_id').order('created_at', { ascending: false }), []),
    ...
    const libMap = Object.fromEntries((libItems || []).map(i => [i.id, i]))
    ...
    const sg = q.size_group_id ? sgMap[q.size_group_id] : null
    const qColors = q.size_group_id ? (colors || []).filter(c => c.size_group_id === q.size_group_id) : []
    let selectedColor = q.color_name ? qColors.find(c => (c.color_name || '').trim().toLowerCase() === (q.color_name || '').trim().toLowerCase()) : null
    if (!selectedColor && qColors.length === 1) selectedColor = qColors[0]

    const orderedSizes = sg?.sizes || []
    const sizeQtyMap = {}
    orderedSizes.forEach(sz => {
      const row = selectedColor ? bd.find(b => b.size_group_id === q.size_group_id && b.color_id === selectedColor.id && b.size === sz) : null
      sizeQtyMap[sz] = parseFloat(row?.qty) || 0
    })
    const positiveQtys = orderedSizes.map(sz => Math.round(sizeQtyMap[sz] || 0)).filter(v => v > 0)
    const gcd2 = (a, b) => b ? gcd2(b, a % b) : a
    const ratioBase = positiveQtys.length ? positiveQtys.reduce(gcd2) : 0
    const ratioMap = {}
    orderedSizes.forEach(sz => {
      const v = Math.round(sizeQtyMap[sz] || 0)
      ratioMap[sz] = ratioBase > 0 && v > 0 ? Math.round(v / ratioBase) : 0
    })
    const ratioTotal = Object.values(ratioMap).reduce((s, v) => s + (parseFloat(v) || 0), 0)
    ...
    function calcBomQty(item) {
      ...
      if (rule === 'By Individual Sizes') return Object.entries(ud).reduce((s,[sz,cons]) => s + ((parseFloat(cons)||0) * (parseFloat(sizeQtyMap[sz])||0) * wastageFactor(item.wastage)), 0)
    ...
    const fabricItems = (bom || []).filter(x => x.category === 'Fabric').map(x => {
      ...
      return {
        ...x,
        library: lib,
        shade: lib?.colour || x.specification || x.detail || '—',
        content: lib?.composition || x.detail || '—',
        weight: lib?.gsm ? `${lib.gsm} GSM` : '—',
        width: lib?.width_inches ? `${lib.width_inches}.00 inches` : '—',
        consump: parseFloat(x.base_qty) || 0,
        q_qty: calcBomQty(x),
      }
    })
    const trimItems = (bom || []).filter(x => x.category !== 'Fabric').map(x => ({ ...x, q_qty: calcBomQty(x) }))
    const stitchTrimItems = trimItems.filter(x => String(x.category || '').toLowerCase().includes('stitch'))
    const packingTrimItems = trimItems.filter(x => !String(x.category || '').toLowerCase().includes('stitch'))
    const washItems = (washing || []).filter(x => !q.color_name || !x.color_name || x.color_name === q.color_name)
    const embellishmentLines = (embellishments || []).map(x => `${x.description || x.technique || '—'}${x.placement ? ' @' + x.placement : ''}`)

    const latestTrimPOForItem = (item) => {
      const linked = (poItemsAll || []).filter(pi => {
        if (pi.source_type === 'queue' && pi.source_id === q.id) {
          if (pi.library_item_id && item.library_item_id) return pi.library_item_id === item.library_item_id
          const d = (pi.description || '').toLowerCase()
          return d.includes((item.name || '').toLowerCase())
        }
        return false
      }).sort((a, b) => {
        const pa = poHeadersAll.find(p => p.id === a.po_id)
        const pb = poHeadersAll.find(p => p.id === b.po_id)
        return new Date(pb?.created_at || 0) - new Date(pa?.created_at || 0)
      })
      const first = linked[0]
      if (!first) return null
      const po = poHeadersAll.find(p => p.id === first.po_id)
      return po ? { po_number: po.po_number || '—', status: po.status || '—' } : null
    }

    const trimsForIOS = []
    stitchTrimItems.forEach(item => {
      const linked = latestTrimPOForItem(item)
      trimsForIOS.push({
        type: 'Stitching',
        item: item.name || '—',
        shade: item.specification || item.detail || '—',
        consump: `${(parseFloat(item.base_qty) || 0).toFixed(2)} ${item.unit || ''}`.trim(),
        req: `${Math.ceil(item.q_qty || 0).toLocaleString()} ${item.unit || ''}`.trim(),
        po_number: linked?.po_number || '—',
        status: linked?.status || '—',
      })
    })
    if (stitchTrimItems.length && packingTrimItems.length) trimsForIOS.push({ spacer: true })
    packingTrimItems.forEach(item => {
      const linked = latestTrimPOForItem(item)
      trimsForIOS.push({
        type: 'Packing',
        item: item.name || '—',
    ...
    const cutSizeMap = {}
    orderedSizes.forEach(sz => {
      cutSizeMap[sz] = Math.round((parseFloat(sizeQtyMap[sz]) || 0) * (1 + cuttingPct / 100))
    })
    const cuttingQtyTotal = queueQty

    const fittingBlockName = (fittingBlocks || []).find(b => b.block_name)?.block_name || '—'
    const washName = (washItems[0]?.wash_type || washItems[0]?.wash_ref || '').trim() || '—'
    ...
    const blisterEach = finishingPacks[0]?.pieces_per_inner_pack || finishingPacks[0]?.inner_pieces || finishing?.pieces_per_inner_pack || '—'
    const blisterTotal = finishingPacks.length ? finishingPacks.reduce((s, p) => s + Math.ceil((queueQty || 0) / Math.max(1, parseFloat(p.pieces_per_inner_pack || p.inner_pieces || 1))), 0) : '—'
    const cartonEach = finishingPacks[0]?.pcs_per_carton || finishing?.pcs_per_carton || '—'
    const cartonTotal = finishingPacks.length ? finishingPacks.reduce((s, p) => s + Math.ceil((queueQty || 0) / Math.max(1, parseFloat(p.pcs_per_carton || 1))), 0) : '—'
  ...
  function pseudoQR(text) {
    const size = 21
    let seed = 0
    for (let i = 0; i < text.length; i++) seed = (seed * 31 + text.charCodeAt(i)) % 1000000007
    const cell = 5
    const dim = size * cell
    ...
    function finderRect(x, y) {
      return [
        `<rect x="${x}" y="${y}" width="${7*cell}" height="${7*cell}" fill="#000"/>`,
        `<rect x="${x+cell}" y="${y+cell}" width="${5*cell}" height="${5*cell}" fill="#fff"/>`,
        `<rect x="${x+2*cell}" y="${y+2*cell}" width="${3*cell}" height="${3*cell}" fill="#000"/>`,
      ].join('')
    }
    const rects = [finderRect(0,0), finderRect((size-7)*cell,0), finderRect(0,(size-7)*cell)]
    for (let r = 0; r < size; r++) {
      for (let c = 0; c < size; c++) {
        if (isFinder(r,c)) continue
        seed = (seed * 1664525 + 1013904223) % 4294967296
        if ((seed >>> 0) % 3 === 0) rects.push(`<rect x="${c*cell}" y="${r*cell}" width="${cell}" height="${cell}" fill="#000"/>`)
      }
    }
    return `<svg xmlns="http://www.w3.org/2000/svg" width="${dim}" height="${dim}" viewBox="0 0 ${dim} ${dim}"><rect width="100%" height="100%" fill="#fff"/>${rects.join('')}</svg>`
  }

  function formatIOSDate(dt = new Date()) {
  ...
  function buildIOSHTML(d) {
    ...
    const topPO = d.ord.po_number || '—'
    const sizes = d.orderedSizes || []
    const orderQtyCells = sizes.map(sz => `<td class="ios-center">${escHtml(Math.round(d.sizeQtyMap[sz] || 0))}</td>`).join('')
    const ratioCells = sizes.map(sz => `<td class="ios-center">${escHtml(d.ratioMap[sz] || 0)}</td>`).join('')
    const cutCells = sizes.map(sz => `<td class="ios-center">${escHtml(d.cutSizeMap[sz] || 0)}</td>`).join('')
    const zipperCells = sizes.map(() => `<td class="ios-center">&nbsp;</td>`).join('')
    const hasFabric = d.fabricItems && d.fabricItems.length
    const fabricRows = hasFabric ? d.fabricItems.map(item => `
      <tr>
        <td>${escHtml(item.name || '—')}</td>
        <td>${escHtml(item.shade || '—')}</td>
        <td>${escHtml(item.content || '—')}</td>
        <td>${escHtml(item.weight || '—')}</td>
        <td>${escHtml(item.width || '—')}</td>
        <td class="ios-right">${escHtml((parseFloat(item.consump || 0)).toFixed(2))} ${escHtml(item.unit || '')}</td>
        <td class="ios-right">${escHtml((item.q_qty || 0).toLocaleString(undefined, { maximumFractionDigits: 2 }))} ${escHtml(item.unit || '')}</td>
      </tr>`).join('') : `<tr><td colspan="7">—</td></tr>`
    const trimsRows = d.trimsForIOS && d.trimsForIOS.length ? d.trimsForIOS.map(row => {
      if (row.spacer) return `<tr class="ios-spacer"><td colspan="7"></td></tr>`
      return `<tr>
        <td>${escHtml(row.type)}</td>
        <td>${escHtml(row.item)}</td>
        <td>${escHtml(row.shade)}</td>
        <td class="ios-right">${escHtml(row.consump)}</td>
        <td class="ios-right">${escHtml(row.req)}</td>
        <td class="ios-center">${escHtml(row.po_number)}</td>
        <td>${escHtml(row.status)}</td>
      </tr>`
    }).join('') : `<tr><td colspan="7">—</td></tr>`
    const embellishmentText = d.embellishmentLines?.length ? d.embellishmentLines.map(x => `<div>${escHtml(x)}</div>`).join('') : '—'
    const qrSvg = pseudoQR(`IOS-${qRef}|${d.ord.style_number || ''}|${topPO}`)
    return `
      <div class="ios-page">
        <style>@page{size:A4 portrait;margin:6mm;}
          .ios-page{width:210mm;min-height:0;padding:8px 10px 10px 10px;font-family:"Times New Roman",serif;color:#000;background:#fff;box-sizing:border-box;}
          .ios-topline{display:flex;justify-content:space-between;font-size:11px;line-height:1;margin-bottom:2px;}
          .ios-brandrow{display:grid;grid-template-columns:1.15fr 0.45fr 0.42fr 0.72fr;align-items:start;column-gap:8px;margin-bottom:4px;}
          .ios-brand h1{font-size:28px;line-height:0.9;margin:2px 0 2px 0;font-weight:700;letter-spacing:-0.4px;}
          .ios-brand .sub{font-size:16px;font-weight:400;}
          .ios-oval-wrap{display:flex;justify-content:center;align-items:flex-start;padding-top:8px;}
          .ios-oval{width:82px;height:42px;border:3px solid #000;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:24px;font-weight:700;letter-spacing:-0.6px;}
          .ios-qr-wrap{display:flex;justify-content:center;padding-top:4px;}
          .ios-qno{font-size:60px;line-height:0.82;font-weight:700;text-align:right;padding-top:0;white-space:nowrap;}
          .ios-info{display:grid;grid-template-columns:1fr 1fr 1fr;column-gap:14px;margin-bottom:4px;}
          .ios-info .col{display:grid;grid-template-columns:auto 1fr;column-gap:10px;row-gap:2px;align-items:start;}
          .ios-lbl{font-size:15px;font-weight:700;white-space:nowrap;}
          .ios-val{font-size:14px;white-space:nowrap;}
          .ios-section-title{font-size:22px;font-weight:700;line-height:1;margin:4px 0 3px 0;}
          table.ios-table{width:100%;border-collapse:collapse;margin:0 0 6px 0;font-size:12px;}
          .ios-table th,.ios-table td{border:1.5px solid #000;padding:2px 4px;vertical-align:top;line-height:1.0;}
          .ios-table thead th{background:#000;color:#fff;font-size:12px;font-weight:700;line-height:1.0;}
          .ios-subhead th{background:#d9d9d9 !important;color:#8f96a3 !important;font-size:11px !important;letter-spacing:1px;}
          .ios-breakdown-left{width:170px;font-size:12px;font-weight:700;line-height:1.1;}
          .ios-breakdown-mid{width:150px;font-size:12px;font-weight:700;}
          .ios-center{text-align:center;white-space:nowrap;}
          .ios-right{text-align:right;white-space:nowrap;}
          .ios-blackhead th{background:#000;color:#fff;font-size:12px;}
          .ios-spacer td{height:8px;padding:0;background:#fff;}
          .ios-bottom{display:grid;grid-template-columns:1.1fr 0.9fr 0.9fr;column-gap:10px;align-items:start;margin-top:2px;}
          .ios-box-title{font-size:14px;font-weight:700;line-height:1;margin-bottom:3px;}
          .ios-box{border:1.5px solid #000;min-height:72px;padding:4px 6px;font-size:11px;line-height:1.05;}
          .ios-pack-table{width:100%;border-collapse:collapse;font-size:11px;margin-top:4px;}
          .ios-pack-table th,.ios-pack-table td{border:1.5px solid #000;padding:3px 4px;}
          .ios-pack-table thead th{background:#d9d9d9;color:#8f96a3;font-size:10px;letter-spacing:1px;text-align:left;}
          .ios-tickline{display:flex;align-items:center;gap:6px;margin:4px 0;font-size:11px;font-weight:700;}
          .ios-tick{width:10px;height:10px;border:1.5px solid #000;display:inline-block;}
        </style>
        <div class="ios-topline">
          <div>Document Ref#IOS-${escHtml(qRef)}</div>
          <div>Date Created: ${escHtml(formatIOSDate())} &nbsp; Username: <strong>admin</strong></div>
        </div>
        <div class="ios-brandrow">
          <div class="ios-brand">
            <h1>NIZAMIA APPARELS</h1>
            <div class="sub">Internal Order Sheet</div>
          </div>
          <div class="ios-oval-wrap"><div class="ios-oval">IOS</div></div>
          <div class="ios-qr-wrap">${qrSvg}</div>
          <div class="ios-qno">${escHtml(qRef)}</div>
        </div>
        <div class="ios-info">
          <div class="col">
            <div class="ios-lbl">Brand</div><div class="ios-val">${escHtml(d.ord.brand_name || '—')}</div>
            <div class="ios-lbl">Store</div><div class="ios-val">${escHtml(d.ord.store_name || '—')}</div>
          </div>
          <div class="col">
            <div class="ios-lbl">Style Nr</div><div class="ios-val">${escHtml(d.ord.style_number || '—')}</div>
            <div class="ios-lbl">Fit Name</div><div class="ios-val">${escHtml(d.fittingBlockName || '—')}</div>
          </div>
          <div class="col">
            <div class="ios-lbl">PO#</div><div class="ios-val">${escHtml(topPO)}</div>
            <div class="ios-lbl">Ship Date</div><div class="ios-val">${escHtml(fmtDate(d.ord.ship_date))}</div>
          </div>
        </div>

        <div class="ios-section-title">Order BreakDown</div>
        <table class="ios-table">
          <thead>
            <tr class="ios-blackhead">
              <th style="width:230px">Size Group + Wash</th>
              <th style="width:230px">Wash / Colour</th>
              ${sizes.map(sz => `<th class="ios-center">${escHtml(sz)}</th>`).join('')}
              <th class="ios-center">Total</th>
            </tr>
          </thead>
          <tbody>
            <tr>
              <td class="ios-breakdown-left" rowspan="4">${escHtml(d.groupName || '—')}<br/>${escHtml(d.washName || '—')}</td>
              <td class="ios-breakdown-mid">Order Quantity</td>
              ${orderQtyCells}
              <td class="ios-center">${escHtml(Math.round(d.q.qty || 0))}</td>
            </tr>
            <tr>
              <td class="ios-breakdown-mid">Ratio</td>
              ${ratioCells}
              <td class="ios-center">${escHtml(d.ratioTotal || 0)}</td>
            </tr>
            <tr>
              <td class="ios-breakdown-mid">Cutting Qty <span style="font-size:22px;font-weight:700">+${escHtml(parseFloat(d.cuttingPct || 0).toFixed(0))}%</span></td>
              ${cutCells}
              <td class="ios-center">${escHtml(Math.round(d.cuttingQtyTotal || 0))}</td>
            </tr>
            <tr>
              <td class="ios-breakdown-mid">Zipper Usage/Size</td>
              ${zipperCells}
              <td></td>
            </tr>
          </tbody>
        ...
        <div class="ios-section-title">Materials &amp; Trims Requirements</div>
        <table class="ios-table">
          <thead>
            <tr class="ios-blackhead">
              <th>Item</th><th>Shade</th><th>Content</th><th>Weight</th><th>Width</th><th>Consump</th><th>Required</th>
            </tr>
          </thead>
          <tbody>${fabricRows}</tbody>
        </table>

        <table class="ios-table" style="font-size:10px;margin-bottom:4px"><thead><tr class="ios-blackhead"><th style="width:11%">Type</th><th style="width:19%">Item</th><th style="width:25%">Shade</th><th style="width:17%">Consump</th><th style="width:15%">Req</th><th style="width:6%">PO#</th><th style="width:7%">Status</th></tr></thead><tbody>${trimsRows}</tbody></table>

        <div class="ios-bottom">
          <div>
            <div class="ios-box-title">Embellishment &amp; Decoration</div>
            <div class="ios-box">${embellishmentText}</div>
            <table class="ios-pack-table">
              <thead><tr><th>Packing</th><th>Each</th><th>Total</th></tr></thead>
              <tbody>
                <tr><td>Blisters</td><td class="ios-center">${escHtml(d.blisterEach)}</td><td class="ios-center">${escHtml(d.blisterTotal)}</td></tr>
                <tr><td>Cartons</td><td class="ios-center">${escHtml(d.cartonEach)}</td><td class="ios-center">${escHtml(d.cartonTotal)}</td></tr>
              </tbody>
            </table>
          </div>
          <div>
            <div class="ios-box-title">Packing Instructions</div>
            <div class="ios-box">
              <div class="ios-tickline"><span class="ios-tick"></span> Solid Pack</div>
              <div class="ios-tickline"><span class="ios-tick"></span> Ratio Pack</div>
            </div>
          </div>
          <div>
            <div class="ios-box-title">Merchandiser</div>
            <div class="ios-box"></div>
          </div>
        </div>
      </div>`
  }
  ...
  function coverHTML(d) {
    const sizeCols = Object.keys(d.sizeQtyMap || {})
    const sizeHead = sizeCols.map(sz => `<th class="center">${esc(sz)}</th>`).join('')
    const sizeVals = sizeCols.map(sz => `<td class="center">${esc(d.sizeQtyMap[sz] || 0)}</td>`).join('')
    const stitch = d.trimsForIOS.filter(x => !x.spacer && x.type === 'Stitching')
    const finish = d.trimsForIOS.filter(x => !x.spacer && x.type === 'Packing')
    const fabric = d.fabricItems
    const packSummary = [`Blisters: ${d.blisterTotal}`, `Cartons: ${d.cartonTotal}`].join('<br/>') || '—'
    return `
    <div class="page">
      <div style="font-family:Arial,sans-serif;font-size:11px;color:#111">
        ...
        <div style="display:grid;grid-template-columns:1.2fr 1fr;gap:0;border:1px solid #333;border-bottom:none">
          <div>
            ...
            ${rowsHTML(stitch.slice(0,10), [{label:'Item', key:'item'},{label:'Shade/Code', key:'shade'},{label:'Cons/PC', key:'consump'},{label:'Req Qty', key:'req', align:'right'}])}
          </div>
          <div>
            <div style="background:#4b5563;color:#fff;font-weight:700;padding:4px 8px">FABRICATION</div>
            ${rowsHTML(fabric.slice(0,8), [{label:'Code', render:r=>r.name},{label:'Cons', render:r=>parseFloat(r.consump)||0},{label:'Required', render:r=>Math.ceil(r.q_qty||0), align:'right'}])}
        ...
        <div style="display:grid;grid-template-columns:1.2fr 1fr;gap:0;border:1px solid #333;border-top:none">
          <div>
            ...
            ${rowsHTML(finish.slice(0,16), [{label:'Item', key:'item'},{label:'Shade/Code', key:'shade'},{label:'Cons', key:'consump'},{label:'Req Qty', key:'req', align:'right'}])}
          </div>
          <div>
            <div style="background:#4b5563;color:#fff;font-weight:700;padding:4px 8px">CUTTING QTY</div>
            <table><thead><tr><th>Allowance</th><th class="right">Cut Qty</th></tr></thead><tbody><tr><td>${esc(d.cuttingPct)}%</td><td class="right">${esc(Math.ceil(d.cuttingQtyTotal))}</td></tr></tbody></table>
  ...
  async function buildProgramHTML(kind, q) {
    ...
    if (kind === 'Print Production File') {
      return [
        ...
        sectionDocHTML('Cutting Plan', d, Object.keys(d.sizeQtyMap).map(sz => ({ size: sz, qty: d.sizeQtyMap[sz] })), [{label:'Size', key:'size'},{label:'Qty', key:'qty', align:'right'}], `Cutting qty with allowance (${d.cuttingPct}%): ${Math.ceil(d.cuttingQtyTotal)} pcs`),
        sectionDocHTML('Trims Demand', d, d.trimsForIOS.filter(x => !x.spacer).map(x => ({ name: x.item, category: x.type, specification: x.shade, unit: '', req: x.req })), [{label:'Item', key:'name'},{label:'Category', key:'category'},{label:'Specification', render:r=>r.specification||'—'},{label:'Unit', key:'unit'},{label:'Required', render:r=>r.req, align:'right'}]),
        sectionDocHTML('Washing', d, [{ wash_type: d.washName, wash_ref: d.washName, color_name: d.q.color_name || '—', unit: 'pcs', q_qty: d.q.qty }], [{label:'Wash Type', key:'wash_type'},{label:'Wash Ref', key:'wash_ref'},{label:'Color', key:'color_name'},{label:'Unit', key:'unit'},{label:'Qty', render:r=>Math.ceil(r.q_qty||0), align:'right'}]),
        sectionDocHTML('Embellishment', d, (d.embellishmentLines || []).map(x => ({ description: x, technique: '', placement: '', vendor_id: '—', q_qty: d.q.qty })), [{label:'Description', render:r=>r.description||'Embellishment'},{label:'Technique', key:'technique'},{label:'Placement', key:'placement'},{label:'Vendor', render:r=>r.vendor_id||'—'},{label:'Qty', render:r=>Math.ceil(r.q_qty||0), align:'right'}]),
        sectionDocHTML('Finishing', d, [{ pack_name: 'Blisters', pack_basis: '—', inner_pack_type: '—', pieces_per_inner_pack: d.blisterEach, pcs_per_carton: d.cartonEach, cartons: d.cartonTotal }, { pack_name: 'Cartons', pack_basis: '—', inner_pack_type: '—', pieces_per_inner_pack: d.blisterEach, pcs_per_carton: d.cartonEach, cartons: d.cartonTotal }], [{label:'Pack', render:r=>r.pack_name||'Pack'},{label:'Basis', render:r=>r.pack_basis||'—'},{label:'Inner Pack', render:r=>r.inner_pack_type||'—'},{label:'Pcs/Inner', render:r=>r.pieces_per_inner_pack || '—', align:'right'},{label:'Pcs/Carton', key:'pcs_per_carton', align:'right'},{label:'Cartons', key:'cartons', align:'right'}]),
      ].join('')
    }
    const map = {
      'Internal Order Sheet (IOS)': () => buildIOSHTML(d),
      ...
      'Cutting Plan': () => sectionDocHTML('Cutting Plan', d, Object.keys(d.sizeQtyMap).map(sz => ({ size: sz, qty: d.sizeQtyMap[sz] })), [{label:'Size', key:'size'},{label:'Qty', key:'qty', align:'right'}], `Cutting qty with allowance (${d.cuttingPct}%): ${Math.ceil(d.cuttingQtyTotal)} pcs`),
      'Trims Demand': () => sectionDocHTML('Trims Demand', d, d.trimsForIOS.filter(x => !x.spacer).map(x => ({ name: x.item, category: x.type, specification: x.shade, unit: '', req: x.req })), [{label:'Item', key:'name'},{label:'Category', key:'category'},{label:'Specification', render:r=>r.specification||'—'},{label:'Unit', key:'unit'},{label:'Required', render:r=>r.req, align:'right'}]),
      'Washing': () => sectionDocHTML('Washing', d, [{ wash_type: d.washName, wash_ref: d.washName, color_name: d.q.color_name || '—', unit: 'pcs', q_qty: d.q.qty }], [{label:'Wash Type', key:'wash_type'},{label:'Wash Ref', key:'wash_ref'},{label:'Color', key:'color_name'},{label:'Unit', key:'unit'},{label:'Qty', render:r=>Math.ceil(r.q_qty||0), align:'right'}]),
      'Embellishment': () => sectionDocHTML('Embellishment', d, (d.embellishmentLines || []).map(x => ({ description: x, technique: '', placement: '', vendor_id: '—', q_qty: d.q.qty })), [{label:'Description', render:r=>r.description||'Embellishment'},{label:'Technique', key:'technique'},{label:'Placement', key:'placement'},{label:'Vendor', render:r=>r.vendor_id||'—'},{label:'Qty', render:r=>Math.ceil(r.q_qty||0), align:'right'}]),
      'Finishing': () => sectionDocHTML('Finishing', d, [{ pack_name: 'Blisters', pack_basis: '—', inner_pack_type: '—', pieces_per_inner_pack: d.blisterEach, pcs_per_carton: d.cartonEach, cartons: d.cartonTotal }, { pack_name: 'Cartons', pack_basis: '—', inner_pack_type: '—', pieces_per_inner_pack: d.blisterEach, pcs_per_carton: d.cartonEach, cartons: d.cartonTotal }], [{label:'Pack', render:r=>r.pack_name||'Pack'},{label:'Basis', render:r=>r.pack_basis||'—'},{label:'Inner Pack', render:r=>r.inner_pack_type||'—'},{label:'Pcs/Inner', render:r=>r.pieces_per_inner_pack || '—', align:'right'},{label:'Pcs/Carton', key:'pcs_per_carton', align:'right'},{label:'Cartons', key:'cartons', align:'right'}]),
    }
  ...
  function exportQueuesPDF() {
    ...
    const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Queues Export</title><style>@page{size:A4 portrait;margin:6mm;}body{font-family:Arial,sans-serif;padding:24px} table{width:100%;border-collapse:collapse} th,td{border:1px solid #ddd;padding:8px;font-size:12px;text-align:left} th{background:#f5f5f5}</style></head><body><h2>Queue List</h2><table><thead><tr><th>Q #</th><th>Label</th><th>Split Rule</th><th>Style</th><th>Buyer</th><th>Qty</th><th>Status</th></tr></thead><tbody>${rows}</tbody></table></body></html>`
    ...
    <div style={{ flex: 1, overflowY: 'auto', padding: '16px 24px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
        ...
        <div style={{ marginLeft:'auto', display:'flex', gap:8, alignItems:'center' }}>
          <button className="btn btn-sm" style={{ background: selectedQueuedOnly.length === 0 ? '#e5e7eb' : '#16a34a', color: selectedQueuedOnly.length === 0 ? '#9ca3af' : '#fff', border: '1px solid ' + (selectedQueuedOnly.length === 0 ? '#e5e7eb' : '#16a34a') }} disabled={selectedQueuedOnly.length === 0} onClick={handleActivateQueues}>Activate Q</button>
          <button className="btn btn-sm" style={{ background: selectedOrderIds.length === 0 ? '#f3f4f6' : '#fee2e2', color: selectedOrderIds.length === 0 ? '#9ca3af' : '#dc2626', border: '1px solid ' + (selectedOrderIds.length === 0 ? '#e5e7eb' : '#fecaca') }} disabled={selectedOrderIds.length === 0} onClick={handleRollbackOrderQueues}>Reset Queue</button>
