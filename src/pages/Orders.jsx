...
function JobsTab({ onEditOrder }) {
  ...
  async function loadAll() {
    ...
    const [{ data: js }, { data: ors }] = await Promise.all([
      ...
      supabase.from('orders').select('id,job_id,job_number,buyer_name,style_number,po_number,ship_date,status,total_qty,total_value_usd,season,merchandiser_name,style_image_base64,description').order('ship_date'),
    ])
    setJobs(js || [])
    setOrders(ors || [])
    setLoading(false)
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', gap: 8 }}>
          <button className="btn btn-secondary" style={{ display: 'flex', alignItems: 'center', gap: 6 }}
            onClick={() => {
              ...
              const cols = ['Job','Style','Buyer','PO #','Season','Qty','Value USD','Ship Date','Status']
              const rows = filtered.map(o => [o.job_number||'',o.style_number||'',o.buyer_name||'',o.po_number||'',o.season||'',o.total_qty||'',o.total_value_usd||'',o.ship_date||'',o.status||''])
