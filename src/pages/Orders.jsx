...
function QueuesTab({ onEditOrder }) {
  ...
  async function loadAll() {
    ...
    const [{ data: qs }, { data: ors }] = await Promise.all([
      ...
      supabase.from('orders').select('id,style_number,buyer_name,job_number,brand_name').order('ship_date'),
    ])
    setQueues(qs || [])
    setOrders(ors || [])
    setLoading(false)
  ...
  return (
    <div style={{ flex: 1, overflowY: 'auto', padding: '16px 24px' }}>
      ...
      {selected && (() => {
        const q = queues.find(x => x.id === selected)
        const o = q ? orderMap[q.order_id] : null
        if (!q) return null
        return (
          <div style={{ marginTop: 16, background: '#fff', border: '1px solid #e8e8e6', borderRadius: 8, padding: '16px 20px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
              <span style={{ fontFamily: 'var(--font-mono)', fontWeight: 800, fontSize: 15 }}>{q.q_number}</span>
              <span style={{ fontSize: 13, fontWeight: 600 }}>{q.label}</span>
              <span style={{ fontSize: 11, color: '#9ca3af' }}>{q.split_rule}</span>
              <span style={{ fontFamily: 'var(--font-mono)', fontSize: 13, fontWeight: 700, color: '#0d0d0d', marginLeft: 'auto' }}>{q.qty?.toLocaleString()} pcs</span>
            </div>
            <div style={{ display: 'flex', gap: 8 }}>
              {['Fabric Demand', 'Cutting Plan', 'Trims Demand', 'Washing', 'Embellishment', 'Finishing'].map(prog => (
                <button key={prog} className="btn btn-secondary btn-sm" style={{ fontSize: 11 }}
                  onClick={() => alert(`${prog} report for ${q.q_number} — coming soon`)}>
                  {prog}
                </button>
              ))}
            </div>
