...
function QueuesTab({ onEditOrder }) {
  ...
  function buildIOSHTML(d) {
    ...
    return `
      <div class="ios-page">
        <style>
          .ios-page{width:210mm;min-height:297mm;padding:14px 18px 18px 18px;font-family:"Times New Roman",serif;color:#000;background:#fff;box-sizing:border-box;}
          .ios-topline{display:flex;justify-content:space-between;font-size:14px;line-height:1.1;margin-bottom:4px;}
          .ios-brandrow{display:grid;grid-template-columns:1.2fr 0.55fr 0.55fr 0.8fr;align-items:start;column-gap:14px;margin-bottom:10px;}
          .ios-brand h1{font-size:34px;line-height:0.95;margin:6px 0 4px 0;font-weight:700;letter-spacing:-0.5px;}
          .ios-brand .sub{font-size:22px;font-weight:400;}
          .ios-oval-wrap{display:flex;justify-content:center;align-items:flex-start;padding-top:14px;}
          .ios-oval{width:110px;height:58px;border:4px solid #000;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:34px;font-weight:700;letter-spacing:-1px;}
          .ios-qr-wrap{display:flex;justify-content:center;padding-top:10px;}
          .ios-qno{font-size:86px;line-height:0.85;font-weight:700;text-align:right;padding-top:2px;white-space:nowrap;}
          .ios-info{display:grid;grid-template-columns:1fr 1fr 1fr;column-gap:26px;margin-bottom:10px;}
          .ios-info .col{display:grid;grid-template-columns:auto 1fr;column-gap:18px;row-gap:4px;align-items:start;}
          .ios-lbl{font-size:20px;font-weight:700;white-space:nowrap;}
          .ios-val{font-size:18px;white-space:nowrap;}
          .ios-section-title{font-size:30px;font-weight:700;line-height:1;margin:8px 0 6px 0;}
          table.ios-table{width:100%;border-collapse:collapse;margin:0 0 14px 0;font-size:18px;}
          .ios-table th,.ios-table td{border:2px solid #000;padding:4px 8px;vertical-align:top;}
          .ios-table thead th{background:#000;color:#fff;font-size:17px;font-weight:700;line-height:1.1;}
          .ios-subhead th{background:#d9d9d9 !important;color:#8f96a3 !important;font-size:16px !important;letter-spacing:2px;}
          .ios-breakdown-left{width:220px;font-size:18px;font-weight:700;line-height:1.15;}
          .ios-breakdown-mid{width:220px;font-size:18px;font-weight:700;}
          .ios-center{text-align:center;white-space:nowrap;}
          .ios-right{text-align:right;white-space:nowrap;}
          .ios-blackhead th{background:#000;color:#fff;font-size:17px;}
          .ios-spacer td{height:18px;padding:0;background:#fff;}
          .ios-bottom{display:grid;grid-template-columns:1.15fr 0.95fr 0.95fr;column-gap:20px;align-items:start;margin-top:6px;}
          .ios-box-title{font-size:22px;font-weight:700;line-height:1;margin-bottom:6px;}
          .ios-box{border:2px solid #000;min-height:130px;padding:10px;font-size:18px;}
          .ios-pack-table{width:100%;border-collapse:collapse;font-size:18px;margin-top:14px;}
          .ios-pack-table th,.ios-pack-table td{border:2px solid #000;padding:6px 8px;}
          .ios-pack-table thead th{background:#d9d9d9;color:#8f96a3;font-size:16px;letter-spacing:2px;text-align:left;}
          .ios-tickline{display:flex;align-items:center;gap:10px;margin:8px 0;font-size:18px;font-weight:700;}
          .ios-tick{width:16px;height:16px;border:2px solid #000;display:inline-block;}
        </style>
        ...
        <table class="ios-table">
          <thead>
            <tr class="ios-blackhead">
              <th>Type</th><th>Item</th><th>Shade</th><th>Consump</th><th>Req</th><th>PO#</th><th>Status</th>
            </tr>
          </thead>
          <tbody>${trimsRows}</tbody>
        </table>
  ...
  async function printSelectedPrograms(kind) {
    ...
    try {
      ...
      printHTML(parts.join(''), printWin)
    } catch (err) {
      printWin.document.open()
      printWin.document.write(`<!DOCTYPE html><html><body style="font-family:Arial,sans-serif;padding:24px">Could not prepare print.</body></html>`)
      printWin.document.close()
    }
  }
  ...
  function exportQueuesPDF() {
    ...
    const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Queues Export</title><style>body{font-family:Arial,sans-serif;padding:24px} table{width:100%;border-collapse:collapse} th,td{border:1px solid #ddd;padding:8px;font-size:12px;text-align:left} th{background:#f5f5f5}</style></head><body><h2>Queue List</h2><table><thead><tr><th>Q #</th><th>Label</th><th>Split Rule</th><th>Style</th><th>Buyer</th><th>Qty</th><th>Status</th></tr></thead><tbody>${rows}</tbody></table></body></html>`
