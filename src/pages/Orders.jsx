...
function JobsTab({ onEditOrder }) {
  ...
  async function loadAll() {
    ...
    const [{ data: js }, { data: ors }] = await Promise.all([
      ...
      supabase.from('orders').select('id,job_id,job_number,buyer_name,style_number,po_number,ship_date,status,total_qty,total_value_usd,factory_ref,brand_name,merchandiser_name,style_image_base64,description').order('ship_date'),
    ])
    setJobs(js || [])
    setOrders(ors || [])
    setLoading(false)
...
export default function Orders() {
  ...
  async function loadOrders() {
    setLoading(true)
    const { data } = await supabase.from('orders').select('id,job_id,job_number,buyer_name,buyer_id,style_number,po_number,ship_date,planned_date,status,total_qty,total_value_usd,factory_ref,brand_name,store_name,merchandiser_name,style_image_base64,description,created_at').order('ship_date', { ascending: true, nullsLast: true })
    setOrders(data || [])
    setLoading(false)
  }
  ...
  async function duplicateOrder(o, e) {
    ...
    for (const sg of (sgs || [])) {
      ...
      const { data: colors } = await supabase.from('size_group_colors').select('*').eq('size_group_id', sg.id).order('sort_order')
