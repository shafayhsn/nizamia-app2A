...
function QueuesTab({ onEditOrder }) {
  ...
  const selectedQueues = [...selectedIds].map(id => queues.find(x => x.id === id)).filter(Boolean)
              ...
              {filtered.map(q => {
                ...
                return (
                  <tr key={q.id} style={{ background: isSelected ? '#f0f9ff' : '' }}>
                    ...
                    <td><span style={{ fontSize: 10, fontWeight: 600, padding: '2px 7px', borderRadius: 4, background: statusBg[q.status] || '#fafaf8', color: statusColor[q.status] || '#6b7280' }}>{q.status}</span></td>
                  </tr>
                )
              })}
            </tbody>
...
export default function Orders() {
  ...
  const totalValue   = openFiltered.reduce((s, o) => s + (parseFloat(o.total_value_usd) || 0), 0)
