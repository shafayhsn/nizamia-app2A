...
function QueuesTab({ onEditOrder }) {
  ...
  const queueComponentPayload = (q) => {
    const raw = q?.custom_components ?? String(q?.label || '').split('__APP2A_QC__')[1] ?? ''
  ...
  async function fetchQDocData(q) {
    ...
    const customKeys = queueComponentPayload(q).split('|').map(x => x.trim()).filter(Boolean)
    const isCustomRule = String(q.split_rule || '').trim().toLowerCase().includes('custom')
    const isCustomQueue = isCustomRule
    const customComponentSpecs = []
    const keyMatchesCustomComponent = (key, groupName, colorName) => customKeys.some(k => {
      if (k === key) return true
      const kn = earlyNorm(k)
      return kn.includes(earlyNorm(groupName)) && kn.includes(earlyNorm(colorName))
    })
    ...
    qSgs.forEach((g, gi) => {
      ...
      groupColors.forEach((c, ci) => {
        ...
        const shouldUseCustomComponent = isCustomQueue && customKeys.length && keyMatchesCustomComponent(key, g.group_name, c.color_name)
        if (!isCustomQueue) return
        ...
        if (componentQty > 0) {
          ...
          allCustomCandidates.push(spec)
          if (shouldUseCustomComponent) customComponentSpecs.push(spec)
        }
      })
    })


    // Safety fallback for older rows / DBs where custom_components was not saved.
    // Rebuild selected custom queue by matching queue qty against colour/wash × size-group components.
    if (isCustomQueue && customComponentSpecs.length === 0 && allCustomCandidates.length) {
      const targetQty = parseFloat(q.qty) || 0
      const cents = (n) => Math.round((parseFloat(n) || 0) * 100)
      const target = cents(targetQty)
      let found = null
      const walk = (idx, sum, picked) => {
        if (found) return
        if (sum === target && picked.length) { found = picked.slice(); return }
        if (idx >= allCustomCandidates.length || sum > target) return
        walk(idx + 1, sum + cents(allCustomCandidates[idx].qty), [...picked, allCustomCandidates[idx]])
        walk(idx + 1, sum, picked)
      }
      if (target > 0 && allCustomCandidates.length <= 22) walk(0, 0, [])
      ;(found || allCustomCandidates.filter(x => targetQty <= 0 || x.qty <= targetQty)).forEach(x => customComponentSpecs.push(x))
    }
    const groupName = isCustomQueue ? (cleanQLabel || 'Custom Batch') : (selectedGroupSizes.length ? (cleanQLabel || rawGroupName) : rawGroupName)

    const sizeMap = {}
    if (isCustomQueue) {
      customComponentSpecs.forEach(comp => {
    ...
    const sizeOrder = isCustomQueue
      ? [...new Set(customComponentSpecs.flatMap(comp => comp.sizes || []).filter(sz => (parseFloat(sizeMap[String(sz)]) || 0) > 0))]
      : (effectiveSizes.length ? effectiveSizes : (qSgs.length ? ((qSgs[0].sizes || []).map(String)) : Object.keys(sizeMap)))
    ...
    const getQueueContext = (queue) => {
      ...
      const includeCell = (sgName, colorName) => {
        if (splitRule === 'Custom' && customComponentSpecs.length) {
          return customComponentSpecs.some(comp => norm(comp.groupName) === norm(sgName) && norm(comp.colorName) === norm(colorName))
        }
...
const libMap = Object.fromEntries((libs || []).map(x => [x.id, x]))
    ...
    const variantBlocks = customComponentSpecs.length
      ? customComponentSpecs.map(comp => {
    ...
    return {
      ord, q: { ...q, label: cleanQLabel }, groupName, fitName, washName, sizeMap, sizeOrder, ratioMap, variantBlocks,
  ...
  function buildIOSHTML(d) {
    ...
    const breakdownRows = variantBlocks.map((b) => {
      const blockSizeMap = b.sizeMap || {}
      const blockRatioMap = b.ratioMap || {}
      const blockQty = parseFloat(b.qty) || sizeCols.reduce((sum, sz) => sum + (parseFloat(blockSizeMap[sz]) || 0), 0)
      const blockCutting = parseFloat(b.cuttingQty) || sizeCols.reduce((sum, sz) => sum + Math.round((parseFloat(blockSizeMap[sz]) || 0) * (1 + (parseFloat(d.cuttingPct) || 0) / 100)), 0)
      const ratioTotal = sizeCols.reduce((sum, sz) => sum + (parseFloat(blockRatioMap[sz]) || 0), 0)
      return `
            <tr class="ios-variant-block">
              <td style="${tdC};font-weight:500;" rowspan="4" colspan="2">${formatVariantLabel(b)}</td>
              <td style="${td};font-weight:700;white-space:nowrap;">Order Quantity</td>
              ${sizeCols.map(sz => `<td style="${tdC}">${esc(blockSizeMap[sz] || 0)}</td>`).join('')}
              <td style="${tdC};font-weight:700;">${esc(blockQty || 0)}</td>
            </tr>
            <tr class="ios-variant-block">
              <td style="${td};font-weight:700;">Ratio</td>
              ${sizeCols.map(sz => `<td style="${tdC}">${esc(blockRatioMap[sz] || 0)}</td>`).join('')}
              <td style="${tdC}">${ratioTotal}</td>
            </tr>
            <tr class="ios-variant-block">
              <td style="${td};font-weight:700;">Cutting Qty +${esc(d.cuttingPct || 0)}%</td>
              ${sizeCols.map(sz => `<td style="${tdC}">${Math.round((parseFloat(blockSizeMap[sz]) || 0) * (1 + (parseFloat(d.cuttingPct) || 0) / 100))}</td>`).join('')}
    ...
    const breakdownTotalRow = isMultiVariant ? `
            <tr>
              <td style="${td};font-weight:900;background:#f3f4f6;text-align:right;" colspan="3">QUEUE TOTAL</td>
              ${sizeCols.map(sz => `<td style="${tdC};font-weight:900;background:#f3f4f6;">${esc(d.sizeMap[sz] || 0)}</td>`).join('')}
              <td style="${tdC};font-weight:900;background:#f3f4f6;">${esc(variantTotalQty)}</td>
            </tr>
            <tr>
              <td style="${td};font-weight:900;background:#f3f4f6;text-align:right;" colspan="3">CUTTING TOTAL</td>
              ${sizeCols.map(sz => `<td style="${tdC};font-weight:900;background:#f3f4f6;">${Math.round((parseFloat(d.sizeMap[sz]) || 0) * (1 + (parseFloat(d.cuttingPct) || 0) / 100))}</td>`).join('')}
              <td style="${tdC};font-weight:900;background:#f3f4f6;">${Math.ceil(variantTotalCutting)}</td>
            </tr>` : ''
