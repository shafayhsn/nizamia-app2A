...
import { Search, Printer, Layers, Plus, X, Check, AlertCircle, Copy, Upload, Download, FileText, Table, Trash2, RefreshCcw, Settings, Lock, GripVertical, CheckCircle2, Archive } from 'lucide-react'
...
import { statusBadgeStyle, statusDotStyle } from '../lib/statusColors'
            ...
            return (
              <div key={job.id} style={{ background: '#fff', border: '1px solid #e8e8e6', borderRadius: 8, marginBottom: 8, overflow: 'hidden' }}>
                <div onClick={() => toggleExpand(job.id)}
                  ...
                  <span style={{ fontFamily: 'monospace', fontSize: 13, fontWeight: 700, color: '#0d0d0d', minWidth: 100 }}>{job.job_number}</span>
...
function OldQueuesTab({ onEditOrder }) {
  ...
  const selectedQueuedOnly = selectedQueues.filter(q => { const qn = String(q.q_number || '').trim().toLowerCase(); return !qn || qn === 'queued' || qn === 'nil' || qn === '-' })
  ...
  async function handleRollbackOrderQueues() {
    ...
    if (selectedOrderIds.length !== 1) {
      window.alert('Reset Queue works on one order at a time. Select any one Q from the order you want to reset.')
      return
    }
    const orderId = selectedOrderIds[0]
    const proceed = window.confirm('This will delete all queues and reset the queue setup for this order. This action cannot be undone.')
    if (!proceed) return
    ...
    const { error: orderErr } = await supabase.from('orders').update({ queue_split_rule: 'None' }).eq('id', orderId)
    if (orderErr) { window.alert(orderErr.message || 'Queue setup reset failed.'); return }
...
const libMap = Object.fromEntries((libs || []).map(x => [x.id, x]))
    ...
    const firstPackRaw = (packs || [])[0] || {}
    const firstPackMeta = firstPackRaw.config?.__packMeta || {}
    const firstPack = {
      ...firstPackRaw,
      pieces_per_inner_pack: firstPackRaw.pieces_per_inner_pack || firstPackRaw.inner_pieces || firstPackMeta.pieces_per_inner_pack,
      pcs_per_carton: firstPackRaw.pcs_per_carton || firstPackRaw.units_per_carton,
    }
    ...
    return {
      ...
      finishingPacks: (packs || []).map(p => {
        const cfg = p.config || {}
        const meta = cfg.__packMeta || {}
        return {
          ...p,
          pack_basis: p.pack_basis || p.pack_type || meta.packing_basis || 'By Q#',
          inner_pack_type: p.inner_pack_type || meta.inner_pack_type || '—',
          pieces_per_inner_pack: p.pieces_per_inner_pack || p.inner_pieces || meta.pieces_per_inner_pack || '',
          pcs_per_carton: p.pcs_per_carton || p.units_per_carton || '',
          carton_style: p.carton_style || p.hang_flat || meta.carton_style || 'Flat Packed',
        }
      }),
      packs: packs,
    ...
    <div class="page">
      <div style="font-family:Arial,sans-serif;font-size:11px;color:#111">
        <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8px">
          ...
          <div style="font-size:42px;font-weight:700;line-height:1">${esc(d.q.q_number || 'Queued')}</div>
        </div>
  ...
  function buildIOSHTML(d) {
    if (d.customQueueMissingComponents) {
      return `<div style="width:210mm;min-height:297mm;padding:12mm;font-family:Arial,sans-serif;color:#000;box-sizing:border-box;">
        <div style="font-size:22px;font-weight:700;margin-bottom:8mm;">IOS Print Blocked</div>
    ...
    const th  = 'border:1px solid #000;padding:2.7px 4px;font-size:9.15px;font-weight:700;text-align:center;vertical-align:middle;white-space:nowrap;line-height:1.22;'
    ...
    const formatVariantLabel = (b) => {
      ...
      return `<div style="font-weight:700;line-height:1.2;">${esc(top)}</div>${bottom ? `<div style="font-size:8.3px;font-weight:600;line-height:1.2;margin-top:1px;">${esc(bottom)}</div>` : ''}`
    }
    const renderBreakdownTable = (b) => {
      ...
      return `
        <table class="ios-component-table" style="width:100%;border-collapse:collapse;table-layout:fixed;margin-bottom:1.4mm;page-break-inside:avoid;break-inside:avoid;">
          ...
          <tbody>
            <tr>
              <td style="${tdC};font-weight:700;" rowspan="4">${formatVariantLabel(b)}</td>
    ...
    const breakdownTotalBox = isMultiVariant ? `
      <table style="width:100%;border-collapse:collapse;table-layout:fixed;margin-bottom:1mm;">
        <tbody>
          <tr>
            <td style="${td};font-weight:700;background:#f3f4f6;text-align:right;">QUEUE TOTAL</td>
            <td style="${tdC};font-weight:700;background:#f3f4f6;width:25mm;">${esc(variantTotalQty)}</td>
          </tr>
          <tr>
            <td style="${td};font-weight:700;background:#f3f4f6;text-align:right;">CUTTING TOTAL</td>
            <td style="${tdC};font-weight:700;background:#f3f4f6;width:25mm;">${Math.ceil(variantTotalCutting)}</td>
          </tr>
        </tbody>
      </table>` : ''
    ...
    return `
      ...
      <div class="ios-page" style="width:210mm;min-height:297mm;padding:5mm 4mm 4mm 4mm;font-family:Arial,sans-serif;color:#000;background:#fff;box-sizing:border-box;display:flex;flex-direction:column;gap:1mm;">
        ...
        <div style="display:grid;grid-template-columns:1fr auto 1fr;align-items:center;border-bottom:2px solid #000;padding-bottom:2mm;">
          <div>
            <div style="font-size:18px;font-weight:700;line-height:1.05;">NIZAMIA APPARELS</div>
            <div style="font-size:8px;font-weight:700;letter-spacing:1.8px;text-transform:uppercase;">Internal Order Sheet</div>
          </div>
          <div style="display:flex;justify-content:center;">
            <div style="background:#000;color:#fff;border:2px solid #000;border-radius:50%;width:13mm;height:13mm;display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:700;">IOS</div>
          </div>
          <div style="text-align:right;font-size:24px;font-weight:700;line-height:1;">${esc(qRef)}</div>
        </div>
        ...
        <div style="font-size:10.8px;font-weight:700;margin-bottom:0.7mm;">Order BreakDown</div>
        ${breakdownTables}
        ${breakdownTotalBox}

        <div style="font-size:10.8px;font-weight:700;margin-bottom:0.7mm;">Materials &amp; Trims Requirements</div>
  ...
  function buildCuttingPlanHTML(d) {
    ...
    const th = 'border:1px solid #000;padding:2.4px 4px;font-size:8.9px;font-weight:700;text-align:center;vertical-align:middle;line-height:1.15;white-space:nowrap;'
    ...
    const blockTables = blocks.map((block) => {
      ...
      return `<table style="width:100%;border-collapse:collapse;table-layout:fixed;margin-bottom:1.2mm;page-break-inside:avoid;">
        ...
        <tbody>
          <tr><td style="${tdC};font-weight:700;text-transform:uppercase;" rowspan="4">${esc(titleCase(label))}</td><td style="${td};font-weight:700;">Order Quantity</td>${qtyCells}<td style="${tdR};font-weight:700;">${fmtInt(orderTotal)}</td></tr>
          <tr><td style="${td};font-weight:700;">Ratio</td>${ratioCells}<td style="${tdR};font-weight:700;">${fmtInt(ratioTotal)}</td></tr>
          <tr><td style="${td};font-weight:700;">Cutting Qty +${esc(cuttingPct)}%</td>${cutCells}<td style="${tdR};font-weight:700;">${fmtInt(cutTotal)}</td></tr>
          <tr><td style="${td};color:#555;">Zipper Usage/Size</td>${sizes.map(() => `<td style="${tdC}">&nbsp;</td>`).join('')}<td style="${tdC}">&nbsp;</td></tr>
        </tbody>
      </table>`
    }).join('') || '<div style="border:1px solid #000;padding:6mm;font-size:12px;font-weight:700;">No queue breakdown data found.</div>'
    ...
    return `<div style="width:210mm;min-height:297mm;padding:6mm 8mm;box-sizing:border-box;font-family:Arial,sans-serif;color:#000;background:#fff;">
      ...
      <div class="cp-page" style="min-height:285mm;display:flex;flex-direction:column;">
        ...
        <div style="display:grid;grid-template-columns:1fr auto 1fr;align-items:center;border-bottom:2px solid #000;padding-bottom:2mm;margin-bottom:2.5mm;"><div><div style="font-size:21px;font-weight:700;line-height:.92;">NIZAMIA<br/>APPARELS</div></div><div style="text-align:center;font-size:21px;font-weight:700;text-transform:uppercase;">Cutting Program</div><div style="text-align:right;font-size:30px;font-weight:700;">${esc(qRef)}</div></div>
        <table style="width:100%;border-collapse:collapse;table-layout:fixed;margin-bottom:5mm;"><tr>${[['Brand', d.ord.buyer_name], ['Style Ne', d.ord.style_number], ['Fit Name', d.fitName || '—'], ['PO#', d.ord.po_number], ['Factory Ref', d.ord.factory_ref || d.ord.job_number], ['Store', d.ord.store_name], ['Ship Date', fmtDate(d.ord.ship_date)]].map(([k,v]) => `<td style="font-size:8.8px;font-weight:700;vertical-align:top;">${esc(k)}<br/><span style="font-weight:700;">${esc(v || '—')}</span></td>`).join('')}</tr></table>
        <div style="font-size:12px;font-weight:700;margin-bottom:1mm;">Order Breakdown / Order Quantity</div>
        ${blockTables}
        <div style="font-size:12px;font-weight:700;margin:2mm 0 1mm;">Fabrication Info</div>
  ...
  function buildTrimsProgramHTML(d) {
    if (d.customQueueMissingComponents) {
      return `<div style="width:210mm;min-height:297mm;padding:12mm;font-family:Arial,sans-serif;color:#000;box-sizing:border-box;">
        <div style="font-size:22px;font-weight:700;margin-bottom:8mm;">Trims Program Blocked</div>
        <div style="border:2px solid #000;padding:8mm;font-size:14px;line-height:1.5;">
          <strong>Custom queue missing component data.</strong><br/>
          Regenerate this queue after running the <code>order_queues.custom_components</code> migration.
        </div>
    ...
    const th = 'border:1px solid #000;padding:2.2px 3px;font-size:8.4px;font-weight:700;text-align:center;vertical-align:middle;line-height:1.12;white-space:nowrap;'
    ...
    const parentTd = td + 'background:#e6e6e6;font-weight:700;color:#000;'
    const parentTdC = tdC + 'background:#e6e6e6;font-weight:700;color:#000;'
    const parentTdR = tdR + 'background:#e6e6e6;font-weight:700;color:#000;'
    ...
    return `<div style="width:210mm;min-height:297mm;padding:6mm 8mm;box-sizing:border-box;font-family:Arial,sans-serif;color:#000;background:#fff;"><style>@page{size:A4;margin:0}body{margin:0;background:#fff}*{box-sizing:border-box}tr{page-break-inside:avoid}.tp-page{page-break-after:auto}</style><div class="tp-page" style="min-height:285mm;display:flex;flex-direction:column;"><div style="display:grid;grid-template-columns:1fr 1fr 1fr;align-items:start;font-size:8.5px;margin-bottom:3mm;"><div>Document Ref#TP-${esc(qRef)}</div><div style="text-align:center;">Date Created: ${new Date().toLocaleDateString('en-GB')} - ${new Date().toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'})}</div><div style="text-align:right;">Username: admin</div></div><div style="display:grid;grid-template-columns:1fr auto 1fr;align-items:center;border-bottom:2px solid #000;padding-bottom:2mm;margin-bottom:2.5mm;"><div><div style="font-size:21px;font-weight:700;line-height:.92;">NIZAMIA<br/>APPARELS</div></div><div style="text-align:center;font-size:21px;font-weight:700;text-transform:uppercase;">Trims Program</div><div style="text-align:right;font-size:30px;font-weight:700;">${esc(qRef)}</div></div><table style="width:100%;border-collapse:collapse;table-layout:fixed;margin-bottom:5mm;"><tr>${[['Brand', d.ord.buyer_name], ['Style Ne', d.ord.style_number], ['Fit Name', d.fitName || '—'], ['PO#', d.ord.po_number], ['Factory Ref', d.ord.factory_ref || d.ord.job_number], ['Store', d.ord.store_name], ['Ship Date', fmtDate(d.ord.ship_date)]].map(([k,v]) => `<td style="font-size:8.8px;font-weight:700;vertical-align:top;">${esc(k)}<br/><span style="font-weight:700;">${esc(v || '—')}</span></td>`).join('')}</tr></table><div style="font-size:12px;font-weight:700;margin-bottom:1mm;">Order Breakdown / Order Quantity</div>${blockTables}<div style="font-size:12px;font-weight:700;margin:2mm 0 1mm;">Trims Detailed Sheet</div><table style="width:100%;border-collapse:collapse;table-layout:fixed;margin-bottom:4mm;"><colgroup><col style="width:8%"/><col style="width:14%"/><col style="width:27%"/><col style="width:8%"/><col style="width:9%"/><col style="width:10%"/><col style="width:7%"/><col style="width:8%"/><col style="width:4.5%"/><col style="width:4.5%"/></colgroup><thead><tr>${['Type','Item','Shade / Description','Consump','Pack','Req','PO#','Status','Approval','Testing'].map(h => `<th style="${th}${blk};font-size:7.6px;white-space:normal;letter-spacing:.8px;">${h}</th>`).join('')}</tr></thead><tbody>${trimRows}</tbody></table><table style="width:100%;border-collapse:collapse;table-layout:fixed;margin-top:auto;"><tr>${['Merchandiser','Store Manager','General Manager','Director'].map(lbl => `<td style="border:1px solid #000;height:20mm;position:relative;text-align:center;font-size:11px;font-weight:700;"><div style="position:absolute;left:0;right:0;bottom:3mm;">${esc(lbl)}</div></td>`).join('')}</tr></table></div></div>`
  }
    ...
    <div style={{ flex: 1, overflowY: 'auto', padding: '16px 24px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
        ...
        <div style={{ marginLeft:'auto', display:'flex', gap:8, alignItems:'center' }}>
          ...
          <button className="btn btn-secondary btn-sm" disabled={selectedQueues.length === 0} onClick={() => setShowPrintModal(true)}>Print Reports</button>
          <div style={{ position:'relative' }}>
            <button className="btn btn-secondary btn-sm app-icon-only" disabled={selectedQueues.length === 0} onClick={() => setShowExportMenu(v => !v)} title="Export"><Download size={13}/></button>
              ...
              {filtered.map(q => {
                ...
                return (
                  <tr key={q.id} style={{ background: isSelected ? '#f0f9ff' : '' }}>
                    ...
                    <td>
                      <span style={{ fontFamily: 'var(--font-mono)', fontWeight: 700, fontSize: 13, color: '#0d0d0d', background: '#f3f4f6', padding: '2px 8px', borderRadius: 5 }}>
                        {q.q_number || 'Queued'}
                      </span>
                    </td>
                    ...
                    <td><span style={statusBadgeStyle((q.balance_qty <= 0 && q.shipped_qty > 0) ? 'Completed' : (q.shipped_qty > 0 ? 'Partially Shipped' : q.status))}>{(q.balance_qty <= 0 && q.shipped_qty > 0) ? 'Completed' : (q.shipped_qty > 0 ? 'Partially Shipped' : q.status)}</span></td>
                  </tr>
                )
              })}
            </tbody>
        ...
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.4)', zIndex:300, display:'flex', alignItems:'center', justifyContent:'center' }}>
          <div style={{ background:'#fff', borderRadius:10, padding:24, width:360, boxShadow:'0 16px 48px rgba(0,0,0,0.2)' }}>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
              {['Queue Summary', 'Internal Order Sheet (IOS)', 'Fabric Demand', 'Cutting Plan', 'Trims Demand', 'Washing', 'Embellishment', 'Finishing', 'Print Production File'].map(prog => (
                <button key={prog} className="btn btn-secondary btn-sm" style={{ justifyContent:'flex-start' }} onClick={async () => { setShowPrintModal(false); if (prog === 'Queue Summary') { exportQueuesPDF() } else { await printSelectedPrograms(prog) } }}>{prog}</button>
              ))}
            </div>
...
function DragColumnSettings({ title='Columns', subtitle='Drag to reorder fields', labels, order, visible, onChange, onReset, onClose }) {
  ...
  return <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.35)', zIndex:300, display:'flex', alignItems:'center', justifyContent:'center' }} onClick={onClose}>
    <div style={{ background:'#fff', borderRadius:12, width:460, maxHeight:'80vh', overflow:'hidden', boxShadow:'0 18px 48px rgba(0,0,0,0.18)', display:'flex', flexDirection:'column' }} onClick={e=>e.stopPropagation()}>
      <div style={{ padding:'16px 18px', borderBottom:'1px solid #f3f4f6', display:'flex', justifyContent:'space-between' }}><div><div style={{ fontSize:15, fontWeight:700 }}>{title}</div><div style={{ fontSize:11, color:'#9ca3af' }}>{subtitle}</div></div><button className="btn btn-ghost" onClick={onClose}><X size={16}/></button></div>
      <div style={{ padding:14, display:'flex', flexDirection:'column', gap:8, overflowY:'auto' }}>{order.map(k=><div key={k} draggable onDragStart={()=>setDragKey(k)} onDragOver={e=>e.preventDefault()} onDrop={()=>moveKey(dragKey,k)} onDragEnd={()=>setDragKey(null)} style={{ display:'flex', alignItems:'center', gap:10, padding:'8px 10px', border:'1px solid #f3f4f6', borderRadius:8, background:dragKey===k?'#f9fafb':'#fff', cursor:'grab' }}><span style={{ color:'#9ca3af', fontWeight:700, fontSize:14 }}>⋮⋮</span><input type="checkbox" checked={!!visible[k]} onChange={()=>onChange({ order, visible:{...visible,[k]:!visible[k]} })}/><span style={{ flex:1, fontSize:12, fontWeight:700 }}>{labels[k] || k}</span></div>)}</div>
...
function QueuesTab({ onEditOrder }) {
  ...
  useEffect(() => { loadAll() }, [])
  useEffect(() => {
    const refreshQueues = (event) => {
      if (!event || event.key === 'nzm_tab_notices_v1' || event.key === 'app2a.queue.refresh') loadAll()
    }
  ...
  const isActiveQueue = (q) => {
    const qn = String(q?.q_number || '').trim().toLowerCase()
    return !!qn && qn !== 'queued' && qn !== 'nil' && qn !== '-'
  }
  const activeRows = rows.filter(isActiveQueue)
  const inactiveRows = rows.filter(q => !isActiveQueue(q))
  ...
  const cellText = (q, key) => {
    const o = q.order || {}
    if (key === 'q_number') return isActiveQueue(q) ? q.q_number : 'Queued'
  ...
  const sortVal = (q, col) => col === 'q_number' ? (isActiveQueue(q) ? (parseInt(String(q.q_number || '').replace(/\D/g,'')) || 999999) : 999999) : col === 'qty' || col === 'shipped_qty' || col === 'balance_qty' ? parseFloat(q[col]) || 0 : String(cellText(q,col)).toLowerCase()
  const activeFilterBuyer = buyers.includes(filterBuyer) ? filterBuyer : ''
  const activeFilterStyle = styles.includes(filterStyle) ? filterStyle : ''
  const activeFilterJob = jobs.includes(filterJob) ? filterJob : ''
  const activeFilterRule = rules.includes(filterRule) ? filterRule : ''
  const activeFilterStatus = statuses.includes(filterStatus) ? filterStatus : ''
  const filtered = rows.filter(q => {
    const term = String(search || '').trim().toLowerCase()
    return (!term || DEFAULT_QUEUE_COLUMN_ORDER.some(k => String(cellText(q,k)).toLowerCase().includes(term))) &&
      (!activeFilterBuyer || q.order?.buyer_name === activeFilterBuyer) && (!activeFilterStyle || q.order?.style_number === activeFilterStyle) &&
      (!activeFilterJob || q.order?.job_number === activeFilterJob) && (!activeFilterRule || q.split_rule === activeFilterRule) && (!activeFilterStatus || q.derived_status === activeFilterStatus)
  }).sort((a,b) => {
  ...
  const selectedQueues = selected
  const selectedActive = selected.filter(isActiveQueue)
  const selectedInactive = selected.filter(q => !isActiveQueue(q))
  ...
  async function runAction(mode = actionMode) {
    ...
    if (mode === 'reset') {
      const orderIds = [...new Set(selected.map(q => q.order_id || q.order?.id).filter(Boolean))]
      ...
      if (!window.confirm('This will delete all queues and reset the queue setup for this order. This action cannot be undone.')) return
      const { error: delErr } = await supabase.from('order_queues').delete().eq('order_id', orderId)
      if (delErr) { window.alert(delErr.message || 'Queue reset failed.'); return }
      const { error: orderErr } = await supabase.from('orders').update({ queue_split_rule:'None' }).eq('id', orderId)
      if (orderErr) { window.alert(orderErr.message || 'Queue setup reset failed.'); return }
  ...
  useEffect(() => {
    if (filterBuyer && !buyers.includes(filterBuyer)) setFilterBuyer('')
    if (filterStyle && !styles.includes(filterStyle)) setFilterStyle('')
    if (filterJob && !jobs.includes(filterJob)) setFilterJob('')
    if (filterRule && !rules.includes(filterRule)) setFilterRule('')
    if (filterStatus && !statuses.includes(filterStatus)) setFilterStatus('')
  }, [queues.length, orders.length, filterBuyer, filterStyle, filterJob, filterRule, filterStatus])

  const groupValue = q => groupBy === 'style_number' ? (q.order?.style_number || 'Unassigned') : groupBy === 'job_number' ? (q.order?.job_number || 'Unassigned') : groupBy === 'buyer_name' ? (q.order?.buyer_name || 'Unassigned') : groupBy === 'q_number' ? (isActiveQueue(q) ? q.q_number : 'Queued') : groupBy === 'ship_date' ? (q.order?.ship_date || 'No Delivery Date') : ''
  ...
  const thStyle = { padding:'9px 8px', borderBottom:'1px solid #f3f4f6', fontSize:10, fontWeight:700, color:'#9ca3af', textTransform:'uppercase', letterSpacing:'0.35px', whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', flex:1, minHeight:0, background:'#fff' }}>
      <div style={{ padding:'12px 24px', borderBottom:'1px solid #f3f4f6', display:'flex', alignItems:'center', gap:14, flexShrink:0, minWidth:0 }}>
        <div style={{ flexShrink:0 }}><div className="app-page-title">Queue</div><div className="app-page-subtitle">Production queue status and shipment tracking</div></div>
        <div style={{ display:'flex', gap:8, alignItems:'stretch', minWidth:0, flex:'1 1 auto', overflow:'hidden' }}>
          {[{l:'Total Queues',v:rows.length,q:totalQty},{l:'Active Queues',v:activeRows.length,q:activeQty},{l:'Inactive Queues',v:inactiveRows.length,q:inactiveQty}].map(k => <div key={k.l} className="app-kpi-card" style={{ border:'1px solid var(--border)', borderRadius:8, background:'#fff', minWidth:100 }}><div style={{ fontSize:9, fontWeight:600, color:'var(--text-light)', textTransform:'uppercase', letterSpacing:'0.5px', marginBottom:3 }}>{k.l}</div><div style={{ display:'flex', alignItems:'baseline', gap:8 }}><div style={{ fontSize:18, fontWeight:700, lineHeight:1 }}>{k.v}</div><div style={{ fontSize:10, color:'#6b7280', fontWeight:500 }}>{Number(k.q||0).toLocaleString()} pcs</div></div></div>)}
          <div className="app-kpi-card" style={{ border:'1px solid var(--border)', borderRadius:8, background:'#fff', minWidth:155 }}>
            <div style={{ fontSize:9, fontWeight:600, color:'var(--text-light)', textTransform:'uppercase', letterSpacing:'0.5px', marginBottom:3 }}>Shipped Progress</div>
            <div style={{ display:'flex', alignItems:'baseline', gap:8 }}><div style={{ fontSize:18, fontWeight:700, lineHeight:1 }}>{pct}%</div><div style={{ fontSize:10, color:'#6b7280', fontWeight:700 }}>{shippedQty.toLocaleString()} / {totalQty.toLocaleString()} pcs</div></div>
        ...
        <div style={{ marginLeft:'auto', display:'flex', gap:8, alignItems:'center', flexShrink:0 }}><label className="btn btn-secondary" style={{ height:34, display:'inline-flex', alignItems:'center', gap:7, padding:'0 12px' }}><input type="checkbox" checked={multiSelect} onChange={e=>{ setMultiSelect(e.target.checked); setSelectedIds(new Set()) }} /> Multi-select</label>{multiSelect && <button className="btn btn-secondary" style={{ height:34, padding:'0 12px' }} disabled={filtered.length === 0} onClick={toggleAll}>{filtered.length && filtered.every(q=>selectedIds.has(q.id)) ? 'Clear' : 'Select Filtered'}</button>}{multiSelect && <span style={{ fontSize:11, fontWeight:700, color:selected.size?'#111827':'#9ca3af', minWidth:72 }}>{selected.length} selected</span>}<button className="btn btn-secondary app-icon-only" disabled={filtered.length === 0} onClick={exportCSV} title="Export"><Download size={14}/></button><button className="btn btn-secondary" style={{ height:34, padding:'0 14px' }} disabled={selectedQueues.length === 0} onClick={() => setShowPrintModal(true)}>Print Reports</button><button className="btn btn-primary" style={{ height:34, padding:'0 14px', background:selectedInactive.length && selectedInactive.length === selected.length ? '#16a34a' : '#e5e7eb', color:selectedInactive.length && selectedInactive.length === selected.length ? '#fff' : '#9ca3af', borderColor:'transparent', minWidth:112 }} disabled={!(selectedInactive.length && selectedInactive.length === selected.length)} title={selected.length && selectedInactive.length !== selected.length ? 'Select only inactive/queued rows to activate' : ''} onClick={()=>runAction('activate')}>Activate Queue</button><button className="btn btn-secondary" style={{ height:34, padding:'0 14px', background:selected.length ? '#fee2e2' : '#f3f4f6', color:selected.length ? '#dc2626' : '#9ca3af', borderColor:'transparent', minWidth:100 }} disabled={!selected.length} title={selected.length ? 'Reset the whole queue split for this order' : 'Select any Q to reset its order'} onClick={()=>runAction('reset')}>Reset Queue</button></div>
      </div>
      <div style={{ padding:'8px 24px', borderBottom:'1px solid #f3f4f6', display:'flex', alignItems:'center', gap:8, flexShrink:0, overflow:'hidden', minWidth:0 }}>
        ...
        <button className="btn btn-secondary" style={{ height:32, width:42, padding:0, marginLeft:'auto', flex:'0 0 42px', display:'inline-flex', alignItems:'center', justifyContent:'center', lineHeight:1 }} onClick={()=>setShowCols(true)} title="Column Settings"><Settings size={15} /></button>
      </div>
      <div style={{ flex:1, overflowY:'auto', overflowX:'hidden', minWidth:0 }}>{displayRows.length === 0 ? <div style={{ textAlign:'center', padding:60, color:'#9ca3af', fontSize:12 }}>{rows.length ? 'No queues match the current filters.' : 'No queues yet.'}</div> : <table style={{ width:'100%', borderCollapse:'collapse', tableLayout:'fixed' }}><thead><tr style={{ background:'#fafafa', position:'sticky', top:0, zIndex:10 }}>{multiSelect && <th style={{ width:36, padding:'9px 0 9px 14px', borderBottom:'1px solid #f3f4f6' }}><input type="checkbox" checked={filtered.length>0 && filtered.every(q=>selectedIds.has(q.id))} onChange={toggleAll}/></th>}{visibleKeys.map(k=><th key={k} style={{ ...thStyle, textAlign:['qty','shipped_qty','balance_qty'].includes(k)?'right':'left' }}>{QUEUE_COLUMN_LABELS[k]}</th>)}</tr></thead><tbody>{displayRows.map(row=>{ if(row.type==='group') return <tr key={`g-${row.key}`} style={{ background:'var(--app-group-row-bg, #fdf6e3)', color:'var(--app-group-row-text, #111827)' }}><td colSpan={(multiSelect?1:0)+visibleKeys.length} style={{ padding:'10px 18px', borderBottom:'1px solid #eee', borderTop:'1px solid #eee' }}><div style={{ display:'flex', justifyContent:'space-between', fontSize:12, fontWeight:700 }}><span>{row.key} <span style={{ color:'#9ca3af' }}>— {row.count} queues</span></span><span>Qty: {row.qty.toLocaleString()} · Shipped: {row.shipped.toLocaleString()}</span></div></td></tr>; const q=row.q, isSel=selectedIds.has(q.id); return <tr key={q.id} style={{ background:isSel?'#f0f9ff':'' }}>{multiSelect && <td style={{ padding:'10px 0 10px 14px', borderBottom:'1px solid #f3f4f6' }}><input type="checkbox" checked={isSel} onChange={()=>toggleSelect(q.id)}/></td>}{visibleKeys.map(k=><React.Fragment key={k}>{renderCell(q,k)}</React.Fragment>)}</tr> })}</tbody></table>}</div>
      {showPrintModal && (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.4)', zIndex:300, display:'flex', alignItems:'center', justifyContent:'center' }}>
          <div style={{ background:'#fff', borderRadius:10, padding:24, width:380, boxShadow:'0 16px 48px rgba(0,0,0,0.2)' }}>
            <div style={{ fontSize:16, fontWeight:700, marginBottom:4 }}>Queue Print Center</div>
            <div style={{ fontSize:12, color:'#6b7280', marginBottom:14 }}>{selectedQueues.length} selected queue(s)</div>
            <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
              {['Queue Summary', 'Internal Order Sheet (IOS)', 'Fabric Demand', 'Cutting Plan', 'Trims Demand', 'Washing', 'Embellishment', 'Finishing', 'Print Production File'].map(prog => (
                <button key={prog} className="btn btn-secondary btn-sm" style={{ justifyContent:'flex-start' }} onClick={async () => { setShowPrintModal(false); if (prog === 'Queue Summary') { printList() } else { await printSelectedPrograms(prog) } }}>{prog}</button>
              ))}
            </div>
            <div style={{ display:'flex', justifyContent:'flex-end', marginTop:16 }}><button className="btn btn-secondary" onClick={() => setShowPrintModal(false)}>Close</button></div>
          </div>
...
// ── Order Metadata / A4 Execution Sheet ──────────────────────────────────────
function OrderMetadataModal({ order, onClose, onEdit }) {
  const [data, setData] = useState(null)
  const [loading, setLoading] = useState(true)

  const escHtml = (v) => String(v ?? '—').replace(/[&<>"']/g, m => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]))
  const nfmt = (v) => { const n = Number(v); return Number.isFinite(n) ? n.toLocaleString(undefined, { maximumFractionDigits: 2 }) : (v === null || v === undefined || v === '' ? '—' : String(v)) }
  const clean = (v) => (v === null || v === undefined || v === '' ? '—' : v)

  useEffect(() => { if (order?.id) load() }, [order?.id])
  ...
  async function load() {
    setLoading(true)
    const [sizeGroups, bom, samples, fits, embs, pos] = await Promise.all([
      safeQuery(supabase.from('size_groups').select('*').eq('order_id', order.id).order('sort_order')),
      safeQuery(supabase.from('bom_items').select('*').eq('order_id', order.id).order('sort_order')),
      safeQuery(supabase.from('samples').select('*').eq('order_id', order.id).order('created_at')),
      safeQuery(supabase.from('fitting_blocks').select('*').eq('order_id', order.id).order('sort_order')),
      safeQuery(supabase.from('embellishments').select('*').eq('order_id', order.id).order('created_at')),
      safeQuery(supabase.from('purchase_orders').select('id,po_number,status,created_at,purchase_order_items(*)').eq('order_id', order.id).order('created_at', { ascending:false })),
    ])

    const sgIds = sizeGroups.map(g => g.id)
    const [colors, breakdown, wash] = sgIds.length ? await Promise.all([
      safeQuery(supabase.from('size_group_colors').select('*').in('size_group_id', sgIds).order('sort_order')),
      safeQuery(supabase.from('size_group_breakdown').select('*').in('size_group_id', sgIds)),
      safeQuery(supabase.from('washing').select('*').eq('order_id', order.id)),
    ]) : [[], [], []]

    const poItems = pos.flatMap(po => (po.purchase_order_items || []).map(i => ({ ...i, po_number: po.po_number, po_status: po.status, po_created_at: po.created_at })))
    setData({ sizeGroups, colors, breakdown, wash, bom, samples, fits, embs, pos, poItems })
    setLoading(false)
  }
  ...
  const sizeCols = []
  ;(data?.sizeGroups || []).forEach(g => (g.sizes || []).forEach(sz => { if (!sizeCols.includes(sz)) sizeCols.push(sz) }))
  const colorWashRows = []
  ;(data?.sizeGroups || []).forEach(g => {
    const gColors = (data.colors || []).filter(c => c.size_group_id === g.id)
    const washes = data.wash || []
    gColors.forEach(c => {
      const row = { id:c.id, color:c.color_name, wash:(washes.find(w => w.color_id === c.id || w.color_name === c.color_name)?.wash_type || washes.find(w => w.color_name === c.color_name)?.wash_ref || '—'), size_group:g.group_name }
      sizeCols.forEach(sz => { row[sz] = (data.breakdown || []).filter(b => b.color_id === c.id && b.size === sz).reduce((sum,b)=>sum+(Number(b.qty)||0),0) })
      row.qty = sizeCols.reduce((sum,sz)=>sum+(row[sz]||0),0)
      row.extra = Math.round(row.qty * (Number(order.excess_cutting_pct || 5) / 100))
      row.cut = row.qty + row.extra
      colorWashRows.push(row)
    })
  })
  const totalBySize = Object.fromEntries(sizeCols.map(sz => [sz, colorWashRows.reduce((s,r)=>s+(r[sz]||0),0)]))
  const totalQty = colorWashRows.reduce((s,r)=>s+r.qty,0) || Number(order.total_qty || 0)
  const totalExtra = colorWashRows.reduce((s,r)=>s+r.extra,0)
  const totalCut = colorWashRows.reduce((s,r)=>s+r.cut,0)

  const sampleRows = (data?.samples || []).map(s => ({
    type: s.sample_type || 'Sample',
    colourWash: `${(Array.isArray(s.colours) ? s.colours.join(', ') : s.colours) || 'All'} / ${Array.isArray(s.wash_snapshot) && s.wash_snapshot.length ? s.wash_snapshot.map(w=>w.wash_type || w.wash_ref || w.color_name).filter(Boolean).join(', ') : '—'}`,
    size: Array.isArray(s.sizes) ? s.sizes.join(', ') : (s.base_size || s.sizes || '—'),
    qty: s.req_pcs || '—',
    status: s.status || 'Pending'
  }))

  const fitRows = (data?.fits || []).map(f => ({
    name: [f.block_name, f.spec_code].filter(Boolean).join(' / ') || '—',
    usage: (() => {
      const ud = normalizeUsageData(f.usage_data)
      if (Array.isArray(ud.sizeGroups)) return ud.sizeGroups.join(', ')
      if (Array.isArray(ud.sizes)) return ud.sizes.join(', ')
      if (Array.isArray(ud.colors)) return ud.colors.join(', ')
      return f.usage_rule || 'Generic'
    })()
  }))

  const artworkRows = (data?.embs || []).map(e => ({
    name: e.description || e.technique || e.name || 'Artwork',
    usage: [e.placement, Array.isArray(e.applies_to) ? e.applies_to.join(', ') : e.applies_to].filter(Boolean).join(' / ') || '—'
  }))

  const usageNumericLocal = (v) => {
    if (v == null) return 0
    if (typeof v === 'number') return Number.isFinite(v) ? v : 0
    if (typeof v === 'string') return parseFloat(v) || 0
    if (typeof v === 'object') return parseFloat(v.value ?? v.qty ?? v.consumption ?? v.cons ?? v.amount ?? v.base_qty ?? 0) || 0
    return 0
  }
  const calcBomRequired = (b) => {
    const explicit = Number(b.final_qty)
    if (Number.isFinite(explicit) && explicit > 0) return explicit
    const base = Number(b.base_qty) || 0
    const wastage = 1 + ((Number(b.wastage) || 0) / 100)
    const rule = String(b.usage_rule || 'Generic')
    const ud = normalizeUsageData(b.usage_data) || {}
    const allSizes = sizeCols.map(sz => ({ size: sz, qty: totalBySize[sz] || 0 }))
    const colorTotals = Object.fromEntries(colorWashRows.map(r => [String(r.color || '').toLowerCase(), r.qty || 0]))
    const groupTotals = {}
    ;(data?.sizeGroups || []).forEach(g => {
      const colorsInGroup = (data.colors || []).filter(c => c.size_group_id === g.id).map(c => c.id)
      groupTotals[g.group_name] = (data.breakdown || []).filter(x => colorsInGroup.includes(x.color_id)).reduce((s,x)=>s+(Number(x.qty)||0),0)
    })
    if (rule === 'Generic') return base > 0 ? base * (totalQty || Number(order.total_qty) || 0) * wastage : 0
    if (rule === 'By Color' || rule === 'By Colour') return Object.entries(ud).reduce((s,[k,v]) => s + usageNumericLocal(v) * (colorTotals[String(k).toLowerCase()] || 0) * wastage, 0)
    if (rule === 'By Size Group') return Object.entries(ud).reduce((s,[k,v]) => s + usageNumericLocal(v) * (groupTotals[k] || 0) * wastage, 0)
    if (rule === 'By Individual Sizes' || rule === 'By Individual Size') return Object.entries(ud).reduce((s,[k,v]) => s + usageNumericLocal(v) * ((allSizes.find(x => String(x.size) === String(k)) || {}).qty || 0) * wastage, 0)
    if (rule === 'Configure Own' && Array.isArray(ud.__groups)) return ud.__groups.reduce((s,g) => s + usageNumericLocal(g.consumption) * (g.sizes || []).reduce((q,sz)=>q+(totalBySize[sz]||0),0) * wastage, 0)
    if (rule === 'Configure Own' && Array.isArray(ud.__matrix)) return ud.__matrix.reduce((s,m) => {
      const row = colorWashRows.find(r => String(r.color).toLowerCase() === String(m.colorName).toLowerCase())
      return s + usageNumericLocal(m.consumption) * (row?.qty || 0) * wastage
    }, 0)
    return base || 0
  }

  const bomRows = (data?.bom || []).map((b, idx) => {
    const reqQty = calcBomRequired(b)
    const key = String(b.name || b.detail || b.specification || '').trim().toLowerCase()
    const poMatches = (data.poItems || []).filter(i =>
      String(i.source_id || '') === String(b.id || '') ||
      [i.description, i.name, i.specification].some(x => key && String(x || '').toLowerCase().includes(key))
    )
    const poQty = poMatches.reduce((s,i)=>s+(Number(i.qty)||0),0)
    const poNo = poMatches.map(i=>i.po_number).filter(Boolean)[0] || '—'
    const rawStatus = poMatches.map(i=>i.po_status).filter(Boolean)[0] || b.po_status || ''
    const status = poMatches.length ? (poQty > 0 && reqQty > 0 && poQty < reqQty ? 'PARTIAL' : (rawStatus || 'ISSUED')) : 'No PO'
    return { no:idx+1, desc:b.name || b.detail || b.category || 'Material', spec:b.specification || b.detail || '—', uom:b.unit || '—', req:reqQty || '—', po:poNo, poQty:poQty || '—', status }
  })

  const badgeStyle = st => {
    const s = String(st || '').toLowerCase()
    const good = s.includes('approved') || s.includes('received') || s.includes('complete')
    const warn = s.includes('partial') || s.includes('pending') || s.includes('issue')
    return { display:'inline-flex', alignItems:'center', justifyContent:'center', minWidth:60, padding:'2px 7px', borderRadius:4, fontSize:10, fontWeight:800, color: good ? '#166534' : warn ? '#c2410c' : '#374151', background: good ? '#dcfce7' : warn ? '#ffedd5' : '#f3f4f6' }
  }
  const th = { padding:'7px 8px', border:'1px solid #e5e7eb', background:'#f8fafc', fontSize:10, fontWeight:800, color:'#111827', textTransform:'uppercase', textAlign:'left' }
  const td = { padding:'7px 8px', border:'1px solid #e5e7eb', fontSize:11, fontWeight:600, color:'#111827' }
  const sectionHead = (title, bg='#0f2f5f') => <div style={{ background:bg, color:'#fff', fontSize:12, fontWeight:900, letterSpacing:'.3px', padding:'7px 10px', textTransform:'uppercase' }}>{title}</div>

  function buildPrintHtml() {
    const table = (headers, rows) => `<table><thead><tr>${headers.map(h=>`<th>${escHtml(h)}</th>`).join('')}</tr></thead><tbody>${rows}</tbody></table>`
    const qtyRows = colorWashRows.map(r => `<tr><td>${escHtml(r.color)}</td><td>${escHtml(r.wash)}</td>${sizeCols.map(sz=>`<td>${escHtml(r[sz]||'')}</td>`).join('')}<td>${escHtml(r.qty)}</td><td>${escHtml(r.extra)}</td><td>${escHtml(r.cut)}</td></tr>`).join('') + `<tr class="total"><td colspan="2">TOTAL</td>${sizeCols.map(sz=>`<td>${escHtml(totalBySize[sz]||'')}</td>`).join('')}<td>${escHtml(totalQty)}</td><td>${escHtml(totalExtra)}</td><td>${escHtml(totalCut)}</td></tr>`
    const sampleHtml = sampleRows.map(r=>`<tr><td>${escHtml(r.type)}</td><td>${escHtml(r.colourWash)}</td><td>${escHtml(r.size)}</td><td>${escHtml(r.qty)}</td><td>${escHtml(r.status)}</td></tr>`).join('') || '<tr><td colspan="5">No sample requirements</td></tr>'
    const fitHtml = fitRows.map(r=>`<tr><td>${escHtml(r.name)}</td><td>${escHtml(r.usage)}</td></tr>`).join('') || '<tr><td colspan="2">No fit data</td></tr>'
    const artHtml = artworkRows.map(r=>`<tr><td>${escHtml(r.name)}</td><td>${escHtml(r.usage)}</td></tr>`).join('') || '<tr><td colspan="2">No artwork data</td></tr>'
    const bomHtml = bomRows.slice(0,12).map(r=>`<tr><td>${r.no}</td><td>${escHtml(r.desc)}</td><td>${escHtml(r.spec)}</td><td>${escHtml(r.uom)}</td><td>${escHtml(r.req)}</td><td>${escHtml(r.po)}</td><td>${escHtml(r.poQty)}</td><td>${escHtml(r.status)}</td></tr>`).join('') || '<tr><td colspan="8">No BOM data</td></tr>'
    return `<div class="a4"><style>@page{size:A4 portrait;margin:0}body{margin:0;background:#fff}.a4{width:210mm;min-height:297mm;padding:8mm;box-sizing:border-box;font-family:Arial,sans-serif;color:#0f172a}h1{font-size:24px;margin:0 0 6mm;font-weight:900}.actions{display:none}.meta{display:grid;grid-template-columns:repeat(7,1fr);border:1px solid #d1d5db;margin-bottom:4mm}.cell{padding:3mm 2mm;border-right:1px solid #d1d5db}.cell:last-child{border-right:0}.k{font-size:7px;font-weight:800;text-transform:uppercase}.v{font-size:12px;font-weight:900;margin-top:2mm;color:#0b3ea8}.sec{background:#08285a;color:white;font-size:10px;font-weight:900;padding:2mm 3mm;text-transform:uppercase;margin-top:3mm}table{width:100%;border-collapse:collapse;table-layout:fixed}th{font-size:7.2px;text-transform:uppercase;background:#f8fafc;border:1px solid #d1d5db;padding:1.8mm}td{font-size:7.6px;border:1px solid #d1d5db;padding:1.8mm;font-weight:700}.total td{background:#eff6ff;color:#0b3ea8;font-weight:900}.grid2{display:grid;grid-template-columns:1.05fr .95fr;gap:3mm;margin-top:3mm}.sub{margin:0}.foot{font-size:7px;color:#6b7280;margin-top:3mm;display:flex;justify-content:space-between}</style><h1>ORDER METADATA</h1><div class="meta">${[['STYLE #',order.style_number],['PO #',order.po_number],['JOB #',order.job_number || order.job_no || '—'],['BUYER',order.buyer_name],['SHIP DATE',fmtDate(order.ship_date)],['QUEUE SPLIT',order.queue_split_rule || '—'],['FACTORY REF',order.factory_ref]].map(([k,v])=>`<div class="cell"><div class="k">${escHtml(k)}</div><div class="v">${escHtml(v)}</div></div>`).join('')}</div><div class="sec">Order Quantity (PCS)</div>${table(['Colour','Wash',...sizeCols,'Order Qty','Extra Cut','Total Cut'], qtyRows)}<div class="grid2"><div><div class="sec sub" style="background:#064e3b">Sample Requirement Matrix</div>${table(['Sample Type','Colour / Wash','Size','Qty','Status'], sampleHtml)}</div><div><div class="sec sub" style="background:#3b246b">Fit Matrix</div>${table(['Fit / Block Name','Size Group Usage'], fitHtml)}<div class="sec sub" style="background:#7c3f00;margin-top:3mm">Artwork / Embellishment</div>${table(['Artwork Name','Usage / Placement'], artHtml)}</div></div><div class="sec">Material / BOM Snapshot</div>${table(['#','Material','Specification','UOM','Required','PO #','PO Qty','Status'], bomHtml)}<div class="foot"><span>Created By: ADMIN</span><span>Last Updated: ${new Date().toLocaleString()}</span></div></div>`
  }

  return (
    <div style={{ position:'fixed', inset:0, background:'rgba(15,23,42,.45)', zIndex:420, display:'flex', justifyContent:'center', alignItems:'flex-start', padding:'18px 0', overflowY:'auto' }}>
      <div style={{ width:'210mm', minHeight:'297mm', background:'#fff', boxShadow:'0 24px 80px rgba(0,0,0,.28)', padding:'7mm', boxSizing:'border-box', fontFamily:'Inter, Arial, sans-serif' }}>
        <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:14 }}>
          <h1 style={{ margin:0, fontSize:26, fontWeight:900, letterSpacing:-.5, color:'#0f172a', flex:1 }}>ORDER METADATA</h1>
          <button className="btn btn-primary" onClick={() => onEdit(order)}><FileText size={14}/> Edit Order</button>
          <button className="btn btn-secondary" onClick={() => printHTML(buildPrintHtml())}><Printer size={14}/> Print Sheet</button>
          <button className="btn btn-secondary" onClick={onClose}><X size={15}/> Close</button>
        </div>

        <div style={{ display:'grid', gridTemplateColumns:'repeat(7, 1fr)', border:'1px solid #d1d5db', borderRadius:6, overflow:'hidden', marginBottom:12 }}>
          {[["STYLE #",order.style_number],["PO #",order.po_number],["JOB #",order.job_number || order.job_no || "—"],["BUYER",order.buyer_name],["SHIP DATE",fmtDate(order.ship_date)],["QUEUE SPLIT",order.queue_split_rule || '—'],["FACTORY REF",order.factory_ref]].map(([k,v],i)=>(
            <div key={k} style={{ padding:'10px 8px', borderRight:i===6?'none':'1px solid #d1d5db', textAlign:'center' }}><div style={{ fontSize:9, fontWeight:900, color:'#111827' }}>{k}</div><div style={{ marginTop:6, fontSize:15, fontWeight:900, color:i===0||i===4?'#0b3ea8':'#111827' }}>{clean(v)}</div></div>
          ))}
        </div>

        {loading ? <div style={{ padding:50, textAlign:'center', color:'#9ca3af', fontSize:12 }}>Loading metadata...</div> : <>
          <div style={{ border:'1px solid #d1d5db', borderRadius:6, overflow:'hidden', marginBottom:10 }}>
            {sectionHead('Order Quantity (PCS)')}
            <table style={{ width:'100%', borderCollapse:'collapse', tableLayout:'fixed' }}><thead><tr><th style={th}>Colour</th><th style={th}>Wash</th>{sizeCols.map(sz=><th key={sz} style={{...th,textAlign:'center'}}>{sz}</th>)}<th style={{...th,textAlign:'center'}}>Order Qty</th><th style={{...th,textAlign:'center'}}>Extra Cut</th><th style={{...th,textAlign:'center'}}>Total Cut</th></tr></thead><tbody>{colorWashRows.map(r=><tr key={r.id}><td style={td}>{r.color}</td><td style={td}>{r.wash}</td>{sizeCols.map(sz=><td key={sz} style={{...td,textAlign:'center'}}>{r[sz]||''}</td>)}<td style={{...td,textAlign:'center',fontWeight:900}}>{nfmt(r.qty)}</td><td style={{...td,textAlign:'center'}}>{nfmt(r.extra)}</td><td style={{...td,textAlign:'center',fontWeight:900}}>{nfmt(r.cut)}</td></tr>)}<tr><td style={{...td,background:'#eff6ff',color:'#0b3ea8',fontWeight:900}} colSpan={2}>TOTAL</td>{sizeCols.map(sz=><td key={sz} style={{...td,textAlign:'center',background:'#eff6ff',color:'#0b3ea8',fontWeight:900}}>{nfmt(totalBySize[sz])}</td>)}<td style={{...td,textAlign:'center',background:'#eff6ff',color:'#0b3ea8',fontWeight:900}}>{nfmt(totalQty)}</td><td style={{...td,textAlign:'center',background:'#eff6ff',color:'#0b3ea8',fontWeight:900}}>{nfmt(totalExtra)}</td><td style={{...td,textAlign:'center',background:'#eff6ff',color:'#0b3ea8',fontWeight:900}}>{nfmt(totalCut)}</td></tr></tbody></table>
          </div>

          <div style={{ display:'grid', gridTemplateColumns:'1.1fr .9fr', gap:10, marginBottom:10 }}>
            <div style={{ border:'1px solid #d1d5db', borderRadius:6, overflow:'hidden' }}>{sectionHead('Sample Requirement Matrix','#064e3b')}<table style={{ width:'100%', borderCollapse:'collapse', tableLayout:'fixed' }}><thead><tr>{['Sample Type','Colour / Wash','Size','Qty','Status'].map(h=><th key={h} style={th}>{h}</th>)}</tr></thead><tbody>{sampleRows.length ? sampleRows.map((r,i)=><tr key={i}><td style={td}>{r.type}</td><td style={td}>{r.colourWash}</td><td style={{...td,textAlign:'center'}}>{r.size}</td><td style={{...td,textAlign:'center'}}>{r.qty}</td><td style={{...td,textAlign:'center'}}><span style={badgeStyle(r.status)}>{r.status}</span></td></tr>) : <tr><td style={td} colSpan={5}>No sample requirements</td></tr>}</tbody></table></div>
            <div style={{ display:'grid', gridTemplateRows:'1fr 1fr', gap:10 }}>
              <div style={{ border:'1px solid #d1d5db', borderRadius:6, overflow:'hidden' }}>{sectionHead('Fit Matrix (By Size Group)','#3b246b')}<table style={{ width:'100%', borderCollapse:'collapse', tableLayout:'fixed' }}><thead><tr><th style={th}>Fit / Block Name</th><th style={th}>Size Group Usage</th></tr></thead><tbody>{fitRows.length ? fitRows.map((r,i)=><tr key={i}><td style={td}>{r.name}</td><td style={td}>{r.usage}</td></tr>) : <tr><td style={td} colSpan={2}>No fit data</td></tr>}</tbody></table></div>
              <div style={{ border:'1px solid #d1d5db', borderRadius:6, overflow:'hidden' }}>{sectionHead('Artwork / Embellishment','#7c3f00')}<table style={{ width:'100%', borderCollapse:'collapse', tableLayout:'fixed' }}><thead><tr><th style={th}>Artwork Name</th><th style={th}>Usage / Placement</th></tr></thead><tbody>{artworkRows.length ? artworkRows.map((r,i)=><tr key={i}><td style={td}>{r.name}</td><td style={td}>{r.usage}</td></tr>) : <tr><td style={td} colSpan={2}>No artwork data</td></tr>}</tbody></table></div>
            </div>
          </div>

          <div style={{ border:'1px solid #d1d5db', borderRadius:6, overflow:'hidden' }}>{sectionHead('Material / BOM Snapshot')}<table style={{ width:'100%', borderCollapse:'collapse', tableLayout:'fixed' }}><thead><tr>{['#','Material Description','Specification','UOM','Required','PO #','PO Qty','Status'].map((h,i)=><th key={h} style={{...th,width:i===0?34:undefined}}>{h}</th>)}</tr></thead><tbody>{bomRows.length ? bomRows.slice(0,12).map(r=><tr key={r.no}><td style={{...td,textAlign:'center'}}>{r.no}</td><td style={td}>{r.desc}</td><td style={td}>{r.spec}</td><td style={{...td,textAlign:'center'}}>{r.uom}</td><td style={{...td,textAlign:'center'}}>{nfmt(r.req)}</td><td style={td}>{r.po}</td><td style={{...td,textAlign:'center'}}>{r.poQty}</td><td style={{...td,textAlign:'center'}}><span style={badgeStyle(r.status)}>{r.status}</span></td></tr>) : <tr><td style={td} colSpan={8}>No BOM data</td></tr>}</tbody></table></div>
        </>}
      </div>
    </div>
  )
...
function BulkOrderImportModal({ buyers, existingOrders, downloadImportTemplate, onClose, onImported }) {
  const [step, setStep] = useState(1)
  const [fileName, setFileName] = useState('')
  const [rows, setRows] = useState([])
  const [saving, setSaving] = useState(false)
  const [message, setMessage] = useState(null)
  const buyerMap = Object.fromEntries((buyers || []).map(b => [String(b.name || '').trim().toLowerCase(), b.id]))
  const existingKeys = new Set((existingOrders || []).flatMap(o => [
    o.po_number ? `po:${String(o.po_number).trim().toLowerCase()}` : null,
    o.factory_ref ? `factory:${String(o.factory_ref).trim().toLowerCase()}` : null,
  ].filter(Boolean)))
  const headers = ['Style Number*','Buyer Name*','Brand Name','Store Name','PO Number','Factory Ref','Description','Order Qty','Value USD','Currency','Ship Mode','Incoterms','Port of Loading','Port of Discharge','Ex-Factory Date (YYYY-MM-DD)','In-Store Date (YYYY-MM-DD)','Merchandiser','Agent','Notes']
  const keyOf = (h) => String(h || '').toLowerCase().replace(/[^a-z0-9]/g, '')
  const getIdx = (heads, names) => {
    const norm = heads.map(keyOf)
  ...
  const readCell = (cells, idx) => idx >= 0 ? String(cells[idx] ?? '').trim() : ''
  const buildRows = (matrix) => {
    if (!matrix || matrix.length < 2) return []
    const heads = matrix[0].map(v => String(v || '').trim())
    const idx = {
      style: getIdx(heads, ['Style Number','Style']), buyer: getIdx(heads, ['Buyer Name','Buyer']), brand: getIdx(heads, ['Brand Name','Brand']), store: getIdx(heads, ['Store Name','Store']), po: getIdx(heads, ['PO Number','PO']), factory: getIdx(heads, ['Factory Ref','Factory']), desc: getIdx(heads, ['Description']), qty: getIdx(heads, ['Order Qty','Qty','Quantity']), value: getIdx(heads, ['Value USD','Value','Total Value']), currency: getIdx(heads, ['Currency']), shipMode: getIdx(heads, ['Ship Mode']), incoterms: getIdx(heads, ['Incoterms']), pol: getIdx(heads, ['Port of Loading']), pod: getIdx(heads, ['Port of Discharge']), shipDate: getIdx(heads, ['Ex-Factory Date','Ex Factory','Ship Date']), inStore: getIdx(heads, ['In-Store Date','In Store']), merch: getIdx(heads, ['Merchandiser']), agent: getIdx(heads, ['Agent']), notes: getIdx(heads, ['Notes'])
    }
    return matrix.slice(1).filter(c => c.some(v => String(v || '').trim())).map((cells, i) => {
      const row = {
        rowNo: i + 2,
        checked: true,
        imported: false,
        style_number: readCell(cells, idx.style),
        buyer_name: readCell(cells, idx.buyer),
        brand_name: readCell(cells, idx.brand),
        store_name: readCell(cells, idx.store),
        po_number: readCell(cells, idx.po),
        factory_ref: readCell(cells, idx.factory),
        description: readCell(cells, idx.desc),
        total_qty: readCell(cells, idx.qty),
        total_value_usd: readCell(cells, idx.value),
        currency: readCell(cells, idx.currency) || 'USD',
        ship_mode: readCell(cells, idx.shipMode),
        incoterms: readCell(cells, idx.incoterms),
        port_of_loading: readCell(cells, idx.pol),
        port_of_discharge: readCell(cells, idx.pod),
        ship_date: readCell(cells, idx.shipDate),
        in_store_date: readCell(cells, idx.inStore),
        merchandiser_name: readCell(cells, idx.merch),
        agent_name: readCell(cells, idx.agent),
        notes: readCell(cells, idx.notes),
      }
  ...
  const validateRow = (r) => {
    const errors = []
    if (!String(r.style_number || '').trim()) errors.push('Missing style')
    if (!String(r.buyer_name || '').trim()) errors.push('Missing buyer')
    else if (!buyerMap[String(r.buyer_name).trim().toLowerCase()]) errors.push('Buyer not found')
    if (r.po_number && existingKeys.has(`po:${String(r.po_number).trim().toLowerCase()}`)) errors.push('PO already exists')
    if (r.factory_ref && existingKeys.has(`factory:${String(r.factory_ref).trim().toLowerCase()}`)) errors.push('Factory ref already exists')
    if (r.ship_date && Number.isNaN(new Date(r.ship_date).getTime())) errors.push('Invalid ex-factory date')
    if (r.in_store_date && Number.isNaN(new Date(r.in_store_date).getTime())) errors.push('Invalid in-store date')
    return errors
  }
  const recalc = (row) => ({ ...row, errors: validateRow(row) })
  const selectedValid = rows.filter(r => r.checked && !r.imported && r.errors.length === 0)
  const validCount = rows.filter(r => r.errors.length === 0).length
  const invalidCount = rows.filter(r => r.errors.length > 0).length
  async function handleFile(file) {
    if (!file) return
    setMessage(null); setFileName(file.name)
    let matrix = []
    if (/\.xlsx?$/i.test(file.name)) {
      const wb = XLSX.read(await file.arrayBuffer(), { type:'array' })
      matrix = XLSX.utils.sheet_to_json(wb.Sheets[wb.SheetNames[0]], { header:1, defval:'' })
    } else {
      matrix = parseCsvRows(await file.text())
    }
    const next = buildRows(matrix)
    setRows(next)
    setStep(2)
    if (!next.length) setMessage({ type:'error', text:'No rows found. Use the standard order import template.' })
  }
  function toggleAll(checked) { setRows(prev => prev.map(r => r.imported || r.errors.length ? r : { ...r, checked })) }
  function patchRow(i, key, value) { setRows(prev => prev.map((r, idx) => idx === i ? recalc({ ...r, [key]: value }) : r)) }
  async function importSelected() {
    if (!selectedValid.length) return
    setSaving(true); setMessage(null)
    const payload = selectedValid.map(r => ({
      style_number: r.style_number, buyer_name: r.buyer_name, buyer_id: buyerMap[String(r.buyer_name).trim().toLowerCase()] || null,
      brand_name: r.brand_name || null, store_name: r.store_name || null, po_number: r.po_number || null, factory_ref: r.factory_ref || null,
      description: r.description || null, total_qty: r.total_qty ? Number(String(r.total_qty).replace(/,/g,'')) || null : null,
      total_value_usd: r.total_value_usd ? Number(String(r.total_value_usd).replace(/[$,]/g,'')) || null : null,
      currency: r.currency || 'USD', ship_mode: r.ship_mode || null, incoterms: r.incoterms || null,
      port_of_loading: r.port_of_loading || null, port_of_discharge: r.port_of_discharge || null,
      ship_date: r.ship_date || null, in_store_date: r.in_store_date || null,
      merchandiser_name: r.merchandiser_name || null, agent_name: r.agent_name || null, notes: r.notes || null, status: 'Draft',
    }))
    const { error } = await supabase.from('orders').insert(payload)
    setSaving(false)
    if (error) { setMessage({ type:'error', text:'Import failed: ' + error.message }); return }
    setRows(prev => prev.map(r => r.checked && r.errors.length === 0 ? { ...r, imported:true, checked:false } : r))
    setMessage({ type:'success', text:`${payload.length} order${payload.length > 1 ? 's' : ''} imported successfully.` })
    onImported?.()
    setStep(3)
  }
  const modalTitle = step === 1 ? 'Bulk Order Import' : step === 2 ? 'Validate & Import Orders' : 'Import Complete'
  return <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.45)', zIndex:500, display:'flex', alignItems:'center', justifyContent:'center', padding:14 }}>
    <div style={{ width:'min(1380px, 98vw)', height:'min(780px, 94vh)', background:'#fff', borderRadius:12, boxShadow:'0 22px 70px rgba(0,0,0,0.25)', display:'flex', flexDirection:'column', overflow:'hidden' }}>
      <div style={{ padding:'18px 22px 14px', borderBottom:'1px solid #e5e7eb', display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
        <div><div style={{ fontSize:20, fontWeight:700 }}>{modalTitle}</div><div style={{ display:'flex', gap:8, alignItems:'center', marginTop:8, fontSize:12, fontWeight:700 }}><span style={{ padding:'3px 8px', borderRadius:4, background:step===1?'#111827':'#e5e7eb', color:step===1?'#fff':'#6b7280' }}>1. Upload</span><span>→</span><span style={{ padding:'3px 8px', borderRadius:4, background:step===2?'#111827':'#e5e7eb', color:step===2?'#fff':'#6b7280' }}>2. Validate & Edit</span><span>→</span><span style={{ padding:'3px 8px', borderRadius:4, background:step===3?'#111827':'#e5e7eb', color:step===3?'#fff':'#6b7280' }}>3. Finish</span></div></div>
        <button className="btn btn-ghost" onClick={onClose} style={{ height:34, width:34, padding:0 }}><X size={21}/></button>
      </div>
      {step === 1 && <div style={{ flex:1, display:'flex', alignItems:'center', justifyContent:'center' }}>
        <div onDragOver={e=>e.preventDefault()} onDrop={e=>{ e.preventDefault(); handleFile(e.dataTransfer.files?.[0]) }} style={{ width:560, minHeight:290, border:'2px dashed #cbd5e1', borderRadius:14, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:16, textAlign:'center', padding:30 }}>
          <div style={{ width:58, height:58, borderRadius:'50%', background:'#eff6ff', display:'flex', alignItems:'center', justifyContent:'center', color:'#2563eb' }}><Upload size={30}/></div>
          <div><div style={{ fontSize:18, fontWeight:700 }}>Upload Order Data (.csv / .xlsx)</div><div style={{ fontSize:13, color:'#6b7280', marginTop:8, lineHeight:1.45 }}>Drag and drop your populated template file here, or click to browse.<br/>Preview rows before importing into the system.</div></div>
          <div style={{ display:'flex', gap:10, marginTop:4 }}><label className="btn btn-primary" style={{ cursor:'pointer', height:38 }}><FileText size={16}/> Select File<input type="file" accept=".csv,.xlsx,.xls" style={{ display:'none' }} onChange={e=>handleFile(e.target.files?.[0])}/></label><button className="btn btn-secondary" style={{ height:38 }} onClick={downloadImportTemplate}><Download size={16}/> Download Template</button></div>
          {message && <div style={{ fontSize:12, color:message.type==='error'?'#dc2626':'#16a34a', fontWeight:700 }}>{message.text}</div>}
        </div>
      </div>}
      {step !== 1 && <>
        <div style={{ padding:'12px 22px', borderBottom:'1px solid #f3f4f6', display:'flex', alignItems:'center', gap:12 }}>
          <div style={{ fontSize:12, color:'#6b7280', fontWeight:700, flex:1 }}>{fileName || 'Uploaded file'} · {rows.length} rows · {validCount} valid · {invalidCount} needs attention</div>
          <button className="btn btn-secondary" style={{ height:32 }} onClick={()=>toggleAll(true)}>Select All Valid</button><button className="btn btn-secondary" style={{ height:32 }} onClick={()=>toggleAll(false)}>Clear All</button><button className="btn btn-secondary" style={{ height:32 }} onClick={()=>{ setRows([]); setStep(1); setMessage(null) }}>Upload Another</button>
        </div>
        {message && <div style={{ margin:'10px 22px 0', padding:'9px 12px', borderRadius:8, background:message.type==='error'?'#fef2f2':'#ecfdf5', color:message.type==='error'?'#dc2626':'#15803d', fontSize:12, fontWeight:700 }}>{message.text}</div>}
        <div style={{ flex:1, overflow:'auto', padding: '10px 22px' }}>
          <table style={{ width:'100%', borderCollapse:'collapse', tableLayout:'fixed', fontSize:12 }}><thead><tr style={{ background:'#f9fafb' }}><th style={{ width:36, padding:8 }}></th><th style={{ width:70, padding:8, textAlign:'left' }}>Row</th><th style={{ width:130, padding:8, textAlign:'left' }}>Style</th><th style={{ width:150, padding:8, textAlign:'left' }}>Buyer</th><th style={{ width:120, padding:8, textAlign:'left' }}>PO</th><th style={{ width:110, padding:8, textAlign:'left' }}>Factory Ref</th><th style={{ width:90, padding:8, textAlign:'right' }}>Qty</th><th style={{ width:105, padding:8, textAlign:'right' }}>Value</th><th style={{ width:122, padding:8, textAlign:'left' }}>Ex-Factory</th><th style={{ width:180, padding:8, textAlign:'left' }}>Status</th></tr></thead>
          <tbody>{rows.map((r,i)=><tr key={i} style={{ background:r.imported?'#f0fdf4':r.errors.length?'#fff7ed':'#fff' }}><td style={{ padding:8, borderBottom:'1px solid #f3f4f6' }}><input type="checkbox" checked={!!r.checked} disabled={r.imported || r.errors.length>0} onChange={e=>patchRow(i,'checked',e.target.checked)}/></td><td style={{ padding:8, borderBottom:'1px solid #f3f4f6', color:'#6b7280' }}>{r.rowNo}</td>{['style_number','buyer_name','po_number','factory_ref','total_qty','total_value_usd','ship_date'].map(k=><td key={k} style={{ padding:6, borderBottom:'1px solid #f3f4f6', textAlign:['total_qty','total_value_usd'].includes(k)?'right':'left' }}><input value={r[k] || ''} disabled={r.imported} onChange={e=>patchRow(i,k,e.target.value)} style={{ width:'100%', height:30, border:'1px solid #e5e7eb', borderRadius:6, padding:'0 8px', fontSize:12, textAlign:['total_qty','total_value_usd'].includes(k)?'right':'left', background:r.imported?'#f9fafb':'#fff' }}/></td>)}<td style={{ padding:8, borderBottom:'1px solid #f3f4f6' }}>{r.imported ? <span style={{ color:'#16a34a', fontWeight:700, display:'inline-flex', gap:5, alignItems:'center' }}><CheckCircle2 size={14}/> Imported</span> : r.errors.length ? <span style={{ color:'#d97706', fontWeight:700 }}>{r.errors.join(', ')}</span> : <span style={{ color:'#16a34a', fontWeight:700 }}>Valid</span>}</td></tr>)}</tbody></table>
        </div>
        <div style={{ padding:'14px 22px', borderTop:'1px solid #e5e7eb', display:'flex', justifyContent:'space-between', alignItems:'center' }}><div style={{ fontSize:12, color:'#6b7280' }}>Only checked + valid rows will be imported. Invalid rows stay blocked until fixed.</div><div style={{ display:'flex', gap:10 }}><button className="btn btn-secondary" onClick={onClose}>Close</button><button className="btn btn-primary" disabled={saving || selectedValid.length===0} onClick={importSelected}>{saving ? 'Importing...' : `Import Selected (${selectedValid.length})`}</button></div></div>
      </>}
    </div>
  </div>
}
...
export default function Orders() {
  ...
  async function loadOrders() {
    ...
    const [{ data }, { data: shipmentRows }, { data: queueRows }] = await Promise.all([
      supabase.from('orders').select('id,job_id,job_number,buyer_name,buyer_id,style_number,po_number,ship_date,planned_date,status,total_qty,total_value_usd,factory_ref,brand_name,store_name,merchandiser_name,description,queue_split_rule,excess_cutting_pct,created_at').order('ship_date', { ascending: true, nullsLast: true }),
  ...
  function openMetadata(o) { setMetadataOrder(o) }
  function editFromMetadata(o) { setMetadataOrder(null); openEdit(o) }
  ...
  async function archiveSelected() {
    const ids = [...selected]
    if (!ids.length) return
    if (!await confirm(`Archive ${ids.length} selected order(s)? This hides them from active operational views but keeps all order history.`, { title:'Archive Orders', confirmText:'Archive' })) return
    const { error } = await supabase.from('orders').update({ status: 'Archived' }).in('id', ids)
    if (error) { await alert(error.message || 'Could not archive selected orders.', { title:'Archive Failed' }); return }
    markTabUpdated('ordersPage', 'orders')
    setTabNotices(getPageTabNoticeMap('ordersPage', ['orders','queues']))
    setSelected(new Set())
    loadOrders()
  ...
  function saveCurrentViewPreset() {
    ...
    if (!name) return
    const key = 'app2a.orders.savedViews'
    const view = { name, search, filterBuyer, filterStatus, sort, groupBy }
    let views = []
    try { views = JSON.parse(localStorage.getItem(key) || '[]') } catch (e) {}
    views = [view, ...views.filter(v => v.name !== name)].slice(0, 8)
    localStorage.setItem(key, JSON.stringify(views))
    window.dispatchEvent(new Event('app2a-orders-views-updated'))
  }

  function loadSavedViews() {
    try { return JSON.parse(localStorage.getItem('app2a.orders.savedViews') || '[]') } catch (e) { return [] }
  }

  const [savedViewsTick, setSavedViewsTick] = useState(0)
  useEffect(() => {
    const fn = () => setSavedViewsTick(t => t + 1)
    window.addEventListener('app2a-orders-views-updated', fn)
    return () => window.removeEventListener('app2a-orders-views-updated', fn)
  }, [])
  ...
  const renderColumnCell = (o, key) => {
    ...
    if (key === 'po') return <td style={{ padding: '9px 12px', borderBottom: '1px solid #f9fafb', fontSize: 11, color: '#6b7280', fontFamily: 'monospace' }} onClick={() => openMetadata(o)}>{o.po_number || '—'}</td>
    if (key === 'factory_ref') return <td style={{ padding: '9px 12px', borderBottom: '1px solid #f9fafb', fontSize: 11, color: '#6b7280', fontFamily: 'monospace' }} onClick={() => openMetadata(o)}>{o.factory_ref || '—'}</td>
    if (key === 'qty') return <td style={{ padding: '9px 12px', borderBottom: '1px solid #f9fafb', fontFamily: 'monospace', fontSize: 11 }} onClick={() => openMetadata(o)}><div style={{ fontWeight:700 }}>{o.total_qty?.toLocaleString() || '—'}</div><div style={{ color:'#2563eb', marginTop:2 }}>Ship {Number(o.shipped_qty || 0).toLocaleString()}</div><div style={{ color:'#111827', marginTop:2 }}>Bal {Number(o.balance_qty || 0).toLocaleString()}</div></td>
    if (key === 'value') return <td style={{ padding: '9px 12px', borderBottom: '1px solid #f9fafb', fontSize: 12, fontWeight: 700 }} onClick={() => openMetadata(o)}>{o.total_value_usd ? `$${parseFloat(o.total_value_usd).toLocaleString(undefined, { maximumFractionDigits: 0 })}` : '—'}</td>
    if (key === 'ship_date') return <td style={{ padding: '9px 12px', borderBottom: '1px solid #f9fafb', whiteSpace: 'nowrap' }} onClick={() => openMetadata(o)}>{o.ship_date ? (() => { const days = daysRemaining(o.ship_date); const dl = daysLabel(days); return <div><div style={{ fontSize: 11, color: dateColor(o.ship_date), fontWeight: isOverdue ? 700 : 400 }}>{fmtDate(o.planned_date || o.ship_date)}</div>{dl && <div style={{ fontSize: 10, color: dl.color, fontWeight: dl.weight, marginTop: 1 }}>{dl.text}</div>}</div> })() : <span style={{ color: '#9ca3af' }}>—</span>}</td>
    if (key === 'placed') return <td style={{ padding: '9px 12px', borderBottom: '1px solid #f9fafb', fontSize: 11, color: '#9ca3af', whiteSpace: 'nowrap' }} onClick={() => openMetadata(o)}>{o.created_at ? new Date(o.created_at).toLocaleDateString('en-GB', { day: 'numeric', month: 'short' }) : '—'}</td>
    if (key === 'merchandiser') return <td style={{ padding: '9px 12px', borderBottom: '1px solid #f9fafb', fontSize: 11, color: '#6b7280' }} onClick={() => openMetadata(o)}>{o.merchandiser_name ? o.merchandiser_name.split(' ').map((p, i) => i === 0 ? p[0] + '.' : p).join(' ') : '—'}</td>
    if (key === 'q_count') return <td style={{ padding: '9px 12px', borderBottom: '1px solid #f9fafb', fontSize: 12, fontWeight: 700, color: (o.q_count || 0) ? '#111827' : '#9ca3af', fontFamily: 'monospace' }} onClick={() => openMetadata(o)}>{Number(o.q_count || 0)}</td>
    if (key === 'season') return <td style={{ padding: '9px 12px', borderBottom: '1px solid #f9fafb', fontSize: 11, color: '#6b7280' }} onClick={() => openMetadata(o)}>{o.season || '—'}</td>
    if (key === 'po_count') return <td style={{ padding: '9px 12px', borderBottom: '1px solid #f9fafb', fontSize: 11, color: '#6b7280' }} onClick={() => openMetadata(o)}>{o.po_count ?? '—'}</td>
    if (key === 'sampling_status') return <td style={{ padding: '9px 12px', borderBottom: '1px solid #f9fafb', fontSize: 11, color: '#6b7280' }} onClick={() => openMetadata(o)}>{o.sampling_status || '—'}</td>
    return null
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: '#fff' }}>
      ...
      <div style={{ display: 'flex', padding: '0 24px', borderBottom: '1px solid var(--border)', flexShrink: 0 }}>
        <button style={tabStyle('orders')} onClick={() => setTab('orders')}><span className="app-tab-label" style={{ display:'inline-flex', alignItems:'center', gap:6 }}>Buyer Purchase Orders{tabNotices.orders && tab !== 'orders' ? <span style={{ width:8, height:8, borderRadius:'50%', background:'#F59E0B', display:'inline-block' }} /> : null}</span></button>
        <button style={tabStyle('queues')} onClick={() => setTab('queues')}><span className="app-tab-label" style={{ display:'inline-flex', alignItems:'center', gap:6 }}>Queues{tabNotices.queues && tab !== 'queues' ? <span style={{ width:8, height:8, borderRadius:'50%', background:'#F59E0B', display:'inline-block' }} /> : null}</span></button>
      </div>
      ...
      {tab === 'orders' && <div style={{ padding: '12px 24px', borderBottom: '1px solid #f3f4f6', display: 'flex', alignItems: 'center', gap: 14, flexShrink: 0 }}>
        <div style={{ flexShrink: 0 }}>
          <div className="app-page-title">Orders</div>
          <div className="app-page-subtitle">Production status and delivery tracking</div>
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          {[
            ...
            <div key={kpi.label} style={{ padding: '6px 12px', border: '1px solid var(--border)', borderRadius: 8, background: '#fff', minWidth: 100 }}>
              ...
              <div style={{ fontSize: 18, fontWeight: 700, letterSpacing: '-0.5px', lineHeight: 1, color: kpi.amber ? '#d97706' : '#0d0d0d' }}>{kpi.value}</div>
            </div>
          ))}
        </div>
        <div style={{ marginLeft: 'auto', display: 'flex', gap: 8, alignItems: 'center' }}>
          <button className="btn btn-secondary" style={squareBtnStyle} onClick={() => setShowImportOrders(true)} title="Import Orders">
            <Upload size={15} />
          </button>
      ...
      {tab === 'orders' && (
        <>
          ...
          <div style={{ padding: '8px 24px', borderBottom: '1px solid #f3f4f6', display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
            ...
            <button className="btn btn-secondary" style={actionBtnStyle} disabled={filtered.length === 0} onClick={() => setSelected(new Set(filtered.map(o => o.id)))} title="Select all currently filtered orders">Select Filtered</button>
            <button className="btn btn-secondary" style={actionBtnStyle} disabled={selected.size === 0} onClick={() => setSelected(new Set())} title="Clear selected rows">Clear</button>
            <button className="btn btn-secondary" style={actionBtnStyle} onClick={saveCurrentViewPreset} title="Save current filters, group and sort">Save View</button>
            {loadSavedViews().length > 0 && <select key={savedViewsTick} defaultValue="" onChange={e => { const v = loadSavedViews().find(x => x.name === e.target.value); if (!v) return; setSearch(v.search || ''); setFilterBuyer(v.filterBuyer || ''); setFilterStatus(Array.isArray(v.filterStatus) ? v.filterStatus : []); setSort(v.sort || { col:'ship_date', dir:'asc' }); setGroupBy(v.groupBy || 'none'); }} style={{ ...sortSelectStyle, minWidth: 132 }} title="Load saved view"><option value="">Saved Views</option>{loadSavedViews().map(v => <option key={v.name} value={v.name}>{v.name}</option>)}</select>}
            <div style={{ fontSize:11, fontWeight:700, color:selected.size?'#111827':'#9ca3af', minWidth:72 }}>{selected.size} selected</div>
            <div style={{ marginLeft:'auto', display:'flex', gap:8, alignItems:'center' }}>
              ...
              <button className="btn btn-secondary" style={squareBtnStyle} disabled={selected.size === 0} onClick={archiveSelected} title="Archive selected orders">
                <Archive size={15} />
              </button>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 980 }}>
                ...
                <tbody>
                  {displayedRows.map(row => {
                    if (row.type === 'group') {
                      return (
                        <tr key={`group-${row.key}`} style={{ background:'var(--app-group-row-bg, #fdf6e3)', color:'var(--app-group-row-text, #111827)' }}>
                          <td colSpan={1 + (visibleColumns.picture ? 1 : 0) + 3 + visibleConfigColumns.length} style={{ padding:'10px 18px', borderBottom:'1px solid #eee', borderTop:'1px solid #eee' }}>
                            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:16 }}>
                              <div style={{ fontSize:12, fontWeight:700, color:'#111827' }}>{row.label} <span style={{ color:'#9ca3af', fontWeight:700 }}>— {row.count} order{row.count === 1 ? '' : 's'}</span></div>
                    ...
                    return (
                      ...
                      >
                        ...
                        {visibleColumns.picture && <td style={{ padding: '9px 8px', borderBottom: '1px solid #f9fafb' }} onClick={() => openMetadata(o)}>
                        ...
                        <td style={{ padding: '9px 12px', borderBottom: '1px solid #f9fafb' }} onClick={() => openMetadata(o)}>
                          <div style={{ fontFamily: 'monospace', fontWeight: 700, fontSize: 12, color: '#0d0d0d' }}>{visibleJobNumber(o) || '—'}</div>
                          <div style={{ ...statusBadgeStyle(o.status || 'Draft'), marginTop: 5 }}>
                            <span style={statusDotStyle(o.status || 'Draft')} />
                            <span>{o.status || 'Draft'}</span>
                          </div>
                        </td>
                        <td style={{ padding: '9px 12px', borderBottom: '1px solid #f9fafb', whiteSpace: 'nowrap' }} onClick={() => openMetadata(o)}>
                        ...
                        <td style={{ padding: '9px 12px', borderBottom: '1px solid #f9fafb', maxWidth: 200 }} onClick={() => openMetadata(o)}>
      ...
      {metadataOrder && <OrderMetadataModal order={metadataOrder} onClose={() => setMetadataOrder(null)} onEdit={editFromMetadata} />}
      ...
      {showImportOrders && (
        <BulkOrderImportModal
          ...
          onImported={() => { markTabUpdated('ordersPage', 'orders'); setTabNotices(getPageTabNoticeMap('ordersPage', ['orders','queues'])); loadOrders() }}
        />
      )}
      {showColumnSettings && (
        <div style={{ position:'fixed', inset:0, zIndex:260, pointerEvents:'none' }}>
          ...
          <div style={{ position:'absolute', top:0, right:0, width:300, height:'100%', background:'#fff', borderLeft:'1px solid #e5e7eb', boxShadow:'-16px 0 40px rgba(15,23,42,0.14)', pointerEvents:'auto', display:'flex', flexDirection:'column' }}>
            <div style={{ height:58, display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0 18px', borderBottom:'1px solid #f3f4f6' }}>
              <div style={{ fontSize:16, fontWeight:700, color:'#111827' }}>Columns</div>
            ...
            <div style={{ display:'flex', gap:18, padding:'14px 18px 0 18px', borderBottom:'1px solid #f3f4f6' }}>
              <div style={{ paddingBottom:10, borderBottom:'2px solid #111827', fontSize:12, fontWeight:700, color:'#111827' }}>Manage Columns</div>
            ...
            <div style={{ padding:18, overflowY:'auto', flex:1 }}>
              ...
              <div style={{ fontSize:12, fontWeight:700, color:'#374151', marginBottom:10 }}>Pinned <span style={{ color:'#9ca3af', fontWeight:600 }}>(always visible)</span></div>
              ...
              <div style={{ fontSize:12, fontWeight:700, color:'#374151', marginBottom:10 }}>Visible Columns <span style={{ color:'#9ca3af', fontWeight:600 }}>(drag to reorder)</span></div>
              ...
              <div style={{ fontSize:12, fontWeight:700, color:'#374151', marginBottom:10 }}>Hidden Columns</div>
