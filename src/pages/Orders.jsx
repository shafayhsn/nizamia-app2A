...
function QueuesTab({ onEditOrder }) {
  ...
  const [visibleCols, setVisibleCols] = useState(() => {
    try { return { ...DEFAULT_QUEUE_VISIBLE_COLUMNS, ...JSON.parse(localStorage.getItem('app2a.queue.visibleCols') || '{}') } } catch(e) { return DEFAULT_QUEUE_VISIBLE_COLUMNS }
  })
  const [colOrder, setColOrder] = useState(() => {
    try { return JSON.parse(localStorage.getItem('app2a.queue.colOrder') || 'null') || DEFAULT_QUEUE_COLUMN_ORDER } catch(e) { return DEFAULT_QUEUE_COLUMN_ORDER }
  })

  useEffect(() => { loadAll() }, [])
  useEffect(() => { localStorage.setItem('app2a.queue.visibleCols', JSON.stringify(visibleCols)); localStorage.setItem('app2a.queue.colOrder', JSON.stringify(colOrder)) }, [visibleCols, colOrder])
  ...
  const thStyle = { padding:'9px 12px', borderBottom:'1px solid #f3f4f6', fontSize:10, fontWeight:800, color:'#9ca3af', textTransform:'uppercase', letterSpacing:'0.5px' }
  const tdStyle = { padding:'11px 12px', borderBottom:'1px solid #f3f4f6', fontSize:12 }
  ...
  function exportCSV() {
    const target = selected.length ? selected : filtered
    const csv = [[...visibleKeys.map(k=>QUEUE_COLUMN_LABELS[k]||k)], ...target.map(q => visibleKeys.map(k => cellText(q,k)))].map(r => r.map(v => `"${String(v).replace(/"/g,'""')}"`).join(',')).join('\n')
  ...
  function printList() {
    const target = selected.length ? selected : filtered
    const head = visibleKeys.map(k => `<th style="border:1px solid #ddd;padding:8px;background:#f5f5f5;text-align:left">${QUEUE_COLUMN_LABELS[k]||k}</th>`).join('')
    const body = target.map(q => `<tr>${visibleKeys.map(k => `<td style="border:1px solid #ddd;padding:8px">${cellText(q,k)}</td>`).join('')}</tr>`).join('')
    ...
    <div style={{ display:'flex', flexDirection:'column', flex:1, minHeight:0, background:'#fff' }}>
      <div style={{ padding:'12px 24px', borderBottom:'1px solid #f3f4f6', display:'flex', alignItems:'center', gap:14, flexShrink:0 }}>
        ...
        <div style={{ display:'flex', gap:8 }}>
          {[{l:'Total Queues',v:rows.length,q:totalQty},{l:'Active Queues',v:activeRows.length,q:activeQty},{l:'Inactive Queues',v:inactiveRows.length,q:inactiveQty}].map(k => <div key={k.l} style={{ padding:'6px 12px', border:'1px solid var(--border)', borderRadius:8, background:'#fff', minWidth:120 }}><div style={{ fontSize:9, fontWeight:600, color:'var(--text-light)', textTransform:'uppercase', letterSpacing:'0.5px', marginBottom:2 }}>{k.l}</div><div style={{ display:'flex', alignItems:'baseline', gap:8 }}><div style={{ fontSize:18, fontWeight:800, lineHeight:1 }}>{k.v}</div><div style={{ fontSize:10, color:'#6b7280', fontWeight:700 }}>{Number(k.q||0).toLocaleString()} pcs</div></div></div>)}
        </div>
        <div style={{ marginLeft:'auto', minWidth:260 }}><div style={{ display:'flex', alignItems:'center', gap:10 }}><div style={{ fontSize:13, fontWeight:800, whiteSpace:'nowrap' }}>Shipped Progress</div><div style={{ flex:1, height:8, borderRadius:999, background:'#e5e7eb', overflow:'hidden', border:'1px solid #d1d5db' }}><div style={{ width:`${pct}%`, height:'100%', background:'#22c55e' }} /></div><div style={{ fontSize:12, fontWeight:800, minWidth:36 }}>{pct}%</div></div><div style={{ marginTop:4, fontSize:10, color:'#6b7280', textAlign:'right' }}>{shippedQty.toLocaleString()} / {totalQty.toLocaleString()} pcs shipped</div></div>
        <div style={{ display:'flex', gap:8, alignItems:'center' }}><button className="btn btn-primary" style={{ height:36, background:actionMode === 'activate' ? '#16a34a' : actionMode === 'reset' ? '#374151' : '#e5e7eb', color:actionDisabled ? '#9ca3af' : '#fff', borderColor:'transparent' }} disabled={actionDisabled} title={actionMode === 'mixed' ? 'Mixed selection — select same state queues' : ''} onClick={runAction}>{actionMode === 'reset' ? 'Reset Queue' : 'Activate Queue'}</button><button className="btn btn-secondary" style={{ height:36 }} disabled={selected.length === 0} onClick={printList}>Print</button><button className="btn btn-secondary" style={{ height:36 }} disabled={selected.length === 0} onClick={exportCSV}>Export</button></div>
      </div>
      <div style={{ padding:'8px 24px', borderBottom:'1px solid #f3f4f6', display:'flex', alignItems:'center', gap:8, flexShrink:0 }}>
        <div style={{ position:'relative', width:280, flexShrink:0 }}><Search size={13} style={{ position:'absolute', left:9, top:'50%', transform:'translateY(-50%)', color:'#9ca3af' }} /><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search Q#, label, style, buyer..." style={{ width:'100%', paddingLeft:28, paddingRight:10, height:32, border:'1px solid var(--border)', borderRadius:7, fontSize:12, fontFamily:'var(--font)', outline:'none', background:'#fafafa' }} /></div>
        <select value={groupBy} onChange={e=>setGroupBy(e.target.value)} style={{ height:32, border:'1px solid var(--border)', borderRadius:7, fontSize:12, padding:'0 10px', background:'#fff' }}><option value="none">☰ No Group</option><option value="style_number">☰ By Style</option><option value="job_number">☰ By Job</option><option value="buyer_name">☰ By Buyer</option><option value="q_number">☰ By Serial / Q#</option><option value="ship_date">☰ By Delivery Date</option></select>
        <select value={`${sort.col}:${sort.dir}`} onChange={e=>{ const [col,dir]=e.target.value.split(':'); setSort({col,dir}) }} style={{ height:32, border:'1px solid var(--border)', borderRadius:7, fontSize:12, padding:'0 10px', background:'#fff' }}><option value="q_number:asc">⇅ Q# Asc</option><option value="q_number:desc">⇅ Q# Desc</option><option value="ship_date:asc">⇅ Delivery Asc</option><option value="ship_date:desc">⇅ Delivery Desc</option><option value="qty:desc">⇅ Qty High → Low</option><option value="qty:asc">⇅ Qty Low → High</option></select>
        <select value={filterBuyer} onChange={e=>setFilterBuyer(e.target.value)} style={{ height:32, border:'1px solid var(--border)', borderRadius:7, fontSize:12, padding:'0 10px', background:'#fff' }}><option value="">All Buyers</option>{buyers.map(v=><option key={v}>{v}</option>)}</select>
        <select value={filterStyle} onChange={e=>setFilterStyle(e.target.value)} style={{ height:32, border:'1px solid var(--border)', borderRadius:7, fontSize:12, padding:'0 10px', background:'#fff' }}><option value="">All Styles</option>{styles.map(v=><option key={v}>{v}</option>)}</select>
        <select value={filterJob} onChange={e=>setFilterJob(e.target.value)} style={{ height:32, border:'1px solid var(--border)', borderRadius:7, fontSize:12, padding:'0 10px', background:'#fff' }}><option value="">All Jobs</option>{jobs.map(v=><option key={v}>{v}</option>)}</select>
        <select value={filterRule} onChange={e=>setFilterRule(e.target.value)} style={{ height:32, border:'1px solid var(--border)', borderRadius:7, fontSize:12, padding:'0 10px', background:'#fff' }}><option value="">All Split Rules</option>{rules.map(v=><option key={v}>{v}</option>)}</select>
        <select value={filterStatus} onChange={e=>setFilterStatus(e.target.value)} style={{ height:32, border:'1px solid var(--border)', borderRadius:7, fontSize:12, padding:'0 10px', background:'#fff' }}><option value="">All Status</option>{statuses.map(v=><option key={v}>{v}</option>)}</select>
...
export default function Orders() {
  ...
  const [visibleColumns, setVisibleColumns] = useState(() => {
    try {
      const saved = localStorage.getItem('orders.visibleColumns.v2') || localStorage.getItem('orders.visibleColumns.v1')
  ...
  const [columnOrder, setColumnOrder] = useState(() => {
    try {
      const saved = JSON.parse(localStorage.getItem('orders.columnOrder.v1') || '[]')
  ...
  useEffect(() => {
    try { localStorage.setItem('orders.visibleColumns.v2', JSON.stringify(visibleColumns)) } catch (e) {}
  }, [visibleColumns])
