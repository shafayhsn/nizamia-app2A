  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', flex:1, minHeight:0, background:'#fff' }}>
      <div style={{ padding:'12px 24px', borderBottom:'1px solid #f3f4f6', display:'flex', alignItems:'center', gap:14, flexShrink:0, minWidth:0 }}>
        ...
        <div style={{ marginLeft:'auto', display:'flex', gap:8, alignItems:'center', flexShrink:0 }}><button className="btn btn-secondary" style={{ height:34, padding:'0 14px' }} disabled={filtered.length === 0} onClick={exportCSV}>Export</button><button className="btn btn-secondary" style={{ height:34, padding:'0 14px' }} disabled={selectedQueues.length === 0} onClick={() => setShowPrintModal(true)}>Print Reports</button><button className="btn btn-primary" style={{ height:34, padding:'0 14px', background:selectedInactive.length && selectedInactive.length === selected.length ? '#16a34a' : '#e5e7eb', color:selectedInactive.length && selectedInactive.length === selected.length ? '#fff' : '#9ca3af', borderColor:'transparent', minWidth:112 }} disabled={!(selectedInactive.length && selectedInactive.length === selected.length)} title={selected.length && selectedInactive.length !== selected.length ? 'Select only inactive/queued rows to activate' : ''} onClick={()=>runAction('activate')}>Activate Queue</button><button className="btn btn-secondary" style={{ height:34, padding:'0 14px', background:selected.length ? '#fee2e2' : '#f3f4f6', color:selected.length ? '#dc2626' : '#9ca3af', borderColor:'transparent', minWidth:100 }} disabled={!selected.length} title={selected.length ? 'Reset the whole queue split for this order' : 'Select any Q to reset its order'} onClick={()=>runAction('reset')}>Reset Queue</button></div>
      </div>
