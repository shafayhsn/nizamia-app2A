...
function QueuesTab({ onEditOrder }) {
  ...
  const queueComponentPayload = (q) => String(q?.custom_components || String(q?.label || '').split('__APP2A_QC__')[1] || '')
  ...
  async function fetchQDocData(q) {
    ...
    const isCustomQueue = String(q.split_rule || '').trim() === 'Custom' && customKeys.length > 0
    ...
    qSgs.forEach((g, gi) => {
      ...
      groupColors.forEach((c, ci) => {
        ...
        if (!isCustomQueue || !keyMatchesCustomComponent(key, g.group_name, c.color_name)) return
        ...
        if (componentQty > 0) customComponentSpecs.push({
          key,
