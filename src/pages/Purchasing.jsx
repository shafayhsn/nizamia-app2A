    ...
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.45)', zIndex:300, display:'flex', alignItems:'center', justifyContent:'center', padding:24 }}>
      <div style={{ background:'#fff', borderRadius:12, width:'100%', maxWidth:860, maxHeight:'calc(100vh - 48px)', display:'flex', flexDirection:'column', boxShadow:'0 24px 80px rgba(0,0,0,0.22)', overflow:'hidden' }}>
        <div style={{ padding:'14px 24px', borderBottom:'1px solid #f0f0ee', display:'flex', alignItems:'center', gap:10 }}>
          ...
          <div style={{ flex:1 }} />
          <label style={{ display:'flex', alignItems:'center', gap:6, fontSize:11, fontWeight:700, color:taxEnabled?'#0d0d0d':'#9ca3af', marginRight:6 }}>
            <input type="checkbox" checked={taxEnabled} disabled={locked} onChange={e=>setTaxEnabled(e.target.checked)} />
            Tax {taxEnabled ? `${taxRate}%` : 'OFF'}
          </label>
  ...
  const buildPrefill = () => {
    ...
    for (const r of filtered) {
      ...
      if (selected.has(rowKey(r.id))) {
        ...
        if (usageLines.length) usageLines.forEach((u, ix) => lines.push({ _id:`${r.id}_${ix}`, description:r.name, specification:r.specification, breakdown:u.breakdown, qty:u.req_qty||'', unit:u.unit||r.unit||'pcs', unit_rate:'', amount:'', source_type:r.source_type, source_id:r.source_id }))
        else lines.push({ _id:r.id, description:r.name, specification:r.specification, breakdown:'', qty:r.req_qty||'', unit:r.unit||'pcs', unit_rate:'', amount:'', source_type:r.source_type, source_id:r.source_id })
      }
    }
    return lines
  }
