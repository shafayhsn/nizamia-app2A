...
function MaterialDemandTab({ suppliers }) {
  ...
  const buildQueueLine = (r, q) => ({
    ...
    description: r.name,
    specification: `${q.label || q.q_number || 'Queued'}${r.specification ? ' · ' + r.specification : ''}${q.q_number ? ' [' + q.q_number + ']' : ''}`,
    qty: q.req_qty || '',
    ...
    for (const r of filtered) {
      ...
      if (childKeys.length) {
        ;(r.queue_breakdown || []).forEach(q => {
          if (selected.has(qKey(r.id, queueRef(q)))) lines.push(buildQueueLine(r, q))
        })
        continue
      }
