...
import { Search, Plus, Trash2, Check, X, AlertCircle, Eye, Lock, Unlock, ChevronDown, ChevronUp } from 'lucide-react'
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', zIndex: 400, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ background: '#fff', borderRadius: 10, padding: 28, width: 320, boxShadow: '0 16px 48px rgba(0,0,0,0.2)', textAlign: 'center' }}>
        ...
        <input type="password" value={val} onChange={e => { setVal(e.target.value); setErr(false) }} onKeyDown={e => e.key === 'Enter' && check()}
          autoFocus placeholder="Passkey"
...
// ── PO Form Modal ─────────────────────────────────────────────────────────────
function POModal({ po, orders, suppliers, onClose, onSaved }) {
  const isNew = !po
  ...
  const [items, setItems] = useState(po?.items || [{ _id: '1', description: '', specification: '', qty: '', unit: 'yards', unit_rate: '', amount: '' }])
  const [saving, setSaving] = useState(false)
  const [error, setError] = useState(null)
  const [locked, setLocked] = useState(!isNew)
  const [showPasskey, setShowPasskey] = useState(false)

  // Auto-populate items from BOM when order selected
  ...
  const addItem = () => setItems(its => [...its, { _id: String(Date.now()), description: '', specification: '', qty: '', unit: 'yards', unit_rate: '', amount: '' }])
  ...
  const handleSave = async () => {
    ...
    setSaving(true)
    try {
      let poNum = form.po_number
      if (!poNum) poNum = await generatePONumber()
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <div style={{ background: '#fff', borderRadius: 12, width: '100%', maxWidth: 860, maxHeight: 'calc(100vh - 48px)', display: 'flex', flexDirection: 'column', boxShadow: '0 24px 80px rgba(0,0,0,0.22)', overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '20px 24px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12, marginBottom: 16 }}>
            ...
            <div>
              ...
              <select style={sel} value={form.order_id} onChange={e => handleOrderChange(e.target.value)} disabled={locked}>
                <option value="">— Not linked to order —</option>
                {orders.map(o => <option key={o.id} value={o.id}>{o.style_number} · {o.buyer_name}</option>)}
              </select>
            </div>
            <div>
              <label style={lbl}>PO Date</label>
              <input style={inp} type="date" value={form.po_date} onChange={set('po_date')} disabled={locked} />
            </div>
            <div>
              <label style={lbl}>Delivery Date</label>
              <input style={inp} type="date" value={form.delivery_date} onChange={set('delivery_date')} disabled={locked} />
            </div>
            <div>
            ...
            <div>
              ...
              <input style={inp} value={form.payment_terms} onChange={set('payment_terms')} placeholder="e.g. 30 days net" disabled={locked} />
            </div>
            <div>
            ...
            <div style={{ gridColumn: '2 / 4' }}>
              ...
              <input style={inp} value={form.notes} onChange={set('notes')} placeholder="Any notes..." disabled={locked} />
            </div>
          </div>
              ...
              {items.map((item, idx) => (
                <tr key={item._id || idx}>
                  <td style={{ padding: '3px 4px', minWidth: 160 }}>
                    <input style={{ ...inp, height: 28 }} value={item.description} onChange={e => updItem(idx, 'description', e.target.value)} placeholder="Item description" disabled={locked} />
                  </td>
                  <td style={{ padding: '3px 4px', minWidth: 130 }}>
                    <input style={{ ...inp, height: 28 }} value={item.specification || ''} onChange={e => updItem(idx, 'specification', e.target.value)} placeholder="Spec" disabled={locked} />
                  </td>
                  <td style={{ padding: '3px 4px', width: 80 }}>
                    <input style={{ ...inp, height: 28, textAlign: 'right' }} type="number" value={item.qty} onChange={e => updItem(idx, 'qty', e.target.value)} placeholder="0" disabled={locked} />
                  </td>
                  <td style={{ padding: '3px 4px', width: 80 }}>
                  ...
                  <td style={{ padding: '3px 4px', width: 100 }}>
                    <input style={{ ...inp, height: 28, textAlign: 'right' }} type="number" value={item.unit_rate} onChange={e => updItem(idx, 'unit_rate', e.target.value)} placeholder="0.00" disabled={locked} />
                  </td>
                  <td style={{ padding: '3px 4px', width: 100, textAlign: 'right', fontSize: 12, fontWeight: 700 }}>
                    {item.amount ? parseFloat(item.amount).toLocaleString(undefined, { maximumFractionDigits: 2 }) : '—'}
                  </td>
                  <td style={{ padding: '3px 0 3px 4px', width: 30 }}>
                    {!locked && items.length > 1 && (
                      <button className="btn btn-ghost btn-sm" onClick={() => removeItem(idx)}><Trash2 size={10} /></button>
                    )}
                  </td>
                </tr>
              ))}
          ...
          {!locked && (
            <button className="btn btn-secondary btn-sm" onClick={addItem} style={{ marginBottom: 12 }}><Plus size={12} /> Add Line</button>
          )}

          {total > 0 && (
            <div style={{ display: 'flex', justifyContent: 'flex-end', paddingRight: 34 }}>
              <div style={{ background: '#0d0d0d', color: '#fff', padding: '8px 16px', borderRadius: 6, fontSize: 13, fontWeight: 700 }}>
                Total: {form.currency} {total.toLocaleString(undefined, { maximumFractionDigits: 2 })}
              </div>
            </div>
          )}
...
function WOModal({ wo, orders, suppliers, onClose, onSaved }) {
  ...
  const [form, setForm] = useState({
    ...
    vendor_phone: wo?.vendor_phone || '',
    factory_ref: wo?.factory_ref || '',
    issue_date: wo?.issue_date || new Date().toISOString().slice(0, 10),
    start_date: wo?.start_date || '',
    complete_by: wo?.complete_by || '',
    daily_output: wo?.daily_output || '',
  ...
  const [processes, setProcesses] = useState(wo?.processes || [])
  const [availProcesses, setAvailProcesses] = useState([])
  ...
  // When order changes, load its processes from Step 9
  const handleOrderChange = async (orderId) => {
    setForm(f => ({ ...f, order_id: orderId }))
    if (!orderId) { setAvailProcesses([]); return }
    const { data } = await supabase.from('order_processes').select('process_name').eq('order_id', orderId)
    const procs = data?.map(p => p.process_name) || []
    setAvailProcesses(procs)
    // Auto-populate items from ticked processes
    setItems(procs.map((p, i) => ({ _id: String(i), process_name: p, dept_info: '', qty: '', unit: 'pcs', rate: '', amount: '' })))
  }
  ...
  const handleSave = async () => {
    ...
    setSaving(true)
    try {
      let woNum = form.wo_number
      if (!woNum) woNum = await generateWONumber()
      const payload = { ...form, wo_number: woNum, processes, qty: parseInt(form.qty) || null, daily_output: parseInt(form.daily_output) || null }
      let woId
      if (wo?.id) {
      ...
      if (items.length) {
        const rows = items.filter(i => i.process_name).map((i, idx) => ({
          wo_id: woId, process_name: i.process_name, dept_info: i.dept_info || null,
          qty: parseFloat(i.qty) || null, unit: i.unit || null,
          rate: parseFloat(i.rate) || null, amount: parseFloat(i.amount) || null,
          sort_order: idx,
        }))
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <div style={{ background: '#fff', borderRadius: 12, width: '100%', maxWidth: 800, maxHeight: 'calc(100vh - 48px)', display: 'flex', flexDirection: 'column', boxShadow: '0 24px 80px rgba(0,0,0,0.22)', overflow: 'hidden' }}>
        <div style={{ padding: '14px 24px', borderBottom: '1px solid #f0f0ee', display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '20px 24px' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12, marginBottom: 16 }}>
            ...
            <div>
              ...
              <input style={inp} value={form.vendor_phone} onChange={set('vendor_phone')} placeholder="03xx-xxxxxxx" disabled={locked} />
            </div>
            <div>
              <label style={lbl}>Order (optional)</label>
              <select style={sel} value={form.order_id} onChange={e => handleOrderChange(e.target.value)} disabled={locked}>
            ...
            <div>
              <label style={lbl}>Factory Ref</label>
              <input style={inp} value={form.factory_ref} onChange={set('factory_ref')} disabled={locked} />
            </div>
            <div>
              <label style={lbl}>Colour</label>
              <input style={inp} value={form.color} onChange={set('color')} placeholder="e.g. Stone Blue" disabled={locked} />
            </div>
            <div>
              <label style={lbl}>Qty</label>
              <input style={inp} type="number" value={form.qty} onChange={set('qty')} disabled={locked} />
            </div>
            <div>
              <label style={lbl}>Issue Date</label>
              <input style={inp} type="date" value={form.issue_date} onChange={set('issue_date')} disabled={locked} />
            </div>
            <div>
              <label style={lbl}>Start Date</label>
              <input style={inp} type="date" value={form.start_date} onChange={set('start_date')} disabled={locked} />
            </div>
            <div>
              <label style={lbl}>Complete By</label>
              <input style={inp} type="date" value={form.complete_by} onChange={set('complete_by')} disabled={locked} />
            </div>
            <div>
              <label style={lbl}>Daily Output</label>
              <input style={inp} type="number" value={form.daily_output} onChange={set('daily_output')} placeholder="pcs/day" disabled={locked} />
            </div>
            <div>
              <label style={lbl}>Payment Terms</label>
              <input style={inp} value={form.payment_terms} onChange={set('payment_terms')} placeholder="e.g. On completion" disabled={locked} />
            </div>
            <div>
              <label style={lbl}>Status</label>
              <select style={sel} value={form.status} onChange={set('status')} disabled={locked}>
          ...
          {/* Processes */}
          {availProcesses.length > 0 && (
            <div style={{ marginBottom: 16 }}>
              <div style={{ fontSize: 12, fontWeight: 700, marginBottom: 8 }}>Processes (from order Step 9)</div>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                {availProcesses.map(p => {
                  const on = processes.includes(p)
                  return (
                    <button key={p} onClick={() => !locked && setProcesses(ps => on ? ps.filter(x => x !== p) : [...ps, p])}
                      style={{ padding: '4px 10px', borderRadius: 5, fontSize: 11, fontWeight: 600, border: 'none', cursor: locked ? 'default' : 'pointer', background: on ? '#0d0d0d' : '#f0f0ee', color: on ? '#fff' : '#374151' }}>
                      {p}
                    </button>
                  )
                })}
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: 10 }}>
            <thead>
              <tr>
                {['Process', 'Dept / Info', 'Qty', 'Unit', 'Rate', 'Amount', ''].map(h => (
            ...
            <tbody>
              {items.map((item, idx) => (
                <tr key={item._id || idx}>
                  <td style={{ padding: '3px 4px', minWidth: 130 }}>
                    <input style={{ ...inp, height: 28 }} value={item.process_name} onChange={e => updItem(idx, 'process_name', e.target.value)} placeholder="Process" disabled={locked} />
                  </td>
                  <td style={{ padding: '3px 4px', minWidth: 110 }}>
                    <input style={{ ...inp, height: 28 }} value={item.dept_info || ''} onChange={e => updItem(idx, 'dept_info', e.target.value)} placeholder="Dept/info" disabled={locked} />
                  </td>
                  <td style={{ padding: '3px 4px', width: 80 }}>
                    <input style={{ ...inp, height: 28, textAlign: 'right' }} type="number" value={item.qty} onChange={e => updItem(idx, 'qty', e.target.value)} disabled={locked} />
                  </td>
                  <td style={{ padding: '3px 4px', width: 80 }}>
                  ...
                  <td style={{ padding: '3px 4px', width: 90 }}>
                    <input style={{ ...inp, height: 28, textAlign: 'right' }} type="number" value={item.rate} onChange={e => updItem(idx, 'rate', e.target.value)} placeholder="0.00" disabled={locked} />
                  </td>
                  <td style={{ padding: '3px 4px', width: 90, textAlign: 'right', fontSize: 12, fontWeight: 700 }}>
                    {item.amount ? parseFloat(item.amount).toLocaleString(undefined, { maximumFractionDigits: 2 }) : '—'}
                  </td>
                  <td style={{ padding: '3px 0 3px 4px', width: 30 }}>
          ...
          {!locked && (
            <button className="btn btn-secondary btn-sm" onClick={() => setItems(its => [...its, { _id: String(Date.now()), process_name: '', dept_info: '', qty: '', unit: 'pcs', rate: '', amount: '' }])} style={{ marginBottom: 12 }}>
          ...
          {total > 0 && (
            <div style={{ display: 'flex', justifyContent: 'flex-end', paddingRight: 34 }}>
              <div style={{ background: '#0d0d0d', color: '#fff', padding: '8px 16px', borderRadius: 6, fontSize: 13, fontWeight: 700 }}>
                Total: PKR {total.toLocaleString(undefined, { maximumFractionDigits: 2 })}
              </div>
            </div>
          )}

          <div style={{ marginTop: 12 }}>
            <label style={lbl}>Notes / T&Cs</label>
...
function DemandTab({ suppliers }) {
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true)
      console.log('[DemandTab] load() called')
      ...
      console.log('[DemandTab] ords:', ords?.length, 'jobOrders:', jobOrders.length)
      setOrders(ords || [])
      if (!jobOrders.length) { setLoading(false); return }
      const { data: bom, error: bomErr } = await supabase.from('bom_items')
        .select('*').order('created_at')
      console.log('[DemandTab] bom items:', bom?.length, 'error:', bomErr?.message)
      const jobOrderIds = new Set(jobOrders.map(o => o.id))
      setBomItems((bom || []).filter(b => jobOrderIds.has(b.order_id)))
      setLoading(false)
    }
    load()
  }, [])
  ...
  const rows = bomItems.map(item => {
    ...
    return { ...item, order, cat: item.category === 'Fabric' ? 'Fabric' : item.category === 'Stitching Trim' ? 'S. Trim' : 'P. Trim' }
  }).filter(Boolean)

  const filtered = rows.filter(r => {
    const q = search.toLowerCase()
    const ms = !q || [r.name, r.order.buyer_name, r.order.style_number, r.order.job_number].some(f => f?.toLowerCase().includes(q))
    const mc = !filterCat    || r.cat === filterCat
    const mst = !filterStatus || (r.po_status || 'No PO') === filterStatus
    return ms && mc && mst
  })
  ...
  filtered.forEach(r => {
    ...
    if (!grouped[key]) grouped[key] = { order: r.order, rows: [] }
  ...
  const catStyle = { Fabric: { color: '#2563eb', bg: '#eff6ff' }, 'S. Trim': { color: '#d97706', bg: '#fff7ed' }, 'P. Trim': { color: '#7c3aed', bg: '#f5f3ff' } }
  ...
  return (
    <>
      ...
      <div style={{ flex: 1, overflowY: 'auto', overflowX: 'auto' }}>
        {loading ? (
          <div style={{ padding: 60, textAlign: 'center', color: '#9ca3af', fontSize: 12 }}>Loading...</div>
        ) : filtered.length === 0 ? (
          <div style={{ padding: 60, textAlign: 'center', color: '#9ca3af', fontSize: 12 }}>
            {orders.filter(o => o.job_id).length === 0
              ? 'No job-assigned orders yet. Assign orders to a job from the Orders page to see material demand here.'
              : 'No BOM items found matching your filters.'}
          </div>
        ) : (
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 860 }}>
            <thead>
              <tr style={{ background: '#fafafa', position: 'sticky', top: 0, zIndex: 10 }}>
                ...
                {['Job', 'Cat', 'Material / Spec', 'Req Qty', 'Supplier', 'Status'].map(h => (
            ...
            <tbody>
              {Object.values(grouped).map(grp => (
                <React.Fragment key={grp.order.id}>
                  <tr>
                    <td colSpan={7} style={{ padding: '7px 18px', background: '#f9fafb', borderBottom: '1px solid #f0f0ee', borderTop: '1px solid #f0f0ee' }}>
                  ...
                  {grp.rows.map(r => {
                    ...
                    const ss = stStyle[r.po_status || 'No PO'] || {}
                    return (
                      <tr key={r.id}
                        ...
                        <td style={{ padding: '8px 12px', borderBottom: '1px solid #f9fafb' }}>
                          <span style={{ background: cs.bg, color: cs.color, fontSize: 10, fontWeight: 600, padding: '2px 7px', borderRadius: 4 }}>{r.cat}</span>
                        </td>
                        ...
                        <td style={{ padding: '8px 12px', borderBottom: '1px solid #f9fafb', fontSize: 12, fontWeight: 700, fontFamily: 'monospace' }}>
                          {r.final_qty || r.base_qty ? `${(r.final_qty || r.base_qty).toLocaleString()} ${r.unit}` : '—'}
                        </td>
                        <td style={{ padding: '8px 12px', borderBottom: '1px solid #f9fafb', fontSize: 12 }}>
                          {suppliers.find(s => s.id === r.supplier_id)?.name || <span style={{ color: '#9ca3af', fontStyle: 'italic' }}>Unassigned</span>}
                        </td>
                        <td style={{ padding: '8px 12px', borderBottom: '1px solid #f9fafb' }}>
                          <span style={{ background: ss.bg || '#f9fafb', color: ss.color || '#6b7280', fontSize: 10, fontWeight: 600, padding: '2px 7px', borderRadius: 4 }}>{r.po_status || 'No PO'}</span>
                        </td>
                      </tr>
                    )
                  })}
      ...
      {showPOModal && <POModal orders={orders} suppliers={suppliers} onClose={() => setShowPOModal(false)} onSaved={() => setShowPOModal(false)} />}
    </>
  )
}
  ...
  return (
    <>
      <div style={{ padding: '10px 24px', borderBottom: '1px solid #f3f4f6', display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
        ...
        <div style={{ marginLeft: 'auto' }}>
          <button className="btn btn-primary btn-sm" onClick={() => { setEditPO(null); setShowModal(true) }}><Plus size={12} /> New PO</button>
        </div>
      </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', overflowX: 'auto' }}>
        {loading ? (
          ...
          <div style={{ padding: 60, textAlign: 'center', color: '#9ca3af', fontSize: 12 }}>No purchase orders yet. Click "+ New PO" to create one.</div>
        ) : (
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 760 }}>
            <thead>
              <tr style={{ background: '#fafafa', position: 'sticky', top: 0, zIndex: 10 }}>
                {['PO Number', 'Supplier', 'Order / Style', 'PO Date', 'Delivery Date', 'Items', 'Status', ''].map(h => (
            ...
            <tbody>
              {filtered.map(p => {
                ...
                const ss = stStyle[p.status] || {}
                const total = (p.purchase_order_items || []).reduce((s, i) => s + (parseFloat(i.amount) || 0), 0)
                return (
                  <tr key={p.id} style={{ cursor: 'pointer' }}
                    onClick={() => { setEditPO({ ...p, items: p.purchase_order_items || [] }); setShowModal(true) }}
                    ...
                    <td style={{ padding: '10px 12px', borderBottom: '1px solid #f9fafb', fontSize: 11 }}>
                      ...
                      {total > 0 && <span style={{ fontWeight: 700, color: '#0d0d0d', marginLeft: 6 }}>{p.currency} {total.toLocaleString(undefined, { maximumFractionDigits: 0 })}</span>}
                    </td>
                    <td style={{ padding: '10px 12px', borderBottom: '1px solid #f9fafb' }}>
                      <span style={{ background: ss.bg || '#f9fafb', color: ss.color || '#6b7280', fontSize: 10, fontWeight: 600, padding: '2px 7px', borderRadius: 4 }}>{p.status}</span>
                    </td>
                    <td style={{ padding: '10px 12px', borderBottom: '1px solid #f9fafb' }}>
  ...
  return (
    <>
      <div style={{ padding: '10px 24px', borderBottom: '1px solid #f3f4f6', display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
        ...
        <div style={{ marginLeft: 'auto' }}>
          <button className="btn btn-primary btn-sm" onClick={() => { setEditWO(null); setShowModal(true) }}><Plus size={12} /> New Work Order</button>
        </div>
      </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', overflowX: 'auto' }}>
        {loading ? (
          ...
          <div style={{ padding: 60, textAlign: 'center', color: '#9ca3af', fontSize: 12 }}>No work orders yet.</div>
        ) : (
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 760 }}>
            <thead>
              <tr style={{ background: '#fafafa', position: 'sticky', top: 0, zIndex: 10 }}>
                {['WO Number', 'Vendor', 'Order / Style', 'Colour', 'Qty', 'Complete By', 'Processes', 'Status', ''].map(h => (
                  <th key={h} style={{ fontSize: 10, fontWeight: 600, color: '#9ca3af', padding: '9px 12px', textAlign: 'left', textTransform: 'uppercase', letterSpacing: '0.5px', borderBottom: '1px solid #f3f4f6', whiteSpace: 'nowrap' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {filtered.map(w => {
                ...
                const ss = stStyle[w.status] || {}
                const total = (w.work_order_items || []).reduce((s, i) => s + (parseFloat(i.amount) || 0), 0)
                const overdue = w.complete_by && new Date(w.complete_by) < today && w.status !== 'Completed'
                return (
                  <tr key={w.id} style={{ cursor: 'pointer' }}
                    onClick={() => { setEditWO({ ...w, items: w.work_order_items || [] }); setShowModal(true) }}
                    onMouseEnter={e => e.currentTarget.style.background = '#fafafa'}
                    onMouseLeave={e => e.currentTarget.style.background = ''}>
                    ...
                    <td style={{ padding: '10px 12px', borderBottom: '1px solid #f9fafb', fontSize: 11 }}>
                      {(w.processes || []).slice(0, 3).join(', ')}{(w.processes || []).length > 3 ? ` +${w.processes.length - 3}` : ''}
                    </td>
                    <td style={{ padding: '10px 12px', borderBottom: '1px solid #f9fafb' }}>
                      ...
                      {total > 0 && <span style={{ fontSize: 11, fontWeight: 700, color: '#0d0d0d', marginLeft: 6 }}>PKR {total.toLocaleString(undefined, { maximumFractionDigits: 0 })}</span>}
                    </td>
                    <td style={{ padding: '10px 12px', borderBottom: '1px solid #f9fafb' }}><Eye size={13} color="#9ca3af" /></td>
                  </tr>
                )
              })}
            </tbody>
...
function DeliveriesTab({ orders, suppliers }) {
  ...
  const [wos, setWos]   = useState([])
  const [loading, setLoading] = useState(true)
  const [filter, setFilter]   = useState('all') // all | overdue | thisweek | thismonth
  ...
  async function load() {
    ...
    const [{ data: p }, { data: w }] = await Promise.all([
      ...
      supabase.from('work_orders').select('*').not('status', 'eq', 'Cancelled').order('complete_by', { nullsLast: true }),
    ])
    setPos(p || [])
    setWos(w || [])
    setLoading(false)
  ...
  function statusOf(dateStr, docStatus) {
    if (docStatus === 'Delivered') return 'Delivered'
  ...
  const stStyle = {
    ...
    'This Week':{ color: '#d97706', bg: '#fff7ed' },
    'This Month':{ color: '#2563eb', bg: '#eff6ff' },
    Upcoming:   { color: '#6b7280', bg: '#f9fafb' },
    Delivered:  { color: '#16a34a', bg: '#f0fdf4' },
    'No Date':  { color: '#9ca3af', bg: '#f9fafb' },
  }
  ...
  const poRows = pos.map(p => {
    ...
    const total    = items.reduce((s, i) => s + (parseFloat(i.amount) || 0), 0)
    const status   = statusOf(p.delivery_date, p.status)
    return { type: 'PO', ref: p.po_number, date: p.delivery_date, party: supplier?.name || '—', order, items: items.length + ' lines', value: total ? `${p.currency || 'PKR'} ${total.toLocaleString(undefined,{maximumFractionDigits:0})}` : '—', status, docStatus: p.status }
  })

  const woRows = wos.map(w => {
    const order  = orders.find(o => o.id === w.order_id)
    const status = statusOf(w.complete_by, w.status)
    const total  = (w.items || []).reduce((s, i) => s + (parseFloat(i.amount) || 0), 0)
    return { type: 'WO', ref: w.wo_number, date: w.complete_by, party: w.vendor_name || '—', order, items: (w.processes || []).join(', ') || '—', value: total ? `PKR ${total.toLocaleString(undefined,{maximumFractionDigits:0})}` : '—', status, docStatus: w.status }
  })

  const all = [...poRows, ...woRows].sort((a, b) => {
    const priority = { Overdue: 0, 'This Week': 1, 'This Month': 2, Upcoming: 3, 'No Date': 4, Delivered: 5 }
    return (priority[a.status] ?? 3) - (priority[b.status] ?? 3) || (a.date || '').localeCompare(b.date || '')
  })
  ...
  const counts = {
    ...
    thisweek:  all.filter(r => r.status === 'This Week').length,
    thismonth: all.filter(r => r.status === 'This Month').length,
  }
  ...
  return (
    <>
      ...
      <div style={{ padding: '10px 24px', borderBottom: '1px solid #f3f4f6', display: 'flex', gap: 12, flexShrink: 0, alignItems: 'center' }}>
        {[
          { label: 'Overdue',    count: counts.overdue,   color: '#dc2626', f: 'overdue' },
          { label: 'This Week',  count: counts.thisweek,  color: '#d97706', f: 'thisweek' },
          { label: 'This Month', count: counts.thismonth, color: '#2563eb', f: 'thismonth' },
        ].map(k => (
          <button key={k.f} onClick={() => setFilter(filter === k.f ? 'all' : k.f)}
            ...
            <span style={{ fontSize: 15, fontWeight: 800, color: k.color, lineHeight: 1 }}>{k.count}</span>
      ...
      <div style={{ flex: 1, overflowY: 'auto', overflowX: 'auto' }}>
        {loading ? (
          ...
          <div style={{ padding: 60, textAlign: 'center', color: '#9ca3af', fontSize: 12 }}>No deliveries match this filter.</div>
        ) : (
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 780 }}>
            <thead>
              <tr style={{ background: '#fafafa', position: 'sticky', top: 0, zIndex: 10 }}>
                {['Type', 'Reference', 'Party', 'Order / Style', 'Due Date', 'Items', 'Value', 'Status'].map(h => (
                  <th key={h} style={{ fontSize: 10, fontWeight: 600, color: '#9ca3af', padding: '9px 12px', textAlign: 'left', textTransform: 'uppercase', letterSpacing: '0.5px', borderBottom: '1px solid #f3f4f6', whiteSpace: 'nowrap' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {filtered.map((r, i) => {
                ...
                const typeColor = r.type === 'PO' ? { color: '#2563eb', bg: '#eff6ff' } : { color: '#7c3aed', bg: '#f5f3ff' }
                return (
                  <tr key={i}
                    onMouseEnter={e => e.currentTarget.style.background = '#fafafa'}
                    onMouseLeave={e => e.currentTarget.style.background = ''}>
                    <td style={{ padding: '9px 12px', borderBottom: '1px solid #f9fafb' }}>
                      <span style={{ background: typeColor.bg, color: typeColor.color, fontSize: 10, fontWeight: 700, padding: '2px 7px', borderRadius: 4 }}>{r.type}</span>
                    </td>
                    ...
                    <td style={{ padding: '9px 12px', borderBottom: '1px solid #f9fafb', fontSize: 11, color: '#6b7280' }}>
                      {r.order ? `${r.order.style_number} · ${r.order.buyer_name}` : '—'}
                    </td>
                    ...
                    <td style={{ padding: '9px 12px', borderBottom: '1px solid #f9fafb', fontSize: 11, color: '#6b7280', maxWidth: 200, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{r.items}</td>
                    <td style={{ padding: '9px 12px', borderBottom: '1px solid #f9fafb', fontSize: 11, fontFamily: 'monospace', fontWeight: 600 }}>{r.value}</td>
                    <td style={{ padding: '9px 12px', borderBottom: '1px solid #f9fafb' }}>
                      <span style={{ background: ss.bg || '#f9fafb', color: ss.color || '#6b7280', fontSize: 10, fontWeight: 600, padding: '2px 7px', borderRadius: 4 }}>{r.status}</span>
                    </td>
                  </tr>
                )
              })}
...
export default function Purchasing() {
  ...
  useEffect(() => {
    supabase.from('orders').select('id,job_id,job_number,buyer_name,style_number,description,total_qty,buyer_id').not('status', 'eq', 'Cancelled').order('job_number')
  ...
  const tabStyle = (t) => ({
    ...
    fontSize: 13, fontWeight: tab === t ? 600 : 400, color: tab === t ? '#0d0d0d' : '#6b7280',
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: '#fff' }}>
      <div style={{ padding: '16px 24px 0', flexShrink: 0 }}>
        ...
        <div style={{ fontSize: 11, color: '#9ca3af', marginBottom: 12 }}>Manage material demand, purchase orders, and work orders</div>
        <div style={{ display: 'flex', borderBottom: '1px solid #e5e7eb' }}>
          {TABS.map(t => <button key={t.id} style={tabStyle(t.id)} onClick={() => setTab(t.id)}>{t.label}</button>)}
        </div>
      </div>

      {tab === 'demand'     && <DemandTab     orders={orders} suppliers={suppliers} />}
