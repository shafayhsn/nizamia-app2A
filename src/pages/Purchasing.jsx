...
function MaterialDemandTab({ suppliers }) {
  ...
  async function load() {
    ...
    const getQueueContext = (q, pd) => {
      ...
      const inferredColor = (!q.color_name && splitParts.length >= 1) ? splitParts[0] : null
      const inferredGroup = (!q.size_group_id && splitParts.length >= 2) ? splitParts[splitParts.length - 1] : null
      const queueColorName = q.color_name || inferredColor || null
      const groupName = q.size_group_id
        ? (pd?.sizeGroupMap?.[q.size_group_id]?.name || null)
        : inferredGroup
      const includeCell = (sgName, colorName) => {
        if (splitRule === 'By Colour' || splitRule === 'By Wash') return colorName === queueColorName
        if (splitRule === 'By Size Group') return sgName === groupName
        if (splitRule === 'Colour × Size Group' || splitRule === 'By Ratio') return sgName === groupName && colorName === queueColorName
        return true
      }
    ...
    const calcReqQtyForQueue = (b, ord, q) => {
      ...
      if (rule === 'Configure Own') {
        if (ud.__matrix) {
          return ud.__matrix.reduce((s,m)=> {
            const matchesGroup = !groupName || m.sgName === groupName || (q.split_rule === 'By Colour' && queueColorName === m.colorName)
            const matchesColor = !queueColorName || m.colorName === queueColorName || q.split_rule === 'By Size Group'
