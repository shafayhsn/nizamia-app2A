...
function MaterialDemandTab({ suppliers }) {
  ...
  async function load() {
    ...
    ;(emb||[]).filter(e=>idSet.has(e.order_id)).forEach(e=>{
      ...
      // Embellishment demand uses usage_rule logic same as BOM
      ...
      if (eUd && ePd) {
        if (eRule==='By Color') { eReqQty=Object.entries(eUd).reduce((s,[n,cons])=>{ const c=ePd.colors.find(c=>c.name===n); return s+(parseFloat(cons)||0)*(c?.qty||0) },0)||eReqQty }
      ...
      built.push({ id:e.id, order_id:e.order_id, order:ord, source_type:'embellishment', source_id:e.id, cat:'Embellishment', name:e.description||e.technique||'Embellishment', specification:[e.technique,e.placement,e.dimensions].filter(Boolean).join(' · '), req_qty:eReqQty, purchased:purchasedMap[e.id]||0, unit:e.unit||'pcs', usage_rule:eRule, usage_data:eUd })
    })
    ;(wash||[]).filter(w=>idSet.has(w.order_id)).forEach(w=>{
      const ord=allOrds.find(o=>o.id===w.order_id); if (!ord) return
      const wPd=poDataMap[w.order_id]; const wColorQty=wPd?.colors.find(c=>c.name===w.color_name)?.qty||parseFloat(w.qty)||0
      built.push({ id:w.id, order_id:w.order_id, order:ord, source_type:'washing', source_id:w.id, cat:'Wash', name:[w.wash_type,w.color_name].filter(Boolean).join(' — ')||'Washing', specification:w.wash_ref||'', req_qty:wColorQty, purchased:purchasedMap[w.id]||0, unit:'pcs' })
    })
    setRows(built)
    setLoading(false)
  }
            ...
            <tbody>
              {Object.values(grouped).map(grp=>(
                <React.Fragment key={grp.order.id}>
                  ...
                  {grp.rows.map(r=>{
                    const cs=catStyle[r.cat]||{}; const bal=r.req_qty-r.purchased
                    return (
