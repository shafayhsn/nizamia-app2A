...
function POModal({ po, orders, suppliers, prefillItems, onClose, onSaved }) {
  ...
  const handlePrint = async () => {
    if (!po) return
    const targetWindow = window.open('', '_blank', 'width=1000,height=800')
    if (!targetWindow) { window.alert('Popup blocked. Please allow popups for this site and try again.'); return }
    const supplier=suppliers.find(s=>s.id===po.supplier_id)
    const order=orders.find(o=>o.id===po.order_id)
    await printPurchaseOrder({ ...po, items }, supplier, order, null, targetWindow)
  }
...
function WOModal({ wo, orders, suppliers, prefillProcessIds, prefillItems, onClose, onSaved }) {
  ...
  const handlePrint = async () => {
    ...
    await printWorkOrder({ ...wo, items }, vendor, order, targetWindow)
  }
...
function PurchaseOrdersTab({ orders, suppliers }) {
  ...
  const handlePrint = async (p,e) => {
    ...
    const targetWindow = window.open('', '_blank', 'width=1000,height=800')
    if (!targetWindow) { window.alert('Popup blocked. Please allow popups for this site and try again.'); return }
    await printPurchaseOrder({ ...p, items:p.purchase_order_items||[] }, suppliers.find(s=>s.id===p.supplier_id), orders.find(o=>o.id===p.order_id), null, targetWindow)
  }
...
function WorkOrdersTab({ orders, suppliers }) {
  ...
  const handlePrint = async (w,e) => {
    ...
    const targetWindow = window.open('', '_blank', 'width=1000,height=800')
    if (!targetWindow) { window.alert('Popup blocked. Please allow popups for this site and try again.'); return }
    await printWorkOrder({ ...w, items:w.work_order_items||[] }, suppliers.find(s=>s.id===w.vendor_id), orders.find(o=>o.id===w.order_id), targetWindow)
  }
