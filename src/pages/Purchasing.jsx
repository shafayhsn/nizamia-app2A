...
function MaterialDemandTab({ suppliers }) {
  ...
  const qKey = (rowId, qNo) => `q:${rowId}:${qNo}`
  const selectedQueueKeysForRow = (r) => (r.queue_breakdown || []).map(q => qKey(r.id, q.q_number)).filter(k => selected.has(k))
  const rowSelectionState = (r) => {
    const childKeys = (r.queue_breakdown || []).map(q => qKey(r.id, q.q_number))
  ...
  const toggleQueueSelection = (r, q) => {
    const key = qKey(r.id, q.q_number)
    setSelected(prev => {
      const s = new Set(prev)
      s.has(key) ? s.delete(key) : s.add(key)
      s.delete(rowKey(r.id))
  ...
  const getAllSelectableKeys = () => filtered.flatMap(r => (r.queue_breakdown || []).length ? (r.queue_breakdown || []).map(q => qKey(r.id, q.q_number)) : [rowKey(r.id)])
  const selectionCount = selected.size
  const buildQueueLine = (r, q) => ({
    _id: `${r.id}_${q.q_number}`,
    description: r.name,
    specification: `${q.label || q.q_number}${r.specification ? ' · ' + r.specification : ''} [${q.q_number}]`,
    qty: q.req_qty || '',
    ...
    for (const r of filtered) {
      ...
      if (childKeys.length) {
        ;(r.queue_breakdown || []).forEach(q => {
          if (selected.has(qKey(r.id, q.q_number))) lines.push(buildQueueLine(r, q))
        })
        continue
      }
