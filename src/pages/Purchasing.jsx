...
function MaterialDemandTab({ suppliers }) {
  ...
  async function load() {
    ...
    ;(wash||[]).filter(w=>idSet.has(w.order_id)).forEach(w=>{
      ...
      built.push({ id:w.id, order_id:w.order_id, order:ord, source_type:'washing', source_id:w.id, cat:'Wash', name:[w.wash_type,w.color_name].filter(Boolean).join(' — ')||'Washing', specification:w.wash_ref||'', req_qty:parseFloat(w.qty)||0, purchased:purchasedMap[w.id]||0, unit:w.unit||'pcs' })
    })
    setRows(built)
    setLoading(false)
  }
