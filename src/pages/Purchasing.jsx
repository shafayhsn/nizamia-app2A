...
function PurchaseOrdersTab({ orders, suppliers }) {
  ...
  const handleDelete = async (p, e) => {
    e.stopPropagation()
    if (!window.confirm(`Delete PO ${p.po_number}?`)) return
    const { error } = await supabase.from('purchase_orders').delete().eq('id', p.id)
    if (error) { window.alert(error.message || 'Could not delete PO.'); return }
    load()
  }
            ...
            <tbody>{filtered.map(p=>{
              ...
              return (
                <tr key={p.id} style={{ cursor:'pointer' }} onClick={()=>{ setEditPO({...p,items:p.purchase_order_items||[]}); setShowModal(true) }}
                  ...
                  <td style={{ padding:'9px 8px', borderBottom:'1px solid #f9fafb', whiteSpace:'nowrap' }}>
                    <button className="btn btn-ghost btn-sm" onClick={e=>handlePrint(p,e)}><Printer size={12} color="#9ca3af"/></button>
                    <button className="btn btn-ghost btn-sm" onClick={e=>handleDelete(p,e)}><Trash2 size={12} color="#dc2626"/></button>
                  </td>
                </tr>
              )
            })}</tbody>
...
function WorkOrdersTab({ orders, suppliers }) {
  ...
  const handleDelete = async (w, e) => {
    e.stopPropagation()
    if (!window.confirm(`Delete WO ${w.wo_number}?`)) return
    const { error } = await supabase.from('work_orders').delete().eq('id', w.id)
    if (error) { window.alert(error.message || 'Could not delete WO.'); return }
    load()
  }
            ...
            <tbody>{filtered.map(w=>{
              ...
              return (
                <tr key={w.id} style={{ cursor:'pointer' }} onClick={()=>{ setEditWO({...w,items:w.work_order_items||[]}); setShowModal(true) }}
                  ...
                  <td style={{ padding:'9px 8px', borderBottom:'1px solid #f9fafb', whiteSpace:'nowrap' }}>
                    <button className="btn btn-ghost btn-sm" onClick={e=>handlePrint(w,e)}><Printer size={12} color="#9ca3af"/></button>
                    <button className="btn btn-ghost btn-sm" onClick={e=>handleDelete(w,e)}><Trash2 size={12} color="#dc2626"/></button>
                  </td>
                </tr>
              )
            })}</tbody>
