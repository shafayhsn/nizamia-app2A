        ...
        <div style={{ flex:1, overflowY:'auto', padding:'20px 24px' }}>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:12, marginBottom:16 }}>
            ...
            <div><label style={lbl}>Order (optional)</label>
              <select style={mkSel(locked)} value={form.order_id} onChange={set('order_id')} disabled={locked}>
                <option value="">— Not linked —</option>
                {orders.map(o=><option key={o.id} value={o.id}>{o.style_number} · {o.buyer_name}</option>)}
              </select></div>
            <div><label style={lbl}>PO Date</label><input style={mkInp(locked)} type="date" value={form.po_date} onChange={set('po_date')} disabled={locked} /></div>
          ...
          <table style={{ width:'100%', borderCollapse:'collapse', marginBottom:10 }}>
            <thead><tr>{['Description','Specification','Qty','Unit','Unit Rate','Amount',''].map(h=>(
            ...
            <tbody>{items.map((item,idx)=>(
              <tr key={item._id||idx}>
                ...
                <td style={{ padding:'3px 4px', width:80 }}><select style={{ ...mkSel(locked), height:28 }} value={item.unit||'yards'} onChange={e=>updItem(idx,'unit',e.target.value)} disabled={locked}>{['yards','meters','pcs','kg','cone','set','sht','ctn','roll','ltr'].map(u=><option key={u}>{u}</option>)}</select></td>
...
function WOModal({ wo, orders, suppliers, prefillProcessIds, onClose, onSaved }) {
  const isNew = !wo
  const [form, setForm] = useState({
  ...
  useEffect(() => {
    if (!prefillProcessIds?.length || !processesLoaded || !orderProcesses.length || items.length) return
  ...
  }, [processesLoaded, orderProcesses]) // eslint-disable-line
            ...
            <table style={{ width:'100%', borderCollapse:'collapse', marginBottom:10 }}>
              ...
              <tbody>{items.map((item,idx)=>(
                <tr key={item._id||idx}>
                  ...
                  <td style={{ padding:'3px 4px', width:80 }}><input style={{ ...mkInp(locked), height:28, textAlign:'right' }} type="number" value={item.qty} onChange={e=>updItem(idx,'qty',e.target.value)} disabled={locked}/></td>
                  <td style={{ padding:'3px 4px', width:70 }}><select style={{ ...mkSel(locked), height:28 }} value={item.unit||'pcs'} onChange={e=>updItem(idx,'unit',e.target.value)} disabled={locked}>{['pcs','yards','meters','kg','set'].map(u=><option key={u}>{u}</option>)}</select></td>
...
function MaterialDemandTab({ suppliers }) {
  ...
  async function load() {
    setLoading(true)
    const { data:ords } = await supabase.from('orders').select('id,job_id,job_number,buyer_name,style_number,total_qty,queue_split_rule').not('status','eq','Cancelled').order('job_number')
    const allOrds = ords||[]
    ...
    const [{ data:bom }, { data:emb }, { data:wash }, { data:poItems }, { data:sizeGroups }, { data:queues }] = await Promise.all([
    ...
    const calcReqQty = (b, ord) => {
      ...
      if (rule === 'By Color') {
        return Object.entries(ud).reduce((s,[n,cons])=>{
          ...
          return s + (parseFloat(cons)||0) * totalColorQty * (1+wastage/100)
        },0)
      }
    ...
    const getQueueContext = (q, pd) => {
      const groupName = q.size_group_id ? (pd?.sizeGroupMap?.[q.size_group_id]?.name || null) : null
      const splitRule = q.split_rule || 'None'
      const includeCell = (sgName, colorName) => {
        if (splitRule === 'By Colour' || splitRule === 'By Wash') return colorName === q.color_name
        if (splitRule === 'By Size Group') return sgName === groupName
        if (splitRule === 'Colour × Size Group' || splitRule === 'By Ratio') return sgName === groupName && colorName === q.color_name
        return true
      }
      ...
      return { groupName, splitRule, includeCell, sizeQtyForQueue, colorQtyForQueue, sizeGroupQtyForQueue, queueQty }
    }

    const calcReqQtyForQueue = (b, ord, q) => {
      const pd = poDataMap[b.order_id]
      if (!pd) return 0
      const { groupName, queueQty, sizeQtyForQueue, colorQtyForQueue, sizeGroupQtyForQueue } = getQueueContext(q, pd)
      const rule = b.usage_rule || 'Generic'
      const ud = b.usage_data || {}
      ...
      if (rule === 'By Color') {
        return Object.entries(ud).reduce((s,[colorName, cons]) => s + ((parseFloat(cons)||0) * colorQtyForQueue(colorName) * wastageFactor), 0)
      }
      if (rule === 'By Size Group') {
        return Object.entries(ud).reduce((s,[sgName, cons]) => s + ((parseFloat(cons)||0) * sizeGroupQtyForQueue(sgName) * wastageFactor), 0)
      }
      if (rule === 'By Individual Sizes') {
        return Object.entries(ud).reduce((s,[size, cons]) => s + ((parseFloat(cons)||0) * sizeQtyForQueue(size) * wastageFactor), 0)
      }
      if (rule === 'Configure Own') {
        if (ud.__matrix) {
          return ud.__matrix.reduce((s,m)=> {
            const matchesGroup = !groupName || m.sgName === groupName || (q.split_rule === 'By Colour' && q.color_name === m.colorName)
            const matchesColor = !q.color_name || m.colorName === q.color_name || q.split_rule === 'By Size Group'
            if (!matchesGroup || !matchesColor) return s
            const cellQty = pd.sgColorTotals?.[m.sgName]?.[m.colorName] || 0
            return s + ((parseFloat(m.consumption)||0) * cellQty * wastageFactor)
          },0)
        }
        const groups = ud.__groups || []
        return groups.reduce((s,g)=> {
          ...
          return s + ((parseFloat(g.consumption)||0) * qty * wastageFactor)
        },0)
      }
      return 0
    }
    ...
    const calcEmbellishmentReqForQueue = (e, ord, q) => {
      ...
      const { colorQtyForQueue, sizeGroupQtyForQueue, sizeQtyForQueue, groupName } = getQueueContext(q, pd)
      if (rule === 'By Color') return Object.entries(ud).reduce((s,[n,cons])=>s+((parseFloat(cons)||0)*colorQtyForQueue(n)),0)
      if (rule === 'By Size Group') return Object.entries(ud).reduce((s,[n,cons])=>s+((parseFloat(cons)||0)*sizeGroupQtyForQueue(n)),0)
      if (rule === 'By Individual Sizes') return Object.entries(ud).reduce((s,[sz,cons])=>s+((parseFloat(cons)||0)*sizeQtyForQueue(sz)),0)
      if (rule === 'Configure Own' && ud.__matrix) {
        return ud.__matrix.reduce((s,m)=> {
          ...
          if (!matchesGroup || !matchesColor) return s
          const cellQty = pd.sgColorTotals?.[m.sgName]?.[m.colorName] || 0
          return s + ((parseFloat(m.consumption)||0) * cellQty)
        },0)
      }
    ...
    ;(bom||[]).filter(b=>idSet.has(b.order_id)).forEach(b=>{
      ...
      const queue_breakdown = (queuesByOrder[b.order_id] || []).map(q => ({
        id: `${b.id}_${q.id}`,
        q_number: q.q_number,
        label: q.label,
        ...
        unit: b.unit || '',
      })).filter(x => x.req_qty > 0)
      built.push({ id:b.id, order_id:b.order_id, order:ord, source_type:'bom', source_id:b.id, cat, name:b.name, specification:b.specification||b.detail||'', req_qty, purchased:purchasedMap[b.id]||0, unit:b.unit||'', usage_rule:b.usage_rule, usage_data:b.usage_data, wastage:b.wastage, base_qty:b.base_qty, queue_breakdown })
    })
    ;(emb||[]).filter(e=>idSet.has(e.order_id)).forEach(e=>{
      ...
      if (eUd && ePd) {
        if (eRule==='By Color') { eReqQty=Object.entries(eUd).reduce((s,[n,cons])=>{ const totalColorQty = (ePd.colors||[]).filter(c=>c.name===n).reduce((ss,c)=>ss+(c.qty||0),0); return s+(parseFloat(cons)||0)*totalColorQty },0)||eReqQty }
        else if (eRule==='By Size Group') { eReqQty=Object.entries(eUd).reduce((s,[n,cons])=>{ const g=ePd.sizeGroups.find(x=>x.name===n); return s+(parseFloat(cons)||0)*(g?.qty||0) },0)||eReqQty }
        else if (eRule==='By Individual Sizes') { eReqQty=Object.entries(eUd).reduce((s,[sz,cons])=>{ const f=ePd.allSizes.find(x=>x.size===sz); return s+(parseFloat(cons)||0)*(f?.qty||0) },0)||eReqQty }
        else if (eRule==='Configure Own'&&eUd.__matrix) { eReqQty=eUd.__matrix.reduce((s,m)=>{ const sg=ePd.sgMatrix?.find(x=>x.sgName===m.sgName); const c=sg?.colors.find(x=>x.colorName===m.colorName); return s+(parseFloat(m.consumption)||0)*(c?.qty||0) },0)||eReqQty }
        else if (eRule==='Generic'&&eUd.generic) { eReqQty=(parseFloat(eUd.generic)||0)*(parseFloat(ord.total_qty)||0)||eReqQty }
      }
      const queue_breakdown = (queuesByOrder[e.order_id] || []).map(q => ({
        id: `${e.id}_${q.id}`,
        q_number: q.q_number,
        label: q.label,
        split_rule: q.split_rule,
      ...
      if (r.source_type === 'bom' && r.usage_rule && r.usage_rule !== 'Generic' && r.usage_data) {
        ...
        if (rule === 'By Color' && pd) {
          expanded = Object.entries(ud).map(([colorName, cons]) => {
            const totalColorQty = (pd.colors||[]).filter(c=>c.name===colorName).reduce((ss,c)=>ss+(c.qty||0),0)
            const qty = Math.round((parseFloat(cons) || 0) * totalColorQty * (1 + wastage / 100) * 100) / 100
    ...
    <>
      <div style={{ padding:'10px 24px', borderBottom:'1px solid #f3f4f6', display:'flex', alignItems:'center', gap:8, flexShrink:0 }}>
        ...
        <div style={{ marginLeft:'auto' }}>
          <button className="btn btn-primary btn-sm" onClick={()=>setShowPOModal(true)} disabled={selected.size===0}>+ Generate PO {selected.size>0?`(${selected.size})`:''}</button>
        </div>
      </div>
      {selected.size>0&&(
        ...
        :(
          <table style={{ width:'100%', borderCollapse:'collapse', minWidth:980 }}>
            <thead><tr style={{ background:'#fafafa', position:'sticky', top:0, zIndex:10 }}>
              <th style={{ width:40, padding:'9px 0 9px 18px', borderBottom:'1px solid #f3f4f6' }}><input type="checkbox" checked={selected.size===filtered.length&&filtered.length>0} onChange={()=>setSelected(selected.size===filtered.length?new Set():new Set(filtered.map(r=>r.id)))}/></th>
                  ...
                  {grp.rows.map(r=>{
                    ...
                    return (
                      <React.Fragment key={r.id}>
                        <tr style={{ background:selected.has(r.id)?'#f0f9ff':'' }}
                          ...
                          <td style={{ padding:'8px 0 8px 18px', borderBottom:isOpen?'none':'1px solid #f9fafb' }}><input type="checkbox" checked={selected.has(r.id)} onChange={()=>setSelected(p=>{ const s=new Set(p); s.has(r.id)?s.delete(r.id):s.add(r.id); return s })}/></td>
                        ...
                        {isOpen && (r.queue_breakdown || []).map(q => (
                          <tr key={q.id} style={{ background:'#fcfcfc' }}>
                            <td></td>
                            <td></td>
                            <td colSpan={2} style={{ padding:'8px 12px 8px 24px', borderBottom:'1px solid #f3f4f6' }}>
                              <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                                <span style={{ fontFamily:'var(--font-mono)', fontWeight:800, fontSize:11, color:'#0d0d0d', background:'#f3f4f6', padding:'2px 7px', borderRadius:5 }}>{q.q_number}</span>
...
function ProcessDemandTab({ suppliers }) {
  ...
  async function load() {
    ...
    if (!jobOrds.length) { setRows([]); setLoading(false); return }
    const idSet = new Set(jobOrds.map(o=>o.id))
    const [{ data:procs }, { data:woItems }] = await Promise.all([
    ...
    ;(procs||[]).filter(p=>idSet.has(p.order_id)).forEach(p=>{
      ...
      built.push({ id:p.id, order_id:p.order_id, order:ord, process_name:p.process_name, req_qty:parseFloat(ord.total_qty)||0, assigned:assignedMap[p.id]||0, unit:'pcs' })
    })
    setRows(built)
    setLoading(false)
  }
  ...
  const handleGenerateWO = () => {
    const firstRow = rows.find(r=>selected.has(r.id))
    if (!firstRow) return
  ...
  return (
    <>
      <div style={{ padding:'10px 24px', borderBottom:'1px solid #f3f4f6', display:'flex', alignItems:'center', gap:8, flexShrink:0 }}>
        ...
        <div style={{ marginLeft:'auto' }}>
          <button className="btn btn-primary btn-sm" onClick={handleGenerateWO} disabled={selected.size===0}>+ Generate WO {selected.size>0?`(${selected.size})`:''}</button>
        </div>
      </div>
      {selected.size>0&&(
        <div style={{ padding:'7px 24px', background:'#f5f3ff', borderBottom:'1px solid #ede9fe', display:'flex', alignItems:'center', gap:8, flexShrink:0 }}>
          <span style={{ fontSize:12, color:'#7c3aed', fontWeight:600 }}>{selected.size} processes selected</span>
          <button className="btn btn-sm btn-secondary" onClick={()=>setSelected(new Set())}><X size={11}/> Clear</button>
        </div>
      )}
      <div style={{ flex:1, overflowY:'auto', overflowX:'auto' }}>
        ...
        :(
          <table style={{ width:'100%', borderCollapse:'collapse', minWidth:760 }}>
            ...
            <tbody>
              {Object.values(grouped).map(grp=>(
                <React.Fragment key={grp.order.id}>
                  <tr><td colSpan={6} style={{ padding:'7px 18px', background:'#f9fafb', borderBottom:'1px solid #f0f0ee', borderTop:'1px solid #f0f0ee' }}>
                    <span style={{ fontSize:11, fontWeight:700, fontFamily:'monospace', color:'#0d0d0d' }}>{grp.order.job_number||'—'}</span>
                    <span style={{ fontSize:11, color:'#6b7280', marginLeft:8 }}>{grp.order.buyer_name} · {grp.order.style_number}</span>
                    {grp.order.total_qty&&<span style={{ fontSize:11, color:'#9ca3af', marginLeft:8 }}>{grp.order.total_qty.toLocaleString()} pcs</span>}
                  </td></tr>
                  {grp.rows.map(r=>{
                    ...
                    return (
                      <tr key={r.id} style={{ background:selected.has(r.id)?'#faf5ff':'' }}
                        onMouseEnter={e=>{ if(!selected.has(r.id)) e.currentTarget.style.background='#fafafa' }}
                        onMouseLeave={e=>{ if(!selected.has(r.id)) e.currentTarget.style.background='' }}>
                        <td style={{ padding:'8px 0 8px 18px', borderBottom:'1px solid #f9fafb' }}><input type="checkbox" checked={selected.has(r.id)} onChange={()=>setSelected(p=>{ const s=new Set(p); s.has(r.id)?s.delete(r.id):s.add(r.id); return s })}/></td>
      ...
      {showWOModal&&(
        <WOModal wo={prefillOrderId?{order_id:prefillOrderId}:null} orders={orders} suppliers={suppliers} prefillProcessIds={[...selected]}
        ...
        :(
          <table style={{ width:'100%', borderCollapse:'collapse', minWidth:760 }}>
            <thead><tr style={{ background:'#fafafa', position:'sticky', top:0, zIndex:10 }}>
              ...
              <th style={{ borderBottom:'1px solid #f3f4f6', width:40 }}/>
            </tr></thead>
            <tbody>{filtered.map(p=>{
              ...
              return (
                <tr key={p.id} style={{ cursor:'pointer' }} onClick={()=>{ setEditPO({...p,items:p.purchase_order_items||[]}); setShowModal(true) }}
                  ...
                  <td style={{ padding:'9px 8px', borderBottom:'1px solid #f9fafb' }}><button className="btn btn-ghost btn-sm" onClick={e=>handlePrint(p,e)}><Printer size={12} color="#9ca3af"/></button></td>
                </tr>
              )
            })}</tbody>
          </table>
            ...
            <tbody>{filtered.map(w=>{
              ...
              return (
                <tr key={w.id} style={{ cursor:'pointer' }} onClick={()=>{ setEditWO({...w,items:w.work_order_items||[]}); setShowModal(true) }}
                  ...
                  <td style={{ padding:'9px 8px', borderBottom:'1px solid #f9fafb' }}><button className="btn btn-ghost btn-sm" onClick={e=>handlePrint(w,e)}><Printer size={12} color="#9ca3af"/></button></td>
                </tr>
              )
            })}</tbody>
          </table>
