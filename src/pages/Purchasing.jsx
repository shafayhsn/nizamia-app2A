                    ...
                    return (
                      <React.Fragment key={r.id}>
                        ...
                        {isOpen && (r.queue_breakdown || []).map(q => {
                          const qSelected = selected.has(qKey(r.id, q.q_number))
                          return (
...
function ProcessDemandTab({ suppliers }) {
  ...
  async function load() {
    ...
    ;(procs||[]).filter(p=>idSet.has(p.order_id)).forEach(p=>{
      ...
      const queue_breakdown = (queues||[]).filter(q=>q.order_id===p.order_id).map(q => ({
        id: `${p.id}_${q.id}`,
        q_number: q.q_number,
        label: q.label,
        split_rule: q.split_rule,
      ...
      const req_qty = queue_breakdown.reduce((s,x)=>s+(parseFloat(x.req_qty)||0),0) || parseFloat(ord.total_qty)||0
  ...
  const buildPrefillItems = () => {
    ...
    for (const r of filtered) {
      ...
      if (childKeys.length) {
        ;(r.queue_breakdown || []).forEach(q => {
          if (selected.has(qKey(r.id, q.q_number))) {
            lines.push({
              _id: `${r.id}_${q.q_number}`,
  ...
  const handleGenerateWO = () => {
    const firstRow = filtered.find(r => selected.has(rowKey(r.id)) || (r.queue_breakdown || []).some(q => selected.has(qKey(r.id, q.q_number))))
    if (!firstRow) return
                            ...
                            <tr key={q.id} style={{ background:qSelected?'#fdf4ff':'#fcfcfd' }}>
                              ...
                              <td style={{ padding:'8px 12px', borderBottom:'1px solid #f3f4f6', fontSize:12 }}>
                                <div style={{ display:'flex', alignItems:'center', gap:8, flexWrap:'wrap' }}>
                                  <span style={{ fontFamily:'monospace', fontSize:11, fontWeight:700, color:'#7c3aed' }}>{q.q_number}</span>
