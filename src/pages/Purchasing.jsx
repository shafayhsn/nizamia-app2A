  ...
  return (
    <>
      <div style={{ padding:'10px 24px', borderBottom:'1px solid #f3f4f6', display:'flex', alignItems:'center', gap:8, flexShrink:0 }}>
        ...
        <div style={{ marginLeft:'auto' }}><button className="btn btn-primary btn-sm" onClick={()=>{ if(!access.canCreatePO(user)){ alert('Access Denied'); return } setEditPO(null); setShowModal(true) }}><Plus size={12}/> New PO</button></div>
      </div>
      <div style={{ flex:1, overflowY:'auto', overflowX:'auto' }}>
        {loading?<div style={{ padding:60, textAlign:'center', color:'#9ca3af', fontSize:12 }}>Loading...</div>
        ...
        :(
          <table style={{ width:'100%', borderCollapse:'collapse', minWidth:760 }}>
            ...
            <tbody>{filtered.map(p=>{
              ...
              return (
                <tr key={p.id} style={{ cursor:'pointer' }} onClick={()=>{ if(!access.canEditPO(user)){ alert('Access Denied'); return } setEditPO({...p,items:p.purchase_order_items||[]}); setShowModal(true) }}
