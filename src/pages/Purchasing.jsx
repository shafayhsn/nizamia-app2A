...
function stripPOMeta(raw = '') {
  return String(raw || '').replace(NOTES_META_RE, '').replace(NOTES_TAX_RE, '').trim()
}
function encodeNotes(notes, taxEnabled, taxRate, version = 1) {
  const clean = stripPOMeta(notes)
  const taxMeta = `[PO_TAX:${JSON.stringify({ enabled: !!taxEnabled, rate: parseFloat(taxRate) || 0 })}]`
  const poMeta = `[PO_META:${JSON.stringify({ version: Math.max(1, parseInt(version, 10) || 1) })}]`
  return `${clean}${clean ? '\n' : ''}${taxMeta}\n${poMeta}`
}
function decodeNotes(notes) {
  const raw = String(notes || '')
  const taxMatch = raw.match(NOTES_TAX_RE)
  const metaMatch = raw.match(NOTES_META_RE)
  let tax = null
  let meta = { version: 1 }
  if (taxMatch) { try { tax = JSON.parse(taxMatch[1]) } catch { tax = null } }
  if (metaMatch) { try { meta = { ...meta, ...JSON.parse(metaMatch[1]) } } catch { meta = { version: 1 } } }
  return { notes: stripPOMeta(raw), tax, meta: { version: Math.max(1, parseInt(meta.version, 10) || 1) } }
}
function getPOTerms() {
  return localStorage.getItem(PO_TERMS_KEY) || DEFAULT_PO_TERMS
}
function getPOVersion(notes) {
  return decodeNotes(notes).meta.version || 1
}
...
function POModal({ po, orders, suppliers, prefillItems, onClose, onSaved }) {
  ...
  const [poVersion, setPOVersion] = useState(decodedPO.meta?.version || 1)
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const nextVersion = po?.id ? Math.max(1, poVersion + 1) : 1
      const payload = { ...form, po_number:poNum, notes: encodeNotes(form.notes, taxEnabled, taxRate, nextVersion) }
      let poId
      if (po?.id) {
        await supabase.from('purchase_orders').update(payload).eq('id', po.id)
        setPOVersion(nextVersion)
        poId = po.id
  ...
  const handlePrint = async () => {
    ...
    await printPurchaseOrder({ ...po, notes: encodeNotes(form.notes, taxEnabled, taxRate, poVersion), items }, supplier, order, taxEnabled ? taxRate : null)
  }
    ...
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.45)', zIndex:300, display:'flex', alignItems:'center', justifyContent:'center', padding:24 }}>
      <div style={{ background:'#fff', borderRadius:12, width:'100%', maxWidth:860, maxHeight:'calc(100vh - 48px)', display:'flex', flexDirection:'column', boxShadow:'0 24px 80px rgba(0,0,0,0.22)', overflow:'hidden' }}>
        <div style={{ padding:'14px 24px', borderBottom:'1px solid #f0f0ee', display:'flex', alignItems:'center', gap:10 }}>
          ...
          <div style={{ flex:1 }} />
          <button type="button" disabled={locked} onClick={()=>!locked&&setTaxEnabled(v=>!v)} style={{ border:`2px solid ${taxEnabled?'#0d0d0d':'#d1d5db'}`, background:taxEnabled?'#0d0d0d':'#fff', color:taxEnabled?'#fff':'#6b7280', borderRadius:999, padding:'7px 12px', fontSize:12, fontWeight:700, letterSpacing:'0.3px', cursor:locked?'not-allowed':'pointer' }}>
            ...
            <tbody>
              {Object.values(grouped).map(grp=>(
                <React.Fragment key={grp.order.id}>
                  <tr><td colSpan={7} style={{ padding:'7px 18px', background:'var(--app-group-row-bg, #f9fafb)', color:'var(--app-group-row-text, #111827)', borderBottom:'1px solid #f0f0ee', borderTop:'1px solid #f0f0ee' }}>
                          ...
                          <tr key={q.id} style={{ background:qSelected?'#eff6ff':'#fcfcfc' }}>
                            ...
                            <td colSpan={2} style={{ padding:'8px 12px 8px 24px', borderBottom:'1px solid #f3f4f6' }}>
                              <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                                <span style={{ fontFamily:'var(--font-mono)', fontWeight:700, fontSize:11, color:'#0d0d0d', background:'#f3f4f6', padding:'2px 7px', borderRadius:5 }}>{q.q_number || 'Queued'}</span>
...
function PurchaseOrdersTab({ orders, suppliers }) {
  ...
  const selectedPOs = [...selectedIds].map(id => filtered.find(p => p.id === id) || pos.find(p => p.id === id)).filter(Boolean)
  const toggleSelect = (id) => setSelectedIds(prev => { const next = new Set(prev); next.has(id) ? next.delete(id) : next.add(id); return next })
  const toggleAll = () => setSelectedIds(filtered.length && filtered.every(p => selectedIds.has(p.id)) ? new Set() : new Set(filtered.map(p => p.id)))
  const stStyle = { draft:{color:'#d97706',bg:'#fff7ed'}, approved:{color:'#16a34a',bg:'#f0fdf4'}, Draft:{color:'#d97706',bg:'#fff7ed'}, Issued:{color:'#2563eb',bg:'#eff6ff'}, Acknowledged:{color:'#7c3aed',bg:'#f5f3ff'}, Delivered:{color:'#16a34a',bg:'#f0fdf4'}, Cancelled:{color:'#dc2626',bg:'#fef2f2'} }

  const handlePrint = async (p,e) => {
    e.stopPropagation()
    if (!access.canPrintPO(user)) { alert('Access Denied'); return }
  ...
  const printSelectedPOs = async () => {
    const targets = selectedPOs.length ? selectedPOs : filtered
    for (const p of targets) await printPurchaseOrder({ ...p, items:p.purchase_order_items||[] }, suppliers.find(s=>s.id===p.supplier_id), orders.find(o=>o.id===p.order_id), null)
  }
  const archiveSelectedPOs = async () => {
    if (!selectedPOs.length) return
    if (!window.confirm(`Cancel/archive ${selectedPOs.length} selected PO(s)?`)) return
    await supabase.from('purchase_orders').update({ status:'Cancelled' }).in('id', selectedPOs.map(p=>p.id))
    setSelectedIds(new Set()); load()
  }

  const handleDeletePO = async (p,e) => {
    e.stopPropagation()
    if (!user || user.role !== 'admin') { alert('Access Denied'); return }
  ...
  const handleTogglePOApproval = async (p, e) => {
    e.stopPropagation()
    if (!access.canApprovePO(user)) { alert('Access Denied'); return }
    const nextStatus = String(p.status || '').toLowerCase() === 'approved' ? 'draft' : 'approved'
    if (!window.confirm(`${nextStatus === 'approved' ? 'Approve' : 'Unapprove'} PO ${p.po_number}?`)) return
    await supabase.from('purchase_orders').update({ status: nextStatus }).eq('id', p.id)
    load()
  }
    ...
    <>
      <div style={{ padding:'10px 24px', borderBottom:'1px solid #f3f4f6', display:'flex', alignItems:'center', gap:8, flexShrink:0 }}>
        ...
        <div style={{ marginLeft:'auto', display:'flex', gap:8, alignItems:'center' }}>
          <label className="btn btn-secondary btn-sm" style={{ display:'inline-flex', alignItems:'center', gap:6 }}><input type="checkbox" checked={multiSelect} onChange={e => { setMultiSelect(e.target.checked); setSelectedIds(new Set()) }} /> Multi-select</label>
          {multiSelect && <button className="btn btn-secondary btn-sm" disabled={!filtered.length} onClick={toggleAll}>{filtered.length && filtered.every(p => selectedIds.has(p.id)) ? 'Clear' : 'Select Filtered'}</button>}
          {multiSelect && <span style={{ fontSize:11, fontWeight:700, color:selectedPOs.length?'#111827':'#9ca3af' }}>{selectedPOs.length} selected</span>}
          {multiSelect && <button className="btn btn-secondary btn-sm" disabled={!selectedPOs.length && !filtered.length} onClick={printSelectedPOs}>Print</button>}
          {multiSelect && <button className="btn btn-secondary btn-sm" disabled={!selectedPOs.length} onClick={archiveSelectedPOs}>Archive</button>}
          <button className="btn btn-primary btn-sm" onClick={()=>{ if(!access.canCreatePO(user)){ alert('Access Denied'); return } setEditPO(null); setShowModal(true) }}><Plus size={12}/> New PO</button>
        </div>
      </div>
      <div style={{ flex:1, overflowY:'auto', overflowX:'auto' }}>
        {loading?<div style={{ padding:60, textAlign:'center', color:'#9ca3af', fontSize:12 }}>Loading...</div>
        ...
        :(
          <table style={{ width:'100%', borderCollapse:'collapse', minWidth:760 }}>
            <thead><tr style={{ background:'#fafafa', position:'sticky', top:0, zIndex:10 }}>
              {multiSelect && <th style={{ borderBottom:'1px solid #f3f4f6', width:36 }}><input type="checkbox" checked={filtered.length>0 && filtered.every(p => selectedIds.has(p.id))} onChange={toggleAll} /></th>}<TH>PO Number</TH><TH>Supplier</TH><TH>Order / Style</TH><TH>PO Date</TH><TH>Delivery</TH><TH>Items</TH><TH>Status</TH>
            ...
            <tbody>{filtered.map(p=>{
              ...
              return (
                <tr key={p.id} style={{ cursor:'pointer', background:selectedIds.has(p.id)?'#f0f9ff':'' }} onClick={()=>{ if(!access.canEditPO(user)){ alert('Access Denied'); return } setEditPO({...p,items:p.purchase_order_items||[]}); setShowModal(true) }}
                  onMouseEnter={e=>e.currentTarget.style.background='#fafafa'} onMouseLeave={e=>e.currentTarget.style.background=''}>
                  {multiSelect && <td style={{ padding:'9px 8px', borderBottom:'1px solid #f9fafb' }}><input type="checkbox" checked={selectedIds.has(p.id)} onClick={e=>e.stopPropagation()} onChange={() => toggleSelect(p.id)} /></td>}
                  ...
                  <td style={{ padding:'9px 8px', borderBottom:'1px solid #f9fafb', whiteSpace:'nowrap' }}><button className="btn btn-ghost btn-sm" title="Edit" onClick={e=>{ e.stopPropagation(); if(!access.canEditPO(user)){ alert('Access Denied'); return } setEditPO({...p,items:p.purchase_order_items||[]}); setShowModal(true) }}><Pencil size={12} color="#6b7280"/></button><button className="btn btn-ghost btn-sm" title="Print" onClick={e=>handlePrint(p,e)}><Printer size={12} color="#6b7280"/></button>{access.canApprovePO(user)&&<button className="btn btn-ghost btn-sm" title={String(p.status||'').toLowerCase()==='approved'?'Unapprove':'Approve'} onClick={e=>handleTogglePOApproval(p,e)}>{String(p.status||'').toLowerCase()==='approved'?<Unlock size={12} color="#d97706"/>:<Lock size={12} color="#16a34a"/>}</button>}<button className="btn btn-ghost btn-sm" title="Delete" onClick={e=>handleDeletePO(p,e)}><Trash2 size={12} color="#dc2626"/></button></td>
                </tr>
              )
            })}</tbody>
          </table>
...
function WorkOrdersTab({ orders, suppliers }) {
  ...
  const selectedWOs = [...selectedIds].map(id => filtered.find(w => w.id === id) || wos.find(w => w.id === id)).filter(Boolean)
  const toggleSelect = (id) => setSelectedIds(prev => { const next = new Set(prev); next.has(id) ? next.delete(id) : next.add(id); return next })
  const toggleAll = () => setSelectedIds(filtered.length && filtered.every(w => selectedIds.has(w.id)) ? new Set() : new Set(filtered.map(w => w.id)))
  const stStyle = { draft:{color:'#d97706',bg:'#fff7ed'}, approved:{color:'#16a34a',bg:'#f0fdf4'}, Draft:{color:'#d97706',bg:'#fff7ed'}, Issued:{color:'#2563eb',bg:'#eff6ff'}, 'In Progress':{color:'#7c3aed',bg:'#f5f3ff'}, Completed:{color:'#16a34a',bg:'#f0fdf4'}, Cancelled:{color:'#dc2626',bg:'#fef2f2'} }
  ...
  const printSelectedWOs = async () => {
    const targets = selectedWOs.length ? selectedWOs : filtered
    for (const w of targets) await printWorkOrder({ ...w, items:w.work_order_items||[] }, suppliers.find(s=>s.id===w.vendor_id), orders.find(o=>o.id===w.order_id))
  }
  const archiveSelectedWOs = async () => {
    if (!selectedWOs.length) return
    if (!window.confirm(`Cancel/archive ${selectedWOs.length} selected WO(s)?`)) return
    await supabase.from('work_orders').update({ status:'Cancelled' }).in('id', selectedWOs.map(w=>w.id))
    setSelectedIds(new Set()); load()
  }
  ...
  return (
    <>
      <div style={{ padding:'10px 24px', borderBottom:'1px solid #f3f4f6', display:'flex', alignItems:'center', gap:8, flexShrink:0 }}>
        ...
        <div style={{ marginLeft:'auto', display:'flex', gap:8, alignItems:'center' }}>
          <label className="btn btn-secondary btn-sm" style={{ display:'inline-flex', alignItems:'center', gap:6 }}><input type="checkbox" checked={multiSelect} onChange={e => { setMultiSelect(e.target.checked); setSelectedIds(new Set()) }} /> Multi-select</label>
          {multiSelect && <button className="btn btn-secondary btn-sm" disabled={!filtered.length} onClick={toggleAll}>{filtered.length && filtered.every(w => selectedIds.has(w.id)) ? 'Clear' : 'Select Filtered'}</button>}
          {multiSelect && <span style={{ fontSize:11, fontWeight:700, color:selectedWOs.length?'#111827':'#9ca3af' }}>{selectedWOs.length} selected</span>}
          {multiSelect && <button className="btn btn-secondary btn-sm" disabled={!selectedWOs.length && !filtered.length} onClick={printSelectedWOs}>Print</button>}
          {multiSelect && <button className="btn btn-secondary btn-sm" disabled={!selectedWOs.length} onClick={archiveSelectedWOs}>Archive</button>}
      ...
      <div style={{ flex:1, overflowY:'auto', overflowX:'auto' }}>
        ...
        :(
          <table style={{ width:'100%', borderCollapse:'collapse', minWidth:820 }}>
            <thead><tr style={{ background:'#fafafa', position:'sticky', top:0, zIndex:10 }}>
              {multiSelect && <th style={{ borderBottom:'1px solid #f3f4f6', width:36 }}><input type="checkbox" checked={filtered.length>0 && filtered.every(w => selectedIds.has(w.id))} onChange={toggleAll} /></th>}<TH>WO Number</TH><TH>Vendor</TH><TH>Type</TH><TH>Order / Style</TH><TH>Colour</TH><TH right>Qty</TH><TH>Processes</TH><TH>Complete By</TH><TH>Status</TH>
              <th style={{ borderBottom:'1px solid #f3f4f6', width:110 }}>Actions</th>
            </tr></thead>
            <tbody>{filtered.map(w=>{
              ...
              return (
                <tr key={w.id} style={{ cursor:'pointer', background:selectedIds.has(w.id)?'#f0f9ff':'' }} onClick={()=>{ setEditWO({...w,items:w.work_order_items||[]}); setShowModal(true) }}
                  onMouseEnter={e=>e.currentTarget.style.background='#fafafa'} onMouseLeave={e=>e.currentTarget.style.background=''}>
                  {multiSelect && <td style={{ padding:'9px 8px', borderBottom:'1px solid #f9fafb' }}><input type="checkbox" checked={selectedIds.has(w.id)} onClick={e=>e.stopPropagation()} onChange={() => toggleSelect(w.id)} /></td>}
                  ...
                  <TD><span style={statusBadgeStyle(w.status)}>{w.status}</span></TD>
      ...
      <div style={{ padding:'10px 24px', borderBottom:'1px solid #f3f4f6', display:'flex', gap:10, flexShrink:0, alignItems:'center', flexWrap:'wrap' }}>
        {[{label:'Overdue',count:counts.overdue,color:'#dc2626',f:'overdue'},{label:'This Week',count:counts.thisweek,color:'#d97706',f:'thisweek'},{label:'This Month',count:counts.thismonth,color:'#2563eb',f:'thismonth'}].map(k=>(
          <button key={k.f} onClick={()=>setFilter(filter===k.f?'all':k.f)}
            ...
            <span style={{ fontSize:15, fontWeight:700, color:k.color }}>{k.count}</span>
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', height:'100%', background:'#fff' }}>
      <div style={{ padding:'16px 24px 0', flexShrink:0 }}>
        <h1 style={{ fontSize:20, fontWeight:700, letterSpacing:'-0.4px', marginBottom:2 }}>Purchasing</h1>
