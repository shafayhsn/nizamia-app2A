...
function normalizeQueueBreakdown(rows = []) {
  ...
  return Array.from(map.values())
    ...
    .sort((a, b) => String(a.q_number).localeCompare(String(b.q_number), undefined, { numeric: true, sensitivity: 'base' }))
}
      ...
      if (rule === 'Configure Own') {
        if (ud.__matrix) {
          return ud.__matrix.reduce((s,m)=> {
            ...
            if (!cons || m.na) return s
            let cellQty = 0
            if (q.split_rule === 'Colour × Size Group' || q.split_rule === 'By Ratio') {
              if (m.sgName === groupName && m.colorName === q.color_name) cellQty = queueQty
            } else if (q.split_rule === 'By Colour' || q.split_rule === 'By Wash') {
              if (m.colorName === q.color_name) cellQty = queueQty
            } else if (q.split_rule === 'By Size Group') {
              if (m.sgName === groupName) cellQty = pd.sgColorTotals?.[m.sgName]?.[m.colorName] || 0
            } else {
              cellQty = pd.sgColorTotals?.[m.sgName]?.[m.colorName] || 0
            }
            return s + (cons * cellQty * wastageFactor)
          },0)
        }
        const groups = ud.__groups || []
        return groups.reduce((s,g)=> {
