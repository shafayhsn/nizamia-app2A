...
const USER_COLUMNS = 'id, email, username, role, role_id, is_active, created_at, roles(id, name)'

function getRole(value) {
  if (!value) return null
  if (typeof value === 'string') return value
  return value.role || value.roles?.name || null
}
...
function UsersAndRights() {
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      if (error) throw error
      setUsers((data || []).map(u => ({ ...u, username: u.username || '', roleName: u.roles?.name || u.role || 'merchandiser' })))
    } catch (err) {
        ...
        <table style={styles.table}><thead><tr><th style={styles.th}>Email</th><th style={styles.th}>Username</th><th style={styles.th}>Role</th><th style={styles.th}>Status</th><th style={styles.th}>Created</th><th style={styles.th}>Actions</th></tr></thead>
          <tbody>{users.length === 0 ? <tr><td colSpan="6" style={{ ...styles.td, textAlign: 'center', color: '#9ca3af' }}>No users yet</td></tr> : users.map((u) => (
            <tr key={u.id}>
              ...
              <td style={styles.td}><span style={getBadgeStyle(u.roleName)}>{u.is_active ? 'Active' : 'Inactive'}</span></td>
              <td style={styles.td}>{u.created_at ? new Date(u.created_at).toLocaleDateString() : '—'}</td>
              <td style={styles.td}><button style={{ ...styles.button, ...styles.buttonSecondary }} onClick={() => handleToggleActive(u.id, u.is_active)}>{u.is_active ? 'Deactivate' : 'Activate'}</button></td>
            </tr>))}</tbody>
        </table>
