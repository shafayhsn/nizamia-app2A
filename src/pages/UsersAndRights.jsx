...
const USER_COLUMNS_WITH_USERNAME = 'id, email, username, role, is_active, created_at'
const USER_COLUMNS_BASIC = 'id, email, role, is_active, created_at'

function isMissingUsernameColumn(error) {
  const msg = String(error?.message || '').toLowerCase()
  return msg.includes('username') && (msg.includes('column') || msg.includes('schema cache') || msg.includes('does not exist'))
}
...
function UsersAndRights() {
  ...
  const handleAdminLogin = async () => {
    ...
    try {
      ...
      if (role !== 'admin') {
        setLoginError('Login successful, but this user is not an admin.')
      }
    } catch (err) {
  ...
  const fetchUsers = async () => {
    ...
    try {
      let { data, error } = await supabase
        .from('users')
        .select(USER_COLUMNS_WITH_USERNAME)
        .order('created_at', { ascending: false })

      if (error && isMissingUsernameColumn(error)) {
        const fallback = await supabase
          .from('users')
          .select(USER_COLUMNS_BASIC)
          .order('created_at', { ascending: false })
        data = fallback.data
      ...
      setUsers((data || []).map(u => ({ ...u, username: u.username || '' })))
    } catch (err) {
  ...
  const handleSaveUserProfile = async () => {
    ...
    try {
      ...
      let { error } = await supabase.from('users').insert(userPayload)
  ...
  const styles = {
    ...
    form: { display: 'grid', gridTemplateColumns: '1.2fr 1fr 1fr 150px auto auto', gap: '12px', alignItems: 'end', marginBottom: '20px' },
    ...
    return (
      <div style={styles.container}>
        <div style={{ ...styles.card, maxWidth: 460 }}>
          ...
          <input type="email" placeholder="Admin email or username" value={loginEmail} onChange={(e) => setLoginEmail(e.target.value)} style={{ ...styles.input, marginBottom: 10 }} />
  ...
  if (!isAdmin) {
    return (
      <div style={{ padding: '40px', textAlign: 'center', color: '#9ca3af' }}>
        ...
        <p style={{ fontSize: 12 }}>Current role: {currentRole || 'not found'}</p>
        <button style={{ ...styles.button, ...styles.buttonSecondary }} onClick={logout}>Logout</button>
      </div>
    )
  }
      ...
      <div style={styles.card}>
        ...
        {showCreateForm && (
          <div>
            ...
            <div style={styles.note}>
              Create users here only. No Supabase Auth setup required. Users can log in with either email or username.
            </div>
          </div>
        )}
        ...
        <table style={styles.table}>
          ...
          <tbody>
            {users.length === 0 ? (
              <tr>
                <td colSpan="6" style={{ ...styles.td, textAlign: 'center', color: '#9ca3af' }}>No users yet</td>
              </tr>
            ) : (
              users.map((u) => (
                <tr key={u.id}>
