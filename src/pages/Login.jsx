...
const REMEMBER_LOGIN_KEY = 'nizamia_app2a_login_identifier'
...
export default function Login() {
  ...
  const [identifier, setIdentifier] = useState(() => remembered ? (localStorage.getItem(REMEMBER_LOGIN_KEY) || '') : '')
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (remember) {
        ...
        localStorage.setItem(REMEMBER_LOGIN_KEY, cleanIdentifier)
      } else {
      ...
      <div style={{ width: 'min(1120px, 100%)', minHeight: 650, background: '#fff', border: '1px solid #eef0f4', borderRadius: 26, boxShadow: '0 24px 70px rgba(15, 23, 42, 0.10)', display: 'grid', gridTemplateColumns: '1.08fr 0.92fr', overflow: 'hidden' }}>
        <section style={{ margin: 10, borderRadius: 20, overflow: 'hidden', position: 'relative', background: 'radial-gradient(circle at 12% 14%, #11d1d9 0, transparent 28%), radial-gradient(circle at 38% 78%, #0fc0d1 0, transparent 30%), radial-gradient(circle at 82% 12%, #2b7cff 0, transparent 32%), linear-gradient(145deg, #0327b8 0%, #003cdd 46%, #001f99 100%)' }}>
          ...
          <div style={{ position: 'relative', color: '#fff', padding: '48px 52px' }}>
            ...
            <div style={{ fontSize: 35, fontWeight: 600, lineHeight: 1.12, letterSpacing: -1.2, maxWidth: 440 }}>Speed up your work<br />with Nizamia OMS</div>
          </div>
        </section>
        ...
        <section style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '54px 64px' }}>
          <form onSubmit={handleSubmit} style={{ width: '100%', maxWidth: 360 }}>
            ...
            <h1 style={{ margin: 0, color: '#050505', fontSize: 35, lineHeight: 1.05, letterSpacing: -1.5, fontWeight: 700 }}>Get Started Now</h1>
            <p style={{ margin: '12px 0 24px', color: '#9ca3af', fontSize: 13 }}>Login with your Nizamia username or email.</p>

            <Field label="Email or Username" icon={<UserRound size={15} />}>
              <input value={identifier} onChange={e => setIdentifier(e.target.value)} placeholder="email@nizamia.com or username" autoComplete="username" style={inputStyle} />
            </Field>
            ...
            <button type="submit" disabled={busy} style={{ width: '100%', height: 42, border: 0, borderRadius: 10, background: busy ? '#93a8ff' : '#1746e8', color: '#fff', fontSize: 13, fontWeight: 700, marginTop: 22, cursor: busy ? 'default' : 'pointer', boxShadow: '0 9px 18px rgba(23,70,232,0.22)' }}>
              {busy ? 'Logging in...' : 'Log in'}
            </button>

            <div style={{ textAlign: 'center', marginTop: 16, color: '#6b7280', fontSize: 12 }}>Users are managed inside App-2A User Management.</div>
          </form>
        </section>
      </div>
...
const labelStyle = { display: 'block', color: '#111827', fontSize: 12, fontWeight: 700 }
