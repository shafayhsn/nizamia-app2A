  ...
  return (
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,.35)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:400 }}>
      <div style={{ ...card, width:760, padding:18, maxHeight:'88vh', overflow:'auto' }}>
        <div style={{ fontSize:16, fontWeight:700, marginBottom:12 }}>Create Parcel</div>
...
export default function Parcels() {
  ...
  const selectedParcels = parcels.filter(p => selectedParcelIds.has(p.id))
  function toggleParcel(id) { setSelectedParcelIds(prev => { const n = new Set(prev); n.has(id) ? n.delete(id) : n.add(id); return n }) }
  function toggleAllParcels() { setSelectedParcelIds(prev => parcels.length && parcels.every(p => prev.has(p.id)) ? new Set() : new Set(parcels.map(p => p.id))) }
  ...
  return (
    <div className="page-content" style={{ display:'flex', flexDirection:'column', gap:14, background:'#fff' }}>
      <div className="section-header">
        <div>
          <div className="app-page-title" style={{ color:'#111827' }}>Parcels</div>
        ...
        <div style={{ display:'flex', gap:8 }}>
          <label className="btn btn-secondary" style={{ height:34, display:'inline-flex', alignItems:'center', gap:7, padding:'0 12px' }}><input type="checkbox" checked={multiSelect} onChange={e=>{ setMultiSelect(e.target.checked); setSelectedParcelIds(new Set()) }} /> Multi-select</label>{multiSelect && <button className="btn btn-secondary" disabled={!parcels.length} onClick={toggleAllParcels}>{selectedParcelIds.size === parcels.length && parcels.length ? 'Clear' : 'Select Filtered'}</button>}{multiSelect && <span style={{ fontSize:11, fontWeight:700, color:selectedParcelIds.size?'#111827':'#9ca3af' }}>{selectedParcelIds.size} selected</span>}<button className="btn btn-secondary" onClick={load}><RefreshCw size={14} /> Refresh</button>
      ...
      <div style={{ ...card, padding:14 }}>
        <div style={{ fontWeight:700, marginBottom:8 }}>Pending for Parcel</div>
      ...
      <div style={{ ...card, overflow:'hidden' }}>
        <div className="table-wrap">
          <table>
            <thead><tr>{multiSelect && <th style={{ width:36 }}><input type="checkbox" checked={parcels.length>0 && selectedParcelIds.size === parcels.length} onChange={toggleAllParcels}/></th>}<th>Parcel</th><th>Items</th><th>Status</th><th>Courier</th><th>Tracking #</th><th>Dispatch Date</th><th style={{ textAlign:'right' }}>Actions</th></tr></thead>
            <tbody>
              {parcels.map(p => (
                <tr key={p.id} style={{ background:selectedParcelIds.has(p.id)?'#f0f9ff':undefined }}>
                  {multiSelect && <td><input type="checkbox" checked={selectedParcelIds.has(p.id)} onChange={() => toggleParcel(p.id)} /></td>}
              ...
              {parcels.length === 0 && <tr><td colSpan={multiSelect ? 8 : 7} style={{ textAlign:'center', padding:'28px 12px', color:'#9ca3af' }}>No parcels created yet.</td></tr>}
            </tbody>
          </table>
        </div>
        {active && (() => {
          ...
          return <div style={{ borderTop:'1px solid #ececec', padding:14, background:'#fafafa' }}>
            <div style={{ fontWeight:700, marginBottom:8 }}>Parcel Contents · {p.id}</div>
            <div style={{ display:'grid', gap:8 }}>
              {(p.items || []).map((it, idx) => {
                const s = sampleMap[it.sampleId] || {}
                const o = orderMap[s.order_id] || {}
