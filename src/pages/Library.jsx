...
const FABRIC_UNITS = ['yards', 'meters', 'cm', 'inch', 'ft']
const TRIM_UNITS   = ['pcs', 'kg', 'gram', 'cone', 'set', 'sht', 'ctn', 'roll', 'ltr', 'meters', 'yards', 'cm', 'inch', 'ft']
...
function SizeGroupsTab() {
  ...
  const handleDelete = async (id) => {
    if (!await confirm('Size groups cannot be deleted. Deactivate this size group instead?', { title:'Remove Size Group', confirmText:'Deactivate', tone:'danger' })) return
    await supabase.from('size_group_templates').update({ is_active: false }).eq('id', id)
    loadGroups()
  }
...
export default function Library() {
  ...
  const handleDelete = async (id) => {
    if (!await confirm('Library items cannot be deleted. Deactivate this item instead?', { title:'Remove Library Item', confirmText:'Deactivate', tone:'danger' })) return
    await supabase.from('library_items').update({ is_active: false }).eq('id', id); load()
  }

  const ALL_TABS = [...BOM_CATEGORIES, 'Size Groups']
  const tabItems = items.filter(i => i.category === activeTab)
  const selectedItems = [...selectedIds].map(id => tabItems.find(i => i.id === id)).filter(Boolean)
  const toggleSelect = (id) => setSelectedIds(prev => { const next = new Set(prev); next.has(id) ? next.delete(id) : next.add(id); return next })
  const toggleAll = () => setSelectedIds(selectedIds.size === tabItems.length && tabItems.length ? new Set() : new Set(tabItems.map(i => i.id)))
  const archiveSelected = async () => {
    const ids = selectedItems.map(i => i.id)
    if (!ids.length) return
    if (!await confirm(`Deactivate ${ids.length} selected item(s)?`, { title:'Archive Library Items', confirmText:'Deactivate', tone:'danger' })) return
    await supabase.from('library_items').update({ is_active:false }).in('id', ids)
    setSelectedIds(new Set()); load()
  }
  const duplicateSelected = async () => {
    if (selectedItems.length !== 1) return
    const src = selectedItems[0]
    const { id, created_at, updated_at, ...copy } = src
    await supabase.from('library_items').insert([{ ...copy, name: `${copy.name || 'Item'} Copy` }])
    setSelectedIds(new Set()); load()
  }
  ...
  return (
    <div className="page-content">
      ...
      {activeTab === 'Size Groups' ? (
        ...
        <>
          <div className="section-header">
            <div style={{ fontSize: 13, color: 'var(--text-mid)' }}>{tabItems.length} items{multiSelect && selectedIds.size ? ` · ${selectedIds.size} selected` : ''}</div>
            <div style={{ display:'flex', gap:8, alignItems:'center' }}>
              <label className="btn btn-secondary" style={{ height:34, display:'inline-flex', alignItems:'center', gap:7 }}><input type="checkbox" checked={multiSelect} onChange={e => { setMultiSelect(e.target.checked); setSelectedIds(new Set()) }} /> Multi-select</label>
              {multiSelect && <button className="btn btn-secondary" disabled={!tabItems.length} onClick={toggleAll}>{selectedIds.size === tabItems.length && tabItems.length ? 'Clear' : 'Select Filtered'}</button>}
              {multiSelect && <button className="btn btn-secondary" disabled={selectedItems.length !== 1} onClick={duplicateSelected}>Duplicate</button>}
              {multiSelect && <button className="btn btn-secondary" disabled={!selectedItems.length} onClick={archiveSelected}>Archive</button>}
            ...
            <div className="table-wrap">
              {tabItems.length === 0 ? (
                ...
                <table>
                  <thead>
                    <tr>
                      {multiSelect && <th style={{ width:36 }}><input type="checkbox" checked={selectedIds.size === tabItems.length && tabItems.length > 0} onChange={toggleAll} /></th>}
                      <th>Name</th>
                      <th>Composition</th>
                      <th>Colour</th>
                  ...
                  <tbody>
                    {tabItems.map(item => (
                      <tr key={item.id} style={{ background:selectedIds.has(item.id)?'#f0f9ff':'' }}>
                        {multiSelect && <td><input type="checkbox" checked={selectedIds.has(item.id)} onChange={() => toggleSelect(item.id)} /></td>}
