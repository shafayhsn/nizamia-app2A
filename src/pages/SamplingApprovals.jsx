  ...
  return (
    <div className="page-content" style={{ display:'flex', flexDirection:'column', gap:14, background:'#fff' }}>
      <div className="section-header">
        <div>
          <div className="app-page-title" style={{ color:'#111827' }}>Sampling & Approvals</div>
      ...
      <div style={{ ...card, padding:12 }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', gap:10, marginBottom:12, flexWrap:'wrap' }}>
          <div style={{ display:'flex', alignItems:'center', gap:8, flexWrap:'wrap' }}><label className="btn btn-secondary" style={{ height:32, display:'inline-flex', alignItems:'center', gap:7, padding:'0 12px' }}><input type="checkbox" checked={multiSelect} onChange={e=>{ setMultiSelect(e.target.checked); setSelectedIds([]) }} /> Multi-select</label>{multiSelect && <button className="btn btn-secondary" style={{ height:32 }} disabled={!rows.length} onClick={() => setSelectedIds(selectedIds.length === rows.length && rows.length ? [] : rows.map(r => r.id))}>{selectedIds.length === rows.length && rows.length ? 'Clear' : 'Select Filtered'}</button>}<div style={{ fontSize:12, color:selectedIds.length?'#111827':'#6b7280', fontWeight:700 }}>{selectedIds.length ? `${selectedIds.length} selected` : 'Select a sample to enable actions'}</div></div>
        ...
        <div className="table-wrap">
          <table>
            <thead>
              <tr>
                {multiSelect && <th style={{ width:36 }}><input type="checkbox" checked={rows.length > 0 && selectedIds.length === rows.length} onChange={e => setSelectedIds(e.target.checked ? rows.map(r => r.id) : [])} /></th>}<th>SAM#</th><th>Job / Style</th><th>Type / Ver.</th><th>Qty</th><th>Stage</th><th>Approval</th>
              </tr>
            </thead>
            <tbody>
              {rows.map(r => (
                <tr key={r.id}>
                  {multiSelect && <td><input type="checkbox" checked={selectedIds.includes(r.id)} onChange={e => setSelectedIds(s => e.target.checked ? [...new Set([...s, r.id])] : s.filter(x => x !== r.id))} /></td>}
                  <td>
              ...
              {rows.length === 0 && <tr><td colSpan={multiSelect ? 7 : 6} style={{ textAlign:'center', padding:'28px 12px', color:'#9ca3af' }}>No sample requests found.</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
