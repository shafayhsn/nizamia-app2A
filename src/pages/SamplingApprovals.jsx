...
const STAGES = ['Pattern', 'Cutting', 'Stitching', 'Washing', 'Finishing', 'Ready']
...
export default function SamplingApprovals() {
  ...
  async function saveStage(sampleId, stage) {
    updateSampleMeta(sampleId, { stage })
    setRefreshKey(x => x + 1)
  }
  ...
  async function createRevision(sample) {
    ...
    if (error || !inserted) {
      alert(error?.message || 'Could not create revision sample')
      return
    }
      ...
      <div style={{ ...card, overflow:'hidden' }}>
        <div className="table-wrap">
          <table>
            <thead>
              <tr>
                <th>SAM#</th><th>Job / Style</th><th>Type / Ver.</th><th>Qty</th><th>Stage</th><th>Dispatch</th><th>Approval</th><th>Latest Comment</th><th style={{ textAlign:'right' }}>Actions</th>
              </tr>
            </thead>
            <tbody>
              {rows.map(r => (
                <tr key={r.id}>
                  ...
                  <td>{r.qty || '—'}</td>
                  <td>
                    <select style={{ ...inp, height:30, minWidth:110 }} value={r.stage} onChange={e => saveStage(r.id, e.target.value)}>
                      {STAGES.map(s => <option key={s}>{s}</option>)}
                    </select>
                  </td>
                  <td>{r.dispatchStatus.includes('Dispatched') ? statusPill('#dcfce7','#166534',r.dispatchStatus) : statusPill('#eef2ff','#3730a3',r.dispatchStatus)}</td>
                  ...
                  <td style={{ maxWidth:240 }}>
                    <div style={{ fontSize:11, color:'#374151', lineHeight:1.35 }}>{r.latestCommentText ? `${r.latestCommentText.slice(0, 88)}${r.latestCommentText.length > 88 ? '…' : ''}` : '—'}</div>
                    {r.latest && <div style={{ fontSize:10, color:'#9ca3af', marginTop:4 }}>{r.latest.type} · {formatDate(r.latest.date)} · {r.latest.by}</div>}
                  </td>
                  <td>
                    <div style={{ display:'flex', gap:6, justifyContent:'flex-end', flexWrap:'wrap' }}>
                      <button style={btn} onClick={() => setCommenting(r)}><MessageSquarePlus size={13} /> Add Comment</button>
                      <button style={btn} onClick={() => createRevision(r)}><ClipboardCheck size={13} /> Create V+</button>
                      <button style={btn} onClick={() => setDispatching(r)} disabled={r.stage !== 'Ready' || r.dispatchStatus === 'Dispatched'}><PackagePlus size={13} /> Send to Parcels</button>
                      <button style={btn} onClick={() => printSampleProgram(r)}><Printer size={13} /> Program</button>
                    </div>
                  </td>
                </tr>
              ))}
              {rows.length === 0 && <tr><td colSpan={9} style={{ textAlign:'center', padding:'28px 12px', color:'#9ca3af' }}>No sample requests found.</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
