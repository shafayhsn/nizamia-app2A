...
import { Building2, Calculator, FileText, Users, ShieldCheck, Hash, Ruler, SlidersHorizontal, X } from 'lucide-react'
...
export default function Settings() {
  ...
  const cards = [
    ...
    { id: 'roles', title: 'Roles & Permissions', desc: 'Structured permission matrix', icon: ShieldCheck, status: 'UI Ready' },
    ...
    <div style={{ padding: '28px', overflowY: 'auto', height: '100%', background: '#f7f7f5' }}>
      ...
      {active && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 80, background: 'rgba(15,23,42,0.34)', display: 'grid', placeItems: 'center', padding: 24 }} onMouseDown={e => { if (e.target === e.currentTarget) setActive(null) }}>
          <div style={{ width: active === 'users' ? 'min(1120px, 96vw)' : 'min(760px, 94vw)', maxHeight: '88vh', overflow: 'hidden', background: '#fff', borderRadius: 18, boxShadow: '0 24px 70px rgba(15,23,42,0.22)', border: '1px solid #eef0f4', display: 'flex', flexDirection: 'column' }}>
  ...
  return (
    <button onClick={onClick} style={{ textAlign: 'left', minHeight: 145, border: '1px solid #e5e7eb', background: '#fff', borderRadius: 16, padding: 18, cursor: 'pointer', boxShadow: '0 8px 24px rgba(15,23,42,0.05)' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
        ...
        <span style={{ fontSize: 10, fontWeight: 900, color: card.status === 'Active' ? '#166534' : '#6b7280', background: card.status === 'Active' ? '#dcfce7' : '#f3f4f6', borderRadius: 999, padding: '4px 8px' }}>{card.status}</span>
      </div>
