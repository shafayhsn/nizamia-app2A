  ...
  return (
    <div style={{ padding: '28px', overflowY: 'auto', height: '100%', background: '#f7f7f5' }}>
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: 24 }}>
        <div><h1 className="app-page-title" style={{ margin: 0, color: '#111827' }}>Settings</h1><div style={{ fontSize: 12, color: 'var(--text-light)', marginTop: 5 }}>Open each setup area from the cards below.</div></div>
      </div>
      ...
      {active && <div style={{ position: 'fixed', inset: 0, zIndex: 80, background: 'rgba(15,23,42,0.34)', display: 'grid', placeItems: 'center', padding: 24 }} onMouseDown={e => { if (e.target === e.currentTarget) setActive(null) }}><div style={{ width: active === 'users' || active === 'roles' ? 'min(1120px, 96vw)' : 'min(920px, 94vw)', maxHeight: '88vh', overflow: 'hidden', background: '#fff', borderRadius: 18, boxShadow: '0 24px 70px rgba(15,23,42,0.22)', border: '1px solid #eef0f4', display: 'flex', flexDirection: 'column' }}><div style={{ height: 58, borderBottom: '1px solid #eef0f4', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 18px' }}><div style={{ fontSize: 16, fontWeight: 700, color: '#111827' }}>{activeCard?.title}</div><button onClick={() => setActive(null)} style={{ width: 32, height: 32, borderRadius: 10, border: '1px solid #e5e7eb', background: '#fff', display: 'grid', placeItems: 'center', cursor: 'pointer' }}><X size={16} /></button></div><div style={{ padding: 18, overflowY: 'auto' }}>{renderContent()}</div></div></div>}
    </div>
  )
}

function SettingsCard({ card, onClick }) { const Icon = card.icon; return <button onClick={onClick} style={{ textAlign:'left', minHeight:145, border:'1px solid #e5e7eb', background:'#fff', borderRadius:16, padding:18, cursor:'pointer', boxShadow:'0 8px 24px rgba(15,23,42,0.05)' }}><div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:20 }}><div style={{ width:44, height:44, borderRadius:14, background:'#f3f4f6', display:'grid', placeItems:'center', color:'#111827' }}><Icon size={21} strokeWidth={1.9} /></div><span style={statusBadgeStyle(card.status)}>{card.status}</span></div><div style={{ fontSize:15, fontWeight:700, color:'#111827', marginBottom:5 }}>{card.title}</div><div style={{ fontSize:12, color:'#6b7280', lineHeight:1.45 }}>{card.desc}</div></button> }
...
function JobsControl() {
  ...
  return <div><Help text="Admin-only cleanup for mistakenly created / idle jobs. Jobs linked to orders are protected and cannot be deleted." /><div style={{ display:'flex', justifyContent:'space-between', gap:12, alignItems:'center', marginBottom:12, flexWrap:'wrap' }}><label style={{ display:'flex', alignItems:'center', gap:8, fontSize:12, fontWeight:700, color:'#374151' }}><input type="checkbox" checked={idleOnly} onChange={e=>setIdleOnly(e.target.checked)} /> Idle Jobs Only</label><div style={{ display:'flex', gap:8, flexWrap:'wrap' }}><button className="btn btn-secondary btn-sm" onClick={loadJobs} disabled={loading}>{loading ? 'Loading...' : 'Refresh'}</button><button className="btn btn-secondary btn-sm" onClick={resetJobSerial} disabled={loading}>Reset Serial to 01</button><button className="btn btn-danger btn-sm" onClick={deleteSelected} disabled={loading || selected.size === 0}>Delete Selected</button></div></div>{message && <div style={{ fontSize:12, fontWeight:700, color: message.includes('deleted') ? '#16a34a' : '#dc2626', marginBottom:10 }}>{message}</div>}<Table headers={[<input type="checkbox" checked={visible.length>0 && visible.every(j=>selected.has(j.id))} onChange={toggleAll} />, 'Job Number','Linked Orders','Created','Delete Allowed']} rows={visible.map(j=><tr key={j.id}><Td><input type="checkbox" checked={selected.has(j.id)} onChange={()=>toggle(j.id)} disabled={j.order_count>0}/></Td><Td><span style={{ fontFamily:'monospace', fontWeight:700 }}>{j.job_number || '—'}</span></Td><Td><span style={statusBadgeStyle(j.order_count ? 'Active' : 'Inactive')}>{j.order_count}</span></Td><Td>{j.created_at ? new Date(j.created_at).toLocaleDateString() : '—'}</Td><Td>{j.order_count === 0 ? <span style={{ color:'#16a34a', fontWeight:700 }}>Yes</span> : <span style={{ color:'#dc2626', fontWeight:700 }}>No</span>}</Td></tr>)} /></div>
}
...
function Table({ headers, rows }) { return <div style={{ overflowX:'auto', border:'1px solid #eef0f4', borderRadius:12 }}><table style={{ width:'100%', borderCollapse:'collapse', fontSize:12 }}><thead><tr>{headers.map(h=><th key={h} style={{ textAlign:'left', padding:10, background:'#fafafa', borderBottom:'1px solid #eef0f4', color:'#6b7280', fontWeight:700 }}>{h}</th>)}</tr></thead><tbody>{rows}</tbody></table></div> }
...
const labelStyle={ fontSize:11, fontWeight:700, color:'#6b7280', display:'block', marginBottom:6, textTransform:'uppercase', letterSpacing:.4 }
