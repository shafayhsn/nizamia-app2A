...
function JobsControl() {
  ...
  const loadJobs = async () => {
    ...
    try {
      const [{ data: jobRows, error: jobsError }, { data: orderRows, error: ordersError }] = await Promise.all([
        supabase.from('jobs').select('id, job_number, buyer_name, created_at').order('created_at', { ascending:false }),
  ...
  return <div><Help text="Admin-only cleanup for mistakenly created / idle jobs. Jobs linked to orders are protected and cannot be deleted." /><div style={{ display:'flex', justifyContent:'space-between', gap:12, alignItems:'center', marginBottom:12, flexWrap:'wrap' }}><label style={{ display:'flex', alignItems:'center', gap:8, fontSize:12, fontWeight:800, color:'#374151' }}><input type="checkbox" checked={idleOnly} onChange={e=>setIdleOnly(e.target.checked)} /> Idle Jobs Only</label><div style={{ display:'flex', gap:8 }}><button className="btn btn-secondary btn-sm" onClick={loadJobs} disabled={loading}>{loading ? 'Loading...' : 'Refresh'}</button><button className="btn btn-danger btn-sm" onClick={deleteSelected} disabled={loading || selected.size === 0}>Delete Selected</button></div></div>{message && <div style={{ fontSize:12, fontWeight:800, color: message.includes('deleted') ? '#16a34a' : '#dc2626', marginBottom:10 }}>{message}</div>}<Table headers={[<input type="checkbox" checked={visible.length>0 && visible.every(j=>selected.has(j.id))} onChange={toggleAll} />, 'Job Number','Buyer','Linked Orders','Created','Delete Allowed']} rows={visible.map(j=><tr key={j.id}><Td><input type="checkbox" checked={selected.has(j.id)} onChange={()=>toggle(j.id)} disabled={j.order_count>0}/></Td><Td><span style={{ fontFamily:'monospace', fontWeight:900 }}>{j.job_number || '—'}</span></Td><Td>{j.buyer_name || '—'}</Td><Td><span style={statusBadgeStyle(j.order_count ? 'Active' : 'Inactive')}>{j.order_count}</span></Td><Td>{j.created_at ? new Date(j.created_at).toLocaleDateString() : '—'}</Td><Td>{j.order_count === 0 ? <span style={{ color:'#16a34a', fontWeight:900 }}>Yes</span> : <span style={{ color:'#dc2626', fontWeight:900 }}>No</span>}</Td></tr>)} /></div>
}
