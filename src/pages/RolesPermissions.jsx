...
export default function RolesPermissions() {
  ...
  const styles = {
    ...
    roleBtn: active => ({ width: '100%', textAlign: 'left', border: '1px solid ' + (active ? '#111827' : '#e5e7eb'), background: active ? '#111827' : '#fff', color: active ? '#fff' : '#111827', borderRadius: 10, padding: '10px 12px', fontWeight: 700, fontSize: 13, marginBottom: 8, cursor: 'pointer' }),
    input: { width: '100%', height: 34, border: '1px solid #e5e7eb', borderRadius: 9, padding: '0 10px', fontSize: 13, boxSizing: 'border-box' },
    btn: { height: 34, border: 'none', background: '#111827', color: '#fff', borderRadius: 9, padding: '0 12px', fontSize: 12, fontWeight: 700, cursor: 'pointer' },
  ...
  return (
    <div style={styles.wrap}>
      <div style={styles.side}>
        <div style={{ fontSize: 12, color: '#6b7280', fontWeight: 700, marginBottom: 10, textTransform: 'uppercase' }}>Roles</div>
        {roles.map(r => (
      ...
      <div style={{ border: '1px solid #e5e7eb', borderRadius: 14, background: '#fff', overflow: 'hidden' }}>
        <div style={{ height: 50, padding: '0 14px', borderBottom: '1px solid #e5e7eb', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
            <div style={{ fontWeight: 700, color: '#111827' }}>{selectedRole?.name || 'Select role'}</div>
