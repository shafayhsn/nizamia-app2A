...
function CreateShipmentModal({ existingShipments, allOrders = [], onClose, onDone, user }) {
  ...
  return (
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.4)', zIndex:300, display:'flex', alignItems:'center', justifyContent:'center', padding:20 }}>
      <div style={{ ...card, width:'min(1280px, 97vw)', maxHeight:'92vh', display:'flex', flexDirection:'column' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'300px 1fr', minHeight:560, overflow:'hidden' }}>
    ...
    <div className="page-content" style={{ display:'flex', flexDirection:'column', gap:14, padding:'16px 24px', overflowY:'auto', height:'100%' }}>
      ...
      <div style={{ ...card }}>
        <div style={{ padding:'12px 16px', borderBottom:'1px solid #eef2f7', display:'flex', alignItems:'center', justifyContent:'space-between', gap:10 }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:8 }}>
            <div style={{ position:'relative', width:280 }}><Search size={13} style={{ position:'absolute', left:9, top:'50%', transform:'translateY(-50%)', color:'#9ca3af' }} /><input style={{ ...inp, paddingLeft:28 }} value={search} onChange={e => setSearch(e.target.value)} placeholder="Search shipment..." /></div>
      ...
      <div style={{ ...card }}>
        ...
        <table style={{ width:'100%', borderCollapse:'collapse', minWidth:900 }}>
          <thead><tr style={{ background:'#fafafa' }}><th style={th}>Buyer</th><th style={th}>Style / PO</th><th style={{...th, textAlign:'right'}}>Order Qty</th><th style={{...th, textAlign:'right'}}>Shipped</th><th style={{...th, textAlign:'right'}}>Balance</th><th style={th}>POL</th><th style={th}>POD</th></tr></thead>
          <tbody>
            {openOrders.map(o => <tr key={o.id}>
              <td style={td}>{o.buyer_name || '—'}</td>
              <td style={td}><div style={{ fontWeight:700 }}>{o.style_number || '—'}</div><div style={{ fontSize:11, color:'#6b7280' }}>{o.po_number || o.job_number || '—'}</div></td>
              <td style={{...td, textAlign:'right', fontFamily:'monospace'}}>{num(o.total_qty).toLocaleString()}</td>
              <td style={{...td, textAlign:'right', fontFamily:'monospace', color:'#2563eb'}}>{num(o.shipped_qty).toLocaleString()}</td>
              <td style={{...td, textAlign:'right', fontFamily:'monospace', fontWeight:800}}>{num(o.balance_qty).toLocaleString()}</td>
              <td style={td}>{getOrderPortLoading(o)}</td><td style={td}>{getOrderPortDestination(o)}</td>
            </tr>)}
