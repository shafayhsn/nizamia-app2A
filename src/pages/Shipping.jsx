...
function getQueueValue(order, queue) {
  ...
  const byName = groups.find(g => g?.group_name && norm(cleanLabel).includes(norm(g.group_name)))
...
function buildQueueRows(queues, orders, shipmentLines, sizeGroups=[]) {
  const groupsByOrder = {}
  ;(sizeGroups || []).forEach(g => { if (g.order_id) (groupsByOrder[g.order_id] ||= []).push(g) })
...
function ShipmentModal({ selectedQueues, shipments, onClose, onDone, draft }) {
  ...
  const [header, setHeader] = useState(() => ({
    ...
    container_no: draft?.shipment?.container_no || '',
    cro_lp_no: draft?.shipment?.cro_lp_no || '',
    eta: draft?.shipment?.eta || '',
    etd: draft?.shipment?.etd || '',
    notes: draft?.shipment?.notes || ''
  }))
  const [lines, setLines] = useState(() => {
    ...
    return Object.fromEntries(selectedQueues.map(q => {
      ...
      return [q.id, {
        qty: l.ship_qty ?? l.shipped_qty ?? q.balance_qty ?? q.qty ?? '',
        cartons: l.cartons ?? '',
        cbm: l.cbm ?? '',
        value: l.value ?? (q.value ? Number(q.value).toFixed(2) : ''),
        valueManual: l.value !== undefined && l.value !== null,
        close_queue: !!l.close_queue,
  ...
  function printSummary() {
    ...
    printHTML(`<div style="font-family:Arial;padding:24px"><h2>Shipment Summary ${header.shipment_no || ''}</h2><p>Date: ${header.shipment_date || '—'} · Container: ${header.container_no || '—'} · Booking Ref/CRO/LP: ${header.cro_lp_no || '—'}</p><table style="width:100%;border-collapse:collapse"><thead><tr>${['Q #','Style','Buyer','Qty','Cartons','CBM','Value'].map(h=>`<th style="border:1px solid #ddd;padding:8px;background:#f5f5f5;text-align:${['Qty','Cartons','CBM','Value'].includes(h)?'right':'left'}">${h}</th>`).join('')}</tr></thead><tbody>${rows}</tbody><tfoot><tr><td colspan="3" style="border:1px solid #ddd;padding:8px;font-weight:bold">Total</td><td style="border:1px solid #ddd;padding:8px;text-align:right;font-weight:bold">${totals.qty.toLocaleString()}</td><td style="border:1px solid #ddd;padding:8px;text-align:right;font-weight:bold">${totals.cartons.toLocaleString()}</td><td style="border:1px solid #ddd;padding:8px;text-align:right;font-weight:bold">${money(totals.cbm)}</td><td style="border:1px solid #ddd;padding:8px;text-align:right;font-weight:bold">${money(totals.value)}</td></tr></tfoot></table><p>${header.notes || ''}</p></div>`)
  }
  ...
  async function save(status) {
    ...
    setSaving(true)
    const headerPayload = { shipment_no:header.shipment_no, shipment_date:header.shipment_date, container_no:header.container_no || null, cro_lp_no:header.cro_lp_no || null, etd:header.etd || null, eta:header.eta || null, notes:header.notes || null, status }
    let sh = draft?.shipment || null
    let error = null
    if (isEdit) {
      const res = await supabase.from('shipments').update(headerPayload).eq('id', draft.shipment.id).select().single()
      sh = res.data; error = res.error
    } else {
      const res = await supabase.from('shipments').insert([headerPayload]).select().single()
      sh = res.data; error = res.error
    }
    if (error && /container_no|cro_lp_no|schema cache/i.test(error.message || '')) {
      const fallback = { shipment_no:header.shipment_no, shipment_date:header.shipment_date, etd:header.etd || null, eta:header.eta || null, notes:[header.notes, header.container_no ? `Container No: ${header.container_no}` : '', header.cro_lp_no ? `Booking Ref / CRO No / LP Ref: ${header.cro_lp_no}` : ''].filter(Boolean).join('\n'), status }
      const res = isEdit ? await supabase.from('shipments').update(fallback).eq('id', draft.shipment.id).select().single() : await supabase.from('shipments').insert([fallback]).select().single()
      sh = res.data; error = res.error
    }
    ...
    <div style={{ position:'fixed', inset:0, background:'rgba(17,24,39,0.48)', zIndex:400, display:'flex', alignItems:'center', justifyContent:'center', padding:16 }}>
      <div style={{ width:'min(1360px, 96vw)', height:'min(88vh, 760px)', background:'#fff', borderRadius:14, boxShadow:'0 24px 70px rgba(0,0,0,0.25)', display:'flex', flexDirection:'column', overflow:'hidden' }}>
        ...
        <div style={{ padding:'10px 18px', borderTop:'1px solid #eef2f7', display:'flex', justifyContent:'space-between', alignItems:'center', background:'#fff', flexShrink:0 }}>
          <div style={{ fontSize:11, color:'#6b7280' }}>Factory close means Q is manually closed even with over/short qty; reason is mandatory.</div>
          <div style={{ display:'flex', gap:8 }}><button className="btn btn-secondary" onClick={printSummary}><Printer size={14}/> Print Summary</button><button className="btn btn-secondary" disabled={saving} onClick={()=>save('Draft')}>Save as Draft</button><button className="btn btn-primary" disabled={saving} onClick={()=>save('Created')}>{saving ? 'Saving...' : (isEdit ? 'Update Shipment' : 'Create Shipment')}</button></div>
        </div>
      </div>
    </div>
  )
...
function ShipmentsTab({ shipments, shipmentLines, rows, onRefresh, onEdit }) {
  ...
  const data = shipments.map(s => {
    ...
    const qty=lines.reduce((a,l)=>a+num(l.shipped_qty ?? l.ship_qty),0)
    const cartons=lines.reduce((a,l)=>a+num(l.cartons),0)
    const cbm=lines.reduce((a,l)=>a+num(l.cbm),0)
    const value=lines.reduce((a,l)=>a+num(l.value),0)
    const qs=lines.map(l=>rowMap[l.queue_id]?.q_number || displayQueueLabel(rowMap[l.queue_id]?.label)).filter(Boolean).join(', ')
    return { ...s, qty, cartons, cbm, value, qs, lineCount:lines.length, booking_ref:s.cro_lp_no, tracking:trackingDays(s.eta) }
  })
  ...
  const filtered = useMemo(() => {
    ...
    return data.filter(s => {
      const hay = [s.shipment_no, s.container_no, s.cro_lp_no, s.qs, s.status].join(' ').toLowerCase()
  ...
  function cellValue(s,k,jsx=true) {
    ...
    if (k === 'booking_ref') return s.cro_lp_no || '—'
    if (k === 'queues') return s.qs || `${s.lineCount} line(s)`
    if (['qty','cartons'].includes(k)) return num(s[k]).toLocaleString()
    if (['cbm','value'].includes(k)) return money(s[k])
  ...
  function exportCSV() { const head = visibleKeys.map(k=>labels[k]); const lines = [head.join(',')].concat(filtered.map(s=>visibleKeys.map(k=>`"${String(cellValue(s,k,false)).replace(/"/g,'""')}"`).join(','))); const a=document.createElement('a'); a.href=URL.createObjectURL(new Blob([lines.join('\n')],{type:'text/csv'})); a.download='shipments.csv'; a.click() }
  function printList() { const head = visibleKeys.map(k=>`<th style="border:1px solid #ddd;padding:8px;background:#f5f5f5;text-align:left">${labels[k]}</th>`).join(''); const body = filtered.map(s=>`<tr>${visibleKeys.map(k=>`<td style="border:1px solid #ddd;padding:8px">${cellValue(s,k,false)}</td>`).join('')}</tr>`).join(''); printHTML(`<div style="font-family:Arial;padding:24px"><h2>Shipments</h2><table style="width:100%;border-collapse:collapse"><thead><tr>${head}</tr></thead><tbody>${body}</tbody></table></div>`) }
  ...
  return <div style={{ display:'flex', flexDirection:'column', flex:1, minHeight:0, background:'#fff' }}>
    <div style={{ padding:'12px 24px', borderBottom:'1px solid #f3f4f6', display:'flex', justifyContent:'space-between', alignItems:'center', flexShrink:0 }}>
      ...
      <div style={{ display:'flex', gap:8 }}><button className="btn btn-secondary" onClick={onRefresh}>Refresh</button><button className="btn btn-secondary" disabled={!filtered.length} onClick={exportCSV}>Export</button><button className="btn btn-secondary" disabled={!filtered.length} onClick={printList}>Print</button></div>
    </div>
    <div style={{ padding:'8px 24px', borderBottom:'1px solid #f3f4f6', display:'flex', alignItems:'center', gap:8, flexShrink:0, overflowX:'auto' }}>
      ...
      <button className="btn btn-secondary" style={{ height:32, marginLeft:'auto' }} onClick={()=>setShowCols(true)} title="Column Settings"><Settings size={15}/></button>
    </div>
    <div style={{ flex:1, overflow:'auto' }}><table style={{ width:'100%', minWidth:1260, borderCollapse:'collapse' }}><thead><tr style={{ background:'#fafafa', position:'sticky', top:0, zIndex:10 }}>{visibleKeys.map(k=><th key={k} style={{ ...thStyle, textAlign:['qty','cartons','cbm','value'].includes(k)?'right':'left' }}>{labels[k]}</th>)}<th style={thStyle}>Actions</th></tr></thead><tbody>{displayRows.length===0 ? <tr><td colSpan={visibleKeys.length+1} style={{ padding:50, textAlign:'center', color:'#9ca3af', fontSize:12 }}>No shipments yet.</td></tr> : displayRows.map(row => { if (row.type==='group') return <tr key={`g-${row.key}`} style={{ background:'#fdf6e3' }}><td colSpan={visibleKeys.length+1} style={{ padding:'10px 18px', borderBottom:'1px solid #eee', borderTop:'1px solid #eee', fontSize:12, fontWeight:800 }}>{row.key} <span style={{ color:'#9ca3af' }}>— {row.count} shipments</span><span style={{ float:'right' }}>Qty: {row.qty.toLocaleString()}</span></td></tr>; return <tr key={row.s.id}>{visibleKeys.map(k=>renderCell(row.s,k))}<td style={{ ...tdStyle, display:'flex', gap:6 }}><button className="btn btn-secondary btn-sm" onClick={()=>onEdit(row.s)}>Edit</button><button className="btn btn-secondary btn-sm" onClick={()=>printShipment(row.s)}>Print</button><button className="btn btn-secondary btn-sm" onClick={()=>deleteShipment(row.s)}>Delete</button></td></tr> })}</tbody></table></div>
...
export default function Shipping() {
  ...
  async function load() {
    setLoading(true)
    const [{ data: qs }, { data: ors }, { data: sg }, { data: sh }, { data: sl }] = await Promise.all([
    ...
    setQueues(qs || []); setOrders(ors || []); setSizeGroups(sg || []); setShipments(sh || []); setShipmentLines(sl || []); setLoading(false)
  }
  const rows = useMemo(() => buildQueueRows(queues, orders, shipmentLines, sizeGroups), [queues, orders, shipmentLines, sizeGroups])
