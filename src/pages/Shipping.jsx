...
function ShipmentModal({ selectedQueues, shipments, onClose, onDone, draft }) {
  ...
  return (
    <div style={{ position:'fixed', inset:0, background:'rgba(17,24,39,0.48)', zIndex:400, display:'flex', alignItems:'center', justifyContent:'center', padding:20 }}>
      <div style={{ width:'min(1380px, 96vw)', height:'min(92vh, 900px)', maxHeight:'92vh', background:'#fff', borderRadius:14, boxShadow:'0 24px 70px rgba(0,0,0,0.25)', display:'flex', flexDirection:'column', overflow:'hidden' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'30% 70%', flex:1, minHeight:0, overflow:'hidden' }}>
        ...
        <div style={{ padding:'12px 18px', borderTop:'1px solid #eef2f7', display:'flex', justifyContent:'space-between', alignItems:'center', background:'#fff', flexShrink:0 }}>
...
export default function Shipping() {
  ...
  function openNewShipment() { if (!selectedQueues.length) { alert('Select at least one queue to create shipment.'); return } if (!isAdmin && selectedQueues.some(isQueueRestricted)) { alert('Only admins can book/ship inactive or unassigned Qs.'); return } setShowModal(true) }
  return <div style={{ display:'flex', flexDirection:'column', height:'100%', background:'#fff', overflow:'hidden' }}>
    ...
    {activeTab === 'create' ? <CreateShipmentTab loading={loading} rows={rows} selectedIds={selectedIds} setSelectedIds={setSelectedIds} onNewShipment={openNewShipment} isAdmin={isAdmin} /> : <ShipmentsTab shipments={shipments} shipmentLines={shipmentLines} rows={rows} onRefresh={load} />}
