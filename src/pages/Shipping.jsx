...
function CreateShipmentModal({ existingShipments, allOrders = [], onClose, onDone, user, preselectedOrder }) {
  ...
  return <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,.42)', zIndex:300, display:'flex', alignItems:'center', justifyContent:'center', padding:20 }}>
    <div style={{ ...card, width:'min(1120px,96vw)', maxHeight:'88vh', display:'flex', flexDirection:'column' }}>
      ...
      <div style={{ padding:18, minHeight:460, overflow:'auto' }}>
        {step === 1 && <>
...
export default function Shipping() {
  ...
  const pendingRowsRaw = useMemo(() => (orders || []).map(o => { const price=priceMap[o.id] || { unit_price:n(o.unit_price), currency:o.currency || 'USD' }; const total=n(o.total_qty); const shipped=shippedByOrder[o.id]||0; const bal=Math.max(0,total-shipped); return { id:o.id, job:o.job_number || '—', status:'Open Balance', buyer:o.buyer_name || '—', style:o.style_number || '—', po:o.po_number || '—', qty:total, shipped, balance:bal, value:bal*n(price.unit_price), currency:price.currency, pol:getPOL(o), pod:getPOD(o), date:o.ship_date || o.delivery_date || o.created_at, order:o } }).filter(r=>r.balance>0), [orders, shippedByOrder, priceMap])
  const shippedRowsRaw = useMemo(() => (shipments || []).map(s => { const rows=(lines||[]).filter(l=>l.shipment_id===s.id); const first=rows[0]||{}; const o=orderMap[first.order_id] || {}; const qty=rows.reduce((a,b)=>a+n(b.shipped_qty),0); const cartons=rows.reduce((a,b)=>a+n(b.total_cartons),0); const cbm=rows.reduce((a,b)=>a+n(b.cbm),0); const value=rows.reduce((a,b)=>a+n(b.shipment_value),0); const eta=first.eta_destination_port || s.eta || ''; return { id:s.id, shipment:s.shipment_no, date:s.shipment_date, shipment_date:s.shipment_date, buyer:o.buyer_name || '—', style:o.style_number || '—', po:o.po_number || o.job_number || '—', qty, cartons, cbm, value, currency:first.currency || o.currency || 'USD', pol:first.port_of_loading || getPOL(o), pod:first.port_of_destination || getPOD(o), eta, tracking:s.tracking_link, source:s.source === 'backfill' ? 'Backfill' : 'Queue', status:s.status || 'Open', container:s.container_no, booking:s.booking_ref } }), [shipments, lines, orderMap])
