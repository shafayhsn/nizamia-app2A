...
function ShipmentsTab({ shipments, shipmentLines, rows, onRefresh, onEdit }) {
  ...
  const labelStyle = { fontSize:10.5, fontWeight:800, color:'#6b7280', display:'block', marginBottom:4 }
  const now = new Date(); const monthStart = new Date(now.getFullYear(), now.getMonth(), 1); const nextMonth = new Date(now.getFullYear(), now.getMonth()+1, 1); const weekStart = new Date(now); weekStart.setDate(now.getDate()-now.getDay()); weekStart.setHours(0,0,0,0); const weekEnd = new Date(weekStart); weekEnd.setDate(weekStart.getDate()+7)
  const shippedThisMonth = data.filter(s=>{ const d=new Date(s.shipment_date); return d>=monthStart && d<nextMonth })
  const etaThisWeek = data.filter(s=>{ const d=new Date(s.eta); return d>=weekStart && d<weekEnd }).length
  const etaThisMonth = data.filter(s=>{ const d=new Date(s.eta); return d>=monthStart && d<nextMonth }).length
  ...
  return <div style={{ display:'flex', flexDirection:'column', flex:1, minHeight:0, background:'#fff' }}>
    <div style={{ padding:'12px 24px', borderBottom:'1px solid #f3f4f6', display:'flex', justifyContent:'space-between', alignItems:'center', gap:16, flexShrink:0 }}>
      <div style={{ display:'flex', alignItems:'center', gap:16 }}>
        ...
        <div style={{ display:'flex', gap:8, alignItems:'stretch', minWidth:0 }}>
          <div style={{ padding:'7px 12px', border:'1px solid var(--border)', borderRadius:8, background:'#fff', minWidth:180 }}><div style={{ fontSize:9, fontWeight:700, color:'var(--text-light)', textTransform:'uppercase', letterSpacing:'0.5px', marginBottom:3 }}>Shipped This Month</div><div style={{ display:'flex', alignItems:'baseline', gap:8 }}><div style={{ fontSize:18, fontWeight:800, lineHeight:1 }}>{String(shippedThisMonth.length).padStart(2,'0')}</div><div style={{ fontSize:10, color:'#6b7280', fontWeight:700 }}>{monthQty.toLocaleString()} pcs · {moneyCurrency(monthValue, monthCurrency)}</div></div></div>
          <div style={{ padding:'7px 12px', border:'1px solid var(--border)', borderRadius:8, background:'#fff', minWidth:120 }}><div style={{ fontSize:9, fontWeight:700, color:'var(--text-light)', textTransform:'uppercase', letterSpacing:'0.5px', marginBottom:3 }}>ETA This Week</div><div style={{ display:'flex', alignItems:'baseline', gap:8 }}><div style={{ fontSize:18, fontWeight:800, lineHeight:1 }}>{String(etaThisWeek).padStart(2,'0')}</div><div style={{ fontSize:10, color:'#6b7280', fontWeight:700 }}>shipments</div></div></div>
          <div style={{ padding:'7px 12px', border:'1px solid var(--border)', borderRadius:8, background:'#fff', minWidth:128 }}><div style={{ fontSize:9, fontWeight:700, color:'var(--text-light)', textTransform:'uppercase', letterSpacing:'0.5px', marginBottom:3 }}>ETA This Month</div><div style={{ display:'flex', alignItems:'baseline', gap:8 }}><div style={{ fontSize:18, fontWeight:800, lineHeight:1 }}>{String(etaThisMonth).padStart(2,'0')}</div><div style={{ fontSize:10, color:'#6b7280', fontWeight:700 }}>shipments</div></div></div>
        </div>
      </div>
...
function paymentStatus(row) {
  const partsDone = !!row.buyer_done && !!row.tax_done && !!row.rebate_done
  if (partsDone) return 'Closed'
  const today = new Date(); today.setHours(0,0,0,0)
  const due = row.buyer_due_date ? new Date(row.buyer_due_date) : null
  if (due && Number.isFinite(due.getTime())) { due.setHours(0,0,0,0); if (due < today && !row.buyer_done) return 'Overdue' }
  if (row.buyer_done || row.tax_done || row.rebate_done) return 'Partial'
  return 'Pipeline'
}
function settlementDefaults(s, rowData) {
  const shipDate = s.shipment_date || new Date().toISOString().slice(0,10)
  const value = num(rowData?.value)
  const currency = rowData?.currency || 'USD'
  const taxPct = 12
  const rebatePct = 1
  return {
    buyer_due_date: rowData?.eta || shipDate,
    buyer_received_fcy: '',
    ...
    buyer_notes: '',
    buyer_done: false,
    tax_refund_pct: taxPct,
    tax_expected_amount: (value * taxPct / 100).toFixed(2),
    ...
    rebate_expected_amount: (value * rebatePct / 100).toFixed(2),
    rebate_due_date: '',
...
function PaymentSettlementModal({ row, settlement, onClose, onSave }) {
  const [form, setForm] = useState(() => ({ ...settlement }))
  const labelStyle = { fontSize:10.5, fontWeight:800, color:'#6b7280', display:'block', marginBottom:4 }
  const expectedPkr = num(row.value) * num(form.bank_realisation_rate)
  const fxDiff = expectedPkr - num(form.received_pkr)
  const buyerBalance = num(row.value) - num(form.buyer_received_fcy)
  const taxDiff = num(form.tax_expected_amount) - num(form.tax_received_amount)
  const rebateDiff = num(form.rebate_expected_amount) - num(form.rebate_received_amount)
  ...
  function field(label,key,type='text', extra={}) { return <div style={extra.full ? { gridColumn:'1 / -1' } : null}><label style={labelStyle}>{label}</label><input type={type} style={inputStyle} value={form[key] ?? ''} onChange={e=>set(key, type==='checkbox'?e.target.checked:e.target.value)} {...extra}/></div> }
  return <div style={{ position:'fixed', inset:0, background:'rgba(17,24,39,0.48)', zIndex:420, display:'flex', alignItems:'center', justifyContent:'center', padding:16 }}>
    <div style={{ width:'min(1120px, 96vw)', maxHeight:'88vh', background:'#fff', borderRadius:14, boxShadow:'0 24px 70px rgba(0,0,0,0.25)', display:'flex', flexDirection:'column', overflow:'hidden' }}>
      <div style={{ padding:'14px 18px', borderBottom:'1px solid #eef2f7', display:'flex', alignItems:'center', justifyContent:'space-between', flexShrink:0 }}><div><div style={{ fontSize:17, fontWeight:900 }}>Shipment Settlement</div><div style={{ fontSize:11, color:'#6b7280', marginTop:2 }}>{row.shipment_no} · {row.buyer || '—'} · Invoice {moneyCurrency(row.value, row.currency)}</div></div><button className="btn btn-ghost" onClick={onClose}><X size={16}/></button></div>
      <div style={{ padding:16, overflow:'auto', display:'grid', gap:12 }}>
        {card('Buyer Payment + Bank Realisation', <>
          <div><label style={labelStyle}>Invoice Currency</label><input style={{...inputStyle, background:'#f9fafb'}} value={row.currency || 'USD'} readOnly/></div>
          <div><label style={labelStyle}>Invoice Value (FCY)</label><input style={{...inputStyle, background:'#f9fafb', textAlign:'right'}} value={moneyCurrency(row.value, row.currency)} readOnly/></div>
          {field('Buyer Due Date','buyer_due_date','date')}
          {field('Received FCY','buyer_received_fcy','number', { step:'0.01' })}
          {field('Bank Realisation Rate','bank_realisation_rate','number', { step:'0.01', placeholder:'PKR per currency unit' })}
          <div><label style={labelStyle}>Expected PKR</label><input style={{...inputStyle, background:'#f9fafb', textAlign:'right'}} value={expectedPkr ? moneyCurrency(expectedPkr, 'PKR') : ''} readOnly/></div>
          {field('Received PKR','received_pkr','number', { step:'0.01' })}
          <div><label style={labelStyle}>FX Diff / Bank Charges</label><input style={{...inputStyle, background:'#f9fafb', textAlign:'right'}} value={form.received_pkr ? moneyCurrency(fxDiff, 'PKR') : ''} readOnly/></div>
          {field('Received Date','buyer_received_date','date')}
          <div><label style={labelStyle}>FCY Balance</label><input style={{...inputStyle, background:'#f9fafb', textAlign:'right'}} value={moneyCurrency(buyerBalance, row.currency)} readOnly/></div>
          <label style={{ display:'flex', alignItems:'center', gap:8, fontSize:12, fontWeight:800, marginTop:18 }}><input type="checkbox" checked={!!form.buyer_done} onChange={e=>set('buyer_done',e.target.checked)}/> Buyer Payment Complete</label>
          <div style={{ gridColumn:'1 / -1' }}><label style={labelStyle}>Buyer Payment Notes</label><textarea style={{...inputStyle, height:52, padding:8, resize:'none'}} value={form.buyer_notes || ''} onChange={e=>set('buyer_notes',e.target.value)}/></div>
        </>)}
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
          {card('Sales Tax Refund', <>
            {field('Refund %','tax_refund_pct','number', { step:'0.01', onChange:e=>setForm(f=>({...f,tax_refund_pct:e.target.value,tax_expected_amount:(num(row.value)*num(e.target.value)/100).toFixed(2)})) })}
            {field('Expected Refund','tax_expected_amount','number', { step:'0.01' })}
            {field('Expected Date','tax_due_date','date')}
            {field('Received Amount','tax_received_amount','number', { step:'0.01' })}
            {field('Received Date','tax_received_date','date')}
            <div><label style={labelStyle}>Difference</label><input style={{...inputStyle, background:'#f9fafb', textAlign:'right'}} value={moneyCurrency(taxDiff, row.currency)} readOnly/></div>
            <label style={{ display:'flex', alignItems:'center', gap:8, fontSize:12, fontWeight:800 }}><input type="checkbox" checked={!!form.tax_done} onChange={e=>set('tax_done',e.target.checked)}/> Refund Received</label>
            <div style={{ gridColumn:'1 / -1' }}><label style={labelStyle}>Notes</label><textarea style={{...inputStyle, height:46, padding:8, resize:'none'}} value={form.tax_notes || ''} onChange={e=>set('tax_notes',e.target.value)}/></div>
          </>)}
          {card('Export Rebate', <>
            {field('Rebate %','rebate_pct','number', { step:'0.01', onChange:e=>setForm(f=>({...f,rebate_pct:e.target.value,rebate_expected_amount:(num(row.value)*num(e.target.value)/100).toFixed(2)})) })}
            {field('Expected Rebate','rebate_expected_amount','number', { step:'0.01' })}
            {field('Expected Date','rebate_due_date','date')}
            {field('Received Amount','rebate_received_amount','number', { step:'0.01' })}
            {field('Received Date','rebate_received_date','date')}
            <div><label style={labelStyle}>Difference</label><input style={{...inputStyle, background:'#f9fafb', textAlign:'right'}} value={moneyCurrency(rebateDiff, row.currency)} readOnly/></div>
            <label style={{ display:'flex', alignItems:'center', gap:8, fontSize:12, fontWeight:800 }}><input type="checkbox" checked={!!form.rebate_done} onChange={e=>set('rebate_done',e.target.checked)}/> Rebate Received</label>
            <div style={{ gridColumn:'1 / -1' }}><label style={labelStyle}>Notes</label><textarea style={{...inputStyle, height:46, padding:8, resize:'none'}} value={form.rebate_notes || ''} onChange={e=>set('rebate_notes',e.target.value)}/></div>
          </>)}
        </div>
      </div>
      <div style={{ padding:'12px 18px', borderTop:'1px solid #eef2f7', display:'flex', justifyContent:'space-between', alignItems:'center', background:'#fff', flexShrink:0 }}><div style={{ fontSize:12, fontWeight:800 }}>Closes only when Buyer Payment + Sales Tax Refund + Rebate are complete.</div><div style={{ display:'flex', gap:8 }}><button className="btn btn-secondary" onClick={onClose}>Cancel</button><button className="btn btn-primary" onClick={()=>onSave(form)}>Save Settlement</button></div></div>
    </div>
  </div>
}

function PaymentsTab({ shipments, shipmentLines, rows }) {
  const [search, setSearch] = useState('')
  const [groupBy, setGroupBy] = useState('none')
  ...
  const labels = { shipment_no:'Shipment No', buyer:'Buyer', ship_date:'Ship Date', qty:'Quantity', invoice_value:'Invoice Value', buyer_due_date:'Buyer Due', received_pkr:'Received PKR', fx_diff:'FX Diff', tax_refund:'Tax Refund', rebate:'Rebate', net_pending:'Net Pending', status:'Status' }
  const [visibleCols, setVisibleCols] = useState(defaultCols)
  const [colOrder, setColOrder] = useState(['shipment_no','buyer','ship_date','qty','invoice_value','buyer_due_date','received_pkr','fx_diff','tax_refund','rebate','net_pending','status'])
  const rowMap = Object.fromEntries(rows.map(q => [q.id, q]))
  const baseData = shipments.map(s => {
    ...
    return { ...s, buyer: buyers.join(', ') || '—', ship_date:s.shipment_date, qty, value, currency }
  })
  const data = baseData.map(s => {
    const st = { ...settlementDefaults(s, s), ...(settlements[s.id] || {}) }
    const expectedPkr = num(s.value) * num(st.bank_realisation_rate)
    ...
    const buyerPending = Math.max(0, num(s.value) - num(st.buyer_received_fcy))
    const netPending = buyerPending + taxPending + rebatePending
    const row = { ...s, ...st, expected_pkr:expectedPkr, fx_diff:fxDiff, tax_pending:taxPending, rebate_pending:rebatePending, buyer_pending:buyerPending, net_pending:netPending }
    row.payment_status = paymentStatus(row)
    return row
  })
  const statuses = ['Pipeline','Partial','Overdue','Closed']
  const visibleKeys = colOrder.filter(k => visibleCols[k])
  const filtered = useMemo(() => {
    const q = search.toLowerCase()
    return data.filter(s => {
      const hay = [s.shipment_no, s.buyer, s.payment_status].join(' ').toLowerCase()
      return (!q || hay.includes(q)) && (!filterStatus || s.payment_status === filterStatus)
    }).sort((a,b) => {
  ...
  function cellValue(s,k,jsx=true){
    if (k==='ship_date' || k==='buyer_due_date') return fmtDate(s[k])
    if (k==='qty') return num(s.qty).toLocaleString()
    if (k==='invoice_value') return moneyCurrency(s.value, s.currency)
    ...
    if (k==='tax_refund') return `${moneyCurrency(s.tax_expected_amount, s.currency)}${s.tax_done ? ' ✓' : ''}`
    if (k==='rebate') return `${moneyCurrency(s.rebate_expected_amount, s.currency)}${s.rebate_done ? ' ✓' : ''}`
    if (k==='net_pending') return moneyCurrency(s.net_pending, s.currency)
    if (k==='status' && jsx) return <span style={{ padding:'3px 8px', borderRadius:999, background:s.payment_status==='Closed'?'#ecfdf5':s.payment_status==='Overdue'?'#fef2f2':s.payment_status==='Partial'?'#fff7ed':'#eff6ff', fontWeight:800, fontSize:11 }}>{s.payment_status}</span>
  ...
  function renderCell(s,k){ const right=['qty','invoice_value','received_pkr','fx_diff','tax_refund','rebate','net_pending'].includes(k); return <td key={k} style={{ ...tdStyle, textAlign:right?'right':'left' }}>{cellValue(s,k,true)}</td> }
  return <div style={{ display:'flex', flexDirection:'column', flex:1, minHeight:0, background:'#fff' }}>
    <div style={{ padding:'12px 24px', borderBottom:'1px solid #f3f4f6', display:'flex', justifyContent:'space-between', alignItems:'center', flexShrink:0 }}><div><div style={{ fontSize:20, fontWeight:800 }}>Payments</div><div style={{ fontSize:11, color:'#6b7280' }}>Shipment settlement: buyer payment, bank realisation, sales tax refund and rebate</div></div></div>
    <div style={{ padding:'8px 24px', borderBottom:'1px solid #f3f4f6', display:'flex', alignItems:'center', gap:8, flexShrink:0, overflowX:'auto' }}>
      <div style={{ position:'relative', width:260, flex:'0 0 260px' }}><Search size={13} style={{ position:'absolute', left:9, top:'50%', transform:'translateY(-50%)', color:'#9ca3af' }} /><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search shipment, buyer..." style={{ ...inputStyle, paddingLeft:28, background:'#fafafa' }} /></div>
      <select value={groupBy} onChange={e=>setGroupBy(e.target.value)} style={inputStyle}><option value="none">☰ No Group</option><option value="status">☰ By Status</option><option value="buyer">☰ By Buyer</option></select>
      <select value={`${sort.col}:${sort.dir}`} onChange={e=>{ const [col,dir]=e.target.value.split(':'); setSort({col,dir}) }} style={inputStyle}><option value="buyer_due_date:asc">⇅ Due Soon First</option><option value="buyer_due_date:desc">⇅ Due Later First</option><option value="invoice_value:desc">⇅ Value High → Low</option><option value="ship_date:desc">⇅ Ship Date New → Old</option><option value="net_pending:desc">⇅ Pending High → Low</option></select>
      <select value={filterStatus} onChange={e=>setFilterStatus(e.target.value)} style={inputStyle}><option value="">All Status</option>{statuses.map(v=><option key={v}>{v}</option>)}</select>
      <div style={{ marginLeft:'auto', display:'flex', alignItems:'center', gap:6, flexShrink:0 }}><span style={{ fontSize:11, color:'#6b7280', fontWeight:800 }}>{selectedIds.size} selected</span><button className="btn btn-secondary" style={actionBtnStyle} disabled={selected.length !== 1} onClick={editSelected} title="Edit settlement"><Edit3 size={14}/></button><button className="btn btn-secondary" style={actionBtnStyle} onClick={()=>printList(selected.length?selected:filtered)} title="Print"><Printer size={14}/></button><button className="btn btn-secondary" style={actionBtnStyle} onClick={()=>exportCSV(selected.length?selected:filtered)} title="Export"><Download size={14}/></button><button className="btn btn-secondary" style={actionBtnStyle} onClick={()=>setShowCols(true)} title="Column Settings"><Settings size={15}/></button></div>
    </div>
    <div style={{ flex:1, overflow:'auto' }}><table style={{ width:'100%', minWidth:1280, borderCollapse:'collapse' }}><thead><tr style={{ background:'#fafafa', position:'sticky', top:0, zIndex:10 }}><th style={{ ...thStyle, width:34, textAlign:'center' }}><input type="checkbox" checked={filtered.length>0 && filtered.every(s=>selectedIds.has(s.id))} onChange={toggleAll}/></th>{visibleKeys.map(k=><th key={k} style={{ ...thStyle, textAlign:['qty','invoice_value','received_pkr','fx_diff','tax_refund','rebate','net_pending'].includes(k)?'right':'left' }}>{labels[k]}</th>)}</tr></thead><tbody>{displayRows.length===0 ? <tr><td colSpan={visibleKeys.length+1} style={{ padding:50, textAlign:'center', color:'#9ca3af', fontSize:12 }}>No settlement records yet. Shipments will appear here automatically.</td></tr> : displayRows.map(row => { if(row.type==='group') return <tr key={`pg-${row.key}`} style={{ background:'#fdf6e3' }}><td colSpan={visibleKeys.length+1} style={{ padding:'10px 18px', borderBottom:'1px solid #eee', borderTop:'1px solid #eee', fontSize:12, fontWeight:800 }}>{row.key} <span style={{ color:'#9ca3af' }}>— {row.count} settlements</span></td></tr>; return <tr key={row.s.id}><td style={{ ...tdStyle, textAlign:'center' }}><input type="checkbox" checked={selectedIds.has(row.s.id)} onChange={()=>toggle(row.s.id)}/></td>{visibleKeys.map(k=>renderCell(row.s,k))}</tr> })}</tbody></table></div>
    {editing && <PaymentSettlementModal row={editing} settlement={{ ...settlementDefaults(editing, editing), ...(settlements[editing.id] || {}) }} onClose={()=>setEditing(null)} onSave={saveSettlement}/>} 
    {showCols && <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.35)', zIndex:300, display:'flex', alignItems:'center', justifyContent:'center' }} onClick={()=>setShowCols(false)}><div style={{ background:'#fff', borderRadius:12, width:460, maxHeight:'80vh', overflow:'auto', boxShadow:'0 18px 48px rgba(0,0,0,0.18)' }} onClick={e=>e.stopPropagation()}><div style={{ padding:'16px 18px', borderBottom:'1px solid #f3f4f6', display:'flex', justifyContent:'space-between' }}><div><div style={{ fontSize:15, fontWeight:800 }}>Payment Columns</div><div style={{ fontSize:11, color:'#9ca3af' }}>Add/remove settlement fields</div></div><button className="btn btn-ghost" onClick={()=>setShowCols(false)}><X size={16}/></button></div><div style={{ padding:14, display:'flex', flexDirection:'column', gap:8 }}>{colOrder.map((k,i)=><div key={k} style={{ display:'flex', alignItems:'center', gap:10, padding:'8px 10px', border:'1px solid #f3f4f6', borderRadius:8 }}><input type="checkbox" checked={!!visibleCols[k]} onChange={()=>setVisibleCols(v=>({...v,[k]:!v[k]}))}/><span style={{ flex:1, fontSize:12, fontWeight:700 }}>{labels[k]}</span><button className="btn btn-secondary btn-sm" disabled={i===0} onClick={()=>setColOrder(a=>{const n=[...a];[n[i-1],n[i]]=[n[i],n[i-1]];return n})}>↑</button><button className="btn btn-secondary btn-sm" disabled={i===colOrder.length-1} onClick={()=>setColOrder(a=>{const n=[...a];[n[i+1],n[i]]=[n[i],n[i+1]];return n})}>↓</button></div>)}</div><div style={{ padding:'12px 18px', borderTop:'1px solid #f3f4f6', display:'flex', justifyContent:'space-between' }}><button className="btn btn-secondary" onClick={()=>{setVisibleCols(defaultCols);setColOrder(['shipment_no','buyer','ship_date','qty','invoice_value','buyer_due_date','received_pkr','fx_diff','tax_refund','rebate','net_pending','status'])}}>Reset</button><button className="btn btn-primary" onClick={()=>setShowCols(false)}>Done</button></div></div></div>}
  </div>
}
