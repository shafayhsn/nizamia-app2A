      ...
      <div style={{ padding:16, overflow:'auto', display:'grid', gap:12 }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
          ...
          {card('Export Rebate (PKR)', <>
            <div><label style={labelStyle}>Today PKR Rate</label><input style={{...inputStyle, background:'#f9fafb', textAlign:'right'}} value={pkrRate.toLocaleString(undefined,{maximumFractionDigits:2})} readOnly/></div>
            ...
            {field('Rebate %','rebate_pct','number', { step:'0.01', disabled:receivedLocked.rebate, onChange:e=>setForm(f=>({...f,rebate_pct:e.target.value,rebate_expected_amount:recalcRebate(pkrRate, e.target.value)})) })}
            ...
            <div style={{ gridColumn:'1 / -1', fontSize:10.5, color:'#6b7280', fontWeight:700 }}>Rebate auto-calculates from shipment value × today's PKR rate × rebate %. Actual received PKR is entered and locked when received.</div>
          </>)}
        </div>
      </div>
...
function PaymentsTab({ shipments, shipmentLines, rows }) {
  ...
  const data = baseData.map(s => {
    ...
    const buyerPendingFcy = Math.max(0, num(s.value) - num(st.buyer_received_fcy))
