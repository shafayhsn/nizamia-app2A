...
function PaymentsTab({ shipments, shipmentLines, rows, isAdmin=false }) {
  ...
  function PayKpi({label, usd, pkrValue}) { return <div style={{ padding:'8px 13px', border:'1px solid #e5e7eb', borderRadius:10, background:'#fff', minWidth:170, boxShadow:'0 1px 2px rgba(15,23,42,0.04)' }}><div style={{ fontSize:9, fontWeight:900, color:'#9ca3af', textTransform:'uppercase', letterSpacing:'0.55px', marginBottom:4 }}>{label}</div><div style={{ fontSize:19, fontWeight:900, lineHeight:1 }}>{moneyCurrency(usd, 'USD')}</div><div style={{ fontSize:10, color:'#6b7280', fontWeight:800, marginTop:4 }}>{pkr(pkrValue)}</div></div> }
  ...
  return <div style={{ display:'flex', flexDirection:'column', flex:1, minHeight:0, background:'#fff' }}>
    <div style={{ padding:'12px 24px', borderBottom:'1px solid #f3f4f6', display:'flex', justifyContent:'space-between', alignItems:'center', gap:16, flexShrink:0 }}><div><div style={{ fontSize:20, fontWeight:800 }}>Payments</div><div style={{ fontSize:11, color:'#6b7280' }}>Shipment settlement: buyer payment, bank realisation, sales tax refund and rebate</div></div><div style={{ display:'flex', gap:8, alignItems:'stretch', minWidth:0 }}><PayKpi label="Total Payments" usd={kpiTotalUsd} pkrValue={kpiTotalPkr}/><PayKpi label="Due This Week" usd={kpiWeekUsd} pkrValue={kpiWeekPkr}/><PayKpi label="Due This Month" usd={kpiMonthUsd} pkrValue={kpiMonthPkr}/></div></div>
