...
function settlementDefaults(s, rowData) {
  ...
  const todayRate = num(rowData?.today_pkr_rate || rowData?.bank_realisation_rate)
...
function PaymentSettlementModal({ row, settlement, onClose, onSave }) {
  ...
  const labelStyle = { fontSize:10.5, fontWeight:800, color:'#6b7280', display:'block', marginBottom:4 }
  const pkrRate = num(form.today_pkr_rate || form.bank_realisation_rate)
  ...
  function recalcRebate(rate, pct) { return num(rate) ? (num(row.value) * num(rate) * num(pct) / 100).toFixed(2) : '' }
    ...
    <div style={{ width:'min(1120px, 96vw)', maxHeight:'88vh', background:'#fff', borderRadius:14, boxShadow:'0 24px 70px rgba(0,0,0,0.25)', display:'flex', flexDirection:'column', overflow:'hidden' }}>
      ...
      <div style={{ padding:16, overflow:'auto', display:'grid', gap:12 }}>
        {card('Buyer Payment + Bank Realisation', <>
          ...
          {field('Received FCY','buyer_received_fcy','number', { step:'0.01', disabled:receivedLocked.buyer })}
          {field('Bank Realisation Rate','bank_realisation_rate','number', { step:'0.01', placeholder:'PKR per currency unit', disabled:receivedLocked.buyer })}
          <div><label style={labelStyle}>Expected PKR</label><input style={{...inputStyle, background:'#f9fafb', textAlign:'right'}} value={expectedPkr ? pkr(expectedPkr) : ''} readOnly/></div>
          {field('Received PKR','received_pkr','number', { step:'0.01', disabled:receivedLocked.buyer })}
          <div><label style={labelStyle}>FX Diff / Bank Charges</label><input style={{...inputStyle, background:'#f9fafb', textAlign:'right'}} value={form.received_pkr ? pkr(fxDiff) : ''} readOnly/></div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
          {card('Sales Tax Refund (PKR)', <>
            {field('Linked Purchase Sales Tax (PKR)','tax_purchase_amount_pkr','number', { step:'0.01', disabled:receivedLocked.tax, onChange:e=>setForm(f=>({...f,tax_purchase_amount_pkr:e.target.value,tax_expected_amount:recalcTax(e.target.value, f.tax_refund_pct)})) })}
            {field('Refundable %','tax_refund_pct','number', { step:'0.01', disabled:receivedLocked.tax, onChange:e=>setForm(f=>({...f,tax_refund_pct:e.target.value,tax_expected_amount:recalcTax(f.tax_purchase_amount_pkr, e.target.value)})) })}
            {field('Expected Refund PKR','tax_expected_amount','number', { step:'0.01', disabled:receivedLocked.tax })}
            ...
            <div><label style={labelStyle}>Difference</label><input style={{...inputStyle, background:'#f9fafb', textAlign:'right'}} value={taxExpected || form.tax_received_amount ? pkr(taxDiff) : ''} readOnly/></div>
          ...
          {card('Export Rebate (PKR)', <>
            {field("Today's PKR Rate",'today_pkr_rate','number', { step:'0.01', disabled:receivedLocked.rebate, onChange:e=>setForm(f=>({...f,today_pkr_rate:e.target.value,rebate_expected_amount:recalcRebate(e.target.value, f.rebate_pct)})) })}
            ...
            {field('Rebate %','rebate_pct','number', { step:'0.01', disabled:receivedLocked.rebate, onChange:e=>setForm(f=>({...f,rebate_pct:e.target.value,rebate_expected_amount:recalcRebate(f.today_pkr_rate, e.target.value)})) })}
            {field('Expected Rebate PKR','rebate_expected_amount','number', { step:'0.01', disabled:receivedLocked.rebate })}
            ...
            <div><label style={labelStyle}>Difference</label><input style={{...inputStyle, background:'#f9fafb', textAlign:'right'}} value={rebateExpected || form.rebate_received_amount ? pkr(rebateDiff) : ''} readOnly/></div>
...
function PaymentsTab({ shipments, shipmentLines, rows }) {
  ...
  const labels = { shipment_no:'Shipment No', buyer:'Buyer', ship_date:'Ship Date', qty:'Quantity', invoice_value:'Invoice Value', buyer_due_date:'Buyer Due', received_pkr:'Received PKR', fx_diff:'FX Diff', tax_refund:'Tax Refund PKR', rebate:'Rebate PKR', net_pending:'PKR Pending', status:'Status' }
  const [visibleCols, setVisibleCols] = useState(defaultCols)
