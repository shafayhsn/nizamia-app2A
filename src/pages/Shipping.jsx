  ...
  return <div style={{ display:'flex', flexDirection:'column', flex:1, minHeight:0, background:'#fff' }}>
    ...
    <div style={{ padding:'8px 24px', borderBottom:'1px solid #f3f4f6', display:'flex', alignItems:'center', gap:8, flexShrink:0, overflowX:'auto' }}>
      ...
      <div style={{ marginLeft:'auto', display:'flex', alignItems:'center', gap:6, flexShrink:0 }}>
        ...
        <button className="btn btn-secondary" style={{ height:32, width:34, padding:0 }} onClick={onRefresh} title="Refresh"><RefreshCw size={14}/></button>
        <button className="btn btn-secondary" style={{ height:32, width:34, padding:0 }} disabled={selectedShipments.length !== 1} onClick={editSelected} title="Edit selected"><Edit3 size={14}/></button>
        <button className="btn btn-secondary" style={{ height:32, width:34, padding:0 }} disabled={!selectedShipments.length && !filtered.length} onClick={printSelected} title="Print selected/all"><Printer size={14}/></button>
        <button className="btn btn-secondary" style={{ height:32, width:34, padding:0 }} disabled={!selectedShipments.length && !filtered.length} onClick={exportSelected} title="Export selected/all"><Download size={14}/></button>
        <button className="btn btn-secondary" style={{ height:32, width:34, padding:0 }} disabled={selectedShipments.length !== 1} onClick={deleteSelected} title="Delete selected"><Trash2 size={14}/></button>
        <button className="btn btn-secondary" style={{ height:32, width:34, padding:0 }} onClick={()=>setShowCols(true)} title="Column Settings"><Settings size={15}/></button>
      </div>
    </div>
