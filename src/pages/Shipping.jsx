...
function PaymentsTab({ shipments, shipmentLines, rows, isAdmin=false }) {
  ...
  function PayKpi({label, usd, pkrValue}) { return <div style={{ padding:'7px 12px', border:'1px solid var(--border)', borderRadius:8, background:'#fff', minWidth:120 }}><div style={{ fontSize:9, fontWeight:700, color:'var(--text-light)', textTransform:'uppercase', letterSpacing:'0.5px', marginBottom:3 }}>{label}</div><div style={{ display:'flex', alignItems:'baseline', gap:8 }}><div style={{ fontSize:18, fontWeight:800, lineHeight:1 }}>{moneyCurrency(usd, 'USD')}</div><div style={{ fontSize:10, color:'#6b7280', fontWeight:700 }}>{pkr(pkrValue)}</div></div></div> }
  ...
  return <div style={{ display:'flex', flexDirection:'column', flex:1, minHeight:0, background:'#fff' }}>
    <div style={{ padding:'12px 24px', borderBottom:'1px solid #f3f4f6', display:'flex', justifyContent:'space-between', alignItems:'center', gap:16, flexShrink:0, overflow:'hidden' }}><div style={{ minWidth:240 }}><div style={{ fontSize:20, fontWeight:800 }}>Payments</div><div style={{ fontSize:11, color:'#6b7280' }}>Buyers Payments, Sales Tax and Rebate Tracker</div></div><div style={{ display:'flex', gap:8, alignItems:'center', minWidth:0, marginLeft:'auto' }}><PayKpi label="Total Payments" usd={kpiTotalUsd} pkrValue={kpiTotalPkr}/><PayKpi label="Due This Week" usd={kpiWeekUsd} pkrValue={kpiWeekPkr}/><PayKpi label="Due This Month" usd={kpiMonthUsd} pkrValue={kpiMonthPkr}/><button className="btn btn-secondary" style={actionBtnStyle} onClick={()=>exportCSV(selected.length?selected:filtered)} title="Export"><Download size={14}/></button><button className="btn btn-secondary" style={actionBtnStyle} onClick={()=>printList(selected.length?selected:filtered)} title="Print"><Printer size={14}/></button><button className="btn btn-primary" style={{ height:32, display:'inline-flex', alignItems:'center', justifyContent:'center' }} onClick={()=>printList(filtered)}>Print Summary</button></div></div>
