...
function PaymentSettlementModal({ row, settlement, onClose, onSave, isAdmin=false }) {
  ...
  const dueAmount = num(form.buyer_amount_due_override) > 0 ? num(form.buyer_amount_due_override) : num(row.value)
  ...
  return <div style={{ position:'fixed', inset:0, background:'rgba(17,24,39,0.48)', zIndex:420, display:'flex', alignItems:'center', justifyContent:'center', padding:16 }}>
    <div style={{ width:'min(1120px, 96vw)', maxHeight:'88vh', background:'#fff', borderRadius:14, boxShadow:'0 24px 70px rgba(0,0,0,0.25)', display:'flex', flexDirection:'column', overflow:'hidden' }}>
      ...
      <div style={{ padding:16, overflow:'auto', display:'grid', gap:12 }}>
        {card('Buyer Payment', <>
          <div><label style={labelStyle}>Amount Due</label>{isAdmin ? <input type="number" step="0.01" style={{...inputStyle, textAlign:'right', fontWeight:800}} value={form.buyer_amount_due_override || row.value || ''} onChange={e=>set('buyer_amount_due_override', e.target.value)} /> : <input style={{...inputStyle, background:'#f9fafb', textAlign:'right', fontWeight:800}} value={moneyCurrency(dueAmount, row.currency)} readOnly/>}</div>
...
function PaymentsTab({ shipments, shipmentLines, rows, isAdmin=false }) {
  ...
  function PayKpi({label, usd, pkrValue}) { return <div style={{ padding:'7px 12px', border:'1px solid var(--border)', borderRadius:8, background:'#fff', minWidth:140 }}><div style={{ fontSize:9, fontWeight:700, color:'var(--text-light)', textTransform:'uppercase', letterSpacing:'0.5px', marginBottom:3 }}>{label}</div><div style={{ display:'flex', alignItems:'baseline', gap:8 }}><div style={{ fontSize:18, fontWeight:800, lineHeight:1 }}>{moneyCurrency(usd, 'USD')}</div></div><div style={{ fontSize:10, color:'#6b7280', fontWeight:700, marginTop:4 }}>{pkr(pkrValue)}</div></div> }
  ...
  return <div style={{ display:'flex', flexDirection:'column', flex:1, minHeight:0, background:'#fff' }}>
    <div style={{ padding:'12px 24px', borderBottom:'1px solid #f3f4f6', display:'flex', justifyContent:'space-between', alignItems:'center', gap:16, flexShrink:0, overflow:'hidden' }}><div style={{ minWidth:220 }}><div style={{ fontSize:20, fontWeight:800 }}>Payments</div><div style={{ fontSize:11, color:'#6b7280' }}>Shipment settlement: buyer payment, bank realisation, sales tax refund and rebate</div></div><div style={{ display:'flex', gap:8, alignItems:'stretch', minWidth:0, marginLeft:'auto' }}><PayKpi label="Total Payments" usd={kpiTotalUsd} pkrValue={kpiTotalPkr}/><PayKpi label="Due This Week" usd={kpiWeekUsd} pkrValue={kpiWeekPkr}/><PayKpi label="Due This Month" usd={kpiMonthUsd} pkrValue={kpiMonthPkr}/><button className="btn btn-secondary" onClick={()=>exportCSV(selected.length?selected:filtered)}>Export</button><button className="btn btn-secondary" onClick={()=>printList(selected.length?selected:filtered)}>Print</button><button className="btn btn-primary" onClick={()=>printList(filtered)}>Print Summary</button></div></div>
