    ...
    <div style={{ padding:'12px 24px', borderBottom:'1px solid #f3f4f6', display:'flex', justifyContent:'space-between', alignItems:'center', gap:16, flexShrink:0 }}>
      <div style={{ display:'flex', alignItems:'center', gap:16 }}>
        ...
        <div style={{ display:'flex', gap:8 }}>
          <div style={{ minWidth:150, border:'1px solid #e5e7eb', borderRadius:10, padding:'8px 12px', background:'#fff' }}><div style={{ fontSize:12, fontWeight:800 }}>Shipped this Month</div><div style={{ fontSize:18, fontWeight:900 }}>{String(shippedThisMonth.length).padStart(2,'0')} <span style={{ fontSize:11, color:'#6b7280' }}>{monthQty.toLocaleString()} pcs - {moneyCurrency(monthValue, monthCurrency)}</span></div></div>
          <div style={{ minWidth:120, border:'1px solid #e5e7eb', borderRadius:10, padding:'8px 12px', background:'#fff' }}><div style={{ fontSize:12, fontWeight:800 }}>ETA This Week</div><div style={{ fontSize:18, fontWeight:900 }}>{String(etaThisWeek).padStart(2,'0')}</div></div>
          <div style={{ minWidth:130, border:'1px solid #e5e7eb', borderRadius:10, padding:'8px 12px', background:'#fff' }}><div style={{ fontSize:12, fontWeight:800 }}>ETA This Month</div><div style={{ fontSize:18, fontWeight:900 }}>{String(etaThisMonth).padStart(2,'0')}</div></div>
        </div>
      </div>
...
function paymentStatus(s) {
  ...
  const today = new Date(); today.setHours(0,0,0,0)
  if (!eta || !Number.isFinite(eta.getTime())) return 'Pipeline'
  eta.setHours(0,0,0,0)
  if (eta < today) return 'Overdue'
  const days = Math.ceil((eta - today) / 86400000)
  return days <= 7 ? 'Due' : 'Pipeline'
}

function PaymentsTab({ shipments, shipmentLines, rows }) {
  const [search, setSearch] = useState('')
  const [groupBy, setGroupBy] = useState('none')
  const [sort, setSort] = useState({ col:'eta', dir:'asc' })
  const [filterStatus, setFilterStatus] = useState('')
  const [showCols, setShowCols] = useState(false)
  const [selectedIds, setSelectedIds] = useState(new Set())
  const defaultCols = { shipment_no:true, buyer:true, ship_date:true, eta:true, qty:true, value:true, status:true, tracking:true }
  const labels = { shipment_no:'Shipment No', buyer:'Buyer', ship_date:'Ship Date', eta:'ETA', qty:'Quantity', value:'Value', status:'Payment Status', tracking:'Tracking' }
  const [visibleCols, setVisibleCols] = useState(defaultCols)
  const [colOrder, setColOrder] = useState(['shipment_no','buyer','ship_date','eta','qty','value','status','tracking'])
  const rowMap = Object.fromEntries(rows.map(q => [q.id, q]))
  const data = shipments.map(s => {
    const meta = shipmentMeta(s)
    const lines = shipmentLines.filter(l => l.shipment_id === s.id)
    ...
    return { ...s, buyer: buyers.join(', ') || '—', ship_date:s.shipment_date, qty, value, currency, payment_status:paymentStatus(s), tracking:trackingDays(s.eta) }
  })
  const statuses = ['Pipeline','Due','Overdue']
  const visibleKeys = colOrder.filter(k => visibleCols[k])
  const filtered = useMemo(() => {
    const q = search.toLowerCase()
    return data.filter(s => {
      ...
      const numeric = ['qty','value'].includes(sort.col)
      const av = numeric ? num(a[sort.col]) : String(a[sort.col] || '')
      const bv = numeric ? num(b[sort.col]) : String(b[sort.col] || '')
      const cmp = numeric ? av - bv : av.localeCompare(bv, undefined, { numeric:true })
      return sort.dir === 'asc' ? cmp : -cmp
  ...
  function cellValue(s,k,jsx=true){
    if (k==='ship_date') return fmtDate(s.ship_date)
    if (k==='eta') return fmtDate(s.eta)
    if (k==='qty') return num(s.qty).toLocaleString()
    if (k==='value') return moneyCurrency(s.value, s.currency)
    if (k==='status' && jsx) return <span style={{ padding:'3px 8px', borderRadius:999, background:s.payment_status==='Overdue'?'#fef2f2':s.payment_status==='Due'?'#fff7ed':'#eff6ff', fontWeight:800, fontSize:11 }}>{s.payment_status}</span>
    if (k==='status') return s.payment_status
    if (k==='tracking') return s.tracking || '—'
    return s[k] || '—'
  }
  function exportCSV(list=filtered){ const head=visibleKeys.map(k=>labels[k]); const lines=[head.join(',')].concat(list.map(s=>visibleKeys.map(k=>`"${String(cellValue(s,k,false)).replace(/"/g,'""')}"`).join(','))); const a=document.createElement('a'); a.href=URL.createObjectURL(new Blob([lines.join('\n')],{type:'text/csv'})); a.download='shipment-payments.csv'; a.click() }
  function printList(list=filtered){ const head=visibleKeys.map(k=>`<th style="border:1px solid #ddd;padding:8px;background:#f5f5f5;text-align:left">${labels[k]}</th>`).join(''); const body=list.map(s=>`<tr>${visibleKeys.map(k=>`<td style="border:1px solid #ddd;padding:8px">${cellValue(s,k,false)}</td>`).join('')}</tr>`).join(''); printHTML(`<div style="font-family:Arial;padding:24px"><h2>Payments Tracking</h2><table style="width:100%;border-collapse:collapse"><thead><tr>${head}</tr></thead><tbody>${body}</tbody></table></div>`) }
  ...
  function renderCell(s,k){ const right=['qty','value'].includes(k); return <td key={k} style={{ ...tdStyle, textAlign:right?'right':'left' }}>{cellValue(s,k,true)}</td> }
  return <div style={{ display:'flex', flexDirection:'column', flex:1, minHeight:0, background:'#fff' }}>
    <div style={{ padding:'12px 24px', borderBottom:'1px solid #f3f4f6', display:'flex', justifyContent:'space-between', alignItems:'center', flexShrink:0 }}><div><div style={{ fontSize:20, fontWeight:800 }}>Payments</div><div style={{ fontSize:11, color:'#6b7280' }}>Shipment-based payment pipeline, due and overdue tracking</div></div></div>
    <div style={{ padding:'8px 24px', borderBottom:'1px solid #f3f4f6', display:'flex', alignItems:'center', gap:8, flexShrink:0, overflowX:'auto' }}>
      ...
      <select value={`${sort.col}:${sort.dir}`} onChange={e=>{ const [col,dir]=e.target.value.split(':'); setSort({col,dir}) }} style={inputStyle}><option value="eta:asc">⇅ Due Soon First</option><option value="eta:desc">⇅ Due Later First</option><option value="value:desc">⇅ Value High → Low</option><option value="ship_date:desc">⇅ Ship Date New → Old</option></select>
      <select value={filterStatus} onChange={e=>setFilterStatus(e.target.value)} style={inputStyle}><option value="">All Status</option>{statuses.map(v=><option key={v}>{v}</option>)}</select>
    ...
    <div style={{ flex:1, overflow:'auto' }}><table style={{ width:'100%', minWidth:1000, borderCollapse:'collapse' }}><thead><tr style={{ background:'#fafafa', position:'sticky', top:0, zIndex:10 }}><th style={{ ...thStyle, width:34, textAlign:'center' }}><input type="checkbox" checked={filtered.length>0 && filtered.every(s=>selectedIds.has(s.id))} onChange={toggleAll}/></th>{visibleKeys.map(k=><th key={k} style={{ ...thStyle, textAlign:['qty','value'].includes(k)?'right':'left' }}>{labels[k]}</th>)}</tr></thead><tbody>{displayRows.length===0 ? <tr><td colSpan={visibleKeys.length+1} style={{ padding:50, textAlign:'center', color:'#9ca3af', fontSize:12 }}>No payment tracking records yet.</td></tr> : displayRows.map(row => { if(row.type==='group') return <tr key={`pg-${row.key}`} style={{ background:'#fdf6e3' }}><td colSpan={visibleKeys.length+1} style={{ padding:'10px 18px', borderBottom:'1px solid #eee', borderTop:'1px solid #eee', fontSize:12, fontWeight:800 }}>{row.key} <span style={{ color:'#9ca3af' }}>— {row.count} shipments</span></td></tr>; return <tr key={row.s.id}><td style={{ ...tdStyle, textAlign:'center' }}><input type="checkbox" checked={selectedIds.has(row.s.id)} onChange={()=>toggle(row.s.id)}/></td>{visibleKeys.map(k=>renderCell(row.s,k))}</tr> })}</tbody></table></div>
    {showCols && <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.35)', zIndex:300, display:'flex', alignItems:'center', justifyContent:'center' }} onClick={()=>setShowCols(false)}><div style={{ background:'#fff', borderRadius:12, width:420, maxHeight:'80vh', overflow:'auto', boxShadow:'0 18px 48px rgba(0,0,0,0.18)' }} onClick={e=>e.stopPropagation()}><div style={{ padding:'16px 18px', borderBottom:'1px solid #f3f4f6', display:'flex', justifyContent:'space-between' }}><b>Payment Columns</b><button className="btn btn-ghost" onClick={()=>setShowCols(false)}><X size={16}/></button></div><div style={{ padding:14, display:'flex', flexDirection:'column', gap:8 }}>{colOrder.map(k=><label key={k} style={{ display:'flex', gap:10, alignItems:'center', padding:'8px 10px', border:'1px solid #f3f4f6', borderRadius:8 }}><input type="checkbox" checked={!!visibleCols[k]} onChange={()=>setVisibleCols(v=>({...v,[k]:!v[k]}))}/><span style={{ fontSize:12, fontWeight:700 }}>{labels[k]}</span></label>)}</div><div style={{ padding:'12px 18px', borderTop:'1px solid #f3f4f6', textAlign:'right' }}><button className="btn btn-primary" onClick={()=>setShowCols(false)}>Done</button></div></div></div>}
  </div>
}
