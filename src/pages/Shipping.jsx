...
const thStyle = { padding:'9px 10px', borderBottom:'1px solid #f3f4f6', fontSize:10, fontWeight:700, color:'#9ca3af', textTransform:'uppercase', letterSpacing:'0.4px', whiteSpace:'nowrap' }
...
function appSetting(section, fallback = {}) {
  try { return { ...fallback, ...JSON.parse(localStorage.getItem('app2a_setting_' + section) || '{}') } } catch { return fallback }
}
function taxDefaults() { return appSetting('tax', { sales_tax_rate:18, sales_tax_refund_percent:100, rebate_percent:1 }) }
function factoryDefaults() { return appSetting('defaults', { payment_terms_days:50, pkr_rate:DEFAULT_PKR_RATE }) }
...
function DragColumnSettings({ title='Columns', subtitle='Drag to reorder fields', labels, order, visible, defaults, onChange, onReset, onClose }) {
  ...
  return <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.35)', zIndex:300, display:'flex', alignItems:'center', justifyContent:'center' }} onClick={onClose}>
    <div style={{ background:'#fff', borderRadius:12, width:460, maxHeight:'80vh', overflow:'hidden', boxShadow:'0 18px 48px rgba(0,0,0,0.18)', display:'flex', flexDirection:'column' }} onClick={e=>e.stopPropagation()}>
      ...
      <div style={{ padding:14, display:'flex', flexDirection:'column', gap:8, overflowY:'auto' }}>{order.map(k=><div key={k} draggable onDragStart={()=>setDragKey(k)} onDragOver={e=>e.preventDefault()} onDrop={()=>moveKey(dragKey,k)} style={{ display:'flex', alignItems:'center', gap:10, padding:'8px 10px', border:'1px solid #f3f4f6', borderRadius:8, background:dragKey===k?'#f9fafb':'#fff', cursor:'grab' }}><span style={{ color:'#9ca3af', fontWeight:700, fontSize:14 }}>⋮⋮</span><input type="checkbox" checked={!!visible[k]} onChange={()=>onChange({ order, visible:{...visible,[k]:!visible[k]} })}/><span style={{ flex:1, fontSize:12, fontWeight:700 }}>{labels[k] || k}</span></div>)}</div>
...
function ShipmentModal({ selectedQueues, shipments, onClose, onDone, draft }) {
  ...
  const labelStyle = { fontSize:10.5, fontWeight:700, color:'#6b7280', display:'block', marginBottom:4 }
    ...
    <div style={{ position:'fixed', inset:0, background:'rgba(17,24,39,0.48)', zIndex:400, display:'flex', alignItems:'center', justifyContent:'center', padding:16 }}>
      <div style={{ width:'min(1360px, 96vw)', height:'min(88vh, 760px)', background:'#fff', borderRadius:14, boxShadow:'0 24px 70px rgba(0,0,0,0.25)', display:'flex', flexDirection:'column', overflow:'hidden' }}>
        <div style={{ padding:'12px 18px', borderBottom:'1px solid #eef2f7', display:'flex', alignItems:'center', justifyContent:'space-between', flexShrink:0 }}>
          <div><div style={{ fontSize:17, fontWeight:700 }}>{isEdit ? 'Edit Shipment' : 'New Shipment'}</div><div style={{ fontSize:11, color:'#6b7280', marginTop:2 }}>{selectedQueues.length} selected queue(s). Compact shipment info left, queue details right.</div></div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'32% 68%', flex:1, minHeight:0, overflow:'hidden' }}>
          <div style={{ padding:14, borderRight:'1px solid #eef2f7', background:'#fafafa', overflow:'hidden' }}>
            <div style={{ fontSize:10, fontWeight:700, color:'#9ca3af', letterSpacing:'.5px', textTransform:'uppercase', marginBottom:8 }}>Shipment Info</div>
        ...
        <div style={{ padding:'10px 18px', borderTop:'1px solid #eef2f7', display:'flex', justifyContent:'space-between', alignItems:'center', background:'#fff', flexShrink:0 }}>
          <div style={{ display:'flex', alignItems:'center', gap:18, fontSize:12, color:'#111827', fontWeight:700 }}><span>Quantity: {totals.qty.toLocaleString()}</span><span>Cartons: {totals.cartons.toLocaleString()}</span><span>CBM: {money(totals.cbm)}</span><span>Value: {money(totals.value)}</span></div>
...
function CreateShipmentTab({ loading, rows, selectedIds, setSelectedIds, onNewShipment, isAdmin }) {
  ...
  return <div style={{ display:'flex', flexDirection:'column', flex:1, minHeight:0, background:'#fff' }}>
    <div style={{ padding:'12px 24px', borderBottom:'1px solid #f3f4f6', display:'flex', alignItems:'center', gap:14, flexShrink:0, minWidth:0 }}>
      <div style={{ flexShrink:0 }}><div style={{ fontSize:20, fontWeight:700, letterSpacing:'-0.4px' }}>Bookings</div><div style={{ fontSize:11, color:'var(--text-light)', marginTop:1 }}>Queue mirror: select Qs first, then create shipment</div></div>
      <div style={{ display:'flex', gap:8, alignItems:'stretch', minWidth:0 }}>
        {[{l:'Total Queues',v:rows.length,q:totalQty},{l:'Active Queues',v:activeRows.length,q:activeQty},{l:'Inactive Queues',v:inactiveRows.length,q:inactiveQty}].map(k => <div key={k.l} style={{ padding:'7px 12px', border:'1px solid var(--border)', borderRadius:8, background:'#fff', minWidth:120 }}><div style={{ fontSize:9, fontWeight:700, color:'var(--text-light)', textTransform:'uppercase', letterSpacing:'0.5px', marginBottom:3 }}>{k.l}</div><div style={{ display:'flex', alignItems:'baseline', gap:8 }}><div style={{ fontSize:18, fontWeight:700, lineHeight:1 }}>{k.v}</div><div style={{ fontSize:10, color:'#6b7280', fontWeight:700 }}>{Number(k.q||0).toLocaleString()} pcs</div></div></div>)}
        <div style={{ padding:'7px 12px', border:'1px solid var(--border)', borderRadius:8, background:'#fff', minWidth:170 }}><div style={{ fontSize:9, fontWeight:700, color:'var(--text-light)', textTransform:'uppercase', letterSpacing:'0.5px', marginBottom:3 }}>Shipped Progress</div><div style={{ display:'flex', alignItems:'baseline', gap:8 }}><div style={{ fontSize:18, fontWeight:700, lineHeight:1 }}>{pct}%</div><div style={{ fontSize:10, color:'#6b7280', fontWeight:700 }}>{shippedQty.toLocaleString()} / {totalQty.toLocaleString()} pcs</div></div><div style={{ marginTop:6, height:6, borderRadius:999, background:'#e5e7eb', overflow:'hidden', border:'1px solid #d1d5db' }}><div style={{ width:`${pct}%`, height:'100%', background:'#22c55e' }} /></div></div>
      </div>
      <div style={{ marginLeft:'auto', display:'flex', gap:8, alignItems:'center', flexShrink:0 }}><label className="btn btn-secondary" style={{ height:36, display:'inline-flex', alignItems:'center', gap:7, padding:'0 12px' }}><input type="checkbox" checked={multiSelect} onChange={e=>{ setMultiSelect(e.target.checked); setSelectedIds(new Set()) }} /> Multi-select</label>{multiSelect && <button className="btn btn-secondary" style={{ height:36 }} disabled={!filtered.length} onClick={toggleAll}>{filtered.length && filtered.every(q=>selectedIds.has(q.id)) ? 'Clear' : 'Select Filtered'}</button>}<span style={{ fontSize:11, color:selectedCount?'#111827':'#6b7280', fontWeight:700 }}>{selectedCount} selected</span><button className="btn btn-secondary app-icon-only" disabled={filtered.length === 0} onClick={exportCSV} title="Export"><Download size={14}/></button><button className="btn btn-secondary" style={{ height:36 }} disabled={filtered.length === 0} onClick={printList}>Print</button><button className="btn btn-primary" style={{ height:36, minWidth:132 }} onClick={onNewShipment}><Plus size={15}/> New Shipment</button></div>
    </div>
...
function ShipmentsTab({ shipments, shipmentLines, rows, onRefresh, onEdit }) {
  ...
  function cellValue(s,k,jsx=true) {
    ...
    if (k === 'tracking') return jsx && s.tracking_url ? <a href={s.tracking_url} target="_blank" rel="noreferrer" style={{ color:'#111827', fontWeight:700, textDecoration:'underline' }}>{s.tracking || 'Track'}</a> : (s.tracking || '—')
    if (k === 'status' && jsx) return <span style={statusBadgeStyle(s.status || 'Created')}>{s.status || 'Created'}</span>
    return s[k] || '—'
  }
  ...
  return <div style={{ display:'flex', flexDirection:'column', flex:1, minHeight:0, background:'#fff' }}>
    <div style={{ padding:'12px 24px', borderBottom:'1px solid #f3f4f6', display:'flex', justifyContent:'space-between', alignItems:'center', gap:16, flexShrink:0 }}>
      <div style={{ display:'flex', alignItems:'center', gap:16 }}>
        <div><div style={{ fontSize:20, fontWeight:700 }}>Shipments</div><div style={{ fontSize:11, color:'#6b7280' }}>Shipment records, drafts and print summaries</div></div>
        <div style={{ display:'flex', gap:8, alignItems:'stretch', minWidth:0 }}>
          <div style={{ padding:'8px 13px', border:'1px solid #e5e7eb', borderRadius:10, background:'#fff', minWidth:190, boxShadow:'0 1px 2px rgba(15,23,42,0.04)' }}><div style={{ fontSize:9, fontWeight:700, color:'#9ca3af', textTransform:'uppercase', letterSpacing:'0.55px', marginBottom:4 }}>Shipped This Month</div><div style={{ display:'flex', alignItems:'baseline', gap:8 }}><div style={{ fontSize:19, fontWeight:700, lineHeight:1 }}>{String(shippedThisMonth.length).padStart(2,'0')}</div><div style={{ fontSize:10, color:'#6b7280', fontWeight:700 }}>{monthQty.toLocaleString()} pcs · {moneyCurrency(monthValue, monthCurrency)}</div></div></div>
          <div style={{ padding:'8px 13px', border:'1px solid #e5e7eb', borderRadius:10, background:'#fff', minWidth:132, boxShadow:'0 1px 2px rgba(15,23,42,0.04)' }}><div style={{ fontSize:9, fontWeight:700, color:'#9ca3af', textTransform:'uppercase', letterSpacing:'0.55px', marginBottom:4 }}>ETA This Week</div><div style={{ display:'flex', alignItems:'baseline', gap:8 }}><div style={{ fontSize:19, fontWeight:700, lineHeight:1 }}>{String(etaThisWeek).padStart(2,'0')}</div><div style={{ fontSize:10, color:'#6b7280', fontWeight:700 }}>shipments</div></div></div>
          <div style={{ padding:'8px 13px', border:'1px solid #e5e7eb', borderRadius:10, background:'#fff', minWidth:140, boxShadow:'0 1px 2px rgba(15,23,42,0.04)' }}><div style={{ fontSize:9, fontWeight:700, color:'#9ca3af', textTransform:'uppercase', letterSpacing:'0.55px', marginBottom:4 }}>ETA This Month</div><div style={{ display:'flex', alignItems:'baseline', gap:8 }}><div style={{ fontSize:19, fontWeight:700, lineHeight:1 }}>{String(etaThisMonth).padStart(2,'0')}</div><div style={{ fontSize:10, color:'#6b7280', fontWeight:700 }}>shipments</div></div></div>
        </div>
      </div>
    ...
    <div style={{ padding:'8px 24px', borderBottom:'1px solid #f3f4f6', display:'flex', alignItems:'center', gap:8, flexShrink:0, overflow:'visible' }}>
      ...
      <div style={{ marginLeft:'auto', display:'flex', alignItems:'center', gap:6, flexShrink:0 }}>
        <label className="btn btn-secondary" style={{ height:32, display:'inline-flex', alignItems:'center', gap:7, padding:'0 12px' }}><input type="checkbox" checked={multiSelect} onChange={e=>{ setMultiSelect(e.target.checked); setSelectedShipmentIds(new Set()) }} /> Multi-select</label>{multiSelect && <button className="btn btn-secondary" style={{ height:32 }} disabled={!filtered.length} onClick={toggleAllShipments}>{filtered.length && filtered.every(s=>selectedShipmentIds.has(s.id)) ? 'Clear' : 'Select Filtered'}</button>}<span style={{ fontSize:11, color:selectedShipmentIds.size?'#111827':'#6b7280', fontWeight:700 }}>{selectedShipmentIds.size} selected</span>
    ...
    <div style={{ flex:1, overflow:'auto' }}><table style={{ width:'100%', minWidth:1180, borderCollapse:'collapse' }}><thead><tr style={{ background:'#fafafa', position:'sticky', top:0, zIndex:10 }}>{multiSelect && <th style={{ ...thStyle, width:34, textAlign:'center' }}><input type="checkbox" checked={filtered.length>0 && filtered.every(s=>selectedShipmentIds.has(s.id))} onChange={toggleAllShipments}/></th>}{visibleKeys.map(k=><th key={k} style={{ ...thStyle, textAlign:['qty','cartons','cbm','value'].includes(k)?'right':'left' }}>{labels[k]}</th>)}</tr></thead><tbody>{displayRows.length===0 ? <tr><td colSpan={visibleKeys.length+(multiSelect?1:0)} style={{ padding:50, textAlign:'center', color:'#9ca3af', fontSize:12 }}>No shipments yet.</td></tr> : displayRows.map(row => { if (row.type==='group') return <tr key={`g-${row.key}`} style={{ background:'var(--app-group-row-bg, #fdf6e3)', color:'var(--app-group-row-text, #111827)' }}><td colSpan={visibleKeys.length+(multiSelect?1:0)} style={{ padding:'10px 18px', borderBottom:'1px solid #eee', borderTop:'1px solid #eee', fontSize:12, fontWeight:700 }}>{row.key} <span style={{ color:'#9ca3af' }}>— {row.count} shipments</span><span style={{ float:'right' }}>Qty: {row.qty.toLocaleString()}</span></td></tr>; return <tr key={row.s.id}>{multiSelect && <td style={{ ...tdStyle, textAlign:'center' }}><input type="checkbox" checked={selectedShipmentIds.has(row.s.id)} onChange={()=>toggleShipment(row.s.id)}/></td>}{visibleKeys.map(k=>renderCell(row.s,k))}</tr> })}</tbody></table></div>
...
function settlementDefaults(s, rowData) {
  ...
  const termsDays = num(rowData?.buyer_payment_terms_days || rowData?.payment_terms_days) || num(factoryDefaults().payment_terms_days)
  const todayRate = num(rowData?.today_pkr_rate || rowData?.bank_realisation_rate) || num(factoryDefaults().pkr_rate) || DEFAULT_PKR_RATE
  const purchaseTaxPkr = num(rowData?.purchase_tax_pkr || rowData?.linked_purchase_tax_pkr || rowData?.tax_purchase_amount_pkr)
  const taxRecoveryPct = num(taxDefaults().sales_tax_refund_percent) || 100
  const rebatePct = num(taxDefaults().rebate_percent) || 1
...
function PaymentSettlementModal({ row, settlement, onClose, onSave, isAdmin=false }) {
  ...
  function card(title, children) { return <div style={{ border:'1px solid #eef2f7', borderRadius:12, background:'#fff', overflow:'hidden' }}><div style={{ padding:'10px 12px', borderBottom:'1px solid #f3f4f6', fontWeight:700, fontSize:13 }}>{title}</div><div style={{ padding:12, display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:10 }}>{children}</div></div> }
  ...
  return <div style={{ position:'fixed', inset:0, background:'rgba(17,24,39,0.48)', zIndex:420, display:'flex', alignItems:'center', justifyContent:'center', padding:16 }}>
    <div style={{ width:'min(1120px, 96vw)', maxHeight:'88vh', background:'#fff', borderRadius:14, boxShadow:'0 24px 70px rgba(0,0,0,0.25)', display:'flex', flexDirection:'column', overflow:'hidden' }}>
      <div style={{ padding:'14px 18px', borderBottom:'1px solid #eef2f7', display:'flex', alignItems:'center', justifyContent:'space-between', flexShrink:0 }}><div><div style={{ fontSize:17, fontWeight:700 }}>Shipment Settlement</div><div style={{ fontSize:11, color:'#6b7280', marginTop:2 }}>{row.shipment_no} · {row.buyer || '—'} · Invoice {moneyCurrency(row.value, row.currency)} · Terms {num(row.buyer_payment_terms_days)} days</div></div><button className="btn btn-ghost" onClick={onClose}><X size={16}/></button></div>
      <div style={{ padding:16, overflow:'auto', display:'grid', gap:12 }}>
        {card('Buyer Payment', <>
          <div><label style={labelStyle}>Amount Due</label>{isAdmin ? <><input type="number" step="0.01" style={{...inputStyle, textAlign:'right', fontWeight:700}} value={buyerAmountOverridden ? form.buyer_amount_due_override : (row.value || '')} onChange={e=>set('buyer_amount_due_override', e.target.value)} /><OverrideReset show={buyerAmountOverridden} onReset={()=>set('buyer_amount_due_override','')} /></> : <input style={{...inputStyle, background:'#f9fafb', textAlign:'right', fontWeight:700}} value={moneyCurrency(dueAmount, row.currency)} readOnly/>}</div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
          {card('Sales Tax Refund', <>
            ...
            <div>{field('Refund %','tax_refund_pct','number', { step:'0.01', disabled:receivedLocked.tax, onChange:e=>setForm(f=>({...f,tax_refund_pct:e.target.value,tax_expected_amount:recalcTax(f.tax_purchase_amount_pkr, e.target.value)})) })}<OverrideReset show={num(form.tax_refund_pct)!==(num(taxDefaults().sales_tax_refund_percent)||100)} onReset={()=>setForm(f=>{ const pct=num(taxDefaults().sales_tax_refund_percent)||100; return {...f,tax_refund_pct:pct,tax_expected_amount:recalcTax(f.tax_purchase_amount_pkr, pct)} })}/></div>
            ...
            <label style={{ display:'flex', alignItems:'center', gap:8, fontSize:12, fontWeight:700 }}><input type="checkbox" checked={!!form.tax_done} onChange={e=>set('tax_done',e.target.checked)}/> Refund Received / Lock</label>
          </>)}
          {card('Export Rebate', <>
...
function PaymentsTab({ shipments, shipmentLines, rows, isAdmin=false }) {
  ...
  function PayKpi({label, usd, pkrValue, wide=false}) { return <div style={{ padding:'8px 13px', border:'1px solid #e5e7eb', borderRadius:10, background:'#fff', minWidth:wide?190:132, boxShadow:'0 1px 2px rgba(15,23,42,0.04)' }}><div style={{ fontSize:9, fontWeight:700, color:'#9ca3af', textTransform:'uppercase', letterSpacing:'0.55px', marginBottom:4 }}>{label}</div><div style={{ display:'flex', alignItems:'baseline', gap:8 }}><div style={{ fontSize:19, fontWeight:700, lineHeight:1 }}>{moneyCurrency(usd, 'USD')}</div><div style={{ fontSize:10, color:'#6b7280', fontWeight:700, whiteSpace:'nowrap' }}>{pkr(pkrValue)}</div></div></div> }
  ...
  function cellValue(s,k,jsx=true){
    ...
    if (k==='status' && jsx) return <span style={statusBadgeStyle(s.payment_status)}>{s.payment_status}</span>
  ...
  return <div style={{ display:'flex', flexDirection:'column', flex:1, minHeight:0, background:'#fff' }}>
    <div style={{ padding:'12px 24px', borderBottom:'1px solid #f3f4f6', display:'flex', justifyContent:'space-between', alignItems:'center', gap:16, flexShrink:0, overflow:'hidden' }}>
      <div style={{ display:'flex', alignItems:'center', gap:16, minWidth:0 }}>
        <div style={{ minWidth:260, flex:'0 0 auto' }}><div style={{ fontSize:20, fontWeight:700 }}>Payments</div><div style={{ fontSize:11, color:'#6b7280' }}>Buyers Payments, Sales Tax and Rebate Tracker</div></div>
    ...
    <div style={{ padding:'8px 24px 0', display:'flex', gap:6, alignItems:'center', flexShrink:0 }}>
      {[['all','All'],['week','Due This Week'],['month','Due This Month'],['overdue','Overdue'],['completed','Completed']].map(([key,label]) => <button key={key} onClick={()=>setQuickFilter(key)} style={{ height:28, padding:'0 10px', borderRadius:999, border:quickFilter===key?'1px solid #111827':'1px solid #e5e7eb', background:quickFilter===key?'#111827':'#fff', color:quickFilter===key?'#fff':'#374151', fontSize:11, fontWeight:700, cursor:'pointer' }}>{label}</button>)}
    </div>
    <div style={{ padding:'8px 24px', borderBottom:'1px solid #f3f4f6', display:'grid', gridTemplateColumns:'minmax(220px, 1.1fr) minmax(150px, .9fr) minmax(180px, 1fr) minmax(150px, .9fr) auto', alignItems:'center', gap:8, flexShrink:0, overflow:'hidden', maxWidth:'100%' }}>
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'flex-end', gap:6, minWidth:0, whiteSpace:'nowrap' }}><label className="btn btn-secondary" style={{ height:32, display:'inline-flex', alignItems:'center', gap:7, padding:'0 12px' }}><input type="checkbox" checked={multiSelect} onChange={e=>{ setMultiSelect(e.target.checked); setSelectedIds(new Set()) }} /> Multi-select</label>{multiSelect && <button className="btn btn-secondary" style={{ height:32 }} disabled={!filtered.length} onClick={toggleAll}>{filtered.length && filtered.every(s=>selectedIds.has(s.id)) ? 'Clear' : 'Select Filtered'}</button>}<span style={{ fontSize:11, color:selectedIds.size?'#111827':'#6b7280', fontWeight:700 }}>{selectedIds.size} selected</span><button className="btn btn-secondary" style={actionBtnStyle} disabled={selected.length !== 1} onClick={editSelected} title="Edit settlement"><Edit3 size={14}/></button><button className="btn btn-secondary" style={actionBtnStyle} onClick={()=>setShowCols(true)} title="Column Settings"><Settings size={15}/></button></div>
    </div>
    <div style={{ flex:1, overflowY:'auto', overflowX:'hidden' }}><table style={{ width:'100%', minWidth:0, tableLayout:'fixed', borderCollapse:'collapse' }}><thead><tr style={{ background:'#fafafa', position:'sticky', top:0, zIndex:10 }}>{multiSelect && <th style={{ ...thStyle, width:34, textAlign:'center' }}><input type="checkbox" checked={filtered.length>0 && filtered.every(s=>selectedIds.has(s.id))} onChange={toggleAll}/></th>}{visibleKeys.map(k=><th key={k} style={{ ...thStyle, textAlign:['qty','invoice_value','received_pkr','fx_diff','tax_refund','rebate','net_pending'].includes(k)?'right':'left' }}>{labels[k]}</th>)}</tr></thead><tbody>{displayRows.length===0 ? <tr><td colSpan={visibleKeys.length+(multiSelect?1:0)} style={{ padding:50, textAlign:'center', color:'#9ca3af', fontSize:12 }}>No settlement records yet. Shipments will appear here automatically.</td></tr> : displayRows.map(row => { if(row.type==='group') return <tr key={`pg-${row.key}`} style={{ background:'var(--app-group-row-bg, #fdf6e3)', color:'var(--app-group-row-text, #111827)' }}><td colSpan={visibleKeys.length+(multiSelect?1:0)} style={{ padding:'10px 18px', borderBottom:'1px solid #eee', borderTop:'1px solid #eee', fontSize:12, fontWeight:700 }}>{row.key} <span style={{ color:'#9ca3af' }}>— {row.count} settlements</span></td></tr>; return <tr key={row.s.id} style={{ background:row.s.payment_status==='Overdue'?'#fff7f7':undefined }}>{multiSelect && <td style={{ ...tdStyle, textAlign:'center' }}><input type="checkbox" checked={selectedIds.has(row.s.id)} onChange={()=>toggle(row.s.id)}/></td>}{visibleKeys.map(k=>renderCell(row.s,k))}</tr> })}</tbody></table></div>
...
export default function Shipping() {
  ...
  return <div style={{ display:'flex', flexDirection:'column', height:'100%', background:'#fff', overflow:'hidden' }}>
    <div style={{ padding:'0 24px', borderBottom:'1px solid #eef0f4', flexShrink:0 }}><div style={{ display:'flex', gap:4 }}><button style={{ padding:'12px 16px 10px', border:'none', borderBottom:activeTab==='create'?'3px solid #111827':'3px solid transparent', background:'transparent', fontSize:13, fontWeight:700, cursor:'pointer', color:activeTab==='create'?'#111827':'#6b7280' }} onClick={()=>setActiveTab('create')}>Bookings</button><button style={{ padding:'12px 16px 10px', border:'none', borderBottom:activeTab==='shipments'?'3px solid #111827':'3px solid transparent', background:'transparent', fontSize:13, fontWeight:700, cursor:'pointer', color:activeTab==='shipments'?'#111827':'#6b7280' }} onClick={()=>setActiveTab('shipments')}>Shipments</button>{isAdmin && <button style={{ padding:'12px 16px 10px', border:'none', borderBottom:activeTab==='payments'?'3px solid #111827':'3px solid transparent', background:'transparent', fontSize:13, fontWeight:700, cursor:'pointer', color:activeTab==='payments'?'#111827':'#6b7280' }} onClick={()=>setActiveTab('payments')}>Payments</button>}</div></div>
