...
async function fetchUserById(id) {
  ...
  const columns = 'id, email, username, name, display_name, role, is_active'
...
async function findUserByIdentifier(identifier) {
  ...
  const richColumns = 'id, email, username, name, display_name, role, is_active, password'
  const basicColumns = 'id, email, role, is_active, password'

  if (hasAt) {
    const byEmail = await withTimeout(supabase.from('users').select(richColumns).eq('email', emailValue).maybeSingle()).catch(error => ({ data: null, error }))
  ...
  const byUsername = await withTimeout(supabase.from('users').select(richColumns).eq('username', clean).maybeSingle()).catch(error => ({ data: null, error }))
...
export function AuthProvider({ children }) {
  ...
  const login = async (identifier, password) => {
    ...
    if (String(data.password || '') !== cleanPassword) return { user: null, error: new Error('Invalid username/email or password.') }
