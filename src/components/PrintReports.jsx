...
export async function printPurchaseOrder(po, supplier, order, taxRate, targetWindow = null) {
  ...
  const statusText = String(po.status || po.po_status || 'ORIGINAL').toUpperCase()
  ...
  <div class="po-a4-page">
    <style>
      @page { size: A4; margin: 0; }
      .po-a4-page{width:210mm;height:297mm;box-sizing:border-box;padding:8mm 6mm 7mm;background:#fff;color:#000;font-family:Arial,Helvetica,sans-serif;position:relative;page-break-after:always}
      .po-a4-page *{box-sizing:border-box} table{border-collapse:collapse;width:100%;table-layout:fixed}
      .po-head{display:grid;grid-template-columns:1fr 40mm;align-items:start;margin-bottom:3mm}.po-small-title{font-size:16px;font-weight:800;line-height:1;margin:0 0 2mm}.po-company{font-family:Georgia,'Times New Roman',serif;font-size:44px;font-weight:900;letter-spacing:-1.4px;line-height:.82;white-space:nowrap}.po-tagline{font-size:11.5px;font-style:italic;font-weight:700;line-height:1.15;margin-top:1mm}.po-address{font-size:9.5px;line-height:1.22;margin-top:1mm;max-width:130mm}.po-status{text-align:right}.po-version{font-size:19px;font-weight:900;line-height:1;margin-bottom:1mm}.po-count{font-size:10px;margin-bottom:2mm}.po-status-box{display:inline-block;background:#000;color:#fff;font-family:Georgia,'Times New Roman',serif;font-size:23px;font-weight:900;letter-spacing:.5px;padding:2mm 3mm;min-width:35mm;text-align:center}
      ...
      .items-table{margin-top:3mm;border:1.4px solid #000}.items-table th{background:#000;color:#fff;border:1.4px solid #000;padding:2.6mm 2.5mm;font-size:13px;font-weight:900;text-align:left}.items-table td{border-left:1.4px solid #000;border-right:1.4px solid #000;border-bottom:1.1px solid #000;padding:2.4mm 2.4mm;font-size:11.2px;height:9mm;vertical-align:middle}.items-table .blank-row td{height:9mm}.items-table .num{text-align:right;font-variant-numeric:tabular-nums}.items-table th:nth-child(1){width:11%}.items-table th:nth-child(2){width:47%}.items-table th:nth-child(3){width:10%}.items-table th:nth-child(4){width:9%}.items-table th:nth-child(5){width:11%}.items-table th:nth-child(6){width:12%}.total-strip td{background:#e8e8e8;font-size:14px;font-weight:900;height:9mm;border-top:1.4px solid #000;border-bottom:1.4px solid #000}.total-strip .total-title{text-align:left}
      .words-tax-table{border-left:1.4px solid #000;border-right:1.4px solid #000;border-bottom:1.4px solid #000}.words-tax-table td{border:1.4px solid #000;padding:3mm 2.5mm;vertical-align:top;font-size:11px}.words-box{width:63%;height:18mm}.tax-box{width:37%;padding:0!important}.tax-row{display:grid;grid-template-columns:1fr 28mm;border-bottom:1.4px solid #000;min-height:9mm;align-items:center}.tax-row:last-child{border-bottom:0}.tax-row div{padding:2mm 2.5mm;font-size:13px;font-weight:900}.tax-row div:last-child{text-align:right;font-variant-numeric:tabular-nums}.terms-block{margin-top:4mm;min-height:27mm}.terms-title{font-size:13px;font-weight:900;margin-bottom:2mm;text-transform:uppercase}.terms-text{font-size:11.2px;line-height:1.27;text-align:justify}.sig-table{margin-top:4mm;border:1.4px solid #000;height:26mm}.sig-table td{border:1.4px solid #000;width:25%;vertical-align:bottom;text-align:center;padding:0 2mm 4mm}.sig-label{font-size:13px;font-weight:900;text-transform:uppercase}.footer-lock{position:absolute;left:6mm;right:6mm;bottom:2.5mm;display:flex;justify-content:space-between;font-size:10px;font-weight:800}
    </style>
    <div class="po-head"><div><div class="po-small-title">PURCHASE ORDER</div><div class="po-company">NIZAMIA APPARELS</div><div class="po-tagline">Manufacturer &amp; Exporter of Knitted and Woven Garments</div><div class="po-address">RCC14, SHED NR 2, ESTATE AVENUE ROAD, SITE INDUSTRIAL AREA,<br/>KARACHI 75700, PAKISTAN | www.nizamia.com</div></div><div class="po-status"><div class="po-version">V${version}</div><div class="po-count">Print Count: ${String(printCount).padStart(2,'0')}</div><div class="po-status-box">${escapeHTML(statusText)}</div></div></div>
    <table class="meta-table"><tr><td class="supplier-cell"><div class="meta-label">Supplier Information</div><div class="meta-value">${escapeHTML(supplier?.name || '—')}</div></td><td class="middle-cell"><div class="mini-stack"><div><div class="meta-label">Job Number</div><div class="meta-value">${escapeHTML(order?.job_number || '—')}</div></div><div><div class="meta-label">Style &amp; Queue Number</div><div class="meta-value">${escapeHTML([order?.style_number, qReference].filter(Boolean).join(' / ') || '—')}</div></div></div></td><td class="right-cell"><div class="right-stack"><div><div class="meta-label">PO Number</div><div class="meta-value">${escapeHTML(po.po_number || '—')}</div></div><div><div class="meta-label">Issue Date</div><div class="meta-value">${fmtDate(po.po_date)}</div></div><div><div class="meta-label">Delivery Date</div><div class="meta-value">${fmtDate(po.delivery_date)}</div></div><div><div class="meta-label">Payment Terms</div><div class="meta-value">${escapeHTML(po.payment_terms || '—')}</div></div></div></td></tr></table>
    ...
    <table class="sig-table"><tr><td><div class="sig-label">Supplier Receiving</div></td><td><div class="sig-label">Merchandiser</div></td><td><div class="sig-label">Store Incharge</div></td><td><div class="sig-label">CEO / Director</div></td></tr></table>
    <div class="footer-lock"><span>V${version}</span><span>Print Count: ${String(printCount).padStart(2,'0')}</span></div>
  </div>`
