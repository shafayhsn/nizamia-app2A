...
export async function printPurchaseOrder(po, supplier, order, taxRate, targetWindow = null) {
  ...
  <div class="po-a4-page">
    ...
    <div class="po-head"><div><div class="po-small-title">PURCHASE ORDER</div><div class="po-company">${escapeHTML(company.name)}</div><div class="po-tagline">${escapeHTML(company.tagline)}</div><div class="po-address">${companyAddressLine(company)}</div></div><div class="po-status"><div class="po-version">V${version}</div><div class="po-count">Print Count: ${String(printCount).padStart(2,'0')}</div><div class="po-status-box">${escapeHTML(statusText)}</div></div></div>
