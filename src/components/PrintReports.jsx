...
const PRINT_CSS = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .page { width: 210mm; min-height: 297mm; padding: 14mm 16mm 14mm 16mm; position: relative; page-break-after: always; }
...
export function printHTML(html, targetWindow = null) {
  const w = targetWindow || window.open('', '_blank', 'width=900,height=700')
  if (!w) {
...
// ─────────────────────────────────────────────────────────────────────────────
export async function printPurchaseOrder(po, supplier, order, taxRate) {
  ...
  const itemRows = items.map((item, i) => `<tr>
    <td class="right" style="color:#9ca3af">${String(i+1).padStart(2,'0')}</td>
    <td><div class="bold">${item.description||'—'}</div>${item.specification ? `<div style="font-size:9px;color:#6b7280;font-family:'Courier New',monospace">${item.specification}</div>` : ''}</td>
    <td class="right mono">${item.qty ? fmtNum(item.qty) : '—'}</td>
    <td>${item.unit||'—'}</td>
    <td class="right mono">${item.unit_rate ? fmtNum(item.unit_rate,2) : '—'}</td>
    <td class="right mono bold">${item.amount ? fmtNum(item.amount,0) : '—'}</td>
  </tr>`).join('')

  const html = `
  <div class="page">
    ${docHeader('Purchasing Document', 'Purchase Order', `${po.po_number} · ${fmtDate(po.po_date)} · ORIGINAL`)}
    <div class="po-header-box">
      <div class="po-left">
        <div class="po-number">${po.po_number}</div>
        <div class="po-meta-row">
          <div class="po-meta-item"><div class="lbl">PO Date</div><div class="val">${fmtDate(po.po_date)}</div></div>
          <div class="po-meta-item"><div class="lbl">Delivery Date</div><div class="val">${fmtDate(po.delivery_date)}</div></div>
        </div>
        <div class="po-meta-row">
          <div class="po-meta-item"><div class="lbl">Job No.</div><div class="val">${order?.job_number||'—'}</div></div>
          <div class="po-meta-item"><div class="lbl">Style</div><div class="val">${order?.style_number||'—'}</div></div>
        </div>
        <div class="po-meta-row">
          <div class="po-meta-item"><div class="lbl">Payment Terms</div><div class="val">${po.payment_terms||'—'}</div></div>
          <div class="po-meta-item"><div class="lbl">Currency</div><div class="val">${po.currency||'PKR'}</div></div>
        </div>
      </div>
      <div class="po-right">
        <div class="info-label">Supplier</div>
        <div class="po-supplier-name">${supplier?.name||'—'}</div>
        <div class="po-supplier-addr">${[supplier?.address, supplier?.city, supplier?.phone].filter(Boolean).join('<br>') || ''}</div>
        <hr class="po-divider" />
        <div class="info-label" style="margin-bottom:4px">Deliver To</div>
        <div style="font-size:11px;font-weight:600">Nizamia Apparels</div>
        <div style="font-size:10px;color:#374151">Factory, Korangi Industrial<br>Karachi — 74900</div>
      </div>
    </div>
    <div class="section-title">Order Items</div>
    <table>
      <thead><tr><th>#</th><th>Item Description</th><th class="right">Quantity</th><th>Unit</th><th class="right">Unit Rate (${po.currency||'PKR'})</th><th class="right">Amount (${po.currency||'PKR'})</th></tr></thead>
      <tbody>${itemRows}</tbody>
    </table>
    <div class="po-totals">
      <div class="po-total-row"><span>Subtotal</span><span>${po.currency||'PKR'} ${fmtNum(subtotal,0)}</span></div>
      ${taxRate ? `<div class="po-total-row"><span>GST ${taxRate}%</span><span>${po.currency||'PKR'} ${fmtNum(tax,0)}</span></div>` : ''}
      <div class="po-total-row grand"><span>Grand Total</span><span>${po.currency||'PKR'} ${fmtNum(grand,0)}</span></div>
    </div>
    <div class="tc-box">
      <div class="tc-label">Terms & Conditions</div>
      <div class="tc-text">1. All goods must conform to the approved specifications and samples. 2. Delivery must be made by the date specified above. 3. Invoice must reference this PO number. 4. Nizamia Apparels reserves the right to reject non-conforming goods. 5. Payment will be processed within terms upon satisfactory receipt and inspection.</div>
    </div>
    <div class="sig-block">
      <div><div class="sig-label">Authorised By</div><div class="sig-line"></div><div class="sig-name">Merchandiser</div></div>
      <div><div class="sig-label">Supplier Acknowledgement</div><div class="sig-line"></div></div>
      <div><div class="sig-label">Date</div><div class="sig-line"></div><div class="sig-name">${today}</div></div>
    </div>
    <div class="doc-footer">
      <span>Nizamia Apparels · Karachi, Pakistan — Purchase Order ${po.po_number} · Original</span>
      <span>01 / 01</span>
    </div>
  </div>`
  printHTML(html)
...
export async function printWorkOrder(wo, vendor, order) {
