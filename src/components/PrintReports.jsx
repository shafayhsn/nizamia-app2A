...
export async function printPurchaseOrder(po, supplier, order, taxRate, targetWindow = null) {
  ...
  const itemRows = items.length ? items.map((item) => `<tr>
    <td>${escapeHTML(item.description || '—')}</td>
    <td>${escapeHTML([item.specification, item.breakdown].filter(Boolean).join(' · ') || '—')}</td>
    <td>${escapeHTML(item.unit || '—')}</td>
    <td class="num">${item.qty ? fmtNum(item.qty,0) : '—'}</td>
    <td class="num">${item.unit_rate ? fmtNum(item.unit_rate,2) : '—'}</td>
    <td class="num">${item.amount ? fmtNum(item.amount,2) : '—'}</td>
  </tr>`).join('') : `<tr><td colspan="6" style="height:120px;color:#9ca3af">No items</td></tr>`

  const html = `
  <div class="po-format-page">
    <style>
      .po-format-page { width:210mm; height:297mm; padding:10mm 12mm; background:#fff; color:#000; font-family: Arial, Helvetica, sans-serif; position:relative; page-break-after:always; display:flex; flex-direction:column; }
      .po-brand { text-align:center; line-height:1.15; }
      .po-brand h1 { font-size:20px; font-weight:800; margin:0; letter-spacing:.3px; }
      .po-brand h2 { font-size:18px; font-weight:800; margin:2px 0 0; }
      .po-brand .sub { font-size:11px; font-weight:600; margin-top:2px; }
      .po-brand .addr { font-size:10px; margin-top:3px; line-height:1.35; }
      .po-topgrid { display:grid; grid-template-columns: 1.2fr .8fr; border:1px solid #000; margin-top:10px; min-height:30mm; }
      .po-box { border-right:1px solid #000; padding:6px 7px; }
      .po-box:last-child { border-right:0; }
      .po-label { font-size:10px; font-weight:800; text-transform:uppercase; margin-bottom:5px; }
      .po-value { font-size:13px; font-weight:700; min-height:18px; }
      .po-detail-grid { display:grid; grid-template-columns:1fr 1fr 1fr 1fr; border-left:1px solid #000; border-right:1px solid #000; }
      .po-detail { min-height:13mm; border-bottom:1px solid #000; border-right:1px solid #000; padding:4px 6px; }
      .po-detail:nth-child(4n) { border-right:0; }
      .po-detail .po-label { font-size:8.5px; margin-bottom:3px; }
      .po-detail .po-value { font-size:10.5px; font-weight:700; }
      .po-original { text-align:right; font-size:10px; font-weight:800; padding:4px 0 5px; }
      .po-items { width:100%; border-collapse:collapse; margin:0; border:1px solid #000; table-layout:fixed; }
      .po-items th { border:1px solid #000; padding:5px 5px; font-size:9px; font-weight:800; color:#000; text-align:left; letter-spacing:0; background:#fff; }
      .po-items td { border-left:1px solid #000; border-right:1px solid #000; padding:5px 5px; font-size:10px; color:#000; height:9mm; }
      .po-items .num { text-align:right; }
      .po-items th:nth-child(1){width:22%}.po-items th:nth-child(2){width:36%}.po-items th:nth-child(3){width:8%}.po-items th:nth-child(4){width:10%}.po-items th:nth-child(5){width:10%}.po-items th:nth-child(6){width:14%}
      .po-flex-spacer { flex:1; border-left:1px solid #000; border-right:1px solid #000; min-height:28mm; }
      .po-bottom { border:1px solid #000; border-top:0; }
      .po-terms-total { display:grid; grid-template-columns: 1fr 58mm; border-bottom:1px solid #000; }
      .po-terms { padding:6px 7px; border-right:1px solid #000; min-height:35mm; }
      .po-terms-title { font-size:10px; font-weight:800; margin-bottom:5px; text-transform:uppercase; }
      .po-terms-text { font-size:9.5px; line-height:1.35; text-align:justify; }
      .po-totals-box { padding:0; }
      .po-total-row { display:grid; grid-template-columns:1fr 1fr; border-bottom:1px solid #000; min-height:9mm; align-items:center; }
      .po-total-row:last-child { border-bottom:0; }
      .po-total-row div { padding:4px 6px; font-size:10px; font-weight:800; }
      .po-total-row div:last-child { border-left:1px solid #000; text-align:right; font-weight:600; }
      .po-words { border-bottom:1px solid #000; padding:6px 7px; font-size:10px; min-height:12mm; }
      .po-words b { font-size:10px; }
      .po-sigs { display:grid; grid-template-columns:repeat(5, 1fr); min-height:22mm; }
      .po-sig { border-right:1px solid #000; padding:5px 5px; display:flex; flex-direction:column; justify-content:flex-end; }
      .po-sig:last-child { border-right:0; }
      .po-sig-line { border-top:1px solid #000; height:10px; }
      .po-sig-label { font-size:8.5px; font-weight:800; text-transform:uppercase; text-align:center; }
      .po-foot { display:flex; justify-content:space-between; align-items:center; font-size:9px; font-weight:800; padding-top:4px; }
    </style>
    <div class="po-brand">
      <h1>NIZAMIA APPARELS</h1>
      <h2>PURCHASE ORDER</h2>
      <div class="sub">Manufacturer &amp; Exporter of Knitted and Woven Garments</div>
      <div class="addr">RCC14, SHED NR 2, ESTATE AVENUE ROAD, SITE INDUSTRIAL AREA,<br/>KARACHI 75700, PAKISTAN | www.nizamia.com</div>
    </div>

    <div class="po-topgrid">
      <div class="po-box"><div class="po-label">Supplier Information</div><div class="po-value">${escapeHTML(supplier?.name || '—')}</div></div>
      <div class="po-box"><div class="po-label">Job Number</div><div class="po-value">${escapeHTML(order?.job_number || '—')}</div></div>
    </div>

    <div class="po-detail-grid">
      <div class="po-detail"><div class="po-label">PO Number</div><div class="po-value">${escapeHTML(po.po_number || '—')}</div></div>
      <div class="po-detail"><div class="po-label">Issue Date</div><div class="po-value">${fmtDate(po.po_date)}</div></div>
      <div class="po-detail"><div class="po-label">Delivery Date</div><div class="po-value">${fmtDate(po.delivery_date)}</div></div>
      <div class="po-detail"><div class="po-label">Payment Terms</div><div class="po-value">${escapeHTML(po.payment_terms || '—')}</div></div>
      <div class="po-detail" style="grid-column:1 / 5"><div class="po-label">Style &amp; Queue Number</div><div class="po-value">${escapeHTML([order?.style_number, qReference].filter(Boolean).join(' / ') || '—')}</div></div>
    </div>

    <div class="po-original">ORIGINAL</div>

    <table class="po-items">
      <thead><tr><th>ITEMS</th><th>DESCRIPTION</th><th>UOM</th><th>QTY</th><th>RATE</th><th>AMOUNT</th></tr></thead>
      <tbody>${itemRows}</tbody>
    </table>
    <div class="po-flex-spacer"></div>

    <div class="po-bottom">
      <div class="po-terms-total">
        <div class="po-terms">
          <div class="po-terms-title">Terms &amp; Conditions</div>
          <div class="po-terms-text">${escapeHTML(getPOTerms())}</div>
        </div>
        <div class="po-totals-box">
          <div class="po-total-row"><div>ADD SALES TAX (${fmtNum(effectiveTaxRate,0)}%)</div><div>${fmtNum(tax,2)}</div></div>
          <div class="po-total-row"><div>TOTAL</div><div>${fmtNum(subtotal,2)}</div></div>
          <div class="po-total-row"><div>TOTAL AMOUNT (${escapeHTML(currency)})</div><div>${fmtNum(grand,2)}</div></div>
        </div>
      </div>
      <div class="po-words"><b>AMOUNT IN WORDS:</b> ${escapeHTML(amountInWords(grand, currency))}</div>
      <div class="po-sigs">
        <div class="po-sig"><div class="po-sig-line"></div><div class="po-sig-label">Supplier</div></div>
        <div class="po-sig"><div class="po-sig-line"></div><div class="po-sig-label">Receiving</div></div>
        <div class="po-sig"><div class="po-sig-line"></div><div class="po-sig-label">Merchandiser</div></div>
        <div class="po-sig"><div class="po-sig-line"></div><div class="po-sig-label">Store Incharge</div></div>
        <div class="po-sig"><div class="po-sig-line"></div><div class="po-sig-label">CEO / Director</div></div>
      </div>
    </div>
    <div class="po-foot"><span>V${version}</span><span>Print Count: ${String(printCount).padStart(2,'0')}</span></div>
  </div>`
