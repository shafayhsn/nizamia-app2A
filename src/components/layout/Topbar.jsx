  ...
  return (
    <header style={{
        ...
        {CITIES.map(({ label, tz }) => {
          ...
          return (
            <div key={label} style={{ display: 'flex', alignItems: 'baseline', gap: 5 }}>
              <span style={{ fontSize: 12, fontWeight: 550, color: '#1a1a2e' }}>{label}</span>
      ...
      <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
        {rates.map(({ sym, rate }) => (
          <div key={sym} style={{ display: 'flex', alignItems: 'baseline', gap: 1 }}>
            <span style={{ fontSize: 11, color: '#1a1a2e', fontWeight: 550 }}>{sym}</span>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        <div style={{ fontSize: 11, fontWeight: 550, background: '#dc2626', padding: '3px 9px', borderRadius: 5, color: '#fff', whiteSpace: 'nowrap' }}>W{week}</div>
        <span style={{ fontSize: 12, color: '#1a1a2e', fontWeight: 550, whiteSpace: 'nowrap' }}>{dayStr}</span>
        <span style={{ fontSize: 13, fontWeight: 550, color: '#1a1a2e', fontVariantNumeric: 'tabular-nums', whiteSpace: 'nowrap' }}>{timeStr}</span>
      </div>
      ...
      <div style={{ position: 'relative' }}>
        <button onClick={() => setNotificationsOpen(v => !v)} title="Deadline notifications" style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#6b7280', padding: 6, position: 'relative', display: 'flex', borderRadius: 6 }}
          ...
          {notificationCount > 0 && <span style={{ position: 'absolute', top: 1, right: 1, minWidth: 14, height: 14, background: '#dc2626', borderRadius: 999, border: '1.5px solid #fff', color: '#fff', fontSize: 8.5, fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', lineHeight: 1 }}>{notificationCount > 9 ? '9+' : notificationCount}</span>}
        </button>
