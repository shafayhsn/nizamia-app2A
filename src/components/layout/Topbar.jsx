  ...
  return (
    <header style={{
        ...
        {CITIES.map(({ label, tz }) => {
          ...
          return (
            <div key={label} style={{ display: 'flex', alignItems: 'baseline', gap: 5 }}>
              ...
              <span style={{ fontSize: 12, fontWeight: 700, fontVariantNumeric: 'tabular-nums', color: '#1a1a2e' }}>{time}</span>
            </div>
          )
        })}
      </div>
      ...
      <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
        {rates.map(({ sym, rate }) => (
          <div key={sym} style={{ display: 'flex', alignItems: 'baseline', gap: 1 }}>
            ...
            <span style={{ fontSize: 12, fontWeight: 700, color: '#1a1a2e', fontVariantNumeric: 'tabular-nums' }}>{rate.toFixed(2)}</span>
            <span style={{ fontSize: 9, color: '#1a1a2e', marginLeft: 1, fontWeight: 700 }}>↑</span>
          </div>
        ))}
      </div>
      ...
      <div style={{ position: 'relative' }}>
        <button onClick={() => setNotificationsOpen(v => !v)} title="Deadline notifications" style={{ background: 'none', border: 'none', cursor: 'pointer', color: notificationCount > 0 ? '#dc2626' : '#6b7280', padding: 6, position: 'relative', display: 'flex', borderRadius: 6 }}
