...
import { Bell, Cloud, CloudOff, RefreshCw, AlertCircle, LogOut, UserCircle } from 'lucide-react'
...
export default function Topbar() {
  ...
  const notificationCount = overdueEvents(events).length + dueWithin24h(events).filter(e => e.status !== 'overdue').length
  const now = useClock()
  ...
  return (
    <header style={{
      ...
      <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
        {rates.map(({ sym, rate }) => (
          <div key={sym} style={{ display: 'flex', alignItems: 'baseline', gap: 1 }}>
            ...
            <span style={{ fontSize: 12, fontWeight: 600, color: '#374151', fontVariantNumeric: 'tabular-nums' }}>{rate.toFixed(2)}</span>
      ...
      {user && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, color: '#374151', marginRight: 8 }} title={user.email}>
          <UserCircle size={15} strokeWidth={1.8} />
          <span style={{ fontSize: 11, fontWeight: 700, maxWidth: 145, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{user.email}</span>
          <button onClick={logout} title="Logout" style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#6b7280', padding: 4, display: 'flex', borderRadius: 6 }}
      ...
      <div style={{ position: 'relative' }}>
        <button onClick={() => setNotificationsOpen(v => !v)} title="Deadline notifications" style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#6b7280', padding: 4, position: 'relative', display: 'flex', borderRadius: 6 }}
          onMouseEnter={e => e.currentTarget.style.background = '#f3f4f6'}
          onMouseLeave={e => e.currentTarget.style.background = 'none'}>
          <Bell size={16} strokeWidth={1.8} />
          {notificationCount > 0 && <span style={{ position: 'absolute', top: 1, right: 1, minWidth: 13, height: 13, background: '#dc2626', borderRadius: 999, border: '1.5px solid #fff', color: '#fff', fontSize: 8, fontWeight: 900, display: 'flex', alignItems: 'center', justifyContent: 'center', lineHeight: 1 }}>{notificationCount > 9 ? '9+' : notificationCount}</span>}
        </button>
