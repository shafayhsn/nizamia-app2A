...
import {
  ...
  Printer, StickyNote, UserCircle, ClipboardCheck, Send, CalendarCheck, CalendarDays, LogOut,
} from 'lucide-react'
...
const NAV_GROUPS = [
  ...
  },
  {
    ...
    items: [
      ...
      { action: 'tools', icon: Wrench, label: 'Production Tools', module: 'tools' },
    ],
  },
]
          ...
          {NAV_GROUPS.map(group => (
            <div key={group.heading} style={{ marginBottom: collapsed ? 3 : 10 }}>
              ...
              {group.items.map(({ to, action, icon: Icon, label, module }) => {
                const allowed = !module || module === 'dashboard' || module === 'tools' || can(userRole, module, 'view')
