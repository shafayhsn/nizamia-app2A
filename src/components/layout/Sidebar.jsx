  ...
  return (
    <>
      <aside style={{
        ...
        {/* Nav */}
        <nav style={{ flex: '1 1 auto', minHeight: 0, padding: '10px 8px 6px', overflowY: 'auto', overflowX: 'hidden' }}>
          <NavLink to="/dashboard" title={collapsed ? 'Dashboard' : undefined} style={({ isActive }) => ({
            display: 'flex', alignItems: 'center',
            gap: 10, padding: collapsed ? '9px 0' : '8px 12px',
            justifyContent: collapsed ? 'center' : 'flex-start',
            textDecoration: 'none', margin: '1px 0 6px', borderRadius: 7,
          ...
          >
            {({ isActive }) => (
              <>
                <LayoutDashboard size={16} strokeWidth={isActive ? 2.2 : 1.8} style={{ flexShrink: 0 }} />
                {!collapsed && <span style={{ whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>Dashboard</span>}
              </>
            )}
          </NavLink>
          ...
          {NAV_GROUPS.map(group => (
            <div key={group.heading} style={{ marginBottom: collapsed ? 3 : 10 }}>
              {!collapsed && (
                <div style={{
                  fontSize: 10, fontWeight: 550, color: '#9ca3af',
        ...
        {/* Bottom tools */}
        <div style={{ padding: '8px', borderTop: `1px solid ${SB_BORDER}`, display: 'flex', flexDirection: 'column', gap: 2, flexShrink: 0 }}>
          <div title={currentName} style={{ display: 'flex', alignItems: 'center', gap: 9, padding: collapsed ? '8px 0' : '8px 10px', justifyContent: collapsed ? 'center' : 'flex-start', borderRadius: 8, background: '#f9fafb', border: '1px solid #f1f5f9', marginBottom: 6 }}>
            ...
            {!collapsed && <div style={{ minWidth: 0, flex: 1 }}>
              <div style={{ fontSize: 10, color: '#9ca3af', fontWeight: 550, textTransform: 'uppercase', letterSpacing: 0.35 }}>Logged in</div>
              <div style={{ fontSize: 12, color: '#111827', fontWeight: 550, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{currentName}</div>
            </div>}
            {!collapsed && (
