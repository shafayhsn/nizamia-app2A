...
export default function OrderWizard({ order, onClose }) {
  ...
  async function loadOrder() {
    const { data } = await supabase.from('orders').select('id,buyer_id,buyer_name,style_number,ship_date,planned_date,in_store_date,po_number,po_date,currency,season,status,total_qty,total_value_usd,job_id,job_number,merchandiser_name,agent_name,factory_ref,description,ship_mode,incoterms,port_of_loading,port_of_discharge,notes,excess_cutting_pct,step_po_matrix,step_bom,step_fitting,step_sampling,step_washing,step_embellishment,step_finishing,step_processes,step_finalize').eq('id', orderId).single()
