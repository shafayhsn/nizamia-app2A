...
export default function OrderWizard({ order, onClose }) {
  ...
  const stepSaveMap  = useRef({})
  // registerSave for active step (wired to stepSaveRef)
  // registerSaveForStep used by all steps to always keep their fn current
  const registerSave = useCallback((fn) => { stepSaveRef.current = fn }, [])
  const makeRegisterSave = useCallback((num) => (fn) => {
    stepSaveMap.current[num] = fn
    if (num === stepSaveRef._activeStep) stepSaveRef.current = fn
  }, [])

  useEffect(() => { if (orderId) loadOrder() }, [orderId])

  async function loadOrder() {
    const { data } = await supabase.from('orders').select('id,buyer_id,buyer_name,style_number,brand_name,store_name,ship_date,planned_date,po_number,po_date,currency,season,status,total_qty,total_value_usd,job_id,job_number,merchandiser_name,agent_name,factory_ref,description,ship_mode,incoterms,port_of_loading,port_of_discharge,excess_cutting_pct,step_po_matrix,step_bom,step_fitting,step_sampling,step_washing,step_embellishment,step_finishing,step_processes,step_finalize').eq('id', orderId).single()
  ...
  // Keep stepSaveRef in sync with active step's latest save fn
  useEffect(() => {
    stepSaveRef._activeStep = step
    if (stepSaveMap.current[step]) stepSaveRef.current = stepSaveMap.current[step]
    else stepSaveRef.current = null
  }, [step])
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 200, background: 'rgba(0,0,0,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <div style={{ background: '#fff', borderRadius: 12, boxShadow: '0 24px 80px rgba(0,0,0,0.22)', width: '100%', maxWidth: 900, height: '88vh', maxHeight: 780, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        {/* Step content — all steps rendered, inactive hidden to prevent remount/refetch */}
        <div style={{ flex: 1, overflowY: 'auto', background: '#fff' }}>
          {Object.entries(STEP_COMPONENTS).map(([num, Comp]) => (
            <div key={num} style={{ display: step === parseInt(num) ? 'block' : 'none', padding: '24px 28px' }}>
              <Comp
                orderId={orderId}
                orderData={orderData}
                onSaved={handleSaved}
                registerSave={makeRegisterSave(parseInt(num))}
                setStep={setStep}
              />
            </div>
          ))}
