...
export default function OrderWizard({ order, onClose }) {
  ...
  const dotColor = saveStatus === 'saving' ? '#f59e0b' : saveStatus === 'saved' ? '#16a34a' : saveStatus === 'error' ? '#dc2626' : autoSave ? '#16a34a' : '#d1d5db'
  const dotText  = saveStatus === 'saving' ? 'Saving...' : saveStatus === 'saved' ? 'Saved' : saveStatus === 'error' ? 'Failed' : autoSave ? 'Auto-save on' : ''
  ...
  return (
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 200, background: 'rgba(0,0,0,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      ...
      {/* Modal card */}
      <div style={{ background: '#fff', borderRadius: 12, boxShadow: '0 24px 80px rgba(0,0,0,0.22)', width: '100%', maxWidth: 1100, maxHeight: 'calc(100vh - 48px)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div style={{ height: 54, borderTop: '1px solid #f0f0ee', display: 'flex', alignItems: 'center', padding: '0 24px', gap: 10, flexShrink: 0, background: '#fff' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
            <div style={{ width: 7, height: 7, borderRadius: '50%', background: dotColor, transition: 'background 0.3s' }} />
            {dotText && <span style={{ fontSize: 11, color: '#9ca3af' }}>{dotText}</span>}
          </div>
          {orderId && (
            <button className="btn btn-secondary btn-sm" onClick={manualSave} disabled={saveStatus === 'saving'}>
          ...
          {step < 10
            ? <button className="btn btn-primary btn-sm" onClick={() => setStep(s => s + 1)} disabled={!canGoNext()}>Next <ChevronRight size={13} /></button>
