...
export default function Step2POMatrix({ orderId, orderData, onSaved, registerSave }) {
  ...
  const doSave = useCallback(async () => {
    ...
    const computedTotal = gs.reduce((sum, g) => sum + (g.colors || []).reduce((s, c) => { const key = c._tempId || c.id; return s + (g.sizes || []).reduce((ss, sz) => ss + (parseInt(g.breakdown?.[key]?.[sz]) || 0), 0) }, 0), 0)
    const targetTotal = parseFloat(orderData?.total_qty) || 0
    if (targetTotal > 0 && computedTotal > targetTotal) throw new Error(`PO Matrix total (${computedTotal}) cannot exceed order total (${targetTotal}).`)
    ...
    const rule  = splitRuleRef.current || 'None'
