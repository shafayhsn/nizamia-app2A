  ...
  const generateQueues = async () => {
    ...
    try {
      ...
      const insertRows = merged.map((r, i) => ({
        ...
        custom_components: (r.component_data || []).length ? r.component_data : [],
        qty:           r.qty || 0,
        status:        'Queued',
        sort_order:    i,
      }))
      ...
      if (error) {
        if (String(error.message || '').toLowerCase().includes('custom_components')) {
          throw new Error('Database column order_queues.custom_components is missing. Run the custom queue migration before generating queues.')
        }
        throw error
      }
