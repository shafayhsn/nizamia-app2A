  ...
  const buildQueueRows = (rule, gs, batches = customBatches) => {
    ...
    if (rule === 'Full PO') {
      ...
      ;(batches || []).forEach((batch, idx) => {
        ...
        const qty = (batch.combos || []).reduce((sum, key) => sum + (Number(comboMap.get(key)?.qty || 0)), 0)
        if (!qty && !(batch.combos || []).length) return
        qRows.push({ label: name, qty, split_type: 'custom', custom_components: (batch.combos || []).join('|') })
      })
    }
    return qRows
  }
  ...
  const generateQueues = async () => {
    ...
    if (qRows.length) {
      ...
      const insertRows = merged.map((r, i) => ({
        ...
        custom_components: r.custom_components || null,
        qty:           r.qty || 0,
        status:        'Queued',
        sort_order:    i,
      }))
      let { error } = await supabase.from('order_queues').insert(insertRows)
      if (error && String(error.message || '').toLowerCase().includes('custom_components')) {
        const fallbackRows = insertRows.map(({ custom_components, ...row }) => ({
          ...row,
          label: custom_components ? `${row.label}__APP2A_QC__${custom_components}` : row.label,
        }))
        ;({ error } = await supabase.from('order_queues').insert(fallbackRows))
      }
