...
export default function Step2POMatrix({ orderId, orderData, onSaved, registerSave }) {
  ...
  const generateQueues = async () => {
    ...
    if (rule === 'Custom') {
      ...
      if (validation.errors.length) { alert(validation.errors[0]); return }
      if (!customBatches.some(b => (b.name || '').trim() && (b.combos || []).length)) { alert('Create at least one named custom batch and assign colour/wash × size group rows.'); return }
    }
    const { data: order } = await supabase.from('orders').select('job_id').eq('id', orderId).single()

    // Build queue rows based on selected split rule
    let qRows = buildQueueRows(rule, gs, customBatches)
    ...
    if (qRows.length) {
      ...
      const insertRows = merged.map((r, i) => ({
        ...
        custom_components: (r.component_data || []).length ? r.component_data : null, // Save full component data as JSONB
        qty:           r.qty || 0,
        status:        'Queued',
        sort_order:    i,
      }))
    ...
    <div>
      ...
      <div style={{ background: '#fafaf8', border: '1px solid #e8e8e6', borderRadius: 8, padding: '12px 16px', marginBottom: 20, display: 'flex', alignItems: 'center', gap: 24 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <select value={splitRule} onChange={e => { const v = e.target.value; setSplitRule(v); setQSaved(false); if (!v) setCustomBatches([{ id: tempId(), name: '', combos: [] }]) }}
