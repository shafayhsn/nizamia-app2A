...
function RatioModal({ g, ci, onClose, onApply }) {
  ...
  const [inputs, setInputs] = useState(() => {
    ...
    const obj = {}
    g.sizes.forEach(sz => { obj[sz] = min > 0 ? ((parseInt(bd[sz]) || 0) / min).toFixed(1) : '' })
    return obj
  })
...
export default function Step2POMatrix({ orderId, orderData, onSaved, registerSave }) {
  ...
  // Load excess cutting % from order
  useEffect(() => {
  ...
  async function loadGroups() {
    ...
    // Load excess cutting %
    const { data: ord } = await supabase.from('orders').select('excess_cutting_pct').eq('id', orderId).single()
  ...
  const doSave = useCallback(async () => {
    ...
    await supabase.from('orders').update({ step_po_matrix: true, total_qty, total_value_usd, excess_cutting_pct: exPct }).eq('id', orderId)
    onSaved(orderId, { step_po_matrix: true, total_qty, total_value_usd, excess_cutting_pct: exPct })
  }, [orderId])
  ...
  return (
    <div>
      {/* Excess Cutting % — order-level field */}
      <div style={{ background: '#fafaf8', border: '1px solid #e8e8e6', borderRadius: 8, padding: '12px 16px', marginBottom: 20, display: 'flex', alignItems: 'center', gap: 16 }}>
        <div>
          <div style={{ fontSize: 13, fontWeight: 700 }}>Excess Cutting %</div>
          <div style={{ fontSize: 11, color: '#9ca3af', marginTop: 2 }}>Applied order-wide. BOM items can use cutting qty instead of order qty.</div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginLeft: 'auto' }}>
          <input type="number" step="0.5" min="0" max="50" value={excessCut}
            onChange={e => setExcessCut(e.target.value)}
            style={{ width: 80, height: 36, padding: '0 10px', border: '1px solid #e5e7eb', borderRadius: 6, fontSize: 16, fontWeight: 700, fontFamily: 'Inter,sans-serif', outline: 'none', textAlign: 'center' }}
            placeholder="0" />
          <span style={{ fontSize: 14, fontWeight: 600, color: '#374151' }}>%</span>
          {excessCut > 0 && (
            <div style={{ fontSize: 11, color: '#7c3aed', background: '#f5f3ff', border: '1px solid #ede9fe', borderRadius: 6, padding: '4px 10px', fontWeight: 600 }}>
              Cutting Qty = Order Qty × {(1 + parseFloat(excessCut) / 100).toFixed(3)}
            </div>
          )}
        </div>
      </div>
