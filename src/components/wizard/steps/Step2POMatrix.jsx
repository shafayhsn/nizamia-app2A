...
export default function Step2POMatrix({ orderId, orderData, onSaved, registerSave }) {
  ...
  const [customBatches, setCustomBatches] = useState([{ id: tempId(), name: '', combos: [] }])
  ...
  async function persistGroups(gs, orderRow = null) {
    ...
    const rule = splitRuleRef.current || orderRow?.queue_split_rule || ''
  ...
  const buildQueueRows = (rule, gs, batches = customBatches) => {
    const qRows = []
    if (!rule) return qRows

    if (rule === 'Full PO') {
      const qty = gs.reduce((sum, g) => sum + grandTotal(g), 0)
      qRows.push({ label: 'Full PO', qty, split_type: 'full_po' })
    } else if (rule === 'By Wash' || rule === 'By Colour') {
      const seen = []
      gs.forEach(g => {
        g.colors.forEach(c => {
      ...
      gs.forEach(g => {
        const qty = g.colors.reduce((sum, c) => {
          const key = c._tempId || c.id
          return sum + g.sizes.reduce((sizeSum, sz) => sizeSum + (parseInt(g.breakdown[key]?.[sz]) || 0), 0)
        }, 0)
      ...
      gs.forEach(g => {
        g.colors.forEach(c => {
          if (!c.color_name?.trim()) return
          const key = c._tempId || c.id
          const qty = g.sizes.reduce((sum, sz) => sum + (parseInt(g.breakdown[key]?.[sz]) || 0), 0)
          if (!qty) return
          qRows.push({ label: `${c.color_name} / ${g.group_name}`, qty, color_name: c.color_name, size_group_name: g.group_name })
        })
      })
      ...
      gs.forEach(g => {
        g.colors.forEach(c => {
          ...
          qRows.push({ label: `${ratioLabel} · ${c.color_name} / ${g.group_name}`, qty, color_name: c.color_name, size_group_name: g.group_name })
        })
      })
    } else if (rule === 'Custom') {
      const comboMap = getCustomComboMap(gs)
      ;(batches || []).forEach((batch, idx) => {
        const name = (batch.name || '').trim() || `Custom Batch ${idx + 1}`
        const comboKeys = batch.combos || []
        const qty = comboKeys.reduce((sum, key) => sum + (Number(comboMap.get(key)?.qty || 0)), 0)
        if (!qty && !comboKeys.length) return
        
        // Build detailed component objects for this batch
        const componentData = buildCustomComponentData(gs, comboKeys)
        
        qRows.push({ 
          label: name, 
          qty, 
          split_type: 'custom', 
          custom_components: comboKeys.join('|'),
          component_data: componentData, // Detailed component objects for IOS
        })
      })
    }
    return qRows
  ...
  const getCustomCombos = (gs = normalizeUniqueGroups(groupsRef.current)) => {
    const combos = []
    ;(gs || []).forEach((g, gi) => {
      ;(g.colors || []).forEach((c, ci) => {
        if (!c.color_name?.trim()) return
        const colorKey = ck(c)
        const qty = (g.sizes || []).reduce((sum, sz) => sum + (parseInt(g.breakdown?.[colorKey]?.[sz]) || 0), 0)
        if (!qty) return
        combos.push({
  ...
  const getCustomComboMap = (gs = normalizeUniqueGroups(groupsRef.current)) => new Map(getCustomCombos(gs).map(c => [c.key, c]))

  // Build detailed component objects for Custom queues with full size/qty/ratio data
  const buildCustomComponentData = (gs = normalizeUniqueGroups(groupsRef.current), selectedComboKeys = []) => {
    const components = []
    const comboMap = new Map()
    
    // Build map of all available combos with their size breakdown
    ;(gs || []).forEach((g, gi) => {
      ;(g.colors || []).forEach((c, ci) => {
        if (!c.color_name?.trim()) return
        const colorKey = c.id || c._tempId
        ...
        ;(g.sizes || []).forEach(sz => {
          const qty = parseInt(g.breakdown?.[colorKey]?.[sz]) || 0
          if (qty > 0) {
            sizeBreakdown[String(sz)] = qty
            totalQty += qty
          }
        })
        ...
        const key = `${gi}__${ci}__${g.group_name}__${c.color_name}`
        comboMap.set(key, {
          key,
          wash: c.color_name,
          colour: c.color_name,
          size_group: g.group_name,
          sizes: (g.sizes || []).map(String),
          qty_by_size: sizeBreakdown,
          total_qty: totalQty,
          ratio_by_size: c.ratio_override || sizeBreakdown, // Use override if set, else size qty as ratio
        })
      })
    })
  ...
  const getCustomValidation = (batches = customBatches) => {
    const used = new Map()
    const errors = []
    ;(batches || []).forEach((b, bi) => {
      if ((b.combos || []).length && !(b.name || '').trim()) errors.push(`Batch ${bi + 1} needs a name`)
      ;(b.combos || []).forEach(key => {
        if (!used.has(key)) used.set(key, [])
        used.get(key).push(bi)
      })
    })
    used.forEach((owners) => { if (owners.length > 1) errors.push('Duplicate colour/wash × size group allocation found') })
    return { used, errors }
  }

  const updateCustomBatch = (id, patch) => setCustomBatches(list => list.map(b => b.id === id ? { ...b, ...patch } : b))
  const addCustomBatch = () => setCustomBatches(list => [...list, { id: tempId(), name: '', combos: [] }])
  const removeCustomBatch = (id) => setCustomBatches(list => list.length <= 1 ? [{ id: tempId(), name: '', combos: [] }] : list.filter(b => b.id !== id))
  const toggleCustomCombo = (batchId, comboKey) => setCustomBatches(list => list.map(b => {
    if (b.id !== batchId) return b
    const exists = (b.combos || []).includes(comboKey)
    return { ...b, combos: exists ? b.combos.filter(k => k !== comboKey) : [...(b.combos || []), comboKey] }
  }))

  // ── Generate Queues ────────────────────────────────────────────────────────
  const generateQueues = async () => {
    if (qGenerating) return
    setQError('')
    setQSaved(false)
    setQGenerating(true)
    try {
      if (!orderId) throw new Error('Save the order first before generating queues.')
      const rule = splitRuleRef.current
      if (!rule) throw new Error('Select a split rule before generating queues.')

      // Persist latest PO Matrix values first, so Generate Qs never uses stale rows.
      await doSave()

      const gs = normalizeUniqueGroups(groupsRef.current)
      if (rule === 'Custom') {
        const validation = getCustomValidation(customBatches)
        if (validation.errors.length) throw new Error(validation.errors[0])
        if (!customBatches.some(b => (b.name || '').trim() && (b.combos || []).length)) {
          throw new Error('Create at least one named custom batch and assign colour/wash × size group rows.')
        }
      }
      const { data: order, error: orderError } = await supabase.from('orders').select('job_id').eq('id', orderId).single()
      if (orderError) throw orderError

      const qRows = buildQueueRows(rule, gs, customBatches)
      if (!qRows.length) throw new Error('No queue rows were created. Check that the PO Matrix has quantities.')

      const { error: deleteError } = await supabase.from('order_queues').delete().eq('order_id', orderId)
      if (deleteError) throw deleteError

      let sgMap = {}
      if (qRows.some(r => r.size_group_name)) {
        const { data: sgs, error: sgError } = await supabase.from('size_groups').select('id,group_name').eq('order_id', orderId)
      ...
      qRows.forEach(r => {
        ...
        if (!qMap.has(key)) qMap.set(key, { ...r, size_group_id: sgId, qty: 0, component_data: [] })
        const existing = qMap.get(key)
        existing.qty += Number(r.qty || 0)
        if (Array.isArray(r.component_data) && r.component_data.length) {
          existing.component_data = [...(existing.component_data || []), ...r.component_data]
        }
      })
      qMap.forEach(v => merged.push(v))

      const baseInsertRows = merged.map((r, i) => ({
        order_id:      orderId,
      ...
      const insertRows = rule === 'Custom'
        ? baseInsertRows.map((row, i) => ({
            ...row,
            custom_components: (merged[i]?.component_data || []).length ? merged[i].component_data : [],
          }))
        : baseInsertRows

      const { error } = await supabase.from('order_queues').insert(insertRows)
      if (error) {
        if (String(error.message || '').toLowerCase().includes('custom_components')) {
          if (rule === 'Custom') {
            throw new Error('Database column order_queues.custom_components is missing. Run the custom queue migration before generating Custom queues.')
          }
          const { error: retryError } = await supabase.from('order_queues').insert(baseInsertRows)
      ...
      try { localStorage.setItem('app2a.queue.refresh', String(Date.now())) } catch (e) {}
      setQSaved(true)
      setTimeout(() => setQSaved(false), 3000)
    } catch (err) {
      const msg = err?.message || 'Queue generation failed.'
      console.error('Generate queues failed:', err)
      setQError(msg)
      alert(msg)
    } finally {
  ...
  const queuePreviewRows = buildQueueRows(splitRule, normalizeUniqueGroups(groups), customBatches)
  const customCombos = getCustomCombos(normalizeUniqueGroups(groups))
  const customValidation = getCustomValidation(customBatches)
  ...
  return (
    <div>
      ...
      <div style={{ background: '#fafaf8', border: '1px solid #e8e8e6', borderRadius: 8, padding: '12px 16px', marginBottom: 20, display: 'flex', alignItems: 'center', gap: 24 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div>
            ...
            <div style={{ fontSize: 10, color: '#9ca3af' }}>Creates queue preview</div>
          </div>
          <select value={splitRule} onChange={e => { const v = e.target.value; setSplitRule(v); setQSaved(false); setQError(''); if (!v) setCustomBatches([{ id: tempId(), name: '', combos: [] }]) }}
            ...
            <option value="">Select Split Rule</option>
          ...
          {splitRule && (
            <button type="button" disabled={qGenerating} onClick={(e) => { e.preventDefault(); e.stopPropagation(); generateQueues() }}
              style={{ height: 34, padding: '0 14px', background: qSaved ? '#16a34a' : '#0d0d0d', color: '#fff', border: 'none', borderRadius: 6, fontSize: 11, fontWeight: 700, cursor: qGenerating ? 'wait' : 'pointer', fontFamily: 'var(--font)', display: 'flex', alignItems: 'center', gap: 5, boxShadow: qSaved ? '0 0 0 4px rgba(22,163,74,.14)' : 'none', transition: 'all .18s ease' }}>
              {qGenerating ? 'Generating…' : qSaved ? '✓ Queues Generated' : 'Generate Qs'}
            </button>
          )}
        </div>
      </div>
      ...
      {qError && (
        <div style={{ marginTop: -10, marginBottom: 12, padding: '8px 10px', border: '1px solid #fecaca', background: '#fef2f2', color: '#991b1b', borderRadius: 8, fontSize: 11, fontWeight: 700 }}>
          {qError}
        </div>
      )}

      {splitRule === 'Custom' && (
        <div style={{ background: '#fff', border: '1px solid #e8e8e6', borderRadius: 10, padding: '12px 16px', marginBottom: 20 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, marginBottom: 10 }}>
            <div>
              <div style={{ fontSize: 12, fontWeight: 700 }}>Custom Queue Batches</div>
              <div style={{ fontSize: 10, color: '#9ca3af' }}>Make manual batches from Colour/Wash × Size Group. Each combination can be used once only.</div>
            </div>
            <button className="btn btn-secondary" onClick={addCustomBatch}><Plus size={13} /> Add Batch</button>
          </div>
          <div style={{ display: 'grid', gap: 10 }}>
            {customBatches.map((batch, bi) => {
              const batchQty = (batch.combos || []).reduce((sum, key) => sum + Number(customCombos.find(c => c.key === key)?.qty || 0), 0)
              return (
                <div key={batch.id} style={{ border: '1px solid #f0f0ee', borderRadius: 8, padding: 10, background: '#fafaf8' }}>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr auto auto', gap: 8, alignItems: 'center', marginBottom: 8 }}>
                    <input style={inp} placeholder={`Batch name ${bi + 1}`} value={batch.name} onChange={e => updateCustomBatch(batch.id, { name: e.target.value })} />
                    <div style={{ fontSize: 12, fontWeight: 700, color: '#111827', minWidth: 90, textAlign: 'right' }}>{batchQty.toLocaleString()} pcs</div>
                    <button className="btn btn-ghost btn-sm" onClick={() => removeCustomBatch(batch.id)}><Trash2 size={12} /></button>
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(190px, 1fr))', gap: 6 }}>
                    {customCombos.length === 0 && <div style={{ fontSize: 11, color: '#9ca3af' }}>Add Colour/Wash and quantities in the matrix first.</div>}
                    {customCombos.map(combo => {
                      const owners = customValidation.used.get(combo.key) || []
                      const checked = (batch.combos || []).includes(combo.key)
                      const lockedByOther = owners.length > 0 && !checked
                      return (
                        <label key={`${batch.id}_${combo.key}`} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 8px', border: '1px solid #e8e8e6', borderRadius: 7, background: checked ? '#eff6ff' : lockedByOther ? '#f3f4f6' : '#fff', color: lockedByOther ? '#9ca3af' : '#111827', cursor: lockedByOther ? 'not-allowed' : 'pointer', fontSize: 11, fontWeight: 700 }}>
                          <input type="checkbox" checked={checked} disabled={lockedByOther} onChange={() => toggleCustomCombo(batch.id, combo.key)} />
                          <span style={{ flex: 1 }}>{combo.label}</span>
                          <span>{Number(combo.qty || 0).toLocaleString()}</span>
                        </label>
                      )
                    })}
                  </div>
          ...
          {customValidation.errors.length > 0 && <div style={{ marginTop: 8, fontSize: 11, color: '#dc2626', fontWeight: 700 }}>{customValidation.errors[0]}</div>}
        </div>
      )}

      {splitRule && queuePreviewRows.length > 0 && (
        <div style={{ background: '#fff', border: '1px solid #e8e8e6', borderRadius: 10, padding: '12px 16px', marginBottom: 20 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, marginBottom: 8 }}>
            <div>
              <div style={{ fontSize: 12, fontWeight: 700 }}>Queues Preview</div>
              <div style={{ fontSize: 10, color: '#9ca3af' }}>Logical queues only. Q numbers are assigned later from the Queues tab.</div>
            </div>
            <div style={{ fontSize: 11, fontWeight: 700, color: '#6b7280', background: '#fafaf8', border: '1px solid #f0f0ee', borderRadius: 999, padding: '4px 10px' }}>{splitRule}</div>
          </div>
          <div style={{ display: 'grid', gap: 6 }}>
            {queuePreviewRows.map((q, i) => (
              <div key={`${q.label}-${i}`} style={{ display: 'grid', gridTemplateColumns: '28px 1fr auto', alignItems: 'center', gap: 10, padding: '7px 10px', border: '1px solid #f0f0ee', borderRadius: 8, background: '#fafaf8' }}>
                <div style={{ width: 22, height: 22, borderRadius: 999, background: '#ede9fe', color: '#7c3aed', fontSize: 10, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{i + 1}</div>
                <div style={{ fontSize: 12, fontWeight: 700, color: '#1f2937' }}>{q.label || 'Queue'}</div>
                <div style={{ fontSize: 12, fontWeight: 700, color: '#111827' }}>{Number(q.qty || 0).toLocaleString()} pcs</div>
              </div>
            ))}
          </div>
        </div>
              ...
              <table style={{ borderCollapse: 'collapse', marginTop: 12, minWidth: 500 }}>
                ...
                <tbody>
                  ...
                  <tr style={{ background: '#fafaf8' }}>
                    ...
                    <td style={{ textAlign: 'right', fontWeight: 700, fontSize: 13, padding: '8px 0 8px 8px' }}>
                  ...
                  {excessCut > 0 && grandTotal(g) > 0 && (
                    <tr style={{ background: '#fdf4ff' }}>
                      ...
                      <td style={{ textAlign: 'right', fontWeight: 700, fontSize: 12, color: '#7c3aed', padding: '6px 0 6px 8px' }}>
      ...
      {groups.length > 1 && (
        <div style={{ background: '#1a1a2e', borderRadius: 8, padding: '10px 16px', marginBottom: 16, display: 'flex', gap: 24 }}>
          <div><div style={{ fontSize: 9, color: '#9ca3af', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Total Qty</div><div style={{ fontSize: 16, fontWeight: 700, color: '#fff' }}>{groups.reduce((s, g) => s + grandTotal(g), 0).toLocaleString()} pcs</div></div>
          <div><div style={{ fontSize: 9, color: '#9ca3af', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Order Value</div><div style={{ fontSize: 16, fontWeight: 700, color: '#fff' }}>{groups.reduce((s, g) => s + groupValue(g), 0).toLocaleString(undefined, { maximumFractionDigits: 0 })} USD</div></div>
          {excessCut > 0 && <div><div style={{ fontSize: 9, color: '#9ca3af', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Total Cutting Qty</div><div style={{ fontSize: 16, fontWeight: 700, color: '#c4b5fd' }}>{Math.ceil(groups.reduce((s, g) => s + grandTotal(g), 0) * (1 + parseFloat(excessCut) / 100)).toLocaleString()} pcs</div></div>}
        </div>
      )}
