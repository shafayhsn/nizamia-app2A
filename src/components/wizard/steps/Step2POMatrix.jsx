  ...
  const doSave = useCallback(async () => {
    ...
    for (const g of gs) {
      ...
      for (const c of g.colors) {
        if (!c.color_name?.trim()) continue
        const colorQty = g.sizes.reduce((s, sz) => s + (parseInt(g.breakdown[c._tempId]?.[sz]) || 0), 0)
        total_qty += colorQty
        total_value_usd += colorQty * unitPrice
      }
    }
    await supabase.from('orders').update({ step_po_matrix: true, total_qty, total_value_usd }).eq('id', orderId)
    onSaved(orderId, { step_po_matrix: true, total_qty, total_value_usd })
  }, [orderId])
