...
export default function Step1GeneralInfo({ orderId, orderData, onSaved, registerSave }) {
  ...
  const r = {
    ...
    ship_date:         useRef(), in_store_date:     useRef(),
    planned_date:      useRef(), ship_mode:         useRef(),
    incoterms:         useRef(), port_of_loading:   useRef(),
    port_of_discharge: useRef(), notes:             useRef(),
  }
  ...
  useEffect(() => {
    ...
    s(r.ship_date, o.ship_date); s(r.in_store_date, o.in_store_date)
    s(r.planned_date, o.planned_date); s(r.ship_mode, o.ship_mode || 'Sea')
    s(r.incoterms, o.incoterms || 'FOB'); s(r.port_of_loading, o.port_of_loading)
    s(r.port_of_discharge, o.port_of_discharge); s(r.notes, o.notes)
      ...
      <div>
        ...
        <Row>
          <FG label="Ex-Factory Date (Internal)">
            <input ref={r.ship_date} style={inp} type="date" defaultValue={orderData?.ship_date || ''}
              onChange={e => {
                if (!e.target.value) return
                const d = new Date(e.target.value)
                d.setDate(d.getDate() - 7)
                const planned = d.toISOString().slice(0, 10)
                if (r.planned_date.current && !r.planned_date.current.value) r.planned_date.current.value = planned
              }}
            />
          </FG>
          ...
          <FG label="In-Store Date">
            <input ref={r.in_store_date} style={inp} type="date" defaultValue={orderData?.in_store_date || ''} />
          </FG>
        </Row>
        <Row>
          <FG label="Ship Mode">
