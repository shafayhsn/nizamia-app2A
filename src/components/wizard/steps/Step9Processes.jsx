...
export default function Step9Processes({ orderId, onSaved, registerSave }) {
  ...
  const doSave = useCallback(async () => {
    ...
    await supabase.from('orders').update({ step_processes: true, notes: notesRef.current }).eq('id', orderId)
    onSaved(orderId, { step_processes: true, notes: notesRef.current })
    markTabUpdated('purchasingPage', 'process')
  }, [orderId])
