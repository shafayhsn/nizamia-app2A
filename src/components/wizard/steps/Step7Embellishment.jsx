...
function EmbUsageModal({ config, onClose, onSave, poData }) {
  ...
  const totalFQ = (() => {
    if (rule === 'Generic') {
      ...
      return (parseFloat(data.generic) || 0) * totalQty || null
    }
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '16px 20px' }}>
          {rule === 'Generic' && (
            <div>
              ...
              <input style={{ ...inp, width: 140, textAlign: 'right' }} type="number" step="1"
                value={data.generic || ''} onChange={e => setData({ generic: e.target.value })} placeholder="1" autoFocus />
            </div>
          )}
...
export default function Step7Embellishment({ orderId, onSaved, registerSave }) {
  ...
  async function loadPoData() {
    ...
    const { data: allColorsRaw } = await supabase.from('size_group_colors').select('*').in('size_group_id', sgs.map(g => g.id))
