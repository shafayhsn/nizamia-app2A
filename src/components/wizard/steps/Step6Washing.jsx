...
export default function Step6Washing({ orderId, onSaved, registerSave }) {
  ...
  async function loadAll() {
    const [{ data: existingWash }] = await Promise.all([
      supabase.from('washing').select('id,order_id,color_id,color_name,wash_type,wash_ref,wash_image_url,recipe,wash_image_base64').eq('order_id', orderId),
    ])
    // no suppliers needed
    ...
    const colorRows = []
    if (sgIds.length) {
      const { data: allColors } = await supabase.from('size_group_colors').select('id,color_name,size_group_id').in('size_group_id', sgIds).order('sort_order')
      ;(sgs || []).forEach(sg => {
        (allColors || []).filter(c => c.size_group_id === sg.id).forEach(c => {
          colorRows.push({ color_id: c.id, color_name: c.color_name, size_group: sg.group_name })
        })
      })
    }
    if (colorRows.length) {
  ...
  const doSave = useCallback(async () => {
    ...
    if (enabledRef.current && rowsRef.current.length) {
      const insertRows = rowsRef.current.map(r => ({
        order_id:          orderId,
        color_id:          r.color_id || null,
        color_name:        r.color_name || null,
        wash_type:         r.wash_type || 'No Wash',
        wash_ref:          r.wash_ref || null,
        wash_image_base64: r.wash_image_base64 || null,
        wash_image_url:    r.wash_image_url || null,
          ...
          {rows.map((r, idx) => (
            <div key={r._tempId || idx} style={{ background: '#fafaf8', border: '1px solid #e8e8e6', borderRadius: 8, padding: '12px 14px' }}>
              <div style={{ display: 'flex', gap: 14, alignItems: 'flex-start' }}>
                ...
                {/* Fields */}
                <div style={{ flex: 1, display: 'grid', gridTemplateColumns: '180px 1fr 1fr', gap: 10, alignItems: 'end' }}>
                  <div>
