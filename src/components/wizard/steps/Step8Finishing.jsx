...
function PackConfigModal({ pack, packIdx, poData, useCuttingQty, orderQty, cuttingQty, onClose, onSave }) {
  ...
  function getDefaultLines(selectedBasis, data, targetQty) {
    switch (selectedBasis) {
      case 'By Q#':
        return (data.queues || []).map(q => ({ key: q.q_number, label: `${q.q_number} · ${q.label || q.q_number}`, qty: q.qty || 0 }))
      case 'By Colour':
...
export default function Step8Finishing({ orderId, orderData, onSaved, registerSave }) {
  ...
  async function loadPoData() {
    ...
    const { data: qs } = await supabase.from('order_queues').select('q_number,label,qty,split_rule,color_name,size_group_id').eq('order_id', orderId).order('sort_order')
    if (!sgs?.length) {
