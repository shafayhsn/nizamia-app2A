...
function RequestModal({ orderId, onClose, onCreated }) {
  ...
  useEffect(() => {
    ...
    supabase.from('size_groups').select('*').eq('order_id', orderId).order('sort_order')
      .then(async ({ data: sgs }) => {
        ...
        const enriched = await Promise.all(sgs.map(async sg => {
          const { data: colours } = await supabase.from('size_group_colors').select('*').eq('size_group_id', sg.id).order('sort_order')
          return { ...sg, colours: colours || [] }
        }))
  ...
  const canCreate = selGroup && selColours.length > 0 && selSizes.length > 0
...
function SampleRow({ s, onUpdate, onDelete }) {
  ...
  return (
    <>
      ...
      {expanded && (
        <tr>
          <td colSpan={8} style={{ padding: '0 10px 10px 36px', borderBottom: '1px solid #f0f0ee', background: '#fafaf8' }}>
      ...
      {samples.length === 0 ? (
        ...
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 720 }}>
            <thead>
              <tr>
                {['Sample No.', 'Type', 'Group / Colours', 'Sizes', 'Due Date', 'Status', 'Notes', ''].map(h => (
