...
import { Plus, Trash2, Check, Library, X } from 'lucide-react'
...
export default function Step3BOM({ orderId, orderData, onSaved, registerSave }) {
  ...
  const [usageConfig, setUsageConfig] = useState(null) // { itemIdx, rule }
    ...
    <div>
              ...
              return (
                <tr key={item.id || item._tempId}>
                  ...
                  <td style={{ padding:'4px 4px', borderBottom:'1px solid #f0f0ee', width:130 }}>
                    <select style={{...sel, color: item.usage_rule && item.usage_rule!=='Generic' ? '#2563eb' : undefined}}
                      value={item.usage_rule||'Generic'}
                      onChange={e => {
                        const rule = e.target.value
                        upd(realIdx,'usage_rule',rule)
                        if (rule !== 'Generic') setUsageConfig({ itemIdx: realIdx, rule, item })
                      }}>
      ...
      {usageConfig && (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.45)', zIndex:500, display:'flex', alignItems:'center', justifyContent:'center' }}>
          <div style={{ background:'#fff', borderRadius:12, width:460, boxShadow:'0 24px 60px rgba(0,0,0,0.2)', overflow:'hidden' }}>
            <div style={{ padding:'16px 20px', borderBottom:'1px solid #f0f0ee', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
              <div>
                <div style={{ fontSize:13, fontWeight:700 }}>Usage Rule: {usageConfig.rule}</div>
                <div style={{ fontSize:11, color:'#9ca3af', marginTop:1 }}>{usageConfig.item?.name || 'BOM Item'}</div>
              </div>
              <button onClick={() => setUsageConfig(null)} style={{ background:'none', border:'none', cursor:'pointer', color:'#9ca3af', display:'flex' }}><X size={16}/></button>
            </div>
            <div style={{ padding:'20px' }}>
              {usageConfig.rule === 'By Color' && (
                <div>
                  <p style={{ fontSize:12, color:'#6b7280', marginBottom:12 }}>Set a different consumption qty per colour. Colours are pulled from the PO Matrix.</p>
                  {(orderData?.colors || []).length === 0
                    ? <div style={{ fontSize:12, color:'#9ca3af', padding:'12px', background:'#fafaf8', borderRadius:6 }}>No colours found. Add colours in Step 2 — PO Matrix first.</div>
                    : <div style={{ fontSize:12, color:'#6b7280', padding:'12px', background:'#eff6ff', borderRadius:6, border:'1px solid #bfdbfe' }}>
                        Configuration of per-colour qtys will be available in a future update. The base qty will apply to all colours for now.
                      </div>
                  }
                </div>
              )}
              {usageConfig.rule === 'By Size Group' && (
                <div style={{ fontSize:12, color:'#6b7280', padding:'12px', background:'#eff6ff', borderRadius:6, border:'1px solid #bfdbfe' }}>
                  Per-size-group consumption configuration coming soon. Base qty applies to all groups for now.
                </div>
              )}
              {usageConfig.rule === 'By Individual Sizes' && (
                <div style={{ fontSize:12, color:'#6b7280', padding:'12px', background:'#eff6ff', borderRadius:6, border:'1px solid #bfdbfe' }}>
                  Per-size consumption configuration coming soon. Base qty applies to all sizes for now.
                </div>
              )}
              {usageConfig.rule === 'Configure Own' && (
                <div style={{ fontSize:12, color:'#6b7280', padding:'12px', background:'#eff6ff', borderRadius:6, border:'1px solid #bfdbfe' }}>
                  Custom usage configuration coming soon. Use base qty for now.
                </div>
              )}
            </div>
            <div style={{ padding:'12px 20px', borderTop:'1px solid #f0f0ee', display:'flex', justifyContent:'flex-end', gap:8 }}>
              <button className="btn btn-ghost" onClick={() => { upd(usageConfig.itemIdx,'usage_rule','Generic'); setUsageConfig(null) }}>Reset to Generic</button>
              <button className="btn btn-primary" onClick={() => setUsageConfig(null)}>OK, understood</button>
            </div>
          </div>
        </div>
      )}
