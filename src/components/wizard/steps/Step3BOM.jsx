...
function getStaleKeys(item, poData) {
  ...
  if (rule === 'By Batch') {
    ...
    const assigned = batches.flatMap(b => (b.cells || []).map(c => `${c.sgName}||${c.colorName}`))
    ...
    if (rule === 'Generic') {
      ...
      for (const b of batchData) {
        for (const cell of (b.cells || [])) {
          const k = `${cell.sgName}||${cell.colorName}`
          if (seen.has(k)) { alert(`${cell.sgName} / ${cell.colorName} is already used in another batch.`); return }
          seen.add(k)
        }
      }
              ...
              {!(poData.sgMatrix || []).length ? (
                ...
                <div style={{ display: 'grid', gap: 12 }}>
                  {batchData.map((batch, batchIdx) => {
                    const usedElsewhere = new Set(batchData.flatMap((b, ix) => ix === batchIdx ? [] : (b.cells || []).map(c => `${c.sgName}||${c.colorName}`)))
                    const selectedHere = new Set((batch.cells || []).map(c => `${c.sgName}||${c.colorName}`))
                    ...
                    const toggleCell = (sgName, colorName) => {
                      const key = `${sgName}||${colorName}`
                      if (usedElsewhere.has(key)) return
                      setBatchData(prev => prev.map((b, ix) => {
                        if (ix !== batchIdx) return b
                        const has = (b.cells || []).some(c => c.sgName === sgName && c.colorName === colorName)
                        const cells = has ? (b.cells || []).filter(c => !(c.sgName === sgName && c.colorName === colorName)) : [...(b.cells || []), { sgName, colorName }]
                        return { ...b, cells }
                      }))
                    }
                    return (
                      <div key={batchIdx} style={{ border: '1px solid #e5e7eb', borderRadius: 8, padding: 12, background: '#fff' }}>
                        ...
                        <div style={{ display: 'grid', gap: 8 }}>
                          {(poData.sgMatrix || []).map(sg => (
                            <div key={sg.sgName}>
                              <div style={{ fontSize: 10, fontWeight: 800, marginBottom: 4 }}>{sg.sgName}</div>
                              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                                {sg.colors.map(c => {
                                  const key = `${sg.sgName}||${c.colorName}`
                                  const disabled = usedElsewhere.has(key)
                                  return <button key={key} type="button" onClick={() => toggleCell(sg.sgName, c.colorName)} disabled={disabled} style={{ fontSize: 11, padding: '4px 8px', borderRadius: 999, border: selectedHere.has(key) ? '1px solid #2563eb' : '1px solid #e5e7eb', background: selectedHere.has(key) ? '#eff6ff' : disabled ? '#f3f4f6' : '#fff', color: disabled ? '#9ca3af' : '#111827', cursor: disabled ? 'not-allowed' : 'pointer' }}>{c.colorName} ({c.qty || 0})</button>
                                })}
                              </div>
                            </div>
                          ))}
