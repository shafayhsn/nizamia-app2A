...
function EventRow({ event, onOpen }) {
  ...
  return (
    <button onClick={() => onOpen(event)} style={{
      ...
      <span>
        <div style={{ fontSize: 12, fontWeight: 700, color: '#111827' }}>{event.title}</div>
      ...
      <span style={{ fontSize: 10, color: danger ? '#dc2626' : '#6b7280', fontWeight: 700, whiteSpace: 'nowrap' }}>{fmt(event.rawDate)}</span>
    </button>
  )
}
      ...
      <div style={{ width: 920, maxWidth: 'calc(100vw - 32px)', height: 650, maxHeight: 'calc(100vh - 32px)', background: '#fff', borderRadius: 18, boxShadow: '0 24px 60px rgba(0,0,0,.24)', display: 'grid', gridTemplateColumns: '1fr 310px', overflow: 'hidden' }}>
        <div style={{ padding: 18, borderRight: '1px solid #e5e7eb', display: 'flex', flexDirection: 'column', minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
            ...
            <div style={{ fontSize: 20, fontWeight: 700 }}>{monthLabel(view)}</div>
            <div style={{ flex: 1 }} />
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 6, marginBottom: 6 }}>
            {['Sun','Mon','Tue','Wed','Thu','Fri','Sat'].map(d => <div key={d} style={{ fontSize: 11, fontWeight: 700, color: '#9ca3af', textAlign: 'center' }}>{d}</div>)}
          </div>
            ...
            {cells.map(d => {
              ...
              return (
                <button key={k} onClick={() => setSelected(k)} style={{
                  ...
                  <div style={{ fontSize: 12, fontWeight: 700, color: hasOverdue ? '#dc2626' : '#111827' }}>{d.getDate()}</div>
        ...
        <div style={{ padding: 16, display: 'flex', flexDirection: 'column', background: '#fafafa' }}>
          <div style={{ fontSize: 15, fontWeight: 700, marginBottom: 4 }}>Events</div>
...
function BellTitle({ count }) {
  return <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}><Clock size={16}/><span style={{ fontSize: 14, fontWeight: 700 }}>Notifications</span>{count > 0 && <span style={{ background: '#dc2626', color: '#fff', borderRadius: 999, padding: '2px 7px', fontSize: 10, fontWeight: 700 }}>{count}</span>}</div>
}
